...
export default function SupplierOrderView() {
  ...
  const { data: entity, isLoading, isError } = useSupplierOrder(parsedId);
  ...
  const { data: auditData } = useQuery({
    queryKey: ['audit', 'SupplierOrder', parsedId],
    queryFn: () => api.get(`/audit/SupplierOrder/${parsedId}`).then((r) => r.data as { items: unknown[]; total: number }),
      ...
      <Paper sx={{ p: 3 }}>
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Typography variant="body1">
              {enmEmailStatusOptions.find((o) => o.value === entity.enmEmailStatus)?.label || String(entity.enmEmailStatus)}
            </Typography>
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
            <Box display="flex" alignItems="center" gap={0.5}>
            ...
            <Typography variant="body1">
              {(() => { const v = entity.sentDate; if (!v || String(v) < '1900') return '-'; return new Date(v).toLocaleDateString(); })()}
            </Typography>
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
            <Box display="flex" alignItems="center" gap={0.5}>
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Typography variant="body1">
              {(() => { const v = entity.requestedDeliveryDate; if (!v || String(v) < '1900') return '-'; return new Date(v).toLocaleDateString(); })()}
            </Typography>
          </Grid>
          <Grid size={{ xs: 12, sm: 6 }}>
            <Box display="flex" alignItems="center" gap={0.5}>
