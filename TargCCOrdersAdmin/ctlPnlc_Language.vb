Public Class ctlPnlc_Language 
 
  Private _Requester As clsRequester  
  
  'The Controls 
  Private WithEvents _ctlLanguageCol As ctlc_LanguageCol 
  Private WithEvents _ctlLanguage As ctlc_Language 
  ...
  'The master 
  Private _LanguageID As Long 
 
  'The data holders 
  Private _LanguageCol As csLanguageCol 
  Private _Language As csLanguage 
  ...
  Public Event evtOverrideLoadCboLanguage(ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String, ByRef rMakeSmart As Boolean, ByRef rAddNewPrompt As String) 
  Private Event evtGetLanguageIDFromIntelliComboText(ByVal vIntelliComboText As String) 
  ''' <summary> 
  ''' If you want to 'PageFromServer', i.e. you don't want to load all the items to the combolist, and want to query the database to get the handful of items at a time,  
  '''   then set the level in MyCache to PageFromServer in evtBeforeLoad 
  ''' This is useful if the population of the combolist is huge....  
  ...
  Public Event evtOverrideFillLanguageCol(ByRef rLanguageCol As csLanguageCol, ByRef rGridTitle As String, ByRef rHowMany As Integer) 
  
  'Override LoadControls
  Private Event evtOverrideLoadCtlLanguageCol(ByRef rLoadParameters As ctlc_LanguageCol.clsLoadParameters) 
  Private Event evtOverrideLoadCtlLanguage(ByRef rLoadParameters As ctlc_Language.clsLoadParameters) 
  
  'Set IgnoreDoubleclicks 
  Private Event evtIgnoreLanguageCol_evtRowDoubleClicked(ByRef rIgnore As Boolean) 
  ...
  'Primary Timer
  Private Event evtLanguageTimerTripped(ByRef rCancel As Boolean) 
  ...
  Private Sub ctl_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load 
    ...
    lnkLanguageCol.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D 
    lnkLanguage.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D 
  ...
  Public Function LoadControl(ByVal vLanguageID As Long, ByVal vRequester As clsRequester) As clsFault 
    Dim pFault As New clsFault 
 
    _Requester = vRequester 
    _LanguageID = CType(vLanguageID, Long) 
    ...
    If _LanguageID > 0 Then 
      ...
      _NestedInMain = True 
      pnlCover.SendToBack() 
    ElseIf _LanguageID = -1 AndAlso MyIntelliCombo.cbo.Items.Count = 1 Then 
  ...
  Private Function ActivateControl(ByVal pControlName As String) As clsFault 
    ...
    If pControlName = "ctlc_Language" OrElse pControlName = "ctlLanguage" Then 
      lnkLanguage.ForeColor = Color.Black : lnkLanguage.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D : lnkLanguage.BackColor = Color.Wheat 
      If _ctlLanguage Is Nothing Then 
        _ctlLanguage = New ctlc_Language() 
        _ctlLanguage.Dock = DockStyle.Fill 
        pnlLanguage.Controls.Add(_ctlLanguage) 
        _ctlLanguage.Visible = False 
      End If 
      If _LanguageID = 0 Then 
        pnlLanguage.Visible = False 
      End If 
      'If _Language Is Nothing Then 
      pFault = RefreshCtlLanguage() 
      If pFault.isOK = False Then Return pFault 
      'End If 
      If _ctlLanguage.Language.IsEmpty AndAlso _LanguageID <> -2 Then 
        pnlLanguage.Visible = False 
      End If 
      _ctlLanguage.Name = "ctlc_Language" 
      _ActiveControl = _ctlLanguage 
      _ctlLanguage.BringToFront() 
      _ctlLanguage.Focus() 
      If pShowGrid = True Then 
        lblSecondaryTitle.Visible = True 
        btnFilter.Visible = False 
      End If 
    ElseIf pControlName = "ctlc_LanguageCol" Then 
      lnkLanguageCol.ForeColor = Color.Black : lnkLanguageCol.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D : lnkLanguageCol.BackColor = Color.Wheat 
      If _ctlLanguageCol Is Nothing Then 
        _ctlLanguageCol = New ctlc_LanguageCol() 
        _ctlLanguageCol.Dock = DockStyle.Fill 
        pnlLanguage.Controls.Add(_ctlLanguageCol) 
        _ctlLanguageCol.Visible = False 
      End If  
      pnlLanguage.Visible = True 
      If _LanguageCol Is Nothing Then 
        pFault = RefreshCtlLanguageCol() 
        If pFault.isOK = False Then Return pFault 
      End If 
      _ctlLanguageCol.Name = "ctlc_LanguageCol" 
      _ActiveControl = _ctlLanguageCol 
      _ctlLanguageCol.BringToFront() 
      _ctlLanguageCol.Focus() 
      If pShowGrid = True Then 
        lblSecondaryTitle.Visible = False 
        btnFilter.Visible = True 
      End If 
    Else 
      Return pFault.LogFreeTextFault(1, "Invalid object:" & pControlName, "", "TRGT-Language-091229-1815", _Requester) 
    End If 
  ...
  Private Sub LoadLocalizedText() 
    ...
    lblTitle.Text = TableNameTranslate("Language", _Requester) 
 
    lnkLanguageCol.Text = CCTextTranslate("List", _Requester) 
    lnkLanguage.Text = CCTextTranslate("Details", _Requester) 
  ...
  Private Sub btnRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRefresh.Click 
    ...
    If pnlLanguage.Controls(0) Is _ctlLanguage Then 
      If _LanguageID = 0 Then 
        pnlLanguage.Visible = False 
      End If 
    ElseIf pnlLanguage.Controls(0) Is _ctlLanguageCol Then 
    Else 
    ...
    If MyIntelliCombo.IsDumb = True AndAlso MyIntelliCombo.DropDownStyle <> ComboBoxStyle.DropDownList Then 
      ...
      Dim pLanguageID As Long = _LanguageID 
      If ccHelper.IsNumeric(pText) Then _LanguageID = ccHelper.ToLong(pText) 
      RaiseEvent evtGetLanguageIDFromIntelliComboText(pText) 
      If pLanguageID <> _LanguageID Then 
        _Language = Nothing 
        pFault = ActivateControl("ctlc_Language") 
        Cursor = Cursors.Default 
    ...
    Else 
      Cursor = Cursors.Default 
      pFault.LogFreeTextFault(1, "Can't refresh - no control found:" & pnlLanguage.Controls(0).Name, "", "TRGT-Language-091229-1810", _Requester) 
    End If 
  ...
  Private Sub btnRefresh_MouseDown(sender As Object, e As MouseEventArgs) Handles btnRefresh.MouseDown 
    ...
    If e.Clicks = 2 Then 
      Dim pFault As clsFault = LoadCboLanguages(True) 
      If pFault.isOK = False Then ShowFault(pFault, _Requester) 
    End If 
  End Sub 
 
  Private Sub _ctlLanguageCol_evtRowClicked(ByVal vLanguage As Object) Handles _ctlLanguageCol.evtRowClicked 
    
    If vLanguage Is Nothing Then Exit Sub 
    
    Cursor = Cursors.WaitCursor 
    Dim pLanguage As csLanguage = CType(vLanguage, csLanguage) 
    _LanguageID = pLanguage.ID 
    
    If _ActiveControl Is _ctlLanguageCol Then 
    ...
    'lblSecondaryTitle.Text = pLanguage.Name
 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub ChooseLanguage() 
    _Language = Nothing 
    lnkLanguage.Visible = True 
  End Sub 
  Private Sub _ctlLanguageCol_evtRowDoubleClicked(ByVal vLanguage As csLanguage, ByRef rHandled As Boolean) Handles _ctlLanguageCol.evtRowDoubleClicked 
    If lnkLanguage.Parent IsNot flpMenu Then Exit Sub 
    If vLanguage Is Nothing Then Exit Sub 
    ...
    If pInGroupBy = False Then 
      ...
      RaiseEvent evtIgnoreLanguageCol_evtRowDoubleClicked(pIgnore) 
      ...
      If pMainForm IsNot Nothing Then 
        If pMainForm.UpdateActiveMenuCodeWithID(Me.Name, vLanguage.ID, vLanguage.DefaultDesignation) = False Then 
    ...
    Else 
      ...
      pFault = _LanguageCol.FillOnTheFly(_SearchFilters, vRequester:=_Requester) : If pFault.isOK = False Then Cursor = Cursors.Default : ShowFault(pFault, _Requester) : Exit Sub 
      ...
      If _LanguageCol.Count = 100 Then 
        pTitle = "Showing 1st 99 rows" 
        _LanguageCol.RemoveAt(99) 
      Else 
        pTitle = String.Format(pShowing0Rows, _LanguageCol.Count) 
      End If 
 
      Dim pLoadParameters As New ctlc_LanguageCol.clsLoadParameters() 
      With pLoadParameters 
        ...
        .ColumnsHeaderText.Add(csLanguage.enmProperty.ID, "ID") 
      End With 
 
      pLoadParameters.GridTitle = CreateGridTitle(pTitle) & "Double click row to see Language" 
      pFault = _ctlLanguageCol.LoadControl(_LanguageCol, pLoadParameters, _Requester) 
      If pFault.isOK = False Then 
        Cursor = Cursors.Default 
        ShowFault(pFault, _Requester) 
      Else 
    ...
    Cursor = Cursors.Default 
  End Sub 
  Private Sub _ctlLanguageCol_evtUnChosen() Handles _ctlLanguageCol.evtUnChosen 
 
    _LanguageID = 0 
    _Language = Nothing 
    lblSecondaryTitle.Text = "" 
    lnkLanguage.Visible = False 
 
  End Sub 
  Private Sub lnk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles _ 
      lnkLanguageCol.Click, 
      lnkLanguage.Click 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pFault As clsFault 
    ...
    If lnk Is lnkLanguage OrElse (lnk Is lnkLanguageCol AndAlso chkGrid.Checked = True) Then 
      _NestedFormsCount = 0 
  ...
  'Refreshes 
  Private Function RefreshCtlLanguageCol() As clsFault 
    Dim pFault As New clsFault 
    ...
    Dim pTestCol As csLanguageCol = Nothing 
    Dim pHowmany As Integer = 100 
    RaiseEvent evtOverrideFillLanguageCol(pTestCol, pTitle, pHowmany) 
    If pTestCol Is Nothing Then 
      _LanguageCol = New csLanguageCol() 
      If IsFiltered() Then 
        ...
        pFault = _LanguageCol.FillOnTheFly(_SearchFilters, vRequester:=_Requester) 
        If pFault.isOK = False Then 
          _ctlLanguageCol.Timer?.Stop() 
          Return pFault 
        End If 
      Else 
        ...
        If _ctlLanguageCol.chkAutoRefresh.Checked Then pHowmany = 15 
        pFault = _LanguageCol.Fill(vRequester:=_Requester, vHowMany:=pHowmany, vDir:=clsEnums.enmFillDirection.DESC) 
        If pFault.isOK = False Then 
          _ctlLanguageCol.Timer?.Stop() 
          Return pFault 
        End If 
      ...
      If _LanguageCol.Count = pHowmany Then 
        pTitle = $"Showing 1st {pHowmany - 1} rows" 
        _LanguageCol.RemoveAt(pHowmany - 1) 
      Else 
        pTitle = String.Format(pShowing0Rows, _LanguageCol.Count) 
      End If 
    Else 
      _LanguageCol = pTestCol 
      If pTitle Is Nothing Then 
        pTitle = String.Format(pShowing0Rows, _LanguageCol.Count) 
      End If 
    End If 
    RaiseEvent evtOverrideLoadCtlLanguageCol(pLoadParameters) 
    pLoadParameters.GridTitle = CreateGridTitle(pTitle) & "Double click row to see Language" 
    ...
    pFault = _ctlLanguageCol.LoadControl(_LanguageCol, pLoadParameters, _Requester) 
    If pFault.isOK = False Then Return pFault 
    _ctlLanguageCol.Visible = True 
    
    _ctlLanguageCol.Refresh() 
    If pLanguageID <> 0 Then 
      Dim pLanguageCol As csLanguageCol = CType(_ctlLanguageCol.bsCtlLanguage.DataSource, csLanguageCol) 
      Dim pLanguage As csLanguage = pLanguageCol.FindByID(pLanguageID) 
      If pLanguage.ID > 0 Then 
        _ctlLanguageCol.bsCtlLanguage.CurrencyManager.Position = pLanguageCol.IndexOf(pLanguage) 
        _ctlLanguageCol.dgvLanguage.Rows(pLanguageCol.IndexOf(pLanguage)).Selected = True 
      End If 
    End If 
    Return pFault 
  End Function 
  Private Function RefreshCtlLanguage() As clsFault 
    Dim pFault As New clsFault 
    
    If _LanguageID > 0 Then 
      ChooseLanguage() 
      _Language = New csLanguage() 
      pFault = _Language.GetByID(_LanguageID, _Requester, False) 
      If pFault.isOK = False Then Return pFault 
    Else 
      _Language = New csLanguage() 
    End If 
    'lblSecondaryTitle.Text = _Language.Name    
     
    Dim pLoadParameters As New ctlc_Language.clsLoadParameters() 
    With pLoadParameters
      .ReadOnly = True 
    End With 
    RaiseEvent evtOverrideLoadCtlLanguage(pLoadParameters)
    pFault = _ctlLanguage.LoadControl(_Language, pLoadParameters, _Requester) : If pFault.isOK = False Then Return pFault 
    _ctlLanguage.Visible = True 
    Return pFault 
  End Function 
 
 
  'Handle events 
  Private Function LoadCboLanguages(ByVal vRenewCache As Boolean) As clsFault 
    Dim pFault As clsFault 
    ...
    Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.c_LanguageDefaultByID 
    Dim pParentID As Long = 0 
    
    RaiseEvent evtOverrideLoadCboLanguage(pComboListTypeToLoad, pParentID, pComboList, pPrompt, pMakeSmart, pAddNewPrompt) 
  ...
  Private Sub MyIntelliCombo_evtComboListMemberChosen(ByVal vComboListMember As clsComboListMember) Handles MyIntelliCombo.evtComboListMemberChosen  
    ...
    If vComboListMember Is Nothing OrElse vComboListMember.KeyType = clsEnums.enmComboListKeyType.UD Then 
      ...
      If ccHelper.IsNumeric(vComboListMember.Text) Then _LanguageID = ccHelper.ToLong(vComboListMember.Text) 
      lblSecondaryTitle.Text = "ID: " & vComboListMember.Text 
    ElseIf vComboListMember.KeyLong = -3 Then 
      sLoading = True 
      If ccHelper.IsNumeric(vComboListMember.Text) Then _LanguageID = ccHelper.ToLong(vComboListMember.Text) 
      RaiseEvent evtGetLanguageIDFromIntelliComboText(vComboListMember.Text) 
      lblSecondaryTitle.Text = vComboListMember.Text 
    Else 
      sLoading = True 
      _LanguageID = vComboListMember.KeyLong 
      lblSecondaryTitle.Text = vComboListMember.Text 
    End If 
    'Check if entity is already open in another tab BEFORE loading 
    If _LanguageID > 0 Then 
      Dim pMainForm As frmMain = TryCast(Me.FindForm(), frmMain) 
      If pMainForm IsNot Nothing Then 
        If pMainForm.UpdateActiveMenuCodeWithID(Me.Name, _LanguageID, vComboListMember.Text) = False Then 
    ...
    If _ActiveControl IsNot Nothing Then 
      If Not _ActiveControl.Name.Equals("ctlc_Language", StringComparison.OrdinalIgnoreCase) AndAlso _LanguageID > 0 Then 
  ...
  Private Sub chkGrid_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkGrid.CheckedChanged 
    ...
    Else 
      ...
      _ActiveControl = _ctlLanguage 
      lblSecondaryTitle.Visible = False 
      pFault = LoadCboLanguages(False) : If pFault.isOK = False Then Cursor = Cursors.Default : ShowFault(pFault, _Requester) : Exit Sub 
      If _LanguageID <> 0 Then 
        pFault = ActivateControl("ctlc_Language") 
        If pFault.isOK = False Then Cursor = Cursors.Default : ShowFault(pFault, _Requester) : Exit Sub  
      Else 
  ...
  Private Sub lnk_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles _ 
                  lnkLanguageCol.MouseLeave, 
                  lnkLanguage.MouseLeave, 
                  lblBack.MouseLeave 
    Dim lnk As Label = CType(sender, Label) 
    If lnk.BackColor = Color.Wheat Then Exit Sub 
  ...
  Private Sub btnFilter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFilter.Click 
    ...
    If _frmSearch Is Nothing Then 
      ...
      _frmSearch.Text = "Filter the Languages"  
      ...
      With _frmSearch 
        ...
        .String01Label.Text = If(_ctlLanguageCol.LoadParameters.ColumnsHeaderText.ContainsKey(csLanguage.enmProperty.Code), _ctlLanguageCol.LoadParameters.ColumnsHeaderText(csLanguage.enmProperty.Code), "Code") 
        .String01Text.Text = "" 
        ...
        .Text01Label.Text = If(_ctlLanguageCol.LoadParameters.ColumnsHeaderText.ContainsKey(csLanguage.enmProperty.ID), _ctlLanguageCol.LoadParameters.ColumnsHeaderText(csLanguage.enmProperty.ID), "ID") 
        .Text01From.Text = "" 
    ...
    With _frmSearch 
      If .String01Text.Text <> "" Then 
        pCode = .String01Text.Text 
        pCodeWildcardType = CType(CType(.String01WCType.SelectedItem, clsComboListMember).KeyEnum, clsEnums.enmWildCardType) 
        _SearchFilters.Add(csLanguageCol.enmFillOnTheFlyParameters.Code, pCode) 
        _SearchFilters.Add(csLanguageCol.enmFillOnTheFlyParameters.CodeWildcardType, pCodeWildcardType) 
      End If 
      If .Text01From.Text <> "" Then 
        If IsNumeric(.Text01From.Text) Then 
          ...
          Else 
            pIDTo = pIDFrom 
          End If 
          _SearchFilters.Add(csLanguageCol.enmFillOnTheFlyParameters.IDFrom, pIDFrom) 
          _SearchFilters.Add(csLanguageCol.enmFillOnTheFlyParameters.IDTo, pIDTo) 
        End If 
      End If 
      If .txtMaxRowsToReturn.Text <> "" AndAlso IsNumeric(.txtMaxRowsToReturn.Text) Then 
        Try 
          ...
          _SearchFilters.Add(csLanguageCol.enmListDefinition.HowMany, pRows) 
        Catch ex As Exception 
      ...
      If .rbtnOldestFirst.Checked = True Then 
        ...
        _SearchFilters.Add(csLanguageCol.enmListDefinition.Dir, pDir) 
      End If 
    ...
    If _SearchFilters.Count > 0 Then 
      If Not _SearchFilters.ContainsKey(csLanguageCol.enmListDefinition.HowMany) Then _SearchFilters.Add(csLanguageCol.enmListDefinition.HowMany, pRows) 
      If Not _SearchFilters.ContainsKey(csLanguageCol.enmListDefinition.Dir) Then _SearchFilters.Add(csLanguageCol.enmListDefinition.Dir, pDir) 
    End If  
 
    Dim pLoadParameters As New ctlc_LanguageCol.clsLoadParameters()
    If pDoSum = False Then
      pLoadParameters = New ctlc_LanguageCol.clsLoadParameters() 
      With pLoadParameters 
        .SpreadsheetShowAllFields = False 
        .ReportTitle = "Summary List" 
        .ReadOnly = True 
      ...
      If IsFiltered() Then 
        ...
        _Tooltip.SetToolTip(lblTitle, "Double click this label to reset the filter") 
        _Tooltip.SetToolTip(btnFilter, "Double click the text to the left to reset the filter") 
        pFault = _LanguageCol.FillOnTheFly(_SearchFilters, vRequester:=_Requester) : If pFault.isOK = False Then ShowFault(pFault, _Requester) : Exit Sub 
      Else 
        btnFilter.BackColor = Me.BackColor 
        lblTitle.ForeColor = Color.Black 
        _Tooltip.SetToolTip(lblTitle, "") 
        _Tooltip.SetToolTip(btnFilter, "") 
        pFault = _LanguageCol.Fill(vRequester:=_Requester, vHowMany:=pRows, vDir:=pDir) 
      End If 
    ...
    Else 
      ...
      _LanguageCol = New csLanguageCol() 
      pFault = _LanguageCol.FillSumOnTheFly(_SearchFilters, vRequester:=_Requester) 
      If pFault.isOK = False Then ShowFault(pFault, _Requester) : Exit Sub 
      ...
      With pLoadParameters 
        ...
        .SpreadsheetShowAllFields = False 
        .GridTitle = CreateGridTitle(String.Format(pShowing0Rows, _LanguageCol.Count)) & "Double click row to see listing" 
        .ReportTitle = "Summary Grouped By List" 
        .ReadOnly = True 
        .SummarizeGrid = True 
        .ColumnsHide.Clear() 
        .ColumnsHeaderText.Clear() 
        .ColumnsHeaderText.Add(csLanguage.enmProperty.ID, "Count") 
        .ColumnsHide.Add(csLanguage.enmProperty.Code) 
        .ColumnsHide.Add(csLanguage.enmProperty.Name) 
        .ColumnsHide.Add(csLanguage.enmProperty.NameLoc) 
        .ColumnsHide.Add(csLanguage.enmProperty.Culture) 
        .ColumnsHide.Add(csLanguage.enmProperty.Tag) 
  ...
  Private Function IsFiltered() As Boolean 
    ...
    If _SearchFilters.Count > 2 Then 
      pIsFiltered = True 
    Else 
      If ccHelper.ToInteger(_SearchFilters(csLanguageCol.enmListDefinition.HowMany)) <> 100 Then 
        pIsFiltered = True 
      End If 
      If CType(_SearchFilters(csLanguageCol.enmListDefinition.Dir), clsEnums.enmFillDirection) <> clsEnums.enmFillDirection.DESC Then 
        pIsFiltered = True 
      End If 
    End If 
  ...
  Private Sub lblBack_Click(sender As System.Object, e As System.EventArgs) Handles lblBack.Click 
    ...
    If _NestedFormsCount > 0 Then 
      pnlLanguage.Controls(0).SendToBack() 
      pFault = ActivateControl(pnlLanguage.Controls(0).Name) 
      If pFault.isOK = False Then ShowFault(pFault, _Requester) 
      pnlCover.SendToBack() 
      _NestedFormsCount -= 1 
    End If 
  ...
  Private Sub _ctlLanguageCol_evtTimerTripped() Handles _ctlLanguageCol.evtTimerTripped 
    Dim pCancel As Boolean = False 
    RaiseEvent evtLanguageTimerTripped(pCancel) 
    If pCancel = True Then Exit Sub 
 
    'get the last one 
    Dim pLastID As Long = _ctlLanguageCol.LanguageCol(0).ID 
    btnRefresh_Click(Me, New EventArgs) 
    Dim pNewLastID As Long = _ctlLanguageCol.LanguageCol(0).ID 
  ...
  Private Function CreateGridTitle(ByVal vShowingRowsText As String) As String 
    ...
    If _LanguageCol.Count = 0 Then Return vShowingRowsText & ". " 
    Dim pFoundFilters As Boolean = False 
    For Each l In _SearchFilters 
      Dim pFieldName As String = l.Key.ToString() 
      Dim pTestRow As New csLanguage() 
      ...
      ElseIf pFieldName.EndsWith("ID") Then 
        Dim pCol As csLanguageCol = CType(CallByName(_LanguageCol, "CloneBy" & pFieldName, CallType.Method, l.Value), csLanguageCol) 
      ...
      ElseIf Not (pFieldName.Equals("HowMany") OrElse pFieldName.Equals("Dir") OrElse 
                  ...
                  ccHelper.GetPropertyTypeName(New csLanguageCol(), $"{pFieldName}Text") = "") Then 
        Dim pCol As csLanguageCol = CType(CallByName(_LanguageCol, "CloneBy" & pFieldName, CallType.Method, l.Value), csLanguageCol) 
        If Not (pCol Is Nothing OrElse pCol.Count = 0) Then 
  ...
  Private Sub cc_ctlPnlLanguage_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
