Public Class ctlc_SystemAuditCol
  ...
  Public Event evtRowClicked(ByVal vSystemAudit As csSystemAudit) 
  Public Event evtRowDoubleClicked(ByVal vSystemAudit As csSystemAudit, ByRef rHandled As Boolean) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of csSystemAudit.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property ColumnsReadOnly As List(Of csSystemAudit.enmProperty) 
    Public Property ColumnsHide As List(Of csSystemAudit.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csSystemAudit.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csSystemAudit.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csSystemAudit.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csSystemAudit.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of csSystemAudit.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _ColumnsReadOnly = New List(Of csSystemAudit.enmProperty) 
      _ColumnsHide = New List(Of csSystemAudit.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csSystemAudit.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csSystemAudit.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csSystemAudit.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csSystemAudit.enmProperty, String) 
  ...
  Private WithEvents _SystemAuditCol As csSystemAuditCol
  Private WithEvents _SystemAuditColFullLength As csSystemAuditCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedSystemAudit]() As csSystemAudit 
    Get 
      If dgvSystemAudit.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvSystemAudit.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvSystemAudit.Rows.Count - 1 Then dgvSystemAudit.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _SystemAuditCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [SystemAuditCol]() As csSystemAuditCol 
    Get 
      Return _SystemAuditCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pSystemAuditCol As New csSystemAuditCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pSystemAuditCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pSystemAuditCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vSystemAuditCol As csSystemAuditCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vSystemAuditCol) 
  End Function
  
  Private Function LoadControl(vSystemAuditCol As csSystemAuditCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csSystemAudit.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csSystemAudit.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csSystemAudit.enmProperty.ID Then colID.ReadOnly = True 
      If l = csSystemAudit.enmProperty.TableName Then colTableName.ReadOnly = True 
      If l = csSystemAudit.enmProperty.RowId Then colRowId.ReadOnly = True 
      If l = csSystemAudit.enmProperty.Operation Then colOperation.ReadOnly = True 
      If l = csSystemAudit.enmProperty.OccurredAt Then colOccurredAt.ReadOnly = True 
      If l = csSystemAudit.enmProperty.SqlCurrentUser Then colSqlCurrentUser.ReadOnly = True 
      If l = csSystemAudit.enmProperty.ChangedByUser Then colChangedByUser.ReadOnly = True 
      If l = csSystemAudit.enmProperty.ActiveLoginID Then colActiveLoginID.ReadOnly = True 
      If l = csSystemAudit.enmProperty.SqlSystemUser Then colSqlSystemUser.ReadOnly = True 
      If l = csSystemAudit.enmProperty.SqlAppName Then colSqlAppName.ReadOnly = True 
      If l = csSystemAudit.enmProperty.SqlHostName Then colSqlHostName.ReadOnly = True 
      If l = csSystemAudit.enmProperty.Changes Then colChanges.ReadOnly = True 
    Next 
    ...
    dgvSystemAudit.DoubleBuffered(True) 
 
    pFault = vSystemAuditCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _SystemAuditColFullLength = vSystemAuditCol.Clone() 
 
    'Truncate the strings 
    _SystemAuditCol = vSystemAuditCol 
    If _LoadParameters.TruncateStrings Then 
      _SystemAuditCol.TruncateStrings() 
    Else 
      dgvSystemAudit.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvSystemAudit.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvSystemAudit.Columns 
    ...
    'Hide columns  
    For Each p As csSystemAudit.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvSystemAudit.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvSystemAudit.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvSystemAudit.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvSystemAudit.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvSystemAudit.ClearSelection()
    bsCtlSystemAudit.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-SystemAudit-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvSystemAudit.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvSystemAudit_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvSystemAudit.SelectedRows.Count > 0 Then 
      pRowIndex = dgvSystemAudit.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvSystemAudit.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvSystemAudit.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlSystemAudit.DataSource = Nothing 
    bsCtlSystemAudit.DataSource = _SystemAuditCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvSystemAudit.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _SystemAuditCol.Count - 2 Then 
          dgvSystemAudit.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _SystemAuditCol.Count - 1 Then 
          dgvSystemAudit.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    dgvSystemAudit.EditMode = DataGridViewEditMode.EditProgrammatically 
    dgvSystemAudit.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
    dgvSystemAudit.AllowUserToDeleteRows = False 
    dgvSystemAudit.AllowUserToAddRows = False 
    If _SystemAuditCol.Count = 0 Then 
    ...
    lblStatus.Text = "" 
    dgvSystemAudit.Refresh() 
  End Sub
  'ExternalButtons 
  'CellFormatting  
  Private Sub dgvSystemAudit_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvSystemAudit.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvSystemAudit.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvSystemAudit.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvSystemAudit.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pSystemAudit Is Nothing Then pSystemAudit = CType(dgvSystemAudit.Rows(e.RowIndex).DataBoundItem, csSystemAudit) ' Only assign it if needed 
    '  If pSystemAudit.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pSystemAudit.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvSystemAudit.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pSystemAudit Is Nothing Then pSystemAudit = CType(dgvSystemAudit.Rows(e.RowIndex).DataBoundItem, csSystemAudit) ' Only assign it if needed
    '  If pSystemAudit.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pSystemAudit.RAV - pSystemAudit.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvSystemAudit.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvSystemAudit.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvSystemAudit.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvSystemAudit.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvSystemAudit.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvSystemAudit.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvSystemAudit.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvSystemAudit_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvSystemAudit.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvSystemAudit_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvSystemAudit.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvSystemAudit.Columns(e.ColumnIndex)
    If bsCtlSystemAudit.Current Is Nothing Then Exit Sub
    ...
    dgvSystemAudit.SuspendLayout()

    Dim pSystemAudit As csSystemAudit
    Dim pID As Long = 0 
    If dgvSystemAudit.SelectedRows.Count > 0 Then 
    pSystemAudit = CType(bsCtlSystemAudit.Current, csSystemAudit)
      pID = pSystemAudit.ID 
    End If 
    ...
    Dim pSystemAuditCol As csSystemAuditCol
    pSystemAuditCol = CType(bsCtlSystemAudit.DataSource, csSystemAuditCol)

    Dim pSummaryRow As csSystemAudit = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pSystemAuditCol(pSystemAuditCol.Count - 1) 
      pSystemAuditCol.RemoveAt(pSystemAuditCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pSystemAuditCol.Count - 1 
          pSystemAuditCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pSystemAuditCol.SortByID()
      ElseIf pNewColumn Is colTableName Then
        pSystemAuditCol.SortByTableName()
      ElseIf pNewColumn Is colRowId Then
        pSystemAuditCol.SortByRowId()
      ElseIf pNewColumn Is colOperation Then
        pSystemAuditCol.SortByOperation()
      ElseIf pNewColumn Is colOccurredAt Then
        pSystemAuditCol.SortByOccurredAt()
      ElseIf pNewColumn Is colSqlCurrentUser Then
        pSystemAuditCol.SortBySqlCurrentUser()
      ElseIf pNewColumn Is colChangedByUser Then
        pSystemAuditCol.SortByChangedByUser()
      ElseIf pNewColumn Is colActiveLoginID Then
        pSystemAuditCol.SortByActiveLoginID()
      ElseIf pNewColumn Is colSqlSystemUser Then
        pSystemAuditCol.SortBySqlSystemUser()
      ElseIf pNewColumn Is colSqlAppName Then
        pSystemAuditCol.SortBySqlAppName()
      ElseIf pNewColumn Is colSqlHostName Then
        pSystemAuditCol.SortBySqlHostName()
      ElseIf pNewColumn Is colChanges Then
        pSystemAuditCol.SortByChanges()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csSystemAudit In pSystemAuditCol 
        ...
        ElseIf pNewColumn Is colChanges Then
          Dim pTest As String = "" 
          For Each p As csSystemAudit In pSystemAuditCol 
            If p.Changes <> pTest Then iCntr += 1 : pTest = p.Changes 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pSystemAuditCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pSystemAuditCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pSystemAuditCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlSystemAudit.Position = bsCtlSystemAudit.IndexOf(pSystemAuditCol.FindByID(pID))
    End If

    'dgvSystemAudit.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvSystemAudit.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pSystemAuditCol As csSystemAuditCol 
      pSystemAuditCol = CType(bsCtlSystemAudit.DataSource, csSystemAuditCol) 
      Dim pSystemAudit As csSystemAudit = pSystemAuditCol.FindByID(pID) 
      If Not pSystemAudit.IsEmpty Then 
        bsCtlSystemAudit.Position = bsCtlSystemAudit.IndexOf(pSystemAuditCol.FindByID(pID)) 
        dgvSystemAudit.Rows(bsCtlSystemAudit.Position).Selected = True 
      Else 
        dgvSystemAudit.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvSystemAudit_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvSystemAudit.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvSystemAudit.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvSystemAudit_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvSystemAudit.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvSystemAudit_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvSystemAudit.Scroll
    dgvSystemAudit.Invalidate() 
  End Sub
 
  Private Sub dgvSystemAudit_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvSystemAudit.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvSystemAudit.Rows.Count - 1 Then Exit Sub
 
    'If dgvSystemAudit.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'SystemAudit', the row with an ID of " & dgvSystemAudit.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'SystemAudit', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvSystemAudit.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvSystemAudit.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-SystemAudit-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvSystemAudit(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvSystemAudit(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvSystemAudit_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvSystemAudit.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvSystemAudit_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvSystemAudit.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvSystemAudit_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvSystemAudit.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvSystemAudit.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvSystemAudit(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pSystemAudit As csSystemAudit = _SystemAuditCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pSystemAudit, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "SystemAudit Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_SystemAudit", pSystemAudit, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvSystemAudit_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvSystemAudit.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvSystemAudit.Rows.Count - 1 Then dgvSystemAudit.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pSystemAudit As csSystemAudit = _SystemAuditCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pSystemAudit) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvSystemAudit_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvSystemAudit.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvSystemAudit.Columns 
  ...
  Private Sub Summarize() 
    If _SystemAuditCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
    ...
    For Each pExistingRow As csSystemAudit In _SystemAuditCol 
      ...
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csSystemAudit.enmSummarizeableProperty.RowId) = csSystemAudit.enmSummarizeableProperty.RowId Then pRowId = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csSystemAudit.enmSummarizeableProperty.ActiveLoginID) = csSystemAudit.enmSummarizeableProperty.ActiveLoginID Then pActiveLoginID = 0
    Dim pSummaryRow As New csSystemAudit( _ 
        vID:=0 _ 
      , vTableName:="" _ 
      , vRowId:=pRowId _ 
      , vOperation:="" _ 
    ...
    _SystemAuditCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\SystemAuditCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\SystemAuditCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\SystemAuditCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\SystemAuditCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\SystemAuditCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvSystemAudit.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvSystemAudit.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _SystemAuditColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _SystemAuditColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _SystemAuditColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _SystemAuditColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _SystemAuditColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-SystemAudit-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "SystemAudits" 
        .SubTitleRight = "Rows: " & _SystemAuditCol.Count.ToString 
        ...
        End If 
        .DataSource = _SystemAuditCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-SystemAudit-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-SystemAudit-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvSystemAudit.Rows 
    ...
    For Each row As DataGridViewRow In dgvSystemAudit.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlSystemAudit), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvSystemAudit.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-SystemAudit-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvSystemAudit.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvSystemAudit.Width - 30) / dgvSystemAudit.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'colChanges.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvSystemAudit.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-SystemAudit-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colChanges.HeaderText = pStrg : mnuColVisibleChanges.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleTableName.Click, mnuColVisibleRowId.Click, mnuColVisibleOperation.Click, mnuColVisibleOccurredAt.Click, mnuColVisibleSqlCurrentUser.Click, mnuColVisibleChangedByUser.Click, mnuColVisibleActiveLoginID.Click, mnuColVisibleSqlSystemUser.Click, mnuColVisibleSqlAppName.Click, mnuColVisibleSqlHostName.Click, mnuColVisibleChanges.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvSystemAudit.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvSystemAudit.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleChanges.Checked = True Then mnuColVisibleChanges.PerformClick() 
    
    _Loading = False 
    'dgvSystemAudit.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvSystemAudit_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvSystemAudit.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the SystemAudit to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvSystemAudit_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvSystemAudit.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvSystemAudit.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvSystemAudit.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvSystemAudit.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvSystemAudit.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvSystemAudit(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvSystemAudit.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvSystemAudit.BeginEdit(True) 
        DirectCast(dgvSystemAudit.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvSystemAudit_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvSystemAudit.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvSystemAudit.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvSystemAudit.MultiSelect = True 
        dgvSystemAudit.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvSystemAudit.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvSystemAudit.MultiSelect = True 
    Else 
      dgvSystemAudit.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlc_SystemAuditCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvSystemAudit.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_SystemAuditCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvSystemAudit.Width 
  ...
  Private Sub ctlc_SystemAuditCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub dgvSystemAudit_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvSystemAudit.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvSystemAudit.Rows(e.RowIndex).Selected Then 
        dgvSystemAudit.ClearSelection() 
        dgvSystemAudit.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvSystemAudit.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvSystemAudit.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvSystemAudit.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _SystemAuditCol.Count Then Exit Sub 
    Dim pSystemAudit As csSystemAudit = _SystemAuditCol(pRowIndex) 
    Dim pTitle As String = "SystemAudit #" & pSystemAudit.ID.ToString() 
    Dim pKey As String = "SystemAudit_" & pSystemAudit.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_SystemAudit" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pSystemAudit, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pSystemAudit 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "SystemAudit_" & pSystemAudit.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "SystemAudit #" & pSystemAudit.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_SystemAudit", pSystemAudit, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvSystemAudit.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvSystemAudit.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _SystemAuditCol.Count Then 
        Dim pSystemAudit As csSystemAudit = _SystemAuditCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pSystemAudit.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvSystemAudit.SelectedRows(0) 
