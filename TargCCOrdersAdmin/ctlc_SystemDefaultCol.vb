Public Class ctlc_SystemDefaultCol
  ...
  Public Event evtUpdated(ByVal vSystemDefault As csSystemDefault) 
  Private Event evtBeforeDelete(ByVal vSystemDefault As csSystemDefault, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vSystemDefault As csSystemDefault) 
  Public Event evtRowDoubleClicked(ByVal vSystemDefault As csSystemDefault, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As csSystemDefault.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property CbosDoNotLoad As List(Of csSystemDefault.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of csSystemDefault.enmProperty) 
    Public Property ColumnsHide As List(Of csSystemDefault.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csSystemDefault.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csSystemDefault.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csSystemDefault.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csSystemDefault.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of csSystemDefault.enmParentProperty) 
      _ColumnsReadOnly = New List(Of csSystemDefault.enmProperty) 
      _ColumnsHide = New List(Of csSystemDefault.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csSystemDefault.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csSystemDefault.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csSystemDefault.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csSystemDefault.enmProperty, String) 
  ...
  Private WithEvents _SystemDefaultCol As csSystemDefaultCol
  Private WithEvents _SystemDefaultColFullLength As csSystemDefaultCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedSystemDefault]() As csSystemDefault 
    Get 
      If dgvSystemDefault.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvSystemDefault.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvSystemDefault.Rows.Count - 1 Then dgvSystemDefault.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _SystemDefaultCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [SystemDefaultCol]() As csSystemDefaultCol 
    Get 
      Return _SystemDefaultCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pSystemDefaultCol As New csSystemDefaultCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pSystemDefaultCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pSystemDefaultCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vSystemDefaultCol As csSystemDefaultCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vSystemDefaultCol) 
  End Function
  
  Private Function LoadControl(vSystemDefaultCol As csSystemDefaultCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csSystemDefault.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csSystemDefault.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csSystemDefault.enmProperty.ID Then colID.ReadOnly = True 
      If l = csSystemDefault.enmProperty.Group Then colGroup.ReadOnly = True 
      If l = csSystemDefault.enmProperty.SettingName Then colSettingName.ReadOnly = True 
      If l = csSystemDefault.enmProperty.SettingValue Then colSettingValue.ReadOnly = True 
      If l = csSystemDefault.enmProperty.SystemDefaultType Then colSystemDefaultType.ReadOnly = True 
      If l = csSystemDefault.enmProperty.Description Then colDescription.ReadOnly = True 
    Next 
    ...
    dgvSystemDefault.DoubleBuffered(True) 
 
    pFault = vSystemDefaultCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _SystemDefaultColFullLength = vSystemDefaultCol.Clone() 
 
    'Truncate the strings 
    _SystemDefaultCol = vSystemDefaultCol 
    If _LoadParameters.TruncateStrings Then 
      _SystemDefaultCol.TruncateStrings() 
    Else 
      dgvSystemDefault.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvSystemDefault.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvSystemDefault.Columns 
    ...
    'Hide columns  
    For Each p As csSystemDefault.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvSystemDefault.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvSystemDefault.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvSystemDefault.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvSystemDefault.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvSystemDefault.ClearSelection()
    bsCtlSystemDefault.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-SystemDefault-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvSystemDefault.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvSystemDefault_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvSystemDefault.SelectedRows.Count > 0 Then 
      pRowIndex = dgvSystemDefault.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvSystemDefault.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvSystemDefault.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlSystemDefault.DataSource = Nothing 
    bsCtlSystemDefault.DataSource = _SystemDefaultCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvSystemDefault.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _SystemDefaultCol.Count - 2 Then 
          dgvSystemDefault.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _SystemDefaultCol.Count - 1 Then 
          dgvSystemDefault.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'EnumSystemDefaultType
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(csSystemDefault.enmParentProperty.SystemDefaultType, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.SystemDefaultType, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmSystemDefaultType.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmSystemDefaultType.UD, pPrompt) 
    bsSystemDefaultType.DataSource = pEnumCol 
    colSystemDefaultType.Tag = pPrompt 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_SystemDefaultUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_SystemDefaultUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_SystemDefaultDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      colSettingValue.ReadOnly = True 
      dgvSystemDefault.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvSystemDefault.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvSystemDefault.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvSystemDefault.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvSystemDefault.AllowUserToDeleteRows = False 
      dgvSystemDefault.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvSystemDefault.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvSystemDefault.CurrentRow.Index 
      '  dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(pCurrentRow).Cells(0) 
      '  dgvSystemDefault.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
    ...
    Else 
      If _SystemDefaultCol.Count = 0 Then 
        btnSpreadsheet.Enabled = False 
        btnReport.Enabled = False 
      Else 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("Group, ") 
    pFieldList.Append("SettingName, ") 
    pFieldList.Append("SystemDefaultType (DB Code), ") 
    pFieldList.Append("Description, ") 
    
    Dim pNumberOfFields As Integer = 5 
    ...
    pMessage &= Environment.NewLine 
    pMessage &= Environment.NewLine 
    pMessage &= "If there is no ID field (the 1st field is Group), then I will delete the table and recreate it with the data in this spreadsheet" 
    pMessage &= Environment.NewLine 
    pMessage &= Environment.NewLine 
    ...
    'Now Load the data 
    Dim pIncomingSystemDefaults As New csSystemDefaultCol() 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingSystemDefault As New csSystemDefault() 
          pIncomingSystemDefault.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = csSystemDefaultCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingSystemDefault.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingSystemDefaults.Add(pIncomingSystemDefault) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingSystemDefault.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingSystemDefault.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Group" 
            pIncomingSystemDefault.Group = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingSystemDefault.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "SettingName" 
            pIncomingSystemDefault.SettingName = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingSystemDefault.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "SystemDefaultType" 
            pIncomingSystemDefault.SystemDefaultType = clsEnums.TranslateEnmSystemDefaultType(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingSystemDefault.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingSystemDefault.Description = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingSystemDefault.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingSystemDefault.Tag = "Row " & pRow.ToString Then 
            pIncomingSystemDefault.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingSystemDefaults.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingSystemDefaults.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingSystemDefaults 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _SystemDefaultCol = pIncomingSystemDefaults 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvSystemDefault.Focused = True AndAlso dgvSystemDefault.SelectedRows.Count > 0 Then 
      pCellRow = dgvSystemDefault.CurrentCell.RowIndex 
      pCellCol = dgvSystemDefault.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(0).Cells(0) 
      dgvSystemDefault.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _SystemDefaultCol.Count > 0 AndAlso _SystemDefaultCol(_SystemDefaultCol.Count - 1).ID = 0 Then 
      _SystemDefaultCol.RemoveAt(_SystemDefaultCol.Count - 1) 
      bsCtlSystemDefault.DataSource = Nothing 
      bsCtlSystemDefault.DataSource = _SystemDefaultCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvSystemDefault.Focus() 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(pCellRow).Cells(pCellCol) 
      dgvSystemDefault.CurrentCell.Selected = True 
    ElseIf _SystemDefaultCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlSystemDefault.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As csSystemDefault 
    pEntity = CType(bsCtlSystemDefault.Current, csSystemDefault) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvSystemDefault.CurrentCell Is Nothing Then Return pFault 
    
    If dgvSystemDefault.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pSystemDefault As csSystemDefault 
      pSystemDefault = CType(bsCtlSystemDefault.Current, csSystemDefault) 
      If pSystemDefault Is Nothing Then 
        pFault.LogFreeTextFault("There is no SystemDefault to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvSystemDefault.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvSystemDefault.CurrentCell.RowIndex 
      'show row as selected  
      dgvSystemDefault.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvSystemDefault.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvSystemDefault.CurrentRow.Selected = True 
      If pSystemDefault.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pSystemDefault.Group & "_" & pSystemDefault.SettingName & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pSystemDefault, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvSystemDefault.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvSystemDefault.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvSystemDefault.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pSystemDefault.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlSystemDefault.Remove(bsCtlSystemDefault.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pSystemDefault As csSystemDefault = CType(bsCtlSystemDefault.Current, csSystemDefault) 
    If pSystemDefault IsNot Nothing Then 
      If pSystemDefault.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _SystemDefaultCol.Count > 0 AndAlso dgvSystemDefault.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvSystemDefault.Columns.Count - 1 
        If dgvSystemDefault.Columns(i).Visible Then 
          dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(dgvSystemDefault.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvSystemDefault.Refresh() 
      dgvSystemDefault.Rows(dgvSystemDefault.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvSystemDefault.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvSystemDefault_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvSystemDefault.RowValidating 
    If _Loading = True OrElse dgvSystemDefault.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvSystemDefault_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvSystemDefault.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvSystemDefault.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvSystemDefault.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvSystemDefault.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pSystemDefault Is Nothing Then pSystemDefault = CType(dgvSystemDefault.Rows(e.RowIndex).DataBoundItem, csSystemDefault) ' Only assign it if needed 
    '  If pSystemDefault.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pSystemDefault.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvSystemDefault.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pSystemDefault Is Nothing Then pSystemDefault = CType(dgvSystemDefault.Rows(e.RowIndex).DataBoundItem, csSystemDefault) ' Only assign it if needed
    '  If pSystemDefault.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pSystemDefault.RAV - pSystemDefault.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvSystemDefault.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvSystemDefault.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvSystemDefault.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvSystemDefault.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvSystemDefault.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvSystemDefault.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvSystemDefault.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvSystemDefault_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvSystemDefault.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvSystemDefault_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvSystemDefault.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvSystemDefault.Columns(e.ColumnIndex)
    If bsCtlSystemDefault.Current Is Nothing Then Exit Sub
    ...
    Cursor = Cursors.WaitCursor
    dgvSystemDefault.SuspendLayout()

    Dim pSystemDefault As csSystemDefault
    Dim pID As Long = 0 
    If dgvSystemDefault.SelectedRows.Count > 0 Then 
    pSystemDefault = CType(bsCtlSystemDefault.Current, csSystemDefault)
      pID = pSystemDefault.ID 
    End If 
    ...
    Dim pSystemDefaultCol As csSystemDefaultCol
    pSystemDefaultCol = CType(bsCtlSystemDefault.DataSource, csSystemDefaultCol)

    Dim pSummaryRow As csSystemDefault = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pSystemDefaultCol(pSystemDefaultCol.Count - 1) 
      pSystemDefaultCol.RemoveAt(pSystemDefaultCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pSystemDefaultCol.Count - 1 
          pSystemDefaultCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pSystemDefaultCol.SortByID()
      ElseIf pNewColumn Is colGroup Then
        pSystemDefaultCol.SortByGroup()
      ElseIf pNewColumn Is colSettingName Then
        pSystemDefaultCol.SortBySettingName()
      ElseIf pNewColumn Is colSettingValue Then
        pSystemDefaultCol.SortBySettingValue()
      ElseIf pNewColumn Is colSystemDefaultType Then
        pSystemDefaultCol.SortBySystemDefaultType()
      ElseIf pNewColumn Is colDescription Then
        pSystemDefaultCol.SortByDescription()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csSystemDefault In pSystemDefaultCol 
        ...
        ElseIf pNewColumn Is colGroup Then
          Dim pTest As String = "" 
          For Each p As csSystemDefault In pSystemDefaultCol 
            If p.Group <> pTest Then iCntr += 1 : pTest = p.Group 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colSettingName Then
          Dim pTest As String = "" 
          For Each p As csSystemDefault In pSystemDefaultCol 
            If p.SettingName <> pTest Then iCntr += 1 : pTest = p.SettingName 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colSettingValue Then
          Dim pTest As String = "" 
          For Each p As csSystemDefault In pSystemDefaultCol 
            If p.SettingValue <> pTest Then iCntr += 1 : pTest = p.SettingValue 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colSystemDefaultType Then
          Dim pTest As clsEnums.enmSystemDefaultType = clsEnums.enmSystemDefaultType.UD 
          For Each p As csSystemDefault In pSystemDefaultCol 
            If p.SystemDefaultType <> pTest Then iCntr += 1 : pTest = p.SystemDefaultType 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDescription Then
          Dim pTest As String = "" 
          ...
          Next 
        End If 
        pSystemDefaultCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pSystemDefaultCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pSystemDefaultCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlSystemDefault.Position = bsCtlSystemDefault.IndexOf(pSystemDefaultCol.FindByID(pID))
    End If

    'dgvSystemDefault.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvSystemDefault.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pSystemDefaultCol As csSystemDefaultCol 
      pSystemDefaultCol = CType(bsCtlSystemDefault.DataSource, csSystemDefaultCol) 
      Dim pSystemDefault As csSystemDefault = pSystemDefaultCol.FindByID(pID) 
      If Not pSystemDefault.IsEmpty Then 
        bsCtlSystemDefault.Position = bsCtlSystemDefault.IndexOf(pSystemDefaultCol.FindByID(pID)) 
        dgvSystemDefault.Rows(bsCtlSystemDefault.Position).Selected = True 
      Else 
        dgvSystemDefault.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvSystemDefault_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvSystemDefault.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvSystemDefault.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvSystemDefault_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvSystemDefault.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvSystemDefault_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvSystemDefault.Scroll
    dgvSystemDefault.Invalidate() 
  End Sub
 
  Private Sub dgvSystemDefault_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvSystemDefault.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvSystemDefault.Rows.Count - 1 Then Exit Sub
 
    'If dgvSystemDefault.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'SystemDefault', the row with an ID of " & dgvSystemDefault.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'SystemDefault', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvSystemDefault.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvSystemDefault.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-SystemDefault-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvSystemDefault(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvSystemDefault(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvSystemDefault_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvSystemDefault.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvSystemDefault.CurrentCell = dgvSystemDefault.Rows(dgvSystemDefault.CurrentCell.RowIndex).Cells(0) 
      dgvSystemDefault.Refresh() 
    End If 
  End Sub 
  Private Sub dgvSystemDefault_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvSystemDefault.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvSystemDefault_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvSystemDefault.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvSystemDefault_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvSystemDefault.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvSystemDefault.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvSystemDefault(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pSystemDefault As csSystemDefault = _SystemDefaultCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pSystemDefault, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "SystemDefault Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_SystemDefault", pSystemDefault, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvSystemDefault_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvSystemDefault.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvSystemDefault.Rows.Count - 1 Then dgvSystemDefault.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pSystemDefault As csSystemDefault = _SystemDefaultCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pSystemDefault) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvSystemDefault_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvSystemDefault.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvSystemDefault.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvSystemDefault.Columns(pNewCol).Visible = False Then 
      dgvSystemDefault.Columns(pNewCol).Visible = True 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.CurrentRow.Cells(pNewCol) 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.CurrentRow.Cells(pOriginalCol) 
      dgvSystemDefault.Columns(pNewCol).Visible = False 
    Else 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.CurrentRow.Cells(pNewCol) 
      dgvSystemDefault.CurrentCell = dgvSystemDefault.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pSystemDefault, csSystemDefault), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pSystemDefault.Update(_Requester) 
    ...
    Else 
      If pFault.isOK = False Then 'AndAlso pFault.Severity = clsEnums.enmFaultSeverity.LogOnly  
        ShowFault(pFault, _Requester) 
      End If 
      dgvSystemDefault.EndEdit() 
      _DVGDirty = False 
      'Reset the SystemDefault collection 
      MyCache.ClearComboList(clsEnums.enmComboListType.c_SystemDefaultDefaultByID) 
      RaiseEvent evtUpdated(pSystemDefault) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvSystemDefault.Columns 
  ...
  Private Sub Summarize() 
    If _SystemDefaultCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As csSystemDefault In _SystemDefaultCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\SystemDefaultCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\SystemDefaultCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\SystemDefaultCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\SystemDefaultCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\SystemDefaultCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvSystemDefault.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvSystemDefault.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _SystemDefaultColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _SystemDefaultColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _SystemDefaultColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _SystemDefaultColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _SystemDefaultColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-SystemDefault-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "SystemDefaults" 
        .SubTitleRight = "Rows: " & _SystemDefaultCol.Count.ToString 
        ...
        End If 
        .DataSource = _SystemDefaultCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-SystemDefault-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-SystemDefault-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvSystemDefault.Rows 
    ...
    For Each row As DataGridViewRow In dgvSystemDefault.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlSystemDefault), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvSystemDefault.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-SystemDefault-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvSystemDefault.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvSystemDefault.Width - 30) / dgvSystemDefault.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'Set Wrap 
    'colSettingValue.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvSystemDefault.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-SystemDefault-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colGroup.HeaderText = pStrg : mnuColVisibleGroup.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_SystemDefault", "SettingName", _Requester) 
    If pStrg <> "" Then colSettingName.HeaderText = pStrg : mnuColVisibleSettingName.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_SystemDefault", "SettingValue", _Requester) 
    If pStrg <> "" Then colSettingValue.HeaderText = pStrg : mnuColVisibleSettingValue.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_SystemDefault", "SystemDefaultType", _Requester) 
    If pStrg <> "" Then colSystemDefaultType.HeaderText = pStrg : mnuColVisibleSystemDefaultType.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleGroup.Click, mnuColVisibleSettingName.Click, mnuColVisibleSettingValue.Click, mnuColVisibleSystemDefaultType.Click, mnuColVisibleDescription.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvSystemDefault.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvSystemDefault.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleGroup.Checked = True Then mnuColVisibleGroup.PerformClick() 
    If mnuColVisibleSettingName.Checked = True Then mnuColVisibleSettingName.PerformClick() 
    If mnuColVisibleSettingValue.Checked = True Then mnuColVisibleSettingValue.PerformClick() 
    If mnuColVisibleSystemDefaultType.Checked = True Then mnuColVisibleSystemDefaultType.PerformClick() 
    If mnuColVisibleDescription.Checked = True Then mnuColVisibleDescription.PerformClick() 
    'Show Defaults 
    If mnuColVisibleGroup.Checked = False Then mnuColVisibleGroup.PerformClick() 
    If mnuColVisibleSettingName.Checked = False Then mnuColVisibleSettingName.PerformClick() 
    
    _Loading = False 
    'dgvSystemDefault.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvSystemDefault_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvSystemDefault.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the SystemDefault to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvSystemDefault_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvSystemDefault.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvSystemDefault.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvSystemDefault.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvSystemDefault.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvSystemDefault.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvSystemDefault(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvSystemDefault.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvSystemDefault.BeginEdit(True) 
        DirectCast(dgvSystemDefault.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvSystemDefault_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvSystemDefault.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvSystemDefault.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvSystemDefault.MultiSelect = True 
        dgvSystemDefault.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvSystemDefault.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvSystemDefault.MultiSelect = True 
    Else 
      dgvSystemDefault.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlc_SystemDefaultCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvSystemDefault.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_SystemDefaultCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvSystemDefault.Width 
  ...
  Private Sub ctlc_SystemDefaultCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlc_SystemDefaultCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
    If _Requester Is Nothing Then Return 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    DoCeaseEdit() 
  End Sub 
  Private Sub cc_ctlc_SystemDefaultCol_evtBeforeLoad() Handles Me.evtBeforeLoad 
    _LoadParameters.ReadOnly = True 
    _LoadParameters.ColumnsHide.Add(csSystemDefault.enmProperty.ID) 
  End Sub 
  Private Sub cc_ctlc_SystemDefaultCol_evtLoaded() Handles Me.evtLoaded 
    'Sample initial sort. This sample sorts on Columns 1st, 2nd then 3rd  
    Dim pE As New System.Windows.Forms.DataGridViewCellMouseEventArgs(colSettingName.Index, 0, 0, 0, New System.Windows.Forms.MouseEventArgs(System.Windows.Forms.MouseButtons.Left, 0, 0, 0, 0)) 
    dgvSystemDefault_ColumnHeaderMouseClick(Me, pE) 
    pE = New System.Windows.Forms.DataGridViewCellMouseEventArgs(colGroup.Index, 0, 0, 0, New System.Windows.Forms.MouseEventArgs(System.Windows.Forms.MouseButtons.Left, 0, 0, 0, 0)) 
    dgvSystemDefault_ColumnHeaderMouseClick(Me, pE) 
  End Sub 
  Private Sub cc_ctlc_SystemDefaultCol_evtCellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles Me.evtCellFormatting 
    If dgvSystemDefault.Columns(e.ColumnIndex).DataPropertyName.Equals("SettingValue", StringComparison.OrdinalIgnoreCase) Then 
      If dgvSystemDefault.Rows(e.RowIndex).Cells(dgvSystemDefault.Columns("colSystemDefaultType").Index).Value.ToString().Equals("encrypted", StringComparison.OrdinalIgnoreCase) Then 
        e.Value = "*****" 
      End If 
    End If 
  End Sub 
  ...
  Private Sub dgvSystemDefault_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvSystemDefault.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvSystemDefault.Rows(e.RowIndex).Selected Then 
        dgvSystemDefault.ClearSelection() 
        dgvSystemDefault.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvSystemDefault.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvSystemDefault.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvSystemDefault.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _SystemDefaultCol.Count Then Exit Sub 
    Dim pSystemDefault As csSystemDefault = _SystemDefaultCol(pRowIndex) 
    Dim pTitle As String = "SystemDefault #" & pSystemDefault.ID.ToString() 
    Dim pKey As String = "SystemDefault_" & pSystemDefault.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_SystemDefault" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pSystemDefault, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pSystemDefault 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "SystemDefault_" & pSystemDefault.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "SystemDefault #" & pSystemDefault.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_SystemDefault", pSystemDefault, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvSystemDefault.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvSystemDefault.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _SystemDefaultCol.Count Then 
        Dim pSystemDefault As csSystemDefault = _SystemDefaultCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pSystemDefault.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvSystemDefault.SelectedRows(0) 
