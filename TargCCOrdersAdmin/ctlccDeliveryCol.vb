Public Class ctlccDeliveryCol
  ...
  Public Event evtUpdated(ByVal vDelivery As clsDelivery) 
  Private Event evtBeforeDelete(ByVal vDelivery As clsDelivery, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vDelivery As clsDelivery) 
  Public Event evtRowDoubleClicked(ByVal vDelivery As clsDelivery, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As clsDelivery.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property CbosDoNotLoad As List(Of clsDelivery.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of clsDelivery.enmProperty) 
    Public Property ColumnsHide As List(Of clsDelivery.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of clsDelivery.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of clsDelivery.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of clsDelivery.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of clsDelivery.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of clsDelivery.enmParentProperty) 
      _ColumnsReadOnly = New List(Of clsDelivery.enmProperty) 
      _ColumnsHide = New List(Of clsDelivery.enmProperty) 
      _ColumnsFormat = New Dictionary(Of clsDelivery.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of clsDelivery.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of clsDelivery.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of clsDelivery.enmProperty, String) 
  ...
  Private WithEvents _DeliveryCol As clsDeliveryCol
  Private WithEvents _DeliveryColFullLength As clsDeliveryCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedDelivery]() As clsDelivery 
    Get 
      If dgvDelivery.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvDelivery.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvDelivery.Rows.Count - 1 Then dgvDelivery.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _DeliveryCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [DeliveryCol]() As clsDeliveryCol 
    Get 
      Return _DeliveryCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pDeliveryCol As New clsDeliveryCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pDeliveryCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pDeliveryCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pDeliveryCol As New clsDeliveryCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "OrderHeader" 
          pFault = pDeliveryCol.FillByOrderHeaderID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vDeliveryCol As clsDeliveryCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vDeliveryCol) 
  End Function
  
  Private Function LoadControl(vDeliveryCol As clsDeliveryCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of clsDelivery.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of clsDelivery.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = clsDelivery.enmProperty.ID Then colID.ReadOnly = True 
      If l = clsDelivery.enmProperty.OrderHeader Then colOrderHeader.ReadOnly = True 
      If l = clsDelivery.enmProperty.DeliveryAddress Then colDeliveryAddress.ReadOnly = True 
      If l = clsDelivery.enmProperty.ContactPhone Then colContactPhone.ReadOnly = True 
      If l = clsDelivery.enmProperty.ContactName Then colContactName.ReadOnly = True 
      If l = clsDelivery.enmProperty.DeliveryMethod Then colDeliveryMethod.ReadOnly = True 
      If l = clsDelivery.enmProperty.OrderedDate Then colOrderedDate.ReadOnly = True 
      If l = clsDelivery.enmProperty.ReceivedDate Then colReceivedDate.ReadOnly = True 
      If l = clsDelivery.enmProperty.ArrivalToHubDate Then colArrivalToHubDate.ReadOnly = True 
      If l = clsDelivery.enmProperty.ArrivalToCustomerDate Then colArrivalToCustomerDate.ReadOnly = True 
      If l = clsDelivery.enmProperty.DeliveryStatus Then colDeliveryStatus.ReadOnly = True 
      If l = clsDelivery.enmProperty.Location Then colLocation.ReadOnly = True 
      If l = clsDelivery.enmProperty.ProductsSummary Then colProductsSummary.ReadOnly = True 
      If l = clsDelivery.enmProperty.Notes Then colNotes.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As clsDelivery.enmParentProperty = clsDelivery.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of clsDelivery.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvDelivery.DoubleBuffered(True) 
 
    pFault = vDeliveryCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _DeliveryColFullLength = vDeliveryCol.Clone() 
 
    'Truncate the strings 
    _DeliveryCol = vDeliveryCol 
    If _LoadParameters.TruncateStrings Then 
      _DeliveryCol.TruncateStrings() 
    Else 
      dgvDelivery.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvDelivery.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        ...
        If colOrderHeader.DataGridView IsNot Nothing Then dgvDelivery.Columns.Remove(colOrderHeader) 
      Else 
        If colOrderHeader.ReadOnly = False Then 
          If colOrderHeaderText.DataGridView IsNot Nothing Then dgvDelivery.Columns.Remove(colOrderHeaderText) 
        Else 
          colOrderHeader.Name = colOrderHeader.Name & "zzzz" 
          colOrderHeaderText.Name = colOrderHeader.Name.Replace("zzzz", "") 
          If colOrderHeader.DataGridView IsNot Nothing Then dgvDelivery.Columns.Remove(colOrderHeader) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(clsDelivery.enmParentProperty.OrderHeader) Then 
            _LoadParameters.CbosDoNotLoad.Add(clsDelivery.enmParentProperty.OrderHeader) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvDelivery.Columns 
    ...
    'Hide columns  
    For Each p As clsDelivery.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvDelivery.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvDelivery.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvDelivery.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvDelivery.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    bsCtlDelivery.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Delivery-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvDelivery.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvDelivery_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvDelivery.SelectedRows.Count > 0 Then 
      pRowIndex = dgvDelivery.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvDelivery.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvDelivery.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlDelivery.DataSource = Nothing 
    bsCtlDelivery.DataSource = _DeliveryCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvDelivery.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _DeliveryCol.Count - 2 Then 
          dgvDelivery.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _DeliveryCol.Count - 1 Then 
          dgvDelivery.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'OrderHeader
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = clsDelivery.enmParentProperty.OrderHeader) = clsDelivery.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.ccOrderHeaderDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(clsDelivery.enmParentProperty.OrderHeader, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-DeliveryCol-200806-1015", _Requester) 
        End If 
      End If 
    ...
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsDelivery.enmParentProperty.DeliveryMethod, Nothing, Nothing, pEnumCol, pPrompt) 
    ...
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsDelivery.enmParentProperty.DeliveryStatus, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.DeliveryStatus, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmDeliveryStatus.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmDeliveryStatus.UD, pPrompt) 
    bsDeliveryStatus.DataSource = pEnumCol 
    colDeliveryStatus.Tag = pPrompt 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_DeliveryUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_DeliveryUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_DeliveryDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      colProductsSummary.ReadOnly = True 
      dgvDelivery.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvDelivery.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      ...
      'Don't automatically set the 1st one If dgvDelivery.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvDelivery.CurrentRow.Index 
      '  dgvDelivery.CurrentCell = dgvDelivery.Rows(pCurrentRow).Cells(0) 
      '  dgvDelivery.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("DeliveryAddress, ") 
    pFieldList.Append("ContactPhone, ") 
    pFieldList.Append("ContactName, ") 
    pFieldList.Append("DeliveryMethod (DB Code), ") 
    pFieldList.Append("OrderedDate, ") 
    pFieldList.Append("ReceivedDate, ") 
    pFieldList.Append("ArrivalToHubDate, ") 
    pFieldList.Append("ArrivalToCustomerDate, ") 
    pFieldList.Append("DeliveryStatus (DB Code), ") 
    pFieldList.Append("Location, ") 
    pFieldList.Append("Notes, ") 
    
    Dim pNumberOfFields As Integer = 13 
    ...
    'Now Load the data 
    Dim pIncomingDeliverys As New clsDeliveryCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingDelivery As New clsDelivery(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingDelivery.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = clsDeliveryCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingDeliverys.Add(pIncomingDelivery) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingDelivery.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            ...
            pIncomingDelivery.OrderHeaderID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "DeliveryAddress" 
            pIncomingDelivery.DeliveryAddress = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ContactPhone" 
            pIncomingDelivery.ContactPhone = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ContactName" 
            pIncomingDelivery.ContactName = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingDelivery.DeliveryMethod = clsEnums.TranslateEnmDeliveryMethod(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "OrderedDate" 
            pIncomingDelivery.OrderedDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ReceivedDate" 
            pIncomingDelivery.ReceivedDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ArrivalToHubDate" 
            pIncomingDelivery.ArrivalToHubDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ArrivalToCustomerDate" 
            pIncomingDelivery.ArrivalToCustomerDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "DeliveryStatus" 
            pIncomingDelivery.DeliveryStatus = clsEnums.TranslateEnmDeliveryStatus(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Location" 
            pIncomingDelivery.Location = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Notes" 
            pIncomingDelivery.Notes = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingDelivery.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingDelivery.Tag = "Row " & pRow.ToString Then 
            pIncomingDelivery.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingDeliverys.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingDeliverys.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingDeliverys 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _DeliveryCol = pIncomingDeliverys 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvDelivery.Focused = True AndAlso dgvDelivery.SelectedRows.Count > 0 Then 
      pCellRow = dgvDelivery.CurrentCell.RowIndex 
      pCellCol = dgvDelivery.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvDelivery.CurrentCell = dgvDelivery.Rows(0).Cells(0) 
      dgvDelivery.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _DeliveryCol.Count > 0 AndAlso _DeliveryCol(_DeliveryCol.Count - 1).ID = 0 Then 
      _DeliveryCol.RemoveAt(_DeliveryCol.Count - 1) 
      bsCtlDelivery.DataSource = Nothing 
      bsCtlDelivery.DataSource = _DeliveryCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvDelivery.Focus() 
      dgvDelivery.CurrentCell = dgvDelivery.Rows(pCellRow).Cells(pCellCol) 
      dgvDelivery.CurrentCell.Selected = True 
    ElseIf _DeliveryCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlDelivery.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As clsDelivery 
    pEntity = CType(bsCtlDelivery.Current, clsDelivery) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvDelivery.CurrentCell Is Nothing Then Return pFault 
    
    If dgvDelivery.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pDelivery As clsDelivery 
      pDelivery = CType(bsCtlDelivery.Current, clsDelivery) 
      If pDelivery Is Nothing Then 
        pFault.LogFreeTextFault("There is no Delivery to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvDelivery.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvDelivery.CurrentCell.RowIndex 
      'show row as selected  
      dgvDelivery.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvDelivery.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvDelivery.CurrentRow.Selected = True 
      If pDelivery.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pDelivery.ID.ToString() & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pDelivery, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvDelivery.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvDelivery.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvDelivery.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pDelivery.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlDelivery.Remove(bsCtlDelivery.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pDelivery As clsDelivery = CType(bsCtlDelivery.Current, clsDelivery) 
    If pDelivery IsNot Nothing Then 
      If pDelivery.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _DeliveryCol.Count > 0 AndAlso dgvDelivery.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvDelivery.Columns.Count - 1 
        If dgvDelivery.Columns(i).Visible Then 
          dgvDelivery.CurrentCell = dgvDelivery.Rows(dgvDelivery.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvDelivery.Refresh() 
      dgvDelivery.Rows(dgvDelivery.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvDelivery.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvDelivery_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvDelivery.RowValidating 
    If _Loading = True OrElse dgvDelivery.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvDelivery.CurrentCell = dgvDelivery.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvDelivery_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvDelivery.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvDelivery.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvDelivery.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvDelivery.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pDelivery Is Nothing Then pDelivery = CType(dgvDelivery.Rows(e.RowIndex).DataBoundItem, clsDelivery) ' Only assign it if needed 
    '  If pDelivery.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pDelivery.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvDelivery.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pDelivery Is Nothing Then pDelivery = CType(dgvDelivery.Rows(e.RowIndex).DataBoundItem, clsDelivery) ' Only assign it if needed
    '  If pDelivery.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pDelivery.RAV - pDelivery.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvDelivery.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvDelivery.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvDelivery.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvDelivery.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvDelivery.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvDelivery.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvDelivery.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvDelivery_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvDelivery.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvDelivery_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvDelivery.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvDelivery.Columns(e.ColumnIndex)
    If bsCtlDelivery.Current Is Nothing Then Exit Sub
    ...
    Dim pDeliveryCol As clsDeliveryCol
    pDeliveryCol = CType(bsCtlDelivery.DataSource, clsDeliveryCol)

    Dim pSummaryRow As clsDelivery = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pDeliveryCol(pDeliveryCol.Count - 1) 
      pDeliveryCol.RemoveAt(pDeliveryCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pDeliveryCol.Count - 1 
          pDeliveryCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      ...
      ElseIf pNewColumn Is colOrderHeader OrElse pNewColumn Is colOrderHeaderText Then
        pDeliveryCol.SortByOrderHeaderText()
      ElseIf pNewColumn Is colDeliveryAddress Then
        pDeliveryCol.SortByDeliveryAddress()
      ElseIf pNewColumn Is colContactPhone Then
        pDeliveryCol.SortByContactPhone()
      ElseIf pNewColumn Is colContactName Then
        pDeliveryCol.SortByContactName()
      ElseIf pNewColumn Is colDeliveryMethod Then
        pDeliveryCol.SortByDeliveryMethod()
      ElseIf pNewColumn Is colOrderedDate Then
        pDeliveryCol.SortByOrderedDate()
      ElseIf pNewColumn Is colReceivedDate Then
        pDeliveryCol.SortByReceivedDate()
      ElseIf pNewColumn Is colArrivalToHubDate Then
        pDeliveryCol.SortByArrivalToHubDate()
      ElseIf pNewColumn Is colArrivalToCustomerDate Then
        pDeliveryCol.SortByArrivalToCustomerDate()
      ElseIf pNewColumn Is colDeliveryStatus Then
        pDeliveryCol.SortByDeliveryStatus()
      ElseIf pNewColumn Is colLocation Then
        pDeliveryCol.SortByLocation()
      ElseIf pNewColumn Is colProductsSummary Then
        pDeliveryCol.SortByProductsSummary()
      ElseIf pNewColumn Is colNotes Then
        pDeliveryCol.SortByNotes()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As clsDelivery In pDeliveryCol 
        ...
        ElseIf pNewColumn Is colDeliveryAddress Then
          Dim pTest As String = "" 
          For Each p As clsDelivery In pDeliveryCol 
            If p.DeliveryAddress <> pTest Then iCntr += 1 : pTest = p.DeliveryAddress 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colContactPhone Then
          Dim pTest As String = "" 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ContactPhone <> pTest Then iCntr += 1 : pTest = p.ContactPhone 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colContactName Then
          Dim pTest As String = "" 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ContactName <> pTest Then iCntr += 1 : pTest = p.ContactName 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDeliveryMethod Then
        ...
        ElseIf pNewColumn Is colOrderedDate Then
          ...
          For Each p As clsDelivery In pDeliveryCol 
            If p.OrderedDate <> pTest Then iCntr += 1 : pTest = p.OrderedDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colReceivedDate Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ReceivedDate <> pTest Then iCntr += 1 : pTest = p.ReceivedDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colArrivalToHubDate Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ArrivalToHubDate <> pTest Then iCntr += 1 : pTest = p.ArrivalToHubDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colArrivalToCustomerDate Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ArrivalToCustomerDate <> pTest Then iCntr += 1 : pTest = p.ArrivalToCustomerDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDeliveryStatus Then
          Dim pTest As clsEnums.enmDeliveryStatus = clsEnums.enmDeliveryStatus.UD 
          For Each p As clsDelivery In pDeliveryCol 
            If p.DeliveryStatus <> pTest Then iCntr += 1 : pTest = p.DeliveryStatus 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLocation Then
          Dim pTest As String = "" 
        ...
        ElseIf pNewColumn Is colProductsSummary Then
          Dim pTest As String = "" 
          For Each p As clsDelivery In pDeliveryCol 
            If p.ProductsSummary <> pTest Then iCntr += 1 : pTest = p.ProductsSummary 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colNotes Then
          Dim pTest As String = "" 
          ...
          Next 
        End If 
        pDeliveryCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pDeliveryCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pDeliveryCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlDelivery.Position = bsCtlDelivery.IndexOf(pDeliveryCol.FindByID(pID))
    End If

    'dgvDelivery.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvDelivery.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pDeliveryCol As clsDeliveryCol 
      pDeliveryCol = CType(bsCtlDelivery.DataSource, clsDeliveryCol) 
      Dim pDelivery As clsDelivery = pDeliveryCol.FindByID(pID) 
      If Not pDelivery.IsEmpty Then 
        bsCtlDelivery.Position = bsCtlDelivery.IndexOf(pDeliveryCol.FindByID(pID)) 
        dgvDelivery.Rows(bsCtlDelivery.Position).Selected = True 
      Else 
        dgvDelivery.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvDelivery_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvDelivery.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvDelivery.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvDelivery_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvDelivery.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvDelivery_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvDelivery.Scroll
    dgvDelivery.Invalidate() 
  End Sub
 
  Private Sub dgvDelivery_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvDelivery.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvDelivery.Rows.Count - 1 Then Exit Sub
 
    'If dgvDelivery.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Delivery', the row with an ID of " & dgvDelivery.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Delivery', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvDelivery.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvDelivery.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Delivery-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvDelivery(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvDelivery(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvDelivery_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvDelivery.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvDelivery.CurrentCell = dgvDelivery.Rows(dgvDelivery.CurrentCell.RowIndex).Cells(0) 
      dgvDelivery.Refresh() 
    End If 
  End Sub 
  Private Sub dgvDelivery_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvDelivery.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvDelivery_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvDelivery.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvDelivery_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvDelivery.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvDelivery.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvDelivery(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pDelivery As clsDelivery = _DeliveryCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pDelivery, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Delivery Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlccDelivery", pDelivery, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvDelivery_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvDelivery.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvDelivery.Rows.Count - 1 Then dgvDelivery.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pDelivery As clsDelivery = _DeliveryCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pDelivery) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvDelivery_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvDelivery.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvDelivery.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvDelivery.Columns(pNewCol).Visible = False Then 
      dgvDelivery.Columns(pNewCol).Visible = True 
      dgvDelivery.CurrentCell = dgvDelivery.CurrentRow.Cells(pNewCol) 
      dgvDelivery.CurrentCell = dgvDelivery.CurrentRow.Cells(pOriginalCol) 
      dgvDelivery.Columns(pNewCol).Visible = False 
    Else 
      dgvDelivery.CurrentCell = dgvDelivery.CurrentRow.Cells(pNewCol) 
      dgvDelivery.CurrentCell = dgvDelivery.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pDelivery, clsDelivery), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pDelivery.Update(_Requester) 
    ...
    Else 
      ...
      'Reset the Delivery collection 
      MyCache.ClearComboList(clsEnums.enmComboListType.ccDeliveryDefaultByID) 
      RaiseEvent evtUpdated(pDelivery) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvDelivery.Columns 
  ...
  Private Sub Summarize() 
    If _DeliveryCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As clsDelivery In _DeliveryCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\DeliveryCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\DeliveryCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\DeliveryCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\DeliveryCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\DeliveryCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvDelivery.Rows 
      ...
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvDelivery.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _DeliveryColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _DeliveryColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _DeliveryColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _DeliveryColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _DeliveryColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Delivery-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Deliveries" 
        .SubTitleRight = "Rows: " & _DeliveryCol.Count.ToString 
        ...
        End If 
        .DataSource = _DeliveryCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Delivery-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Delivery-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvDelivery.Rows 
    ...
    For Each row As DataGridViewRow In dgvDelivery.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlDelivery), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvDelivery.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Delivery-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvDelivery.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvDelivery.Width - 30) / dgvDelivery.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'Set Wrap 
    'colDeliveryAddress.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    'colContactName.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    'colLocation.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    'colProductsSummary.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvDelivery.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Delivery-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colDeliveryAddress.HeaderText = pStrg : mnuColVisibleDeliveryAddress.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "ContactPhone", _Requester) 
    If pStrg <> "" Then colContactPhone.HeaderText = pStrg : mnuColVisibleContactPhone.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "ContactName", _Requester) 
    If pStrg <> "" Then colContactName.HeaderText = pStrg : mnuColVisibleContactName.Text = pStrg
    ...
    If pStrg <> "" Then colOrderedDate.HeaderText = pStrg : mnuColVisibleOrderedDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "ReceivedDate", _Requester) 
    If pStrg <> "" Then colReceivedDate.HeaderText = pStrg : mnuColVisibleReceivedDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "ArrivalToHubDate", _Requester) 
    If pStrg <> "" Then colArrivalToHubDate.HeaderText = pStrg : mnuColVisibleArrivalToHubDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "ArrivalToCustomerDate", _Requester) 
    If pStrg <> "" Then colArrivalToCustomerDate.HeaderText = pStrg : mnuColVisibleArrivalToCustomerDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("Delivery", "DeliveryStatus", _Requester) 
    If pStrg <> "" Then colDeliveryStatus.HeaderText = pStrg : mnuColVisibleDeliveryStatus.Text = pStrg
    ...
    If pStrg <> "" Then colProductsSummary.HeaderText = pStrg : mnuColVisibleProductsSummary.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleOrderHeader.Click, mnuColVisibleDeliveryAddress.Click, mnuColVisibleContactPhone.Click, mnuColVisibleContactName.Click, mnuColVisibleDeliveryMethod.Click, mnuColVisibleOrderedDate.Click, mnuColVisibleReceivedDate.Click, mnuColVisibleArrivalToHubDate.Click, mnuColVisibleArrivalToCustomerDate.Click, mnuColVisibleDeliveryStatus.Click, mnuColVisibleLocation.Click, mnuColVisibleProductsSummary.Click, mnuColVisibleNotes.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvDelivery.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvDelivery.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleDeliveryAddress.Checked = True Then mnuColVisibleDeliveryAddress.PerformClick() 
    If mnuColVisibleContactPhone.Checked = True Then mnuColVisibleContactPhone.PerformClick() 
    If mnuColVisibleContactName.Checked = True Then mnuColVisibleContactName.PerformClick() 
    If mnuColVisibleDeliveryMethod.Checked = True Then mnuColVisibleDeliveryMethod.PerformClick() 
    If mnuColVisibleOrderedDate.Checked = True Then mnuColVisibleOrderedDate.PerformClick() 
    If mnuColVisibleReceivedDate.Checked = True Then mnuColVisibleReceivedDate.PerformClick() 
    If mnuColVisibleArrivalToHubDate.Checked = True Then mnuColVisibleArrivalToHubDate.PerformClick() 
    If mnuColVisibleArrivalToCustomerDate.Checked = True Then mnuColVisibleArrivalToCustomerDate.PerformClick() 
    If mnuColVisibleDeliveryStatus.Checked = True Then mnuColVisibleDeliveryStatus.PerformClick() 
    If mnuColVisibleLocation.Checked = True Then mnuColVisibleLocation.PerformClick() 
    If mnuColVisibleProductsSummary.Checked = True Then mnuColVisibleProductsSummary.PerformClick() 
  ...
  Private Sub dgvDelivery_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvDelivery.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Delivery to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvDelivery_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvDelivery.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvDelivery.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvDelivery.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvDelivery.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvDelivery.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvDelivery(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvDelivery.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvDelivery.BeginEdit(True) 
        DirectCast(dgvDelivery.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvDelivery_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvDelivery.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvDelivery.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvDelivery.MultiSelect = True 
        dgvDelivery.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvDelivery.Rows(i).Selected = True 
        Next 
      End If 
  ...
  Private Sub ctlccDeliveryCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvDelivery.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlccDeliveryCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvDelivery.Width 
  ...
  Private Sub ctlccDeliveryCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlccDeliveryCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvDelivery_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvDelivery.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvDelivery.Rows(e.RowIndex).Selected Then 
        dgvDelivery.ClearSelection() 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvDelivery.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvDelivery.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvDelivery.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _DeliveryCol.Count Then Exit Sub 
    Dim pDelivery As clsDelivery = _DeliveryCol(pRowIndex) 
    Dim pTitle As String = "Delivery #" & pDelivery.ID.ToString() 
    Dim pKey As String = "Delivery_" & pDelivery.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlccDelivery" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pDelivery, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pDelivery 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Delivery_" & pDelivery.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Delivery #" & pDelivery.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlccDelivery", pDelivery, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvDelivery.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvDelivery.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _DeliveryCol.Count Then 
        Dim pDelivery As clsDelivery = _DeliveryCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pDelivery.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvDelivery.SelectedRows(0) 
