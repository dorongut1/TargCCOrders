Public Class ctlc_ProcessCol
  ...
  Public Event evtRowClicked(ByVal vProcess As csProcess) 
  Public Event evtRowDoubleClicked(ByVal vProcess As csProcess, ByRef rHandled As Boolean) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property ColumnsReadOnly As List(Of csProcess.enmProperty) 
    Public Property ColumnsHide As List(Of csProcess.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csProcess.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csProcess.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csProcess.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csProcess.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _ColumnsReadOnly = New List(Of csProcess.enmProperty) 
      _ColumnsHide = New List(Of csProcess.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csProcess.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csProcess.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csProcess.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csProcess.enmProperty, String) 
  ...
  Private WithEvents _ProcessCol As csProcessCol
  Private WithEvents _ProcessColFullLength As csProcessCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedProcess]() As csProcess 
    Get 
      If dgvProcess.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvProcess.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvProcess.Rows.Count - 1 Then dgvProcess.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _ProcessCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [ProcessCol]() As csProcessCol 
    Get 
      Return _ProcessCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pProcessCol As New csProcessCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pProcessCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pProcessCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vProcessCol As csProcessCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vProcessCol) 
  End Function
  
  Private Function LoadControl(vProcessCol As csProcessCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csProcess.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csProcess.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csProcess.enmProperty.ID Then colID.ReadOnly = True 
      If l = csProcess.enmProperty.Name Then colName.ReadOnly = True 
      If l = csProcess.enmProperty.DateChecked Then colDateChecked.ReadOnly = True 
    Next 
    ...
    dgvProcess.DoubleBuffered(True) 
 
    pFault = vProcessCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _ProcessColFullLength = vProcessCol.Clone() 
 
    'Truncate the strings 
    _ProcessCol = vProcessCol 
    If _LoadParameters.TruncateStrings Then 
      _ProcessCol.TruncateStrings() 
    Else 
      dgvProcess.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvProcess.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvProcess.Columns 
    ...
    'Hide columns  
    For Each p As csProcess.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvProcess.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvProcess.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvProcess.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvProcess.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    bsCtlProcess.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Process-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvProcess.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvProcess_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvProcess.SelectedRows.Count > 0 Then 
      pRowIndex = dgvProcess.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvProcess.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvProcess.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlProcess.DataSource = Nothing 
    bsCtlProcess.DataSource = _ProcessCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvProcess.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _ProcessCol.Count - 2 Then 
          dgvProcess.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _ProcessCol.Count - 1 Then 
          dgvProcess.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    dgvProcess.EditMode = DataGridViewEditMode.EditProgrammatically 
  ...
  'ExternalButtons 
  'CellFormatting  
  Private Sub dgvProcess_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvProcess.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvProcess.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvProcess.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvProcess.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pProcess Is Nothing Then pProcess = CType(dgvProcess.Rows(e.RowIndex).DataBoundItem, csProcess) ' Only assign it if needed 
    '  If pProcess.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pProcess.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvProcess.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pProcess Is Nothing Then pProcess = CType(dgvProcess.Rows(e.RowIndex).DataBoundItem, csProcess) ' Only assign it if needed
    '  If pProcess.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pProcess.RAV - pProcess.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvProcess.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvProcess.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvProcess.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvProcess.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvProcess.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvProcess.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvProcess.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvProcess_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvProcess.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvProcess_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvProcess.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvProcess.Columns(e.ColumnIndex)
    If bsCtlProcess.Current Is Nothing Then Exit Sub
    ...
    Dim pID As Long = 0 
    If dgvProcess.SelectedRows.Count > 0 Then 
    pProcess = CType(bsCtlProcess.Current, csProcess)
      pID = pProcess.ID 
    End If 
    ...
    Dim pProcessCol As csProcessCol
    pProcessCol = CType(bsCtlProcess.DataSource, csProcessCol)

    Dim pSummaryRow As csProcess = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pProcessCol(pProcessCol.Count - 1) 
      pProcessCol.RemoveAt(pProcessCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pProcessCol.Count - 1 
          pProcessCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pProcessCol.SortByID()
      ElseIf pNewColumn Is colName Then
        pProcessCol.SortByName()
      ElseIf pNewColumn Is colDateChecked Then
        pProcessCol.SortByDateChecked()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csProcess In pProcessCol 
        ...
        ElseIf pNewColumn Is colDateChecked Then
          ...
          For Each p As csProcess In pProcessCol 
            If p.DateChecked <> pTest Then iCntr += 1 : pTest = p.DateChecked 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pProcessCol.SortByTag() 
      End If 
    ...
    If _Summarized = True Then 
      pProcessCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlProcess.Position = bsCtlProcess.IndexOf(pProcessCol.FindByID(pID))
    End If

    'dgvProcess.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pProcessCol As csProcessCol 
      pProcessCol = CType(bsCtlProcess.DataSource, csProcessCol) 
      Dim pProcess As csProcess = pProcessCol.FindByID(pID) 
      If Not pProcess.IsEmpty Then 
        bsCtlProcess.Position = bsCtlProcess.IndexOf(pProcessCol.FindByID(pID)) 
        dgvProcess.Rows(bsCtlProcess.Position).Selected = True 
      Else 
        dgvProcess.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvProcess_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvProcess.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvProcess.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvProcess_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvProcess.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvProcess_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvProcess.Scroll
    dgvProcess.Invalidate() 
  End Sub
 
  Private Sub dgvProcess_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvProcess.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvProcess.Rows.Count - 1 Then Exit Sub
 
    'If dgvProcess.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Process', the row with an ID of " & dgvProcess.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Process', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvProcess.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvProcess.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Process-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvProcess(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvProcess(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvProcess_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvProcess.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvProcess_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvProcess.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvProcess_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvProcess.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvProcess.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvProcess(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pProcess As csProcess = _ProcessCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pProcess, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Process Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_Process", pProcess, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvProcess_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvProcess.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvProcess.Rows.Count - 1 Then dgvProcess.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pProcess As csProcess = _ProcessCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pProcess) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvProcess_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvProcess.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvProcess.Columns 
  ...
  Private Sub Summarize() 
    If _ProcessCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As csProcess In _ProcessCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\ProcessCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\ProcessCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\ProcessCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\ProcessCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\ProcessCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvProcess.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvProcess.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _ProcessColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _ProcessColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _ProcessColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _ProcessColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _ProcessColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Process-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Processes" 
        .SubTitleRight = "Rows: " & _ProcessCol.Count.ToString 
        ...
        End If 
        .DataSource = _ProcessCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Process-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Process-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvProcess.Rows 
    ...
    For Each row As DataGridViewRow In dgvProcess.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlProcess), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvProcess.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Process-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvProcess.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvProcess.Width - 30) / dgvProcess.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvProcess.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Process-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    pStrg = ccHelper.GetLocalizedFieldName("c_Process", "ID", _Requester) 
    If pStrg <> "" Then colID.HeaderText = pStrg : mnuColVisibleID.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Process", "Name", _Requester) 
    If pStrg <> "" Then colName.HeaderText = pStrg : mnuColVisibleName.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Process", "DateChecked", _Requester) 
    If pStrg <> "" Then colDateChecked.HeaderText = pStrg : mnuColVisibleDateChecked.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleName.Click, mnuColVisibleDateChecked.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvProcess.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvProcess.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleDateChecked.Checked = True Then mnuColVisibleDateChecked.PerformClick() 
    'Show Defaults 
  ...
  Private Sub dgvProcess_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvProcess.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Process to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
      If pMessageBox <> frmMessageOrInputBox.enmButtonReturned.Cancel Then 
        ...
        If pMessageBox = frmMessageOrInputBox.enmButtonReturned.No Then 
          Clipboard.SetText(pProcess.ToCSV) 
        Else 
          Clipboard.SetText(pProcess.ToString.Replace(" ‡ ", Environment.NewLine)) 
        End If 
        frmMessageOrInputBox.ShowMsg("The Process is in your clipboard", frmMessageOrInputBox.enmIconType.Information) 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvProcess_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvProcess.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvProcess.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvProcess.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvProcess.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvProcess.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvProcess(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvProcess.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvProcess.BeginEdit(True) 
        DirectCast(dgvProcess.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvProcess_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvProcess.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvProcess.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvProcess.MultiSelect = True 
        dgvProcess.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvProcess.Rows(i).Selected = True 
        Next 
      End If 
  ...
  Private Sub ctlc_ProcessCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvProcess.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_ProcessCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvProcess.Width 
  ...
  Private Sub ctlc_ProcessCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub dgvProcess_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvProcess.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvProcess.Rows(e.RowIndex).Selected Then 
        dgvProcess.ClearSelection() 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvProcess.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvProcess.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvProcess.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _ProcessCol.Count Then Exit Sub 
    Dim pProcess As csProcess = _ProcessCol(pRowIndex) 
    Dim pTitle As String = "Process #" & pProcess.ID.ToString() 
    Dim pKey As String = "Process_" & pProcess.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_Process" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pProcess, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pProcess 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Process_" & pProcess.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Process #" & pProcess.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_Process", pProcess, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvProcess.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvProcess.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _ProcessCol.Count Then 
        Dim pProcess As csProcess = _ProcessCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pProcess.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvProcess.SelectedRows(0) 
