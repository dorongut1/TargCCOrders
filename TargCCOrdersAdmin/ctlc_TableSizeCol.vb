Public Class ctlc_TableSizeCol
  ...
  Public Event evtRowClicked(ByVal vTableSize As csTableSize) 
  Public Event evtRowDoubleClicked(ByVal vTableSize As csTableSize, ByRef rHandled As Boolean) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of csTableSize.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property ColumnsReadOnly As List(Of csTableSize.enmProperty) 
    Public Property ColumnsHide As List(Of csTableSize.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csTableSize.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csTableSize.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csTableSize.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csTableSize.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of csTableSize.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _ColumnsReadOnly = New List(Of csTableSize.enmProperty) 
      _ColumnsHide = New List(Of csTableSize.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csTableSize.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csTableSize.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csTableSize.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csTableSize.enmProperty, String) 
  ...
  Private WithEvents _TableSizeCol As csTableSizeCol
  Private WithEvents _TableSizeColFullLength As csTableSizeCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedTableSize]() As csTableSize 
    Get 
      If dgvTableSize.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvTableSize.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvTableSize.Rows.Count - 1 Then dgvTableSize.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _TableSizeCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [TableSizeCol]() As csTableSizeCol 
    Get 
      Return _TableSizeCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pTableSizeCol As New csTableSizeCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pTableSizeCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pTableSizeCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vTableSizeCol As csTableSizeCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vTableSizeCol) 
  End Function
  
  Private Function LoadControl(vTableSizeCol As csTableSizeCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csTableSize.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csTableSize.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csTableSize.enmProperty.ID Then colID.ReadOnly = True 
      If l = csTableSize.enmProperty.TableName Then colTableName.ReadOnly = True 
      If l = csTableSize.enmProperty.NumberOfRows Then colNumberOfRows.ReadOnly = True 
      If l = csTableSize.enmProperty.ReservedSizeKb Then colReservedSizeKb.ReadOnly = True 
      If l = csTableSize.enmProperty.DataSizeKb Then colDataSizeKb.ReadOnly = True 
      If l = csTableSize.enmProperty.IndexSizeKb Then colIndexSizeKb.ReadOnly = True 
      If l = csTableSize.enmProperty.UnusedSizeKb Then colUnusedSizeKb.ReadOnly = True 
    Next 
    ...
    dgvTableSize.DoubleBuffered(True) 
 
    pFault = vTableSizeCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _TableSizeColFullLength = vTableSizeCol.Clone() 
 
    'Truncate the strings 
    _TableSizeCol = vTableSizeCol 
    If _LoadParameters.TruncateStrings Then 
      _TableSizeCol.TruncateStrings() 
    Else 
      dgvTableSize.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvTableSize.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvTableSize.Columns 
    ...
    'Hide columns  
    For Each p As csTableSize.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvTableSize.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvTableSize.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvTableSize.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvTableSize.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvTableSize.ClearSelection()
    bsCtlTableSize.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-TableSize-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvTableSize.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvTableSize_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvTableSize.SelectedRows.Count > 0 Then 
      pRowIndex = dgvTableSize.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvTableSize.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvTableSize.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlTableSize.DataSource = Nothing 
    bsCtlTableSize.DataSource = _TableSizeCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvTableSize.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _TableSizeCol.Count - 2 Then 
          dgvTableSize.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _TableSizeCol.Count - 1 Then 
          dgvTableSize.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    dgvTableSize.EditMode = DataGridViewEditMode.EditProgrammatically 
    dgvTableSize.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
    dgvTableSize.AllowUserToDeleteRows = False 
    dgvTableSize.AllowUserToAddRows = False 
    If _TableSizeCol.Count = 0 Then 
    ...
    lblStatus.Text = "" 
    dgvTableSize.Refresh() 
  End Sub
  'ExternalButtons 
  'CellFormatting  
  Private Sub dgvTableSize_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvTableSize.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvTableSize.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvTableSize.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvTableSize.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pTableSize Is Nothing Then pTableSize = CType(dgvTableSize.Rows(e.RowIndex).DataBoundItem, csTableSize) ' Only assign it if needed 
    '  If pTableSize.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pTableSize.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvTableSize.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pTableSize Is Nothing Then pTableSize = CType(dgvTableSize.Rows(e.RowIndex).DataBoundItem, csTableSize) ' Only assign it if needed
    '  If pTableSize.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pTableSize.RAV - pTableSize.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvTableSize.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvTableSize.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvTableSize.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvTableSize.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvTableSize.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvTableSize.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvTableSize.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvTableSize_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvTableSize.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvTableSize_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvTableSize.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvTableSize.Columns(e.ColumnIndex)
    If bsCtlTableSize.Current Is Nothing Then Exit Sub
    ...
    dgvTableSize.SuspendLayout()

    Dim pTableSize As csTableSize
    Dim pID As Long = -1 
    If dgvTableSize.SelectedRows.Count > 0 Then 
    pTableSize = CType(bsCtlTableSize.Current, csTableSize)
      pID = pTableSize.ID 
    End If 
    ...
    Dim pTableSizeCol As csTableSizeCol
    pTableSizeCol = CType(bsCtlTableSize.DataSource, csTableSizeCol)

    Dim pSummaryRow As csTableSize = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pTableSizeCol(pTableSizeCol.Count - 1) 
      pTableSizeCol.RemoveAt(pTableSizeCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pTableSizeCol.Count - 1 
          pTableSizeCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pTableSizeCol.SortByID()
      ElseIf pNewColumn Is colTableName Then
        pTableSizeCol.SortByTableName()
      ElseIf pNewColumn Is colNumberOfRows Then
        pTableSizeCol.SortByNumberOfRows()
      ElseIf pNewColumn Is colReservedSizeKb Then
        pTableSizeCol.SortByReservedSizeKb()
      ElseIf pNewColumn Is colDataSizeKb Then
        pTableSizeCol.SortByDataSizeKb()
      ElseIf pNewColumn Is colIndexSizeKb Then
        pTableSizeCol.SortByIndexSizeKb()
      ElseIf pNewColumn Is colUnusedSizeKb Then
        pTableSizeCol.SortByUnusedSizeKb()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csTableSize In pTableSizeCol 
        ...
        ElseIf pNewColumn Is colNumberOfRows Then
          ...
          For Each p As csTableSize In pTableSizeCol 
            If p.NumberOfRows <> pTest Then iCntr += 1 : pTest = p.NumberOfRows 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colReservedSizeKb Then
          Dim pTest As Integer = 0 
          For Each p As csTableSize In pTableSizeCol 
            If p.ReservedSizeKb <> pTest Then iCntr += 1 : pTest = p.ReservedSizeKb 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDataSizeKb Then
          Dim pTest As Integer = 0 
          For Each p As csTableSize In pTableSizeCol 
            If p.DataSizeKb <> pTest Then iCntr += 1 : pTest = p.DataSizeKb 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colIndexSizeKb Then
          Dim pTest As Integer = 0 
          For Each p As csTableSize In pTableSizeCol 
            If p.IndexSizeKb <> pTest Then iCntr += 1 : pTest = p.IndexSizeKb 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colUnusedSizeKb Then
          Dim pTest As Integer = 0 
          For Each p As csTableSize In pTableSizeCol 
            If p.UnusedSizeKb <> pTest Then iCntr += 1 : pTest = p.UnusedSizeKb 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pTableSizeCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pTableSizeCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pTableSizeCol.Add(pSummaryRow) 
    End If 
 
    If pID > -1 Then
      bsCtlTableSize.Position = bsCtlTableSize.IndexOf(pTableSizeCol.FindByID(pID))
    End If

    'dgvTableSize.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvTableSize.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > -1 Then 
      Dim pTableSizeCol As csTableSizeCol 
      pTableSizeCol = CType(bsCtlTableSize.DataSource, csTableSizeCol) 
      Dim pTableSize As csTableSize = pTableSizeCol.FindByID(pID) 
      If Not pTableSize.IsEmpty Then 
        bsCtlTableSize.Position = bsCtlTableSize.IndexOf(pTableSizeCol.FindByID(pID)) 
        dgvTableSize.Rows(bsCtlTableSize.Position).Selected = True 
      Else 
        dgvTableSize.ClearSelection() 
      End If 
    ElseIf pID = -1 Then 
  ...
  'Grid Resize
  Private Sub dgvTableSize_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvTableSize.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvTableSize.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvTableSize_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvTableSize.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvTableSize_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvTableSize.Scroll
    dgvTableSize.Invalidate() 
  End Sub
 
  Private Sub dgvTableSize_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvTableSize.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvTableSize.Rows.Count - 1 Then Exit Sub
 
    'If dgvTableSize.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'TableSize', the row with an ID of " & dgvTableSize.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'TableSize', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvTableSize.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvTableSize.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-TableSize-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvTableSize(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvTableSize(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvTableSize_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvTableSize.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvTableSize_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvTableSize.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvTableSize_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvTableSize.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvTableSize.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvTableSize(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pTableSize As csTableSize = _TableSizeCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pTableSize, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "TableSize Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_TableSize", pTableSize, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvTableSize_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvTableSize.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvTableSize.Rows.Count - 1 Then dgvTableSize.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pTableSize As csTableSize = _TableSizeCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pTableSize) 
    Finally
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvTableSize.Columns 
  ...
  Private Sub Summarize() 
    If _TableSizeCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pNumberOfRows As Integer 
    Dim pReservedSizeKb As Integer 
    Dim pDataSizeKb As Integer 
    Dim pIndexSizeKb As Integer 
    Dim pUnusedSizeKb As Integer 
    For Each pExistingRow As csTableSize In _TableSizeCol 
      If _SummaryOverFlow.IndexOf("#NumberOfRows#") < 0 Then 
        Try 
          pNumberOfRows += pExistingRow.NumberOfRows 
        Catch ex As System.OverflowException 
          pNumberOfRows = -99999999 
          _SummaryOverFlow &= "NumberOfRows#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#ReservedSizeKb#") < 0 Then 
        Try 
          pReservedSizeKb += pExistingRow.ReservedSizeKb 
        Catch ex As System.OverflowException 
          pReservedSizeKb = -99999999 
          _SummaryOverFlow &= "ReservedSizeKb#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#DataSizeKb#") < 0 Then 
        Try 
          pDataSizeKb += pExistingRow.DataSizeKb 
        Catch ex As System.OverflowException 
          pDataSizeKb = -99999999 
          _SummaryOverFlow &= "DataSizeKb#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#IndexSizeKb#") < 0 Then 
        Try 
          pIndexSizeKb += pExistingRow.IndexSizeKb 
        Catch ex As System.OverflowException 
          pIndexSizeKb = -99999999 
          _SummaryOverFlow &= "IndexSizeKb#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#UnusedSizeKb#") < 0 Then 
        Try 
          pUnusedSizeKb += pExistingRow.UnusedSizeKb 
        Catch ex As System.OverflowException 
          pUnusedSizeKb = -99999999 
          _SummaryOverFlow &= "UnusedSizeKb#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csTableSize.enmSummarizeableProperty.NumberOfRows) = csTableSize.enmSummarizeableProperty.NumberOfRows Then pNumberOfRows = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csTableSize.enmSummarizeableProperty.ReservedSizeKb) = csTableSize.enmSummarizeableProperty.ReservedSizeKb Then pReservedSizeKb = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csTableSize.enmSummarizeableProperty.DataSizeKb) = csTableSize.enmSummarizeableProperty.DataSizeKb Then pDataSizeKb = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csTableSize.enmSummarizeableProperty.IndexSizeKb) = csTableSize.enmSummarizeableProperty.IndexSizeKb Then pIndexSizeKb = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csTableSize.enmSummarizeableProperty.UnusedSizeKb) = csTableSize.enmSummarizeableProperty.UnusedSizeKb Then pUnusedSizeKb = 0
    Dim pSummaryRow As New csTableSize( _ 
        vID:=0 _ 
      , vTableName:="" _ 
      , vNumberOfRows:=pNumberOfRows _ 
      , vReservedSizeKb:=pReservedSizeKb _ 
      , vDataSizeKb:=pDataSizeKb _ 
      , vIndexSizeKb:=pIndexSizeKb _ 
      , vUnusedSizeKb:=pUnusedSizeKb _ 
      , vTag:="" _ 
      , vDateAdded:=Nothing _ 
      )
    _TableSizeCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\TableSizeCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\TableSizeCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\TableSizeCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\TableSizeCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\TableSizeCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvTableSize.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvTableSize.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _TableSizeColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _TableSizeColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _TableSizeColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _TableSizeColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _TableSizeColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-TableSize-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "TableSizes" 
        .SubTitleRight = "Rows: " & _TableSizeCol.Count.ToString 
        ...
        End If 
        .DataSource = _TableSizeCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-TableSize-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-TableSize-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvTableSize.Rows 
    ...
    For Each row As DataGridViewRow In dgvTableSize.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlTableSize), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvTableSize.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-TableSize-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvTableSize.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvTableSize.Width - 30) / dgvTableSize.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvTableSize.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-TableSize-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colNumberOfRows.HeaderText = pStrg : mnuColVisibleNumberOfRows.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_TableSize", "ReservedSizeKb", _Requester) 
    If pStrg <> "" Then colReservedSizeKb.HeaderText = pStrg : mnuColVisibleReservedSizeKb.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_TableSize", "DataSizeKb", _Requester) 
    If pStrg <> "" Then colDataSizeKb.HeaderText = pStrg : mnuColVisibleDataSizeKb.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_TableSize", "IndexSizeKb", _Requester) 
    If pStrg <> "" Then colIndexSizeKb.HeaderText = pStrg : mnuColVisibleIndexSizeKb.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_TableSize", "UnusedSizeKb", _Requester) 
    If pStrg <> "" Then colUnusedSizeKb.HeaderText = pStrg : mnuColVisibleUnusedSizeKb.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleTableName.Click, mnuColVisibleNumberOfRows.Click, mnuColVisibleReservedSizeKb.Click, mnuColVisibleDataSizeKb.Click, mnuColVisibleIndexSizeKb.Click, mnuColVisibleUnusedSizeKb.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvTableSize.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvTableSize.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleNumberOfRows.Checked = True Then mnuColVisibleNumberOfRows.PerformClick() 
    If mnuColVisibleReservedSizeKb.Checked = True Then mnuColVisibleReservedSizeKb.PerformClick() 
    If mnuColVisibleDataSizeKb.Checked = True Then mnuColVisibleDataSizeKb.PerformClick() 
    If mnuColVisibleIndexSizeKb.Checked = True Then mnuColVisibleIndexSizeKb.PerformClick() 
    If mnuColVisibleUnusedSizeKb.Checked = True Then mnuColVisibleUnusedSizeKb.PerformClick() 
    'Show Defaults 
  ...
  Private Sub dgvTableSize_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvTableSize.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the TableSize to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvTableSize_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvTableSize.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvTableSize.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvTableSize.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvTableSize.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvTableSize.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvTableSize(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvTableSize.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvTableSize.BeginEdit(True) 
        DirectCast(dgvTableSize.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvTableSize_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvTableSize.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvTableSize.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvTableSize.MultiSelect = True 
        dgvTableSize.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvTableSize.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvTableSize.MultiSelect = True 
    Else 
      dgvTableSize.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlc_TableSizeCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvTableSize.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_TableSizeCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvTableSize.Width 
  ...
  Private Sub ctlc_TableSizeCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub dgvTableSize_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvTableSize.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvTableSize.Rows(e.RowIndex).Selected Then 
        dgvTableSize.ClearSelection() 
        dgvTableSize.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvTableSize.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvTableSize.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvTableSize.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _TableSizeCol.Count Then Exit Sub 
    Dim pTableSize As csTableSize = _TableSizeCol(pRowIndex) 
    Dim pTitle As String = "TableSize #" & pTableSize.ID.ToString() 
    Dim pKey As String = "TableSize_" & pTableSize.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_TableSize" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pTableSize, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pTableSize 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "TableSize_" & pTableSize.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "TableSize #" & pTableSize.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_TableSize", pTableSize, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvTableSize.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvTableSize.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _TableSizeCol.Count Then 
        Dim pTableSize As csTableSize = _TableSizeCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pTableSize.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvTableSize.SelectedRows(0) 
