...
Namespace vbReport
  ''' <summary>
  ''' <para>
  ''' ReportDocument extends the functionality of the standard <see cref="T:System.Drawing.Printing.PrintDocument" />
  ...
  ''' class. In other words, the standard Print method, print dialog and print preview capabilities
...
#Region " Event declarations "
    ...
    ''' </summary>
    Public Event ReportBegin(ByVal sender As Object,
    ...
    ''' </summary>
    Public Event PrintPageBegin(ByVal sender As Object,
    ByVal e As ReportPageEventArgs)
    ''' <summary>
    ...
    ''' </summary>
    Public Event PrintPageEnd(ByVal sender As Object,
    ByVal e As ReportPageEventArgs)
    ''' <summary>
    ...
    ''' </summary>
    Public Event ReportEnd(ByVal sender As Object,
    ByVal e As ReportPageEventArgs)
...
#Region " Report Properties and Settings "
    ...
    Private mHasSummaryLine As Boolean
    Private WithEvents mBindingList As IBindingList

    Public Sub New()

      mFont = New Font("Courier New", 9)
    ...
    ''' <summary>
    ''' Sets or returns whether or not the report is RTL
    ...
    Public Property HasSummaryLine() As Boolean
      Get
        Return mHasSummaryLine
      End Get
      Set(ByVal Value As Boolean)
        mHasSummaryLine = Value
      End Set
    End Property
...
#Region " AutoDiscover "
    ...
    Private Sub DoAutoDiscover(ByVal ds As DataView)
      ...
      For field = 0 To ds.Table.Columns.Count - 1
        ...
        col.MappingName = ds.Table.Columns(field).Caption
...
#Region " GetField "
    ...
    Private Function GetField(ByVal obj As Object, ByVal FieldName As String, ByVal FormatCode As String) As String
      ...
      Else
        ...
        Try
          ...
          Else
            ...
            If IsNothing(prop.GetValue(obj, Nothing)) Then
              Return ""
            Else
              If FormatCode <> "" Then
                Return Format(prop.GetValue(obj, Nothing), FormatCode)
              Else
...
#Region " Do printing "
    ...
    Private Sub ReportDocument_BeginPrint(ByVal sender As Object,
    ...
    Private Sub ReportDocument_PrintPage(ByVal sender As Object,
      ...
      Dim page As New ReportPageEventArgs(e, mPageNumber, mFont, mBrush, mFooterLines, mRTL)
      ...
      If mColumns.Count > 0 Then
        Dim space As Integer = ccHelper.ToInteger(e.MarginBounds.Width / mColumns.Count)
      ...
      If Not mDataSource Is Nothing AndAlso mColumns.Count > 0 Then
        ...
        While Not e.EndOfPage AndAlso mRow < InnerSource.Count
          ...
          For Field = 0 To mColumns.Count - 1
            If mHasSummaryLine = True AndAlso mRow = InnerSource.Count - 1 Then
              If Field = 0 Then
                e.HorizontalRule()
                e.WriteColumn("", mColumns(Field), True, False)
              Else
                e.WriteColumn(GetField(InnerSource.Item(mRow), mColumns(Field).Field, mColumns(Field).FormatCode), mColumns(Field), True, False)
              End If
            Else
              If mColumns(Field).FormatCode <> "" Then
                e.WriteColumn(GetField(InnerSource.Item(mRow), mColumns(Field).Field, mColumns(Field).FormatCode), mColumns(Field), False, True)
              Else
                e.WriteColumn(GetField(InnerSource.Item(mRow), mColumns(Field).Field), mColumns(Field), True)
              End If
            End If
          Next
          e.WriteLine()
    ...
    Private Sub PrintHeader(ByVal e As ReportPageEventArgs)
      ...
      With e
        ...
        If mColumns.Count > 0 Then
          ...
          For field = 0 To mColumns.Count - 1
            .WriteColumn(mColumns(field).HeaderText, mColumns(field), True, False)
          Next
          .WriteLine()
        End If
