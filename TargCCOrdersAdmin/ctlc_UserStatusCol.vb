Public Class ctlc_UserStatusCol
  ...
  Public Event evtUpdated(ByVal vUserStatus As csUserStatus) 
  Private Event evtBeforeDelete(ByVal vUserStatus As csUserStatus, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vUserStatus As csUserStatus) 
  Public Event evtRowDoubleClicked(ByVal vUserStatus As csUserStatus, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As csUserStatus.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of csUserStatus.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property CbosDoNotLoad As List(Of csUserStatus.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of csUserStatus.enmProperty) 
    Public Property ColumnsHide As List(Of csUserStatus.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csUserStatus.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csUserStatus.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csUserStatus.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csUserStatus.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of csUserStatus.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of csUserStatus.enmParentProperty) 
      _ColumnsReadOnly = New List(Of csUserStatus.enmProperty) 
      _ColumnsHide = New List(Of csUserStatus.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csUserStatus.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csUserStatus.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csUserStatus.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csUserStatus.enmProperty, String) 
  ...
  Private WithEvents _UserStatusCol As csUserStatusCol
  Private WithEvents _UserStatusColFullLength As csUserStatusCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedUserStatus]() As csUserStatus 
    Get 
      If dgvUserStatus.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvUserStatus.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvUserStatus.Rows.Count - 1 Then dgvUserStatus.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _UserStatusCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [UserStatusCol]() As csUserStatusCol 
    Get 
      Return _UserStatusCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pUserStatusCol As New csUserStatusCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pUserStatusCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pUserStatusCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pUserStatusCol As New csUserStatusCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "User" 
          pFault = pUserStatusCol.FillByUserID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vUserStatusCol As csUserStatusCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vUserStatusCol) 
  End Function
  
  Private Function LoadControl(vUserStatusCol As csUserStatusCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csUserStatus.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csUserStatus.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csUserStatus.enmProperty.ID Then colID.ReadOnly = True 
      If l = csUserStatus.enmProperty.User Then colUser.ReadOnly = True 
      If l = csUserStatus.enmProperty.ApplicationName Then colApplicationName.ReadOnly = True 
      If l = csUserStatus.enmProperty.LastLoggedLoginID Then colLastLoggedLoginID.ReadOnly = True 
      If l = csUserStatus.enmProperty.LoginTime Then colLoginTime.ReadOnly = True 
      If l = csUserStatus.enmProperty.LogoutTime Then colLogoutTime.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As csUserStatus.enmParentProperty = csUserStatus.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of csUserStatus.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvUserStatus.DoubleBuffered(True) 
 
    pFault = vUserStatusCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _UserStatusColFullLength = vUserStatusCol.Clone() 
 
    'Truncate the strings 
    _UserStatusCol = vUserStatusCol 
    If _LoadParameters.TruncateStrings Then 
      _UserStatusCol.TruncateStrings() 
    Else 
      dgvUserStatus.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvUserStatus.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        ...
        If colUser.DataGridView IsNot Nothing Then dgvUserStatus.Columns.Remove(colUser) 
      Else 
        If colUser.ReadOnly = False Then 
          If colUserText.DataGridView IsNot Nothing Then dgvUserStatus.Columns.Remove(colUserText) 
        Else 
          colUser.Name = colUser.Name & "zzzz" 
          colUserText.Name = colUser.Name.Replace("zzzz", "") 
          If colUser.DataGridView IsNot Nothing Then dgvUserStatus.Columns.Remove(colUser) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(csUserStatus.enmParentProperty.User) Then 
            _LoadParameters.CbosDoNotLoad.Add(csUserStatus.enmParentProperty.User) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvUserStatus.Columns 
    ...
    'Hide columns  
    For Each p As csUserStatus.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvUserStatus.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvUserStatus.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvUserStatus.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvUserStatus.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvUserStatus.ClearSelection()
    bsCtlUserStatus.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-UserStatus-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvUserStatus.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvUserStatus_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvUserStatus.SelectedRows.Count > 0 Then 
      pRowIndex = dgvUserStatus.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvUserStatus.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvUserStatus.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlUserStatus.DataSource = Nothing 
    bsCtlUserStatus.DataSource = _UserStatusCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvUserStatus.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _UserStatusCol.Count - 2 Then 
          dgvUserStatus.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _UserStatusCol.Count - 1 Then 
          dgvUserStatus.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'User
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = csUserStatus.enmParentProperty.User) = csUserStatus.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.c_UserDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(csUserStatus.enmParentProperty.User, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-UserStatusCol-200806-1015", _Requester) 
        End If 
      End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_UserStatusUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_UserStatusUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_UserStatusDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      dgvUserStatus.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvUserStatus.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvUserStatus.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvUserStatus.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvUserStatus.AllowUserToDeleteRows = False 
      dgvUserStatus.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvUserStatus.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvUserStatus.CurrentRow.Index 
      '  dgvUserStatus.CurrentCell = dgvUserStatus.Rows(pCurrentRow).Cells(0) 
      '  dgvUserStatus.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
    ...
    Else 
      If _UserStatusCol.Count = 0 Then 
        btnSpreadsheet.Enabled = False 
        btnReport.Enabled = False 
      Else 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("LastLoggedLoginID, ") 
    pFieldList.Append("LoginTime, ") 
    pFieldList.Append("LogoutTime, ") 
    ...
    'Now Load the data 
    Dim pIncomingUserStatuss As New csUserStatusCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingUserStatus As New csUserStatus(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingUserStatus.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = csUserStatusCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingUserStatuss.Add(pIncomingUserStatus) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingUserStatus.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "UserID" 
            pIncomingUserStatus.UserID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingUserStatus.ApplicationName = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LastLoggedLoginID" 
            pIncomingUserStatus.LastLoggedLoginID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LoginTime" 
            pIncomingUserStatus.LoginTime = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LogoutTime" 
            pIncomingUserStatus.LogoutTime = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingUserStatus.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingUserStatus.Tag = "Row " & pRow.ToString Then 
            pIncomingUserStatus.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingUserStatuss.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingUserStatuss.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingUserStatuss 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _UserStatusCol = pIncomingUserStatuss 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvUserStatus.Focused = True AndAlso dgvUserStatus.SelectedRows.Count > 0 Then 
      pCellRow = dgvUserStatus.CurrentCell.RowIndex 
      pCellCol = dgvUserStatus.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvUserStatus.CurrentCell = dgvUserStatus.Rows(0).Cells(0) 
      dgvUserStatus.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _UserStatusCol.Count > 0 AndAlso _UserStatusCol(_UserStatusCol.Count - 1).ID = 0 Then 
      _UserStatusCol.RemoveAt(_UserStatusCol.Count - 1) 
      bsCtlUserStatus.DataSource = Nothing 
      bsCtlUserStatus.DataSource = _UserStatusCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvUserStatus.Focus() 
      dgvUserStatus.CurrentCell = dgvUserStatus.Rows(pCellRow).Cells(pCellCol) 
      dgvUserStatus.CurrentCell.Selected = True 
    ElseIf _UserStatusCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlUserStatus.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As csUserStatus 
    pEntity = CType(bsCtlUserStatus.Current, csUserStatus) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvUserStatus.CurrentCell Is Nothing Then Return pFault 
    
    If dgvUserStatus.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pUserStatus As csUserStatus 
      pUserStatus = CType(bsCtlUserStatus.Current, csUserStatus) 
      If pUserStatus Is Nothing Then 
        pFault.LogFreeTextFault("There is no UserStatus to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvUserStatus.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvUserStatus.CurrentCell.RowIndex 
      'show row as selected  
      dgvUserStatus.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvUserStatus.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvUserStatus.CurrentRow.Selected = True 
      If pUserStatus.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete the row with a ID of '" & pUserStatus.ID.ToString & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pUserStatus, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvUserStatus.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvUserStatus.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvUserStatus.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pUserStatus.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlUserStatus.Remove(bsCtlUserStatus.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pUserStatus As csUserStatus = CType(bsCtlUserStatus.Current, csUserStatus) 
    If pUserStatus IsNot Nothing Then 
      If pUserStatus.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _UserStatusCol.Count > 0 AndAlso dgvUserStatus.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvUserStatus.Columns.Count - 1 
        If dgvUserStatus.Columns(i).Visible Then 
          dgvUserStatus.CurrentCell = dgvUserStatus.Rows(dgvUserStatus.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvUserStatus.Refresh() 
      dgvUserStatus.Rows(dgvUserStatus.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvUserStatus.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvUserStatus_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvUserStatus.RowValidating 
    If _Loading = True OrElse dgvUserStatus.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvUserStatus.CurrentCell = dgvUserStatus.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvUserStatus_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvUserStatus.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvUserStatus.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvUserStatus.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvUserStatus.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pUserStatus Is Nothing Then pUserStatus = CType(dgvUserStatus.Rows(e.RowIndex).DataBoundItem, csUserStatus) ' Only assign it if needed 
    '  If pUserStatus.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pUserStatus.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvUserStatus.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pUserStatus Is Nothing Then pUserStatus = CType(dgvUserStatus.Rows(e.RowIndex).DataBoundItem, csUserStatus) ' Only assign it if needed
    '  If pUserStatus.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pUserStatus.RAV - pUserStatus.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvUserStatus.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvUserStatus.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvUserStatus.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvUserStatus.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvUserStatus.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvUserStatus.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvUserStatus.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvUserStatus_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvUserStatus.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvUserStatus_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvUserStatus.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvUserStatus.Columns(e.ColumnIndex)
    If bsCtlUserStatus.Current Is Nothing Then Exit Sub
    ...
    Dim pUserStatusCol As csUserStatusCol
    pUserStatusCol = CType(bsCtlUserStatus.DataSource, csUserStatusCol)

    Dim pSummaryRow As csUserStatus = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pUserStatusCol(pUserStatusCol.Count - 1) 
      pUserStatusCol.RemoveAt(pUserStatusCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pUserStatusCol.Count - 1 
          pUserStatusCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pUserStatusCol.SortByID()
      ElseIf pNewColumn Is colUser OrElse pNewColumn Is colUserText Then
        pUserStatusCol.SortByUserText()
      ElseIf pNewColumn Is colApplicationName Then
        pUserStatusCol.SortByApplicationName()
      ElseIf pNewColumn Is colLastLoggedLoginID Then
        pUserStatusCol.SortByLastLoggedLoginID()
      ElseIf pNewColumn Is colLoginTime Then
        pUserStatusCol.SortByLoginTime()
      ElseIf pNewColumn Is colLogoutTime Then
        pUserStatusCol.SortByLogoutTime()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csUserStatus In pUserStatusCol 
        ...
        ElseIf pNewColumn Is colLastLoggedLoginID Then
          Dim pTest As Long = 0 
          For Each p As csUserStatus In pUserStatusCol 
            If p.LastLoggedLoginID <> pTest Then iCntr += 1 : pTest = p.LastLoggedLoginID 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLoginTime Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As csUserStatus In pUserStatusCol 
            If p.LoginTime <> pTest Then iCntr += 1 : pTest = p.LoginTime 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLogoutTime Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As csUserStatus In pUserStatusCol 
            If p.LogoutTime <> pTest Then iCntr += 1 : pTest = p.LogoutTime 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pUserStatusCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pUserStatusCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pUserStatusCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlUserStatus.Position = bsCtlUserStatus.IndexOf(pUserStatusCol.FindByID(pID))
    End If

    'dgvUserStatus.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvUserStatus.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pUserStatusCol As csUserStatusCol 
      pUserStatusCol = CType(bsCtlUserStatus.DataSource, csUserStatusCol) 
      Dim pUserStatus As csUserStatus = pUserStatusCol.FindByID(pID) 
      If Not pUserStatus.IsEmpty Then 
        bsCtlUserStatus.Position = bsCtlUserStatus.IndexOf(pUserStatusCol.FindByID(pID)) 
        dgvUserStatus.Rows(bsCtlUserStatus.Position).Selected = True 
      Else 
        dgvUserStatus.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvUserStatus_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvUserStatus.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvUserStatus.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvUserStatus_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvUserStatus.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvUserStatus_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvUserStatus.Scroll
    dgvUserStatus.Invalidate() 
  End Sub
 
  Private Sub dgvUserStatus_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvUserStatus.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvUserStatus.Rows.Count - 1 Then Exit Sub
 
    'If dgvUserStatus.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'UserStatus', the row with an ID of " & dgvUserStatus.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'UserStatus', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvUserStatus.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvUserStatus.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-UserStatus-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvUserStatus(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvUserStatus(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvUserStatus_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvUserStatus.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvUserStatus.CurrentCell = dgvUserStatus.Rows(dgvUserStatus.CurrentCell.RowIndex).Cells(0) 
      dgvUserStatus.Refresh() 
    End If 
  End Sub 
  Private Sub dgvUserStatus_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvUserStatus.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvUserStatus_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvUserStatus.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvUserStatus_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUserStatus.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvUserStatus.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvUserStatus(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pUserStatus As csUserStatus = _UserStatusCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pUserStatus, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "UserStatus Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_UserStatus", pUserStatus, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvUserStatus_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvUserStatus.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvUserStatus.Rows.Count - 1 Then dgvUserStatus.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pUserStatus As csUserStatus = _UserStatusCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pUserStatus) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvUserStatus_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUserStatus.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvUserStatus.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvUserStatus.Columns(pNewCol).Visible = False Then 
      dgvUserStatus.Columns(pNewCol).Visible = True 
      dgvUserStatus.CurrentCell = dgvUserStatus.CurrentRow.Cells(pNewCol) 
      dgvUserStatus.CurrentCell = dgvUserStatus.CurrentRow.Cells(pOriginalCol) 
      dgvUserStatus.Columns(pNewCol).Visible = False 
    Else 
      dgvUserStatus.CurrentCell = dgvUserStatus.CurrentRow.Cells(pNewCol) 
      dgvUserStatus.CurrentCell = dgvUserStatus.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pUserStatus, csUserStatus), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pUserStatus.Update(_Requester) 
    ...
    Else 
      If pFault.isOK = False Then 'AndAlso pFault.Severity = clsEnums.enmFaultSeverity.LogOnly  
        ShowFault(pFault, _Requester) 
      End If 
      dgvUserStatus.EndEdit() 
      _DVGDirty = False 
      RaiseEvent evtUpdated(pUserStatus) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvUserStatus.Columns 
  ...
  Private Sub Summarize() 
    If _UserStatusCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pLastLoggedLoginID As Long 
    For Each pExistingRow As csUserStatus In _UserStatusCol 
      If _SummaryOverFlow.IndexOf("#LastLoggedLoginID#") < 0 Then 
        Try 
          pLastLoggedLoginID += pExistingRow.LastLoggedLoginID 
        Catch ex As System.OverflowException 
          pLastLoggedLoginID = -99999999 
          _SummaryOverFlow &= "LastLoggedLoginID#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csUserStatus.enmSummarizeableProperty.LastLoggedLoginID) = csUserStatus.enmSummarizeableProperty.LastLoggedLoginID Then pLastLoggedLoginID = 0
    Dim pSummaryRow As New csUserStatus( _ 
        vID:=0 _ 
      , vUserID:=0 _ 
      , vUserText:="" _ 
      , vApplicationName:="" _ 
      , vLastLoggedLoginID:=pLastLoggedLoginID _ 
      , vLoginTime:=Nothing _ 
      , vLogoutTime:=Nothing _ 
      , vTag:="" _ 
      , vDateAdded:=Nothing _ 
      , vWithParents:=clsEnums.enmLoadParent.TextOnly _ 
      )
    _UserStatusCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\UserStatusCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\UserStatusCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\UserStatusCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\UserStatusCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\UserStatusCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvUserStatus.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvUserStatus.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _UserStatusColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _UserStatusColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _UserStatusColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _UserStatusColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _UserStatusColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-UserStatus-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "UserStatuses" 
        .SubTitleRight = "Rows: " & _UserStatusCol.Count.ToString 
        ...
        End If 
        .DataSource = _UserStatusCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-UserStatus-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-UserStatus-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvUserStatus.Rows 
    ...
    For Each row As DataGridViewRow In dgvUserStatus.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlUserStatus), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvUserStatus.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-UserStatus-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvUserStatus.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvUserStatus.Width - 30) / dgvUserStatus.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvUserStatus.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-UserStatus-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colLastLoggedLoginID.HeaderText = pStrg : mnuColVisibleLastLoggedLoginID.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_UserStatus", "LoginTime", _Requester) 
    If pStrg <> "" Then colLoginTime.HeaderText = pStrg : mnuColVisibleLoginTime.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_UserStatus", "LogoutTime", _Requester) 
    If pStrg <> "" Then colLogoutTime.HeaderText = pStrg : mnuColVisibleLogoutTime.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleUser.Click, mnuColVisibleApplicationName.Click, mnuColVisibleLastLoggedLoginID.Click, mnuColVisibleLoginTime.Click, mnuColVisibleLogoutTime.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvUserStatus.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvUserStatus.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleLastLoggedLoginID.Checked = True Then mnuColVisibleLastLoggedLoginID.PerformClick() 
    If mnuColVisibleLoginTime.Checked = True Then mnuColVisibleLoginTime.PerformClick() 
    If mnuColVisibleLogoutTime.Checked = True Then mnuColVisibleLogoutTime.PerformClick() 
    
    _Loading = False 
    'dgvUserStatus.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvUserStatus_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvUserStatus.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the UserStatus to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvUserStatus_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvUserStatus.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvUserStatus.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvUserStatus.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvUserStatus.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvUserStatus.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvUserStatus(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvUserStatus.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvUserStatus.BeginEdit(True) 
        DirectCast(dgvUserStatus.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvUserStatus_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvUserStatus.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvUserStatus.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvUserStatus.MultiSelect = True 
        dgvUserStatus.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvUserStatus.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvUserStatus.MultiSelect = True 
    Else 
      dgvUserStatus.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlc_UserStatusCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvUserStatus.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_UserStatusCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvUserStatus.Width 
  ...
  Private Sub ctlc_UserStatusCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlc_UserStatusCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvUserStatus_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvUserStatus.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvUserStatus.Rows(e.RowIndex).Selected Then 
        dgvUserStatus.ClearSelection() 
        dgvUserStatus.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvUserStatus.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvUserStatus.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvUserStatus.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _UserStatusCol.Count Then Exit Sub 
    Dim pUserStatus As csUserStatus = _UserStatusCol(pRowIndex) 
    Dim pTitle As String = "UserStatus #" & pUserStatus.ID.ToString() 
    Dim pKey As String = "UserStatus_" & pUserStatus.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_UserStatus" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pUserStatus, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pUserStatus 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "UserStatus_" & pUserStatus.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "UserStatus #" & pUserStatus.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_UserStatus", pUserStatus, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvUserStatus.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvUserStatus.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _UserStatusCol.Count Then 
        Dim pUserStatus As csUserStatus = _UserStatusCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pUserStatus.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvUserStatus.SelectedRows(0) 
