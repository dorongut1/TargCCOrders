Public Class ctlc_RoleCol
  ...
  Public Event evtBeforeUpdate(ByVal vRole As csRole, ByRef rCancel As Boolean) 
  Public Event evtUpdated(ByVal vRole As csRole) 
  Private Event evtBeforeDelete(ByVal vRole As csRole, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vRole As csRole) 
  Public Event evtRowDoubleClicked(ByVal vRole As csRole, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As csRole.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property CbosDoNotLoad As List(Of csRole.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of csRole.enmProperty) 
    Public Property ColumnsHide As List(Of csRole.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csRole.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csRole.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csRole.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csRole.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of csRole.enmParentProperty) 
      _ColumnsReadOnly = New List(Of csRole.enmProperty) 
      _ColumnsHide = New List(Of csRole.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csRole.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csRole.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csRole.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csRole.enmProperty, String) 
  ...
  Private WithEvents _RoleCol As csRoleCol
  Private WithEvents _RoleColFullLength As csRoleCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedRole]() As csRole 
    Get 
      If dgvRole.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvRole.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvRole.Rows.Count - 1 Then dgvRole.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _RoleCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [RoleCol]() As csRoleCol 
    Get 
      Return _RoleCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pRoleCol As New csRoleCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pRoleCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pRoleCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pRoleCol As New csRoleCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "Role" 
          pFault = pRoleCol.FillByBaseRoleID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vRoleCol As csRoleCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vRoleCol) 
  End Function
  
  Private Function LoadControl(vRoleCol As csRoleCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csRole.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csRole.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csRole.enmProperty.ID Then colID.ReadOnly = True 
      If l = csRole.enmProperty.Name Then colName.ReadOnly = True 
      If l = csRole.enmProperty.BaseRole Then colBaseRole.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As csRole.enmParentProperty = csRole.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of csRole.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvRole.DoubleBuffered(True) 
 
    pFault = vRoleCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _RoleColFullLength = vRoleCol.Clone() 
 
    'Truncate the strings 
    _RoleCol = vRoleCol 
    If _LoadParameters.TruncateStrings Then 
      _RoleCol.TruncateStrings() 
    Else 
      dgvRole.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvRole.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        colBaseRole.Name = colBaseRole.Name & "zzzz" 
        colBaseRoleText.Name = colBaseRole.Name.Replace("zzzz", "") 
        If colBaseRole.DataGridView IsNot Nothing Then dgvRole.Columns.Remove(colBaseRole) 
      Else 
        If colBaseRole.ReadOnly = False Then 
          If colBaseRoleText.DataGridView IsNot Nothing Then dgvRole.Columns.Remove(colBaseRoleText) 
        Else 
          colBaseRole.Name = colBaseRole.Name & "zzzz" 
          colBaseRoleText.Name = colBaseRole.Name.Replace("zzzz", "") 
          If colBaseRole.DataGridView IsNot Nothing Then dgvRole.Columns.Remove(colBaseRole) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(csRole.enmParentProperty.BaseRole) Then 
            _LoadParameters.CbosDoNotLoad.Add(csRole.enmParentProperty.BaseRole) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvRole.Columns 
    ...
    'Hide columns  
    For Each p As csRole.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvRole.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvRole.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvRole.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvRole.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Role-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvRole.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvRole_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvRole.SelectedRows.Count > 0 Then 
      pRowIndex = dgvRole.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvRole.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvRole.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvRole.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _RoleCol.Count - 2 Then 
          dgvRole.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _RoleCol.Count - 1 Then 
          dgvRole.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'BaseRole
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = csRole.enmParentProperty.BaseRole) = csRole.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.c_RoleDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(csRole.enmParentProperty.BaseRole, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-RoleCol-200806-1015", _Requester) 
        End If 
      End If 
      pComboList = pComboList.Clone() 
      If pPrompt = "" Then pPrompt = pChoose 
        pComboList.AddToTop(ccHelper.ToLong(0), pPrompt) 
      bsBaseRole.DataSource = pComboList 
      colBaseRole.Tag = pPrompt 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_RoleUpdate, _Requester) = True Then btnEdit.Visible = Not (vInEdit) Else btnEdit.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_RoleUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_RoleUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_RoleDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      dgvRole.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvRole.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvRole.EditMode = DataGridViewEditMode.EditProgrammatically 
      ...
      'Don't automatically set the 1st one If dgvRole.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvRole.CurrentRow.Index 
      '  dgvRole.CurrentCell = dgvRole.Rows(pCurrentRow).Cells(0) 
      '  dgvRole.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("Name, ") 
    pFieldList.Append("BaseRoleID (DB Code), ") 
    
    Dim pNumberOfFields As Integer = 3 
    ...
    'Now Load the data 
    Dim pIncomingRoles As New csRoleCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingRole As New csRole(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingRole.Tag = "Row " & pRow.ToString 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingRole.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingRoles.Add(pIncomingRole) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingRole.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingRole.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Name" 
            pIncomingRole.Name = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingRole.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "BaseRoleID" 
            pIncomingRole.BaseRoleID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingRole.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingRole.Tag = "Row " & pRow.ToString Then 
            pIncomingRole.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingRoles.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingRoles.UpdateFromCollection(_Requester) 
    If pFault.isOK = False Then 
      ShowFault(pFault, _Requester) 
      Exit Sub 
    End If 
    'Reset the Role collection 
    MyCache.ClearComboList(clsEnums.enmComboListType.c_RoleDefaultByID) 
 
    Cursor = Cursors.Default 
 
    'Check that there were no problems 
    pErrorFound = False 
    For Each p In pIncomingRoles 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
  ...
  Private Sub DoEdit() 
    ...
    If dgvRole.Focused = True AndAlso dgvRole.SelectedRows.Count > 0 Then 
      pCellRow = dgvRole.CurrentCell.RowIndex 
      pCellCol = dgvRole.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvRole.CurrentCell = dgvRole.Rows(0).Cells(0) 
      dgvRole.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _RoleCol.Count > 0 AndAlso _RoleCol(_RoleCol.Count - 1).ID = 0 Then 
      _RoleCol.RemoveAt(_RoleCol.Count - 1) 
      bsCtlRole.DataSource = Nothing 
      bsCtlRole.DataSource = _RoleCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvRole.Focus() 
      dgvRole.CurrentCell = dgvRole.Rows(pCellRow).Cells(pCellCol) 
      dgvRole.CurrentCell.Selected = True 
    ElseIf _RoleCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    Dim pEntity As csRole 
    pEntity = CType(bsCtlRole.Current, csRole) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvRole.CurrentCell Is Nothing Then Return pFault 
    
    If dgvRole.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pRole As csRole 
      pRole = CType(bsCtlRole.Current, csRole) 
      If pRole Is Nothing Then 
        pFault.LogFreeTextFault("There is no Role to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvRole.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvRole.CurrentCell.RowIndex 
      'show row as selected  
      dgvRole.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvRole.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvRole.CurrentRow.Selected = True 
      If pRole.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pRole.Name & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pRole, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvRole.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvRole.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvRole.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pRole.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlRole.Remove(bsCtlRole.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pRole As csRole = CType(bsCtlRole.Current, csRole) 
    If pRole IsNot Nothing Then 
      If pRole.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _RoleCol.Count > 0 AndAlso dgvRole.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvRole.Columns.Count - 1 
        If dgvRole.Columns(i).Visible Then 
          dgvRole.CurrentCell = dgvRole.Rows(dgvRole.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvRole.Refresh() 
      dgvRole.Rows(dgvRole.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvRole.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvRole_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvRole.RowValidating 
    If _Loading = True OrElse dgvRole.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvRole.CurrentCell = dgvRole.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvRole_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvRole.CellFormatting 
    ...
    If dgvRole.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvRole.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvRole.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pRole Is Nothing Then pRole = CType(dgvRole.Rows(e.RowIndex).DataBoundItem, csRole) ' Only assign it if needed 
    '  If pRole.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pRole.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvRole.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pRole Is Nothing Then pRole = CType(dgvRole.Rows(e.RowIndex).DataBoundItem, csRole) ' Only assign it if needed
    '  If pRole.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pRole.RAV - pRole.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvRole.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvRole.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvRole.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvRole.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvRole.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvRole.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvRole.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvRole_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvRole.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvRole_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvRole.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvRole.Columns(e.ColumnIndex)
    If bsCtlRole.Current Is Nothing Then Exit Sub
    ...
    Dim pRoleCol As csRoleCol
    pRoleCol = CType(bsCtlRole.DataSource, csRoleCol)

    Dim pSummaryRow As csRole = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pRoleCol(pRoleCol.Count - 1) 
      pRoleCol.RemoveAt(pRoleCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pRoleCol.Count - 1 
          pRoleCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      ...
      ElseIf pNewColumn Is colName Then
        pRoleCol.SortByName()
      ElseIf pNewColumn Is colBaseRole OrElse pNewColumn Is colBaseRoleText Then
        pRoleCol.SortByBaseRoleText()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csRole In pRoleCol 
        ...
        ElseIf pNewColumn Is colBaseRole OrElse pNewColumn Is colBaseRoleText Then
          Dim pTest As String = "" 
          For Each p As csRole In pRoleCol 
            If p.BaseRoleText <> pTest Then iCntr += 1 : pTest = p.BaseRoleText 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pRoleCol.SortByTag() 
    ...
    If _Summarized = True Then 
      pRoleCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlRole.Position = bsCtlRole.IndexOf(pRoleCol.FindByID(pID))
    End If

    'dgvRole.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvRole.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pRoleCol As csRoleCol 
      pRoleCol = CType(bsCtlRole.DataSource, csRoleCol) 
      Dim pRole As csRole = pRoleCol.FindByID(pID) 
      If Not pRole.IsEmpty Then 
        bsCtlRole.Position = bsCtlRole.IndexOf(pRoleCol.FindByID(pID)) 
        dgvRole.Rows(bsCtlRole.Position).Selected = True 
      Else 
        dgvRole.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvRole_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvRole.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvRole.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvRole_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvRole.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvRole_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvRole.Scroll
    dgvRole.Invalidate() 
  End Sub
 
  Private Sub dgvRole_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvRole.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvRole.Rows.Count - 1 Then Exit Sub
 
    'If dgvRole.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Role', the row with an ID of " & dgvRole.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Role', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvRole.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvRole.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Role-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvRole(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvRole(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    ...
    'End If 
  End Sub
  Private Sub dgvRole_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvRole.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvRole.CurrentCell = dgvRole.Rows(dgvRole.CurrentCell.RowIndex).Cells(0) 
      dgvRole.Refresh() 
    End If 
  End Sub 
  Private Sub dgvRole_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvRole.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvRole_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvRole.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvRole_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvRole.CellDoubleClick 
    ...
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvRole.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvRole(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pRole As csRole = _RoleCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pRole, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Role Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_Role", pRole, _Requester) 
  ...
  Private Sub dgvRole_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvRole.SelectionChanged 
    ...
    If _Summarized = True AndAlso RowIndex = dgvRole.Rows.Count - 1 Then dgvRole.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pRole As csRole = _RoleCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pRole) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvRole_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvRole.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvRole.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvRole.Columns(pNewCol).Visible = False Then 
      dgvRole.Columns(pNewCol).Visible = True 
      dgvRole.CurrentCell = dgvRole.CurrentRow.Cells(pNewCol) 
      dgvRole.CurrentCell = dgvRole.CurrentRow.Cells(pOriginalCol) 
      dgvRole.Columns(pNewCol).Visible = False 
    Else 
      dgvRole.CurrentCell = dgvRole.CurrentRow.Cells(pNewCol) 
      dgvRole.CurrentCell = dgvRole.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pRole, csRole), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pRole.Update(_Requester) 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvRole.Columns 
  ...
  Private Sub Summarize() 
    If _RoleCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As csRole In _RoleCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\RoleCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\RoleCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\RoleCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\RoleCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\RoleCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvRole.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvRole.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _RoleColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _RoleColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _RoleColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _RoleColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _RoleColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Role-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Roles" 
        .SubTitleRight = "Rows: " & _RoleCol.Count.ToString 
    ...
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Role-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Role-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvRole.Rows 
    ...
    For Each row As DataGridViewRow In dgvRole.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlRole), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvRole.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Role-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvRole.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvRole.Width - 30) / dgvRole.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvRole.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Role-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    pStrg = ccHelper.GetLocalizedFieldName("c_Role", "ID", _Requester) 
    If pStrg <> "" Then colID.HeaderText = pStrg : mnuColVisibleID.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Role", "Name", _Requester) 
    If pStrg <> "" Then colName.HeaderText = pStrg : mnuColVisibleName.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Role", "BaseRole", _Requester) 
    If pStrg <> "" Then colBaseRole.HeaderText = pStrg : mnuColVisibleBaseRole.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleName.Click, mnuColVisibleBaseRole.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvRole.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvRole.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleBaseRole.Checked = True Then mnuColVisibleBaseRole.PerformClick() 
    'Show Defaults 
  ...
  Private Sub dgvRole_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvRole.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Role to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
      If pMessageBox <> frmMessageOrInputBox.enmButtonReturned.Cancel Then 
        ...
        If pMessageBox = frmMessageOrInputBox.enmButtonReturned.No Then 
          Clipboard.SetText(pRole.ToCSV) 
        Else 
          Clipboard.SetText(pRole.ToString.Replace(" ‡ ", Environment.NewLine)) 
        End If 
        frmMessageOrInputBox.ShowMsg("The Role is in your clipboard", frmMessageOrInputBox.enmIconType.Information) 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvRole_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvRole.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvRole.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvRole.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvRole.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvRole.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvRole(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvRole.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvRole.BeginEdit(True) 
        DirectCast(dgvRole.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvRole_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvRole.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvRole.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvRole.MultiSelect = True 
        dgvRole.ClearSelection() 
  ...
  Private Sub ctlc_RoleCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvRole.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_RoleCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvRole.Width 
  ...
  Private Sub ctlc_RoleCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlc_RoleCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvRole_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvRole.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvRole.Rows(e.RowIndex).Selected Then 
        dgvRole.ClearSelection() 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvRole.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvRole.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvRole.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _RoleCol.Count Then Exit Sub 
    Dim pRole As csRole = _RoleCol(pRowIndex) 
    Dim pTitle As String = "Role #" & pRole.ID.ToString() 
    Dim pKey As String = "Role_" & pRole.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_Role" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pRole, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pRole 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Role_" & pRole.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Role #" & pRole.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_Role", pRole, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvRole.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvRole.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _RoleCol.Count Then 
        Dim pRole As csRole = _RoleCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pRole.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvRole.SelectedRows(0) 
