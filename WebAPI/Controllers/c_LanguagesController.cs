...
{ 
    ...
    { 
        // GET api/languages?page=0&pageSize=25&search=xyz 
        [Route("languages")] 
        [HttpGet] 
        ...
        { 
            ...
            clsFault? fault = null; 
            csLanguageCol languages = new csLanguageCol(requester, ref fault); if (!fault.isOK) return BadRequest(!string.IsNullOrEmpty(fault.FreeText) ? fault.FreeText : fault.Message);
 
            var allItems = languages.Select(p => p.ToDto()); 
            ...
            { 
                ...
                allItems = allItems.Where(item => 
                    ...
                    (item.NameLoc ?? "").ToLower().Contains(searchLower) ||
                    (item.Culture ?? "").ToLower().Contains(searchLower)
                ); 
            } 
            ...
            { 
                var prop = typeof(LanguageDto).GetProperty(sortField, System.Reflection.BindingFlags.IgnoreCase | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance); 
                if (prop != null) 
                { 
        ...
        [HttpGet] 
        //[Authorize] //TODO: enable when auth is configured
        public ActionResult<LanguageDto> GetByID(long id) 
        { 
            clsRequester requester = new clsRequester("*", "View", true); //TODO: replace with JWT/ticket authentication 
            requester.CallingFunctionWithinApplication = "WebAPI"; 
 
            csLanguage language = new csLanguage(); 
            clsFault fault = language.GetByID(id, requester); if (!fault.isOK) return NotFound(fault.Message); 
