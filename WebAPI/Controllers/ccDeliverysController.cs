...
{ 
    [Route("api")] 
    [ApiController] 
    public partial class DeliverysController : ControllerBase 
    { 
        // GET api/deliveries?page=0&pageSize=25&search=xyz 
        [Route("deliveries")] 
        [HttpGet] 
        ...
        { 
            ...
            clsFault? fault = null; 
            clsDeliveryCol deliveries = new clsDeliveryCol(clsEnums.enmLoadParent.DoNotLoad, requester, ref fault); if (!fault.isOK) return BadRequest(!string.IsNullOrEmpty(fault.FreeText) ? fault.FreeText : fault.Message);
 
            var allItems = deliveries.Select(p => p.ToDto()); 
            ...
            { 
                ...
                allItems = allItems.Where(item => 
                    ...
                    (item.DeliveryAddress ?? "").ToLower().Contains(searchLower) ||
                    (item.ContactPhone ?? "").ToLower().Contains(searchLower) ||
                    (item.ContactName ?? "").ToLower().Contains(searchLower) ||
                    item.EnmDeliveryMethod.ToString().ToLower().Contains(searchLower) ||
                    item.EnmDeliveryStatus.ToString().ToLower().Contains(searchLower) ||
                    (item.Location ?? "").ToLower().Contains(searchLower) ||
                    (item.ProductsSummary ?? "").ToLower().Contains(searchLower) ||
            ...
            { 
                var prop = typeof(DeliveryDto).GetProperty(sortField, System.Reflection.BindingFlags.IgnoreCase | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance); 
                if (prop != null) 
                { 
        ...
        { 
            ...
            clsDelivery delivery = new clsDelivery(clsEnums.enmLoadParent.DoNotLoad); 
            clsFault fault = delivery.GetByID(id, requester); if (!fault.isOK) return NotFound(fault.Message); 
        ...
        [HttpPost] 
        [Authorize(Policy = "AdminUI")]
        public ActionResult<DeliveryDto> CreateDelivery(DeliveryUpdateDto deliveryDto) 
        { 
            ...
            if (deliveryDto.Id != 0) return BadRequest($"Received an ID of {deliveryDto.Id}. Expected 0 for a new record."); 
 
            clsDelivery delivery; 
 
            try 
            { 
                delivery = deliveryDto.FromDto(requester); 
            } 
            catch (Exception e) 
            { 
            ...
            clsFault fault = delivery.Update(requester); if (!fault.isOK) return BadRequest(!string.IsNullOrEmpty(fault.FreeText) ? fault.FreeText : fault.Message);
 
            return CreatedAtAction(nameof(GetByID), new { id = delivery.ToDto().Id }, delivery.ToDto()); 
        } 
        ...
        [HttpPut] 
        [Authorize(Policy = "AdminUI")]
        public ActionResult<DeliveryDto> UpdateDelivery(long id, DeliveryUpdateDto deliveryDto) 
        { 
            if (!ModelState.IsValid) return BadRequest(ModelState); 
            ...
            if (deliveryDto.Id != id) return BadRequest($"ID received {id}, but ID in object is {deliveryDto.Id}"); 
            ...
            { 
                ...
                { 
                    try 
                    { 
                        var existing = new clsDelivery(); 
                        existing.GetByID(id, requester, false); 
                        var serverEtag = Dtos.DeliveryExtensions.ComputeETag(existing); 
        ...
        { 
            ...
            clsFault fault = delivery.GetByID(id, requester); if (!fault.isOK) return NotFound(fault.Message); 
            fault = delivery.Delete(requester); if (!fault.isOK) return BadRequest(!string.IsNullOrEmpty(fault.FreeText) ? fault.FreeText : fault.Message);
 
            return NoContent(); 
        } 
 
        // DELETE api/deliveries/batch 
        [Route("deliveries/batch")] 
        [HttpDelete] 
        [Authorize(Policy = "AdminUI")]
        ...
        { 
            ...
            { 
                clsDelivery delivery = new clsDelivery(); 
                clsFault fault = delivery.GetByID(id, requester); 
                if (!fault.isOK) { errors.Add($"ID {id}: {fault.Message}"); continue; } 
                fault = delivery.Delete(requester); 
                if (!fault.isOK) { errors.Add($"ID {id}: {fault.Message}"); continue; } 
                deleted++; 
            } 
        ...
        // PATCH api/deliveries/batch — Update specific fields on multiple records 
        [Route("deliveries/batch")] 
        [HttpPatch] 
        [Authorize(Policy = "AdminUI")]
        ...
        { 
            ...
            { 
                ...
                { 
                    ...
                    var fieldErrors = PatchHelper.ApplyFields(delivery, typeof(DeliveryUpdateDto), request.Fields);
                    if (fieldErrors.Count > 0) { errors.Add($"ID {id}: " + string.Join("; ", fieldErrors)); continue; }

                    fault = delivery.Update(requester); 
                    if (!fault.isOK) { errors.Add($"ID {id}: {fault.Message}"); continue; } 
                    updated++; 
                } 
