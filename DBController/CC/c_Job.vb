Public Class csJob
  ...
  Public Property [NextRunTime]() As Date
    ...
    End Get
    Set(ByVal value As Date)
      If Me._NextRunTime <> value Then 
        bccStatus = clsEnums.enmObjectStatus.Dirty 
        Me._NextRunTime = value 
      End If 
    End Set
  End Property
  Public Property [LastRunTime]() As Date
    ...
    End Get
    Set(ByVal value As Date)
      If Me._LastRunTime <> value Then 
        bccStatus = clsEnums.enmObjectStatus.Dirty 
        Me._LastRunTime = value 
      End If 
    End Set
  End Property
  Public Property [JobStatus]() As clsEnums.enmJobStatus
    Get
      Return Me._JobStatus
    End Get
    Set(ByVal value As clsEnums.enmJobStatus)
      If Me._JobStatus <> value Then 
        bccStatus = clsEnums.enmObjectStatus.Dirty 
        Me._JobStatus = value 
      End If 
    End Set
  ...
  Public Property [JobStatusText]() As String
    ...
    End Set
  End Property
  Public Property [WarningMailSent]() As Boolean
    Get
      Return Me._WarningMailSent
    End Get
    Set(ByVal value As Boolean)
      If Me._WarningMailSent <> value Then 
        bccStatus = clsEnums.enmObjectStatus.Dirty 
        Me._WarningMailSent = value 
      End If 
    End Set
  End Property
  ...
  Friend Sub CleanEntityForXML() 
    ...
    _JobCode = ccHelper.RemoveChrW0(_JobCode) 
    _JobRunnerCode = ccHelper.RemoveChrW0(_JobRunnerCode) 
    _Description = ccHelper.RemoveChrW0(_Description) 
    _Instructions = ccHelper.RemoveChrW0(_Instructions) 
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_GetByPrimaryKey", vRequester) 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = 0 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"Job not found for GetByID, since its value is 0", pFunctionParameters, "TRGT-Job-210927-1527", vRequester, vAdditionalMessageToUser:=$"Job not found") 
      pFault.SetOK() 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      pFault = LoadMeFromDBCache(MyController.DBCache.ccJobCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobGetByID" 
      ...
      Try 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"Job not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-Job-210625-0950", vRequester, vAdditionalMessageToUser:=$"Job not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function GetByJobCodeAndJobRunnerCode(ByVal vJobCode As String, ByVal vJobRunnerCode As String, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_GetByJobCodeAndJobRunnerCode", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      pFault = LoadMeFromDBCache(MyController.DBCache.ccJobCol.FindByJobCodeAndJobRunnerCode(vJobCode, vJobRunnerCode), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobGetByJobCode&JobRunnerCode" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
   
      Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters 
        pLastReadVariableName = "lkpJob" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = ccHelper.LookupNullable(vJobCode) 
        pLastReadVariableName = "lkpJobRunner" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = ccHelper.LookupNullable(vJobRunnerCode) 
        pLastReadVariableName = "" 
        ...
        'Execute query 
        pFault = ccDAL.ExecuteQuery(pCommandText, pDALParameters, vRequester, Me) : If Not pFault.isOK Then Return pFault 
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"Job not found for GetByJobCodeAndJobRunnerCode. See FunctionParameters for values", pFunctionParameters, "TRGT-Job-210625-0950", vRequester, vAdditionalMessageToUser:=$"Job not found") 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090623-1648", vRequester) 
      End Try 
  ...
  Public Function AddUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityAddable.AddUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJob_AddUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function EditUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityEditable.EditUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJob_EditUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Friend Function UpdateFriend(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJob_UpdateFriend", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If Me.isEqual(pJob) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", pFunctionParameters, "TRGT-Job-100113-1638", vRequester) 
    End If 
    ...
    Dim pCommandText As String = "c_JobUpdateFriend"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pCachedJob As csJob 
      If _ID = 0 Then 
        pCachedJob = New csJob() 
        'get last ID 
        Dim pJobCol As csJobCol = MyController.DBCache.ccJobCol.Clone() 
        If pJobCol.Count = 0 Then 
          _ID = 1 
        Else 
          pJobCol.SortByID() 
          Dim pLastID As Long = pJobCol(pJobCol.Count - 1).ID
          _ID = pLastID + 1 
        End If 
        bPrimaryKey = _ID 
        MyController.DBCache.ccJobCol.Add(pCachedJob) 
      Else  
        pCachedJob = MyController.DBCache.ccJobCol.FindByID(_ID) 
      End If 
      pCachedJob.AssignValues(Me) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccJobCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
        ...
        pLastReadVariableName = "lkpJob" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = ccHelper.LookupNullable(_JobCode) 
        pLastReadVariableName = "lkpJobRunner" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = ccHelper.LookupNullable(_JobRunnerCode) 
        ...
        pLastReadVariableName = "Instructions" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 1000).Value = ccHelper.ObjectNullable(_Instructions) 
        pLastReadVariableName = "enmJobType" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = (_JobType.FastToString()) 
        pLastReadVariableName = "WhenToRun" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(_WhenToRun) 
        pLastReadVariableName = "CyclicCount" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Int).Value = (_CyclicCount) 
        pLastReadVariableName = "SendNotificationOnSuccess" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_SendNotificationOnSuccess) 
        pLastReadVariableName = "SendAlarmOnMissed" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_SendAlarmOnMissed) 
        pLastReadVariableName = "TimeOutSec" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Int).Value = (_TimeOutSec) 
        pLastReadVariableName = "Active" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_Active) 
        ...
        pLastReadVariableName = "NextRunTime" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(_NextRunTime) 
        pLastReadVariableName = "LastRunTime" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(_LastRunTime) 
        pLastReadVariableName = "enmJobStatus" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = (_JobStatus.FastToString()) 
        pLastReadVariableName = "WarningMailSent" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_WarningMailSent) 
        pLastReadVariableName = "IsManaged" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_IsManaged) 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            bPrimaryKey = pID 
            If pID = 0 Then 
              pFault.LogFreeTextFault(51, "ID returned is 0!", pFunctionParameters, "TRGT-Job-151114-1147", vRequester) 
            End If 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-Job-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090623-1809", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJob_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Dim pCommandText As String = "c_JobUpdate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Shared Function UpdateSetToNow(ByVal vID As Long, ByVal vNextRunTime As Date, ByVal vActive As Boolean, ByVal vWarningMailSent As Boolean, ByVal vActivatingUser As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJob_UpdateSetToNow", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    'Check if we got an empty object  
    Dim pJob As New csJob 
    If vID = 0 Then 
      pFault.LogFreeTextFault(58, "No object to update. ID=0", pFunctionParameters, "TRGT-Job-140824-1624", vRequester) 
      Return pFault 
    End If 

    Dim pCommandText As String = "c_JobUpdateSetToNow" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    Dim pCancel As Boolean = False 
    RaiseEvent evtBeforeSharedUpdateWithRequester(enmUpdateType.ccUpdateSetToNowShared, pCommandText, pDALParameters, pCancel, vRequester, pFault) 
    If pFault.isOK = False Then Return pFault 
    If pCancel = True Then Return pFault 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pCachedJob As New csJob() : pFault = pCachedJob.GetByID(vID, vRequester, True) : If Not pFault.isOK Then Return pFault 
      pCachedJob._NextRunTime = vNextRunTime 
      pCachedJob._Active = vActive 
      pCachedJob._WarningMailSent = vWarningMailSent 
      pCachedJob._ActivatingUser = vActivatingUser 
      pFault = pCachedJob.Update(vRequester) : If Not pFault.isOK Then Return pFault 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
        ...
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt).Value = (vID) 
        pLastReadVariableName = "NextRunTime" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(vNextRunTime) 
        pLastReadVariableName = "Active" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = ccHelper.ObjectNullable(vActive) 
        pLastReadVariableName = "WarningMailSent" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = ccHelper.ObjectNullable(vWarningMailSent) 
        pLastReadVariableName = "ActivatingUser" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar).Value = ccHelper.ObjectNullable(vActivatingUser) 
        pLastReadVariableName = "ChangedBy" 
        pDALParameters.Add("ChangedBy", ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName
        pLastReadVariableName = "UpdatingLoginID" 
        pDALParameters.Add("UpdatingLoginID", ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID
        ...
        If pFault.isOK = True Then 
          ...
          If Not String.IsNullOrEmpty(pMessage) Then 
            pFault.LogFreeTextFault(51, pMessage, pFunctionParameters, "TRGT-Job-151114-1205", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJob_Delete", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    Dim pCommandText As String = "c_JobDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      MyController.DBCache.ccJobCol.Remove(MyController.DBCache.ccJobCol.FindByID(_ID)) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccJobCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-Job-151114-1147", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-Job-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090623-1813", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJob_DeleteByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      MyController.DBCache.ccJobCol.Remove(MyController.DBCache.ccJobCol.FindByID(vID)) 
      'Save File  
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccJobCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pID As Integer = ccHelper.ToInteger(pTargCCReader(0)) 
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-Job-231207-0845", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-Job-231207-0844", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-231207-0843", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If 
  ...
  Public Function FillJobAlertRecipients(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_FillJobAlertRecipients", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function FillLoggedJobs(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJob_FillLoggedJobs", vRequester) 
    If pFault.isOK = False Then Return pFault 
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Try
      ...
      pLastReadVariableName = "lkpJob" 
      If Not vReader.IsDBNull(1) Then _JobCode = vReader.GetString(1)
      pLastReadVariableName = "lkpJobRunner" 
      If Not vReader.IsDBNull(2) Then _JobRunnerCode = vReader.GetString(2)
      pLastReadVariableName = "Description" 
      If Not vReader.IsDBNull(3) Then _Description = vReader.GetString(3) 
      pLastReadVariableName = "Instructions" 
      If Not vReader.IsDBNull(4) Then _Instructions = vReader.GetString(4) 
      pLastReadVariableName = "enmJobType" 
      If Not vReader.IsDBNull(5) Then _JobType = clsEnums.TranslateEnmJobType(vReader.GetString(5))
      pLastReadVariableName = "WhenToRun" 
      If Not vReader.IsDBNull(6) Then _WhenToRun = vReader.GetDateTime(6)
      pLastReadVariableName = "CyclicCount" 
      If Not vReader.IsDBNull(7) Then _CyclicCount = vReader.GetInt32(7)
      pLastReadVariableName = "SendNotificationOnSuccess" 
      If Not vReader.IsDBNull(8) Then _SendNotificationOnSuccess = vReader.GetBoolean(8)
      pLastReadVariableName = "SendAlarmOnMissed" 
      If Not vReader.IsDBNull(9) Then _SendAlarmOnMissed = vReader.GetBoolean(9)
      pLastReadVariableName = "TimeOutSec" 
      If Not vReader.IsDBNull(10) Then _TimeOutSec = vReader.GetInt32(10)
      pLastReadVariableName = "Active" 
      If Not vReader.IsDBNull(11) Then _Active = vReader.GetBoolean(11)
      pLastReadVariableName = "ActivatingUser" 
      If Not vReader.IsDBNull(12) Then _ActivatingUser = vReader.GetString(12) 
      pLastReadVariableName = "NextRunTime" 
      If Not vReader.IsDBNull(13) Then _NextRunTime = vReader.GetDateTime(13)
      pLastReadVariableName = "LastRunTime" 
      If Not vReader.IsDBNull(14) Then _LastRunTime = vReader.GetDateTime(14)
      pLastReadVariableName = "enmJobStatus" 
      If Not vReader.IsDBNull(15) Then _JobStatus = clsEnums.TranslateEnmJobStatus(vReader.GetString(15))
      pLastReadVariableName = "WarningMailSent" 
      If Not vReader.IsDBNull(16) Then _WarningMailSent = vReader.GetBoolean(16)
      pLastReadVariableName = "IsManaged" 
      If Not vReader.IsDBNull(17) Then _IsManaged = vReader.GetBoolean(17)
      pLastReadVariableName = "LastRunBy" 
      If Not vReader.IsDBNull(18) Then _LastRunBy = vReader.GetString(18) 
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedJob As csJob, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedJob) 
      bccStatus = clsEnums.enmObjectStatus.Clean 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-Job-121122-1607", vRequester) 
    End Try 
...
#End Region
  ...
  Private Sub csJob_evtBeforeUpdateWithRequester(vWhichColumn As enmUpdateType, ByRef rCommandText As String, ByRef rDALParameters As ccDAL.csTargCCParameterCol, ByRef rCancel As Boolean, vRequester As clsRequester, ByRef rFault As clsFault) Handles Me.evtBeforeUpdateWithRequester 
    If vWhichColumn = enmUpdateType.Standard Then 
      'if it was updated externally, have scan job set the next run time 
      _NextRunTime = Nothing 
      _ActivatingUser = vRequester.UserName 
      rFault = UpdateFriend(vRequester, True) ' do a reload, since we don't know what the user wanted, and we prefer not to do another update 
      rCancel = True 
    End If 
  End Sub 
 
  ''' <summary> 
  ''' This updates sets the job to in-process, if it's not in-process. 
  ''' If it was already in-process, it returns an ID of 0; Otherwise, it returns its own ID 
  ''' </summary> 
  ''' <param name="rID"></param> 
  ''' <param name="vLastRunBy"></param> 
  ''' <param name="vRequester"></param> 
  ''' <returns></returns> 
  ''' <remarks></remarks> 
  Friend Shared Function UpdateSetToInProcess(ByRef rID As Long, ByVal vLastRunBy As String, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = String.Format("Job.ID={0}, LastRunBy={1}", rID, vLastRunBy) 
    Dim pFault As New clsFault 
    ...
    Dim pCommandText As String = "c_JobUpdateSetToInProcess" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-Job-150216-2152", vRequester) 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
        pLastReadVariableName = "ID" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt).Value = (rID) 
        pLastReadVariableName = "LastRunBy" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 50).Value = ccHelper.ObjectNullable(vLastRunBy) 
        pLastReadVariableName = "ChangedBy" 
        pDALParameters.Add("ChangedBy", ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName 
        pLastReadVariableName = "UpdatingLoginID" 
        pDALParameters.Add("UpdatingLoginID", ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID 
        ...
        If pFault.isOK = True Then 
          ...
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pID As Long = pTargCCReader.GetInt64(0) 
            If pID = 0 Then pMessage = "ID returned is 0!" 
            If pID <> rID Then pMessage = "ID returned is different than the one sent!" 
          Else 
            pMessage = "No ID returned" 
          End If 
          If Not String.IsNullOrEmpty(pMessage) Then 
...
Public Class csJobCol
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.Clone() 
      pJobsCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pJobsCached.Reverse() 
      If vHowMany > 0 AndAlso pJobsCached.Count > vHowMany Then 
        Dim tmp As New csJobCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pJobsCached(i)) 
        Next 
        pJobsCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByActive(ByVal vActive As Boolean, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByActive", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.CloneByActive(vActive)
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillByActive" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        'set parameters 
        pLastReadVariableName = "Active" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (vActive) 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090624-1702", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByJobCode(ByVal vJobCode As String, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByJobCode", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.CloneByJobCode(vJobCode)
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillByJobCode" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByJobRunnerCode(ByVal vJobRunnerCode As String, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByJobRunnerCode", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.CloneByJobRunnerCode(vJobRunnerCode)
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillByJobRunnerCode" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  ''' <summary>
  ''' Gets a collection of all the items for a specific JobRunnerCodeAndActive, or a sub-collection defined by HowMany and Direction. To append to an existing collection, set vAppend to true (default is false)
  ''' </summary>
  ''' <param name="vHowMany">How Many (sorted by ID) - 0 for all</param>
  ''' <param name="vDir">ASCending or DESCending</param>
  ''' <param name="vAppend">True of False</param>
  ...
  Public Function FillByJobRunnerCodeAndActive(ByVal vJobRunnerCode As String, ByVal vActive As Boolean, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByJobRunnerCodeAndActive", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.CloneByJobRunnerCodeAndActive(vJobRunnerCode, vActive)
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillByJobRunnerCode&Active" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccJobCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccJobCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csJobCol failed: " & pResponse) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pJobsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lJob As New csJob() 
      pFault = lJob.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lJob.IsEmpty Then Me.Add(lJob) 
    Next 
 
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController 
      Dim pJobs As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pJobs, "csJobCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pJobs IsNot Nothing AndAlso Me.Count <> pJobs.Count Then FillFromListOfITargCCEntity(pJobs) 
    End If 
  ...
  Public Overrides Function FillOnTheFly(ByVal vParameters As Dictionary(Of System.Enum, Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-Job-121122-2008", vRequester) 
      Dim pJobsCached As csJobCol = MyController.DBCache.ccJobCol.Clone() 
      Dim pJobsToUse As New csJobCol() 
      For Each l In pJobsCached 
        If vIDFrom.HasValue Then 
          ...
          End If 
        End If 
        If Not String.IsNullOrEmpty(vJobCode) Then 
          If Not l.JobCode.Equals(vJobCode, StringComparison.OrdinalIgnoreCase) Then Continue For 
        End If 
        If Not String.IsNullOrEmpty(vJobRunnerCode) Then 
          If Not l.JobRunnerCode.Equals(vJobRunnerCode, StringComparison.OrdinalIgnoreCase) Then Continue For 
        End If 
        If vActive.HasValue Then 
          If l.Active <> vActive.Value Then Continue For 
        End If 
        pJobsToUse.Add(l) 
      Next 
      pJobsToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pJobsToUse.Reverse() 
      If vHowMany > 0 AndAlso pJobsToUse.Count > vHowMany Then 
        Dim tmp As New csJobCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pJobsToUse(i)) 
        Next 
        pJobsToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pJobsToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overrides Function FillSumOnTheFly(ByVal vParameters As Dictionary(Of [Enum], Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-Job-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_JobsFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = ccHelper.ObjectNullable(vActive) 
        pLastReadVariableName = "lkpJob" 
        pDALParameters.Add("GroupBylkpJob", ccDAL.enmSQLDataType.Bit).Value = vGroupByJobCode
        pLastReadVariableName = "lkpJobRunner" 
        pDALParameters.Add("GroupBylkpJobRunner", ccDAL.enmSQLDataType.Bit).Value = vGroupByJobRunnerCode
        pLastReadVariableName = "Active" 
        pDALParameters.Add("GroupByActive", ccDAL.enmSQLDataType.Bit).Value = vGroupByActive
        pLastReadVariableName = "" 
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.Update
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobView, "csJobCol_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each pExists As csJob In Me 
      ...
      ElseIf pExists.ccStatus = clsEnums.enmObjectStatus.UD Then 
        ...
        Return pFault.LogFreeTextFault("Status should not be UD ", pFunctionParameters, "TRGT-Job-130415-0942", vRequester) 
      End If 
    Next 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobUpdate, "csJobCol_UpdateFromCollection", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_Delete", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      pFault = MyController.DBCache.SaveData(New csJobCol(), vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByActive(ByVal vActive As Boolean, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_DeleteByActive", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDeleteByActive"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllJobs As New csJobCol() : pAllJobs.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredJobs As csJobCol = pAllJobs.CloneByActive(vActive) 
      For Each l In pFilteredJobs 
        pAllJobs.Remove(pAllJobs.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllJobs, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByJobCode(ByVal vJobCode As String, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_DeleteByJobCode", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDeleteByJobCode"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllJobs As New csJobCol() : pAllJobs.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredJobs As csJobCol = pAllJobs.CloneByJobCode(vJobCode) 
      For Each l In pFilteredJobs 
        pAllJobs.Remove(pAllJobs.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllJobs, vRequester) 
    ...
    Else 
      ...
      Try 
        'set parameters  
        pLastReadVariableName = "lkpJob" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = (vJobCode) 
        pLastReadVariableName = "ChangedBy" 
        pDALParameters.Add("ChangedBy", ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName
        pLastReadVariableName = "UpdatingLoginID" 
        pDALParameters.Add("UpdatingLoginID", ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID
  ...
  Public Shared Function DeleteByJobRunnerCode(ByVal vJobRunnerCode As String, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_DeleteByJobRunnerCode", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDeleteByJobRunnerCode"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllJobs As New csJobCol() : pAllJobs.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredJobs As csJobCol = pAllJobs.CloneByJobRunnerCode(vJobRunnerCode) 
      For Each l In pFilteredJobs 
        pAllJobs.Remove(pAllJobs.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllJobs, vRequester) 
    ...
    Else 
      ...
      Try 
        'set parameters  
        pLastReadVariableName = "lkpJobRunner" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = (vJobRunnerCode) 
        pLastReadVariableName = "ChangedBy" 
        pDALParameters.Add("ChangedBy", ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName
        pLastReadVariableName = "UpdatingLoginID" 
        pDALParameters.Add("UpdatingLoginID", ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID
  ...
  Public Shared Function DeleteByJobRunnerCodeAndActive(ByVal vJobRunnerCode As String, ByVal vActive As Boolean, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_DeleteByJobRunnerCodeAndActive", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDeleteByJobRunnerCode&Active"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllJobs As New csJobCol() : pAllJobs.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredJobs As csJobCol = pAllJobs.CloneByJobRunnerCodeAndActive(vJobRunnerCode, vActive) 
      For Each l In pFilteredJobs 
        pAllJobs.Remove(pAllJobs.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllJobs, vRequester) 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_JobDelete, "csJobCol_DeleteByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_JobsDeleteByBoundedID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-Job-150216-2148", vRequester) 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-090210-1341", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    While vReader.Read()
      pJob = New csJob() 
      pFault = pJob.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pJob)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pJob.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedJobCol As csJobCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedJob As csJob In vCachedJobCol 
        pJob = New csJob(pCachedJob) 
        pJob.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pJob) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pJob.ID) 
      Next 
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-Job-121122-1513", vRequester) 
      End Try 
