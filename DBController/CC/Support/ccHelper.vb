...
Public Class ccHelper 
  ...
  ''' <summary> 
  ''' <br>This runs a stored procedure. The stored procedure should end with 'SELECT -1 AS ID', 'XXXX' AS Response. -1 is a flag of success, while XXX is the response you want.</br> 
  ''' <br>If you don't want to return a response, use ''.</br> 
  ''' <br>If there are no parameters simply pass Nothing (null)  </br> 
  ''' <br>If don't sent vCommandTimeoutSec, it will use the default (30 sec)</br> 
  ''' </summary> 
  ''' <param name="vSprocName"></param> 
  ''' <param name="rParameters"></param> 
  ''' <param name="rResponse"></param> 
  ''' <param name="vRequester"></param> 
  ''' <returns></returns> 
  Friend Shared Function RunStoredProcedure(ByVal vSprocName As String, ByVal rParameters As ccDAL.csTargCCParameterCol, ByRef rResponse As String, ByVal vRequester As clsRequester, Optional vCommandTimeoutSec As Integer = 0) As clsFault 
    ...
    Else 
      ...
      Dim pParameters As ccDAL.csTargCCParameterCol = rParameters 
      If pParameters Is Nothing Then pParameters = New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pFault = ccDAL.ExecuteQuery(pCommandText, pParameters, vRequester, pTargCCReader, vCommandTimeoutSec:=vCommandTimeoutSec) 
 
        'I expect to get -1 back     
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pDummyFieldThatSaysWeSucceeded As Integer = ccHelper.ToInteger(pTargCCReader(0)) 
            If pDummyFieldThatSaysWeSucceeded <> -1 Then 
              pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-170612-2135", vRequester) 
            Else 
              If pTargCCReader.FieldCount = 1 Then 
                rResponse = "There was no Response field" 
              Else 
                rResponse = CStr(pTargCCReader(1)) 
              End If 
            End If 
          Else 
  ...
  ''' <summary> 
  ''' This function is used by the web service to mark a document as uploaded. If you want to delete the file, then the filename with '.*' 
  ''' </summary> 
  ...
  ''' <param name="vRequester"></param> 
  ''' <returns></returns> 
  Public Shared Function MarkDocumentAsUploaded(ByVal vTableName As String, ByVal vFieldName As String, ByVal vID As Long, ByVal vFileName As String, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
 
    'anyone can do it, as long as they are logged in 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_ProcessView, "ccHelper_MarkDocumentAsUploaded", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    'Check Filename validity 
    If vFileName.EndsWith(".*", StringComparison.OrdinalIgnoreCase) Then 
      vFileName = "" 
    Else 
      Dim pRoot As String = vFileName.Split("."c)(0) 
      If Not $"{vTableName}_{vFieldName}_{vID}".Equals(pRoot) Then 
        Return pFault.LogFreeTextFault($"Invalid filename received for {vTableName}_{vFieldName}_{vID}", pFunctionParameters, "TRGT-210318-0816", vRequester) 
      End If 
    End If 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-170612-2136", vRequester) 
    Else 
      Dim pCommandText As String = "c__UpdateLoadedFile" 
      ...
      Try 
        ...
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = vTableName 
        pLastReadVariableName = "FieldName" 
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 50).Value = vFieldName 
        pLastReadVariableName = "ID" 
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt).Value = vID 
        pLastReadVariableName = "FileName" 
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 69).Value = vFileName 
        pLastReadVariableName = "ChangedBy" 
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName 
        pLastReadVariableName = "UpdatingLoginID" 
        pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID 
        pLastReadVariableName = "" 
  ...
  Public Shared Function SaveFileToLogLocation(ByVal vData As String, ByVal vFileName As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pTask As String = "" 
    Try 
      pTask = "Unzipping Data" 
      Dim pString As String = UnZip(vData) 
      pTask = "Saving file" 
      IO.File.WriteAllText(MyController.LogLocation & vFileName, pString, Text.Encoding.UTF8) 
    Catch ex As Exception 
      Return pFault.LogFreeTextFault("Failed while " & pTask, "Filename:" & MyController.LogLocation & vFileName, "TRGT-140904-2125", vRequester) 
    End Try 
  ...
  Public Shared Function DownloadFileSecurely(ByVal vTask As String, ByVal vFileName As String, ByRef rFullDownloadedFileName As String, ByVal vRequester As clsRequester) As clsFault  
    ...
    pFault = pSystemDefault.GetByFullSettingName(csSystemDefault.enmFullSettingName.Config_UploadedFilesRootFolder, vRequester, vMustExist:=True) : If Not pFault.isOK Then Return pFault 
    pRoot = pSystemDefault.SettingValue : If Not pRoot.EndsWith("\") Then pRoot &= "\" 
 
    Dim pFullFileName As String = $"{pRoot}{vTask}\{vFileName}" 
 
    'check that the file exists  
    If Not IO.File.Exists(pFullFileName) Then 
      Return pFault.LogFreeTextFault(65, $"File origin: {pFullFileName}", "", "TRGT-240221-121919", vRequester) 
    End If 
    ...
    Try 
      IO.File.Copy(pFullFileName, rFullDownloadedFileName, overwrite:=True) 
    Catch ex As IO.FileNotFoundException 
      Return pFault.LogException(65, ex, $"Tried to copy from: {pFullFileName}", "TRGT-2400602-224528", vRequester) 
    Catch ex As Exception 
      If pFault.FreeText.IndexOf("because it is being used by another process", StringComparison.OrdinalIgnoreCase) >= 0 Then 
        pFault.LogException(159, ex, $"Copy from: {pFullFileName}", "TRGT-240220-171708", vRequester) 
        pFault.AddToUserMessage("The file you are trying to view is locked on your computer.") 
      Else 
        pFault.LogException(ex, $"Copy from: {pFullFileName}", "TRGT-240220-171708", vRequester) 
      End If 
      Return pFault 
    End Try 
 
    If Not IO.File.Exists(rFullDownloadedFileName) Then 
      Return pFault.LogFreeTextFault(65, $"Copy from: {pFullFileName} failed. Check the text logs on the server", "", "TRGT-220220-171751", vRequester) 
    Else 
      Dim p As New IO.FileInfo(rFullDownloadedFileName)  
      If p.Length = 0 Then 
        Try  
          IO.File.Delete(rFullDownloadedFileName)  
        Catch ex As Exception  
        End Try  
        Return pFault.LogFreeTextFault(65, $"Copy from: {pFullFileName} failed (file exists with size = 0). Check the text logs on the server", "", "TRGT-220220-171814", vRequester) 
      End If  
    End If  
  ...
  Public Shared Function DownloadFileAsByteArraySecurely(ByVal vTask As String, ByVal vFileName As String, ByRef rBytes As Byte(), ByVal vRequester As clsRequester) As clsFault 
    Dim pFault As New clsFault() 
    ...
    'check that the file exists 
    If Not IO.File.Exists(pFullFileName) Then 
      Return pFault.LogFreeTextFault(65, $"File origin: {pFullFileName}", "", "TRGT-240220-171635", vRequester) 
    End If 
 
    'Download the data   
    Try 
      rBytes = IO.File.ReadAllBytes(pFullFileName) 
    Catch ex As IO.FileNotFoundException 
      Return pFault.LogException(65, ex, $"Tried to copy from: {pFullFileName}", "TRGT-2400602-224528", vRequester) 
    Catch ex As Exception 
      If pFault.FreeText.IndexOf("because it is being used by another process", StringComparison.OrdinalIgnoreCase) >= 0 Then 
...
''' <summary>  
  ...
  Public Shared Function SendSMSorEmail(ByVal vMessage As String, ByVal vDestinationEmailOrNumber As String, ByVal vRequester As clsRequester, Optional ByVal vFullName As String = "", Optional ByVal vLanguage As clsEnums.enmLanguage = clsEnums.enmLanguage.en, Optional vSubject As String = "") As clsFault 
    Dim pFault As New clsFault 
    Dim pParameters As String = vDestinationEmailOrNumber 
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.prc_DoTasks, "ccHelper_SendSMSorEmail", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    If MyController.InTestMode Then 
      Dim pTextToLog As New Text.StringBuilder() 
      pTextToLog.AppendLine($"DestinationEmailOrNumber: {vDestinationEmailOrNumber}") 
      If Not String.IsNullOrEmpty(vFullName) Then pTextToLog.AppendLine($"FullName: {vFullName}") 
      If Not String.IsNullOrEmpty(vSubject) Then pTextToLog.AppendLine($"Subject: {vSubject}") 
      If vLanguage <> clsEnums.enmLanguage.UD Then pTextToLog.AppendLine($"Language: {vLanguage.FastToString()}") 
      If Not String.IsNullOrEmpty(vMessage) Then pTextToLog.AppendLine($"Message: {vMessage}") 
      Tools.LogToTextFile.WriteMessage(pTextToLog.ToString(), "MailerInTestMode") 
    End If 
 
    If String.IsNullOrEmpty(vMessage) Then 
      Return pFault.LogFreeTextFault(310, "No message to send!", pParameters, "TRGT-180829-1548", vRequester) 
    End If 
    If String.IsNullOrEmpty(vDestinationEmailOrNumber) Then 
      Return pFault.LogFreeTextFault(310, "No destination number or email received!", pParameters, "TRGT-180829-1549", vRequester) 
    End If 
 
    If String.IsNullOrEmpty(vSubject) Then 
      vSubject = ccHelper.GetLocalizedSystemText($"Message from {If(String.IsNullOrWhiteSpace(MyController.ServerNameForMail), MyController.SMTPNameFrom, MyController.ServerNameForMail)}", vRequester, vLanguage) 
    End If 
 
    Dim pIsRTL As Boolean = False 
    If vLanguage = clsEnums.enmLanguage.he Then 
      pIsRTL = True 
    End If 
 
    'Record to database if needed  
    'if it's an SMS and SaveToTablefalse, then save it without the message  
    Dim pMessageID As Long = 0 
    If (vMessage.IndexOf("I couldn't access SQL Server to report the error!", StringComparison.OrdinalIgnoreCase) = -1) Then 'No use if I can't access the server  
      'Save the mails to DB   
      Dim pMails As String() = vDestinationEmailOrNumber.Replace(Environment.NewLine, ";").Split(";"c) 
      For Each l In pMails 
        If String.IsNullOrEmpty(l.Trim) Then Continue For 
        Dim pMail As New csMail() 
        pMail.Body = vMessage 
        If l.IndexOf("@") > 0 Then 
          pMail.MessagingMode = clsEnums.enmMessagingMode.Email 
        Else 
          pMail.MessagingMode = clsEnums.enmMessagingMode.SMS 
        End If 
        If Not MyController.InTestMode Then 
          If pMail.MessagingMode = clsEnums.enmMessagingMode.Email Then Continue For 
        End If 
        pMail.Body = "Asked not to save" 'will eventually remove clsMail 
        pMail.RecipientEmail = l 
        pMail.Subject = If(vSubject.Length > 50, vSubject.Substring(0, 50), vSubject) 
        pMail.WhenSent = DateTimeOffset.Now 
        pFault = pMail.Update(vRequester, False) 
        If Not pFault.isOK Then pFault.SetOK(vRequester) 'it was recorded, now continue on   
        pMessageID = pMail.ID 'note that mails can be multiples, but SMS's are single...  
      Next 
    End If 
 
    If vDestinationEmailOrNumber.IndexOf("@") >= 0 Then 
      Dim pNumMails As Integer = vDestinationEmailOrNumber.Split("@"c).Length - 1 
      Dim pResponse As String = "" 
      If pNumMails = 1 Then 
        'in case of a trailing ';'  
        vDestinationEmailOrNumber = vDestinationEmailOrNumber.Replace(";", "") 
        If vDestinationEmailOrNumber.IndexOf(",") > 0 Then 
          vFullName = vDestinationEmailOrNumber.Split(","c)(0) 
          vDestinationEmailOrNumber = vDestinationEmailOrNumber.Split(","c)(1) 
        End If 
        pResponse = Tools.Mailer.SendMessage(vSubject, vFullName, vDestinationEmailOrNumber, vMessage, vMessageAsHTML:=Tools.Mailer.enmMessageAsHTML.TextToHTML, vRTL:=pIsRTL) 
      Else 
        If vDestinationEmailOrNumber.IndexOf(",") > 0 Then 
          'send per user  
          For Each pPerson In vDestinationEmailOrNumber.Split(";"c) 
            If String.IsNullOrEmpty(pPerson) Then Continue For 
            Dim pPersonName As String = pPerson.Split(","c)(0) 
            Dim pPersonEmail As String = pPerson.Split(","c)(1) 
            pResponse = Tools.Mailer.SendMessage(vSubject, pPersonName, pPersonEmail, vMessage, vMessageAsHTML:=Tools.Mailer.enmMessageAsHTML.TextToHTML, vRTL:=pIsRTL) 
          Next 
        Else 
          pResponse = Tools.Mailer.SendMailToMultipleRecipients("", vSubject, vDestinationEmailOrNumber, vMessage, vMessageAsHTML:=Tools.Mailer.enmMessageAsHTML.TextToHTML, vRTL:=pIsRTL) 
        End If 
      End If 
      If Not (pResponse.Equals("ok", StringComparison.OrdinalIgnoreCase)) Then 
        pFault.LogFreeTextFault(130, "Problem sending mail: " & pResponse, pParameters, "TRGT-190208-1302", vRequester) 
    ...
    If ccHelper.IsNumeric(vDestinationEmailOrNumber) Then 
      'SMS Module not installed 
      Throw New Exception("Missing SMS Module") 
    End If 
 
    pFault.SetOK(vRequester) ' just to be sure.... 
    Return pFault 
  End Function 
  ...
  Public Shared Function UpdateTranslation(ByVal vObjectType As clsEnums.enmObjectType, ByVal vObject As String, ByVal vItem As String, ByVal vInstance As Long, ByVal vText As String, ByVal vLocalizedLanguage As clsEnums.enmLanguage, ByVal vRequester As clsRequester) As clsFault 
    Dim pFault As New clsFault 
 
    pFault = ccHelper.LoadLanguageCache(vRequester) : If Not pFault.isOK Then Return pFault 
  
    If MyController.CacheOn = False OrElse (MyController.CacheOn = True AndAlso MyController.CacheSingleLanguageOnly = True AndAlso vLocalizedLanguage <> vRequester.UILang) Then 
      Dim pObjectToTranslate As New csObjectToTranslate 
      pFault = pObjectToTranslate.GetByObjectTypeAndObjectAndItem(vObjectType, vObject, vItem, vRequester, False) 
      If pFault.isOK = False Then Return pFault 
      If pObjectToTranslate.ID = 0 Then 
        ...
        With pObjectToTranslate 
          ...
          .Item = vItem 
        End With 
        pFault = pObjectToTranslate.Update(vRequester, False) 
        If pFault.isOK = False Then Return pFault 
      End If 
 
      Dim pObjectTranslation As New csObjectTranslation() 
      pFault = pObjectTranslation.GetByObjectToTranslateIDAndInstanceAndLanguage(pObjectToTranslate.ID, vInstance, vLocalizedLanguage, vRequester, False) 
      If pFault.isOK = False Then Return pFault 
      If pObjectTranslation.ID = 0 Then 
        'Create a new one 
        ...
        With pObjectTranslation 
          ...
          .Instance = vInstance 
          .Language = vLocalizedLanguage 
        End With 
      End If 
      If vText <> "" Then 
        pObjectTranslation.Text = vText 
        pFault = pObjectTranslation.Update(vRequester, False) 
      ElseIf vText = "" Then 
        If pObjectTranslation.ID <> 0 Then 
          pFault = pObjectTranslation.Delete(vRequester) 
        End If 
      End If 
      Return pFault 
    Else 
      ...
      If pObjectToTranslate.ID = 0 Then 
        ...
        If pFault.isOK = False Then Return pFault 
        'Add to cache 
        _ObjectToTranslateCache.Add(pObjectToTranslate) 
      End If 
 
      Dim pObjectTranslation As csObjectTranslation 
      pObjectTranslation = _ObjectTranslationCache.FindByObjectToTranslateIDAndInstanceAndLanguage(pObjectToTranslate.ID, vInstance, vLocalizedLanguage) 
      If pObjectTranslation.ID = 0 Then 
        'Create a new one 
        pObjectTranslation = New csObjectTranslation 
        With pObjectTranslation 
        ...
        End With 
      End If 
      Dim pPreviousID As Long = pObjectTranslation.ID 
      If vText <> "" Then 
        pObjectTranslation.Text = vText 
        pFault = pObjectTranslation.Update(vRequester, False) 
        If pFault.isOK = False Then Return pFault 
        If pPreviousID = 0 Then 
          _ObjectTranslationCache.Add(pObjectTranslation) 
        End If 
      ElseIf vText = "" Then 
        If pObjectTranslation.ID > 0 Then 
          pFault = pObjectTranslation.Delete(vRequester) 
          If pFault.isOK = False Then Return pFault 
          _ObjectTranslationCache.Remove(_ObjectTranslationCache.FindByID(pPreviousID)) 
        End If 
      End If 
      Return pFault 
    End If 
  ...
  Public Shared Function DeleteTranslationsForTableDataRow(ByVal vObjectType As clsEnums.enmObjectType, ByVal vObject As String, ByVal vInstance As Long, ByVal vRequester As clsRequester) As clsFault 
    Dim pFault As New clsFault 
 
    If vObjectType <> clsEnums.enmObjectType.TableData Then 
      Dim pMessage As String = "This function removes a row of Table Data!" 
      Return pFault.LogFreeTextFault(pMessage, "", "TRGT-120228-1026", vRequester) 
    End If 
 
    pFault = ccHelper.LoadLanguageCache(vRequester) : If Not pFault.isOK Then Return pFault 
 
    Dim pObjectToTranslateCol As New csObjectToTranslateCol 
    pFault = pObjectToTranslateCol.FillByObjectTypeAndObject(vObjectType, vObject, vRequester) 
    If pFault.isOK = False Then Return pFault 
    For Each pObjectToTranslate In pObjectToTranslateCol 
      Dim pObjectTranslationCol As New csObjectTranslationCol() 
      pFault = pObjectTranslationCol.FillByObjectToTranslateIDAndInstance(pObjectToTranslate.ID, vInstance, vRequester) 
      If pFault.isOK = False Then Return pFault 
      For Each p In pObjectTranslationCol 
        _ObjectTranslationCache.Remove(_ObjectTranslationCache.FindByID(p.ID)) 
      Next 
      pFault = csObjectTranslationCol.DeleteByObjectToTranslateIDAndInstance(pObjectToTranslate.ID, vInstance, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Next 
    Return pFault 
...
Public Class WSLoader 
  'This class pretends it loads the web service with a dummy (simply gets the time) 
  Event evtLoaded() 
