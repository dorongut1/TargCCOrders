...
Public Class clsComboList 
  ...
  Private Event evtFillManualCombolist(vQuery As String, vParentID As Long, vSearchString As String, vHowMany As Long, vSearchID As Long, vSearchCode As String, vRequester As clsRequester, ByRef rFault As clsFault, ByRef rQueryFound As Boolean) 
  ...
  Public Function Fill(ByVal vQuery As clsEnums.enmComboListType, ByVal vRequester As clsRequester, Optional ByVal vParentID As Long = 0, Optional ByVal vSearchString As String = "", Optional ByVal vHowMany As Long = 0, Optional ByVal vSearchID As Long = -1, Optional ByVal vSearchCode As String = "") As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_Fill:" & vQuery.FastToString(), vRequester) 
  ...
  Public Function Fill(ByVal vQuery As String, ByVal vRequester As clsRequester, Optional ByVal vParentID As Long = 0, Optional ByVal vSearchString As String = "", Optional ByVal vHowMany As Long = 0, Optional ByVal vSearchID As Long = -1, Optional ByVal vSearchCode As String = "") As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_Fill:" & vQuery, vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Me.Clear() 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pCol As ITargCCCollection = Nothing 
      Select Case vQuery 
        Case "ccBeehiveBuyerTrackingDefaultByID" 
          pCol = New clsBeehiveBuyerTrackingCol() 
        Case "ccCustomerDefaultByID" 
          pCol = New clsCustomerCol() 
        Case "ccCustomerDebtDefaultByID" 
          pCol = New clsCustomerDebtCol() 
        Case "ccDeliveryDefaultByID" 
          pCol = New clsDeliveryCol() 
        Case "ccOrderHeaderDefaultByID" 
          pCol = New clsOrderHeaderCol() 
        Case "ccOrderLineDefaultByID" 
          pCol = New clsOrderLineCol() 
        Case "ccProductDefaultByID" 
          pCol = New clsProductCol() 
        Case "ccProductPriceDefaultByID" 
          pCol = New clsProductPriceCol() 
        Case "ccProductPriceHistDefaultByID" 
          pCol = New clsProductPriceHistCol() 
        Case "ccSupplierOrderDefaultByID" 
          pCol = New clsSupplierOrderCol() 
        Case "c_AlertMessageDefaultByID" 
          pCol = New csAlertMessageCol() 
        Case "c_AuditIndexedDefaultByID" 
          pCol = New csAuditIndexedCol() 
        Case "c_EnumerationDefaultByID" 
          pCol = New csEnumerationCol() 
        Case "c_IndexFragmentationDefaultByID" 
          pCol = New csIndexFragmentationCol() 
        Case "c_JobDefaultByID" 
          pCol = New csJobCol() 
        Case "c_JobAlertRecipientDefaultByID" 
          pCol = New csJobAlertRecipientCol() 
        Case "c_LanguageDefaultByID" 
          pCol = New csLanguageCol() 
        Case "c_LoggedAlertDefaultByID" 
          pCol = New csLoggedAlertCol() 
        Case "c_LoggedJobDefaultByID" 
          pCol = New csLoggedJobCol() 
        Case "c_LoggedLoginDefaultByID" 
          pCol = New csLoggedLoginCol() 
        Case "c_LoggedRequestDefaultByID" 
          pCol = New csLoggedRequestCol() 
        Case "c_LookupDefaultByID" 
          pCol = New csLookupCol() 
        Case "c_MailDefaultByID" 
          pCol = New csMailCol() 
        Case "c_MFADefaultByID" 
          pCol = New csMFACol() 
        Case "c_ObjectToTranslateDefaultByID" 
          pCol = New csObjectToTranslateCol() 
        Case "c_ObjectTranslationDefaultByID" 
          pCol = New csObjectTranslationCol() 
        Case "c_PermissionDefaultByID" 
          pCol = New csPermissionCol() 
        Case "c_ProcessDefaultByID" 
          pCol = New csProcessCol() 
        Case "c_RoleDefaultByID" 
          pCol = New csRoleCol() 
        Case "c_SystemAuditDefaultByID" 
          pCol = New csSystemAuditCol() 
        Case "c_SystemDefaultDefaultByID" 
          pCol = New csSystemDefaultCol() 
        Case "c_TableDefaultByID" 
          pCol = New csTableCol() 
        Case "c_TableSizeDefaultByID" 
          pCol = New csTableSizeCol() 
        Case "c_UserDefaultByID" 
          pCol = New csUserCol() 
        Case "c_UserLoginKeyDefaultByID" 
          pCol = New csUserLoginKeyCol() 
        Case "c_UserPermissionDefaultByID" 
          pCol = New csUserPermissionCol() 
        Case "c_UserStatusDefaultByID" 
          pCol = New csUserStatusCol() 
        Case "c_RoleNonBaseDefaultByID" 
          Dim tCol As New csRoleCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          For Each l In tCol 
            If l.BaseRoleID = 0 And l.Name.IndexOf("master", StringComparison.OrdinalIgnoreCase) = -1 Then 
              Me.Add(New clsComboListMember(l.ID, l.Name)) 
            End If 
          Next 
        Case "c_RoleWithBaseDefaultByID" 
          Dim tCol As New csRoleCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          For Each l In tCol 
            If l.BaseRoleID > 0 Then 
              Me.Add(New clsComboListMember(l.ID, l.Name)) 
            End If 
          Next 
        Case "c_RoleWithBaseAndMasterDefaultByID" 
          Dim tCol As New csRoleCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          For Each l In tCol 
            If l.BaseRoleID > 0 OrElse l.Name.IndexOf("master", StringComparison.OrdinalIgnoreCase) >= 0 Then 
              Me.Add(New clsComboListMember(l.ID, l.Name)) 
            End If 
          Next 
        Case "c_RoleWithBaseNoSysAdminDefaultByID" 
          Dim tCol As New csRoleCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          Dim pIDSysAdmin As Long = tCol.FindByName("SysAdmin").ID 
          For Each l In tCol 
            If l.BaseRoleID > 0 And l.BaseRoleID <> pIDSysAdmin Then 
              Me.Add(New clsComboListMember(l.ID, l.Name)) 
            End If 
          Next 
        Case "c_UserDefaultNoMasterByID" 
          Dim trCol As New csRoleCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          Dim pIDMaster As Long = trCol.FindByName("Master").ID 
          Dim tCol As New csUserCol(clsEnums.enmLoadParent.DoNotLoad, vRequester, pFault) : If Not pFault.isOK Then Return pFault 
          For Each l In tCol 
            If l.RoleID <> pIDMaster Then 
              Me.Add(New clsComboListMember(l.ID, $"{l.FirstName} {l.LastName}")) 
            End If 
          Next 
        Case "ccTestComboListFillManual" 
          'Ignore this test if from file system 
          Return pFault 
        Case Else 
          Dim pQueryFound As Boolean = False 
          'If we find something in the raised event, the pCol will stay nothing.....  
          RaiseEvent evtFillManualCombolist(vQuery, vParentID, vSearchString, vHowMany, vSearchID, vSearchCode, vRequester, pFault, pQueryFound) : If Not pFault.isOK Then Return pFault 
          If pQueryFound = False Then 
            Return pFault.LogFreeTextFault($"Query {vQuery} has not yet been handled for Fill For ComboList", vQuery, "TRGT-210617-1914", vRequester) 
          End If 
      End Select 
      If pCol IsNot Nothing Then 
        pFault = pCol.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
 
        Dim pIsNumeric As Boolean = True 
        For Each p In pCol 
          Dim pp As cTargCCEntity = CType(p, cTargCCEntity) 
          Dim pComboListMember As New clsComboListMember 
          pComboListMember.KeyLong = pp.PrimaryKey 
          pComboListMember.Text = pp.DefaultDesignation 
          If pIsNumeric = True AndAlso ccHelper.IsNumeric(pp.DefaultDesignation) = False Then pIsNumeric = False 
          If vSearchID = -1 AndAlso String.IsNullOrEmpty(vSearchCode) AndAlso Not String.IsNullOrEmpty(vSearchString) Then 
            Dim pTextToTest As String = pp.DefaultDesignation 
            If pTextToTest.IndexOf(vSearchString) >= 0 Then 
              Me.Add(pComboListMember) 
            End If 
          Else 
            If vSearchID > -1 Then 
              If pp.PrimaryKey = vSearchID Then 
                Me.Add(pComboListMember) 
              End If 
            ElseIf Not String.IsNullOrEmpty(vSearchCode) Then 
              Throw New Exception("SearchCode not yet handled (TRGT-211101-1942)") 
            Else 
              Me.Add(pComboListMember) 
            End If 
          End If 
        Next 
        Me.SortByText() 
        If pIsNumeric Then Me.Reverse() 
      End If 
 
    Else 'If MyController.DBType = MyController.enmDBType.SQL Then 
      Dim pCommandText As String = "c__ComboListFillAuto" 
      Dim pParameters As New ccDAL.csTargCCParameterCol 
 
      Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
        pLastReadVariableName = "ListType" 
        If vQuery.Length > 100 Then Throw New Exception($"Query {vQuery} length is greater than 100.Contact the developer!!") 
        pParameters.Add("ListType", ccDAL.enmSQLDataType.VarChar, 100).Value = vQuery 
        pLastReadVariableName = "ParentID" 
        pParameters.Add("ParentID", ccDAL.enmSQLDataType.BigInt).Value = vParentID 
        If vSearchID > -1 Then 
          pLastReadVariableName = "SearchID" 
          pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt, 100).Value = vSearchID 
        End If 
        If Not String.IsNullOrEmpty(vSearchCode) Then 
          pLastReadVariableName = "SearchCode" 
          pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 100).Value = vSearchCode 
        End If 
        If Not String.IsNullOrEmpty(vSearchString) Then 
          pLastReadVariableName = "SearchText" 
          pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 100).Value = vSearchString 
        End If 
        If vHowMany <> 0 Then 
          pLastReadVariableName = "Top" 
          pParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.BigInt).Value = vHowMany 
        End If 
        pLastReadVariableName = "" 
 
        'Execute query  
        pFault = ccDAL.ExecuteQuery(pCommandText, pParameters, vRequester, Me) 
      Catch ex As Exception 
    ...
    'clean up the text 
    For Each l In Me 
      Dim pText As String = l.Text 
 
      Dim pChanged As Boolean = False 
      If vQuery.Equals("c_LookupDefaultByID", StringComparison.OrdinalIgnoreCase) Then 
        'Remove the ParentID in the code 
        Dim pParts As String() = pText.Split("#"c) 
        If pParts.Length = 3 Then 
          pText = pParts(0) & pParts(2) 
          pChanged = True 
        End If 
      End If 
 
      If pText.IndexOf("()", StringComparison.OrdinalIgnoreCase) > 0 Then 
        pText = pText.Replace("()", "") 
        pChanged = True 
      End If 
 
      If pText.StartsWith(" ", StringComparison.OrdinalIgnoreCase) OrElse pText.EndsWith(" ", StringComparison.OrdinalIgnoreCase) Then 
        pText = pText.Trim() 
        pChanged = True  
      End If  
 
      'If Not (pText.StartsWith(" ", StringComparison.OrdinalIgnoreCase) AndAlso pText.EndsWith(" ", StringComparison.OrdinalIgnoreCase)) Then 
      '  pText = " " & pText.Trim() & " " 
      '  pChanged = True 
      'End If 
 
      Dim pHungarian As String 
      pHungarian = ccHelper.CreateFriendlyTextFromHungarianNotation(pText) 
 
      If Not pHungarian.Equals(pText) Then 
        pText = pHungarian 
        pChanged = True 
      End If 
 
      If pChanged = True Then 
        l.Text = pText 
      End If 
    Next 
  ...
  Public Function FillEnums(ByVal vEnum As clsEnums.enmEnum, ByVal vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_FillEnums:" & vEnum.FastToString(), vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  ''' <param name="vRequester"></param> 
  ''' <returns></returns> 
  Public Function FillLookup(ByVal vLookupType As clsEnums.enmLookup, ByVal vRequester As clsRequester) As clsFault 
    Dim pFault As clsFault 
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_FillLookup:" & vLookupType.FastToString(), vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  ''' <param name="vRequester"></param>  
  ''' <returns></returns>  
  Public Function FillLookup(ByVal vParentLookupType As clsEnums.enmLookup, ByVal vLookupType As clsEnums.enmLookup, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As clsFault 
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_FillLookup:" & vParentLookupType.FastToString() & "_" & vLookupType.FastToString(), vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pLookupCol As csLookupCol 
    pLookupCol = ccHelper.LookupCache.CloneByParentLookupTypeAndLookupType(vParentLookupType, vLookupType) 
    ...
    For Each l In pLookupCol 
      ...
      l.TextLocalized = ccHelper.GetLocalizedLookup(l.ParentLookupType, l.ParentCode, l.LookupType, pCode, vRequester) 
    ...
    'In order to create enums, a base lookup is created with a code of '' or -1 and a text of '' 
 
    For Each p In pLookupCol 
      pComboListMember = New clsComboListMember 
      If pIsString = True Then 
        If p.Code.Equals("") AndAlso String.IsNullOrEmpty(p.Text) Then Continue For 
        pComboListMember.KeyString = p.Code 
      Else 
        If p.Code.Equals("-1") AndAlso String.IsNullOrEmpty(p.Text) Then Continue For 
  ...
  ''' <param name="vRequester"></param> 
  ''' <returns></returns> 
  Public Function FillLookup(ByVal vParentLookupType As clsEnums.enmLookup, ByVal vParentCode As String, ByVal vLookupType As clsEnums.enmLookup, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As clsFault 
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_LookupView, "clsComboList_FillLookup:" & vParentLookupType.FastToString() & "_" & vParentCode & "_" & vLookupType.FastToString(), vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pLookupCol As csLookupCol 
    pLookupCol = ccHelper.LookupCache.CloneByParentLookupTypeAndParentCodeAndLookupType(vParentLookupType, vParentCode, vLookupType) 
    Dim pIsString As Boolean = False 'Use to assign the ComboList Type. If even one is a string, then it's a string 
    For Each l In pLookupCol 
      Dim pCode As String = l.Code 
      l.TextLocalized = ccHelper.GetLocalizedLookup(l.ParentLookupType, l.ParentCode, l.LookupType, pCode, vRequester) 
  ...
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    If _KeyType = clsEnums.enmComboListKeyType.UD Then 
      Dim pKeyType As String = vReader.GetDataTypeName(0).ToLowerInvariant() 
      Select Case pKeyType 
        ...
        Case "varchar", "nvarchar", "char", "nchar" 
          ...
          _KeyTypeName = "String" 
        Case Else 
          pFunctionParameters = "KeyType received In DB" & pKeyType 
          Return pFault.LogFreeTextFault("Combolist can only load Long, Integer Or String as a key from the database!", pFunctionParameters, "TRGT-160429-1129", vRequester) 
      End Select 
    End If 
    pFunctionParameters = "KeyTypeName:" & _KeyTypeName 
    ...
    While vReader.Read() 
      ...
      Try 
        ...
        'I'm not expecting Nulls, so don't check for it..... 
        Try 
          Select Case _KeyType 
            Case clsEnums.enmComboListKeyType.Long : pComboListMember.KeyLong = vReader.GetInt64(0) 
            Case clsEnums.enmComboListKeyType.Integer : pComboListMember.KeyInteger = vReader.GetInt32(0) 
            Case clsEnums.enmComboListKeyType.String : pComboListMember.KeyString = vReader.GetString(0) 
            Case clsEnums.enmComboListKeyType.Enum : pComboListMember.KeyEnum = clsEnums.TranslateToEnum(_KeyTypeName, vReader.GetString(0)) 
            Case Else 
              Return pFault.LogFreeTextFault("Combolist's KeyType was not defined!", pFunctionParameters, "TRGT-160427-1621", vRequester) 
          End Select 
        Catch ex As Exception 
          Return pFault.LogFreeTextFault("Combolist's KeyField was null!!", pFunctionParameters, "TRGT-160427-1715", vRequester)  
        End Try 
        pLastReadVariableName = "Text" 
        Try 
          pComboListMember.Text = vReader.GetString(1) 
        Catch ex As Exception 
          pComboListMember.Text = "NULL" 
        End Try 
        pLastReadVariableName = "" 
        pFault.SetOK() 
      Catch ex As Exception 
        If String.IsNullOrEmpty(pLastReadVariableName) = False Then  
          pFunctionParameters &= "; LastVariable:" & pLastReadVariableName  
        End If  
        pFault.LogException(ex, pFunctionParameters, "TRGT-HCC-0019", vRequester)  
      End Try  
