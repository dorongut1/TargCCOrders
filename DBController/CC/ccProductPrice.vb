Public Class clsProductPrice
  ...
  Private Sub CreateDefaultDesignation() 
    ...
    If pOverridenValue = Nothing Then bDefaultDesignation = ccHelper.CreateFriendlyTextFromHungarianNotation(_ProductID.ToString() & " - " & _CustomerType.FastToString() & ": ?" & _SellingPrice.ToString()) Else bDefaultDesignation = pOverridenValue 
  End Sub 
  ...
  Public Sub New(ByVal vclsProductPrice As clsProductPrice)
    MyBase.New()
    CreateEmpty()
    AssignValues(vclsProductPrice) 
  End Sub
  ...
  'Inherited Gets 
  ''' <summary> 
  ''' This satisfies an interface requirement. All TargCCEntities have this function. It gets the ProductPrice by PrimaryKey (ID) 
  ''' </summary> 
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPrice_GetByPrimaryKey", vRequester) 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(57, ex, pFunctionParameters, "TRGT-ProductPrice-151224_0844", vRequester) 
    End Try 
  ...
  End Enum 
  ''' <summary> 
  ''' This satisfies an interface requirement. All TargCCEntities have this function. It gets the ProductPrice by the chosen parameters. This function may be a bit slower than accessing the ProductPrice's GetBy... directly 
  ''' </summary> 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPrice_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Try 
      Select Case pGetByOptions 
        ...
        Case Else 
          pFault = New clsFault 
          pFault.LogFreeTextFault(56, "", pFunctionParameters, "TRGT-ProductPrice-151223_1715", vRequester) 
      End Select 
    Catch ex As Exception 
      pFault = New clsFault  
      pFault.LogException(57, ex, pFunctionParameters, "TRGT-ProductPrice-151223_1716", vRequester)  
    End Try  
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPrice_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = 0 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"ProductPrice not found for GetByID, since its value is 0", pFunctionParameters, "TRGT-ProductPrice-210927-1527", vRequester, vAdditionalMessageToUser:=$"ProductPrice not found") 
      pFault.SetOK() 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      pFault = LoadMeFromDBCache(MyController.DBCache.ProductPriceCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPriceGetByID" 
      ...
      Try 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"ProductPrice not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-ProductPrice-210625-0950", vRequester, vAdditionalMessageToUser:=$"ProductPrice not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function AddUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityAddable.AddUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceUpdate, "clsProductPrice_AddUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    If _ID <> 0 Then 
      'Invalid EntityID received for the operation 
      Return pFault.LogFreeTextFault(58, "Received and ID of " & _ID, pFunctionParameters, "TRGT-ProductPrice-151227-1738", vRequester) 
    Else 
  ...
  Public Function EditUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityEditable.EditUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceUpdate, "clsProductPrice_EditUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  ''' <summary> 
  ''' This updates the ProductPrice. If there are parents or children in the ProductPrice, they are NOT updated.  
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceUpdate, "clsProductPrice_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Dim pProductPrice As New clsProductPrice(_WithParents) 
    If Me.isEqual(pProductPrice) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", pFunctionParameters, "TRGT-ProductPrice-100113-1638", vRequester) 
    End If 
 
    If _IsTruncated Then 
      Return pFault.LogFreeTextFault(57, "Object is Truncated", pFunctionParameters, "TRGT-ProductPrice-240611-135714", vRequester) 
    End If 
 
    Dim pCommandText As String = "ccProductPriceUpdate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      Dim pCachedProductPrice As clsProductPrice 
      If _ID = 0 Then 
        pCachedProductPrice = New clsProductPrice(_WithParents) 
        'get last ID 
        Dim pProductPriceCol As clsProductPriceCol = MyController.DBCache.ProductPriceCol.Clone() 
        If pProductPriceCol.Count = 0 Then 
          _ID = 1 
        Else 
          pProductPriceCol.SortByID() 
          Dim pLastID As Long = pProductPriceCol(pProductPriceCol.Count - 1).ID
          _ID = pLastID + 1 
        End If 
        bPrimaryKey = _ID 
        MyController.DBCache.ProductPriceCol.Add(pCachedProductPrice) 
      Else  
        pCachedProductPrice = MyController.DBCache.ProductPriceCol.FindByID(_ID) 
      End If 
      pCachedProductPrice.AssignValues(Me) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ProductPriceCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            bPrimaryKey = pID 
            If pID = 0 Then 
              pFault.LogFreeTextFault(51, "ID returned is 0!", pFunctionParameters, "TRGT-ProductPrice-151114-1147", vRequester) 
            End If 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-ProductPrice-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090623-1809", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    Dim pFunctionParameters As String = String.Format("ProductPrice.ID={0}", _ID)
    Dim pFault As clsFault
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPrice_Delete", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    Dim pCommandText As String = "ccProductPriceDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      MyController.DBCache.ProductPriceCol.Remove(MyController.DBCache.ProductPriceCol.FindByID(_ID)) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ProductPriceCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-ProductPrice-151114-1147", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-ProductPrice-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090623-1813", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPrice_DeleteByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      MyController.DBCache.ProductPriceCol.Remove(MyController.DBCache.ProductPriceCol.FindByID(vID)) 
      'Save File  
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ProductPriceCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pID As Integer = ccHelper.ToInteger(pTargCCReader(0)) 
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-ProductPrice-231207-0845", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-ProductPrice-231207-0844", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-231207-0843", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If 
  ...
  Public Overrides Function isEqual(ByVal vTargCCEntityToTest As ITargCCEntity) As Boolean 
    If Not (TypeOf (vTargCCEntityToTest) Is clsProductPrice) Then Return False 
    Dim pProductPriceToTest As clsProductPrice = CType(vTargCCEntityToTest, clsProductPrice) 
    Return isEqual(pProductPriceToTest) 
  End Function 
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Overloads Function isEqual(ByVal vProductPriceToTest As clsProductPrice) As Boolean
    With vProductPriceToTest
  ...
  Public Overrides Function CloneTargCCEntity() As ITargCCEntity 
    Dim pClone As New clsProductPrice(Me) 
    Return pClone 
  End Function 
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function Clone() As clsProductPrice
    Dim pClone As New clsProductPrice(Me)
    Return pClone
  End Function
  ...
  Friend Overrides Function LoadDataRow(ByVal vDataRow As DataRow, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try : vDataRow("ID") = _ID : Catch ex As Exception : Return pFault.LogException(ex, "ID", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("ProductID") = _ProductID : Catch ex As Exception : Return pFault.LogException(ex, "ProductID", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("CustomerType") = _CustomerType : Catch ex As Exception : Return pFault.LogException(ex, "CustomerType", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("SellingPrice") = _SellingPrice : Catch ex As Exception : Return pFault.LogException(ex, "SellingPrice", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("MinQuantity") = _MinQuantity : Catch ex As Exception : Return pFault.LogException(ex, "MinQuantity", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("DiscountPercent") = _DiscountPercent : Catch ex As Exception : Return pFault.LogException(ex, "DiscountPercent", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
    Try : vDataRow("Notes") = _Notes : Catch ex As Exception : Return pFault.LogException(ex, "Notes", "TRGT-ProductPrice-130316-0852", vRequester) : End Try 
  ...
  Public Overrides Function CreateXML(ByRef rXML As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-130515-1236", vRequester) 
    End Try 
  ...
  Public Overrides Function LoadXML(ByVal vXML As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      Dim pProductPrice As clsProductPrice = CType(pXmlSerializer.Deserialize(pStreamReader), clsProductPrice) 
      AssignValues(pProductPrice) 
      pFault.SetOK() 
    Catch ex As Exception 
      CreateEmpty() 
      pFault.LogException(ex, vXML, "TRGT-ProductPrice-130515-1230", vRequester) 
    End Try 
...
          pBinaryWriter.Write(bDateAdded.Ticks) 
      ...
      rFault.SetOK() 
    Catch ex As Exception 
      rFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-150307-2338", vRequester) 
    End Try 
...
          'bDateAdded 
    ...
    Catch ex As Exception 
      CreateEmpty() 
      rFault.LogException(ex, "", "TRGT-ProductPrice-150307-2339", vRequester) 
    End Try 
    bPrimaryKey = _ID 
    CreateDefaultDesignation() 
  End Sub 
  ...
  Public Overrides Function CreateJSON(ByRef rJSON As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-190720-1443", vRequester) 
    End Try 
  ...
  Public Overrides Function LoadJSON(ByVal vJSON As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      Dim pProductPrice As clsProductPrice = Newtonsoft.Json.JsonConvert.DeserializeObject(Of clsProductPrice)(vJSON, pSettings) 
      AssignValues(pProductPrice) 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-190720-1443", vRequester) 
  ...
  ''' </summary>
  ''' <remarks></remarks>
  Private Sub AssignValues(ByVal vProductPrice As clsProductPrice)
    With vProductPrice
      _ID = .ID 
  ...
  Public Overrides Function LoadLookupAndEnumText(ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, "Failed getting: " & pTextToGet, "TRGT-ProductPrice-151124-1900", vRequester) 
    End Try 
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Try
      ...
      pLastReadVariableName = "SellingPrice" 
      If Not vReader.IsDBNull(3) Then _SellingPrice = vReader.GetDecimal(3)
      pLastReadVariableName = "MinQuantity" 
      If Not vReader.IsDBNull(4) Then _MinQuantity = vReader.GetInt32(4)
      pLastReadVariableName = "DiscountPercent" 
      If Not vReader.IsDBNull(5) Then _DiscountPercent = vReader.GetDecimal(5)
      pLastReadVariableName = "Notes" 
      If Not vReader.IsDBNull(6) Then _Notes = vReader.GetString(6) 
      ...
      If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
        ...
        If Not vReader.IsDBNull(8) Then _ProductText = vReader.GetString(8) 
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedProductPrice As clsProductPrice, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedProductPrice) 
      ...
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-121122-1607", vRequester) 
    End Try 
...
Public Class clsProductPriceCol
  Inherits cTargCCCollection(Of clsProductPrice)
  ...
  'Support for FindBys
  Private _SortedDictionaryForFindByID As Dictionary(Of Long, clsProductPrice) 
  ...
  Public Overrides Function ToString() As String 
    ...
    For Each pRow As clsProductPrice In Me 
  ...
  Public Overloads Sub Add(ByVal vProductPrice As clsProductPrice) 
    SyncLock _CollectionLock 
      MyBase.Add(vProductPrice) 
      _RecreateDictionaryForFindByID = True 
    End SyncLock 
  End Sub 
  Public Overloads Sub Insert(ByVal vIndex As Integer, ByVal vProductPrice As clsProductPrice) 
    SyncLock _CollectionLock 
      MyBase.Insert(vIndex, vProductPrice) 
      _RecreateDictionaryForFindByID = True 
    End SyncLock 
  End Sub 
  Public Overloads Sub AddRange(ByVal vProductPriceCol As clsProductPriceCol) 
    SyncLock _CollectionLock 
      MyBase.AddRange(vProductPriceCol) 
      _RecreateDictionaryForFindByID = True 
    End SyncLock 
  End Sub 
  Public Overloads Sub RemoveAt(ByVal vIndex As Integer) 
    ...
    End SyncLock 
  End Sub 
  Public Overloads Sub Remove(ByVal vProductPrice As clsProductPrice) 
    SyncLock _CollectionLock 
      MyBase.Remove(vProductPrice) 
      _RecreateDictionaryForFindByID = True 
    End SyncLock 
  End Sub 
  ...
  Private Sub LoadIDs() 
    ...
    SyncLock _CollectionLock 
      ...
      Dim pTempDictionary As New Dictionary(Of Long, clsProductPrice) 
      
      For Each lProductPrice In Me 
        If lProductPrice.IsEmpty OrElse pTempDictionary.ContainsKey(lProductPrice.ID) Then 
          'Not Unique or no ID 
          Continue For 
        End If 
        Try 
          pTempDictionary.Add(lProductPrice.ID, lProductPrice) 
        Catch ex As Exception 
          Dim pFault As New clsFault 
          pFault.LogException(ex, lProductPrice.ToString, "TRGT-ProductPrice-260111-154655", Nothing) 'Log it 
          Throw New Exception("Failed pTempDictionary for _SortedDictionaryForFindByID:" & ex.Message & ", ProductPrice:" & lProductPrice.ToString() & ", TRGT-ProductPrice-260111-154657") 'Send it up the line 
        End Try 
      Next 
  ...
  Public Sub TruncateStrings(Optional pTruncateLength As Integer = 50) 
    ...
    For Each lProductPrice As clsProductPrice In Me 
      lProductPrice.TruncateStrings(pTruncateLength) 
    Next 
  ...
  Public Sub CleanCollectionForXML() 
    ...
    For Each lProductPrice As clsProductPrice In Me 
      lProductPrice.CleanEntityForXML() 
    Next 
  ...
  Public Enum enmFillByParameterCombination 
    [UD] 
    [None_GetAll] 
    [ProductIDAndCustomerType] 
    [ProductID] 
  End Enum 
  ''' <summary> 
  ''' This satisfies an interface requirement. All TargCCCollections have this function. It gets the ProductPrices by the chosen parameters. This function may be a bit slower than accessing the ProductPrice's FillBy... directly 
  ''' </summary> 
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Try 
      Select Case pGetByOptions 
        ...
        Case enmFillByParameterCombination.ProductIDAndCustomerType 
          pFault = FillByProductIDAndCustomerType(ccHelper.ToLong(vParameters(0)), clsEnums.TranslateEnmCustomerType(CStr(vParameters(1))), vRequester, vHowMany, vDir) 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      Dim pProductPricesCached As clsProductPriceCol = MyController.DBCache.ProductPriceCol.Clone() 
      pProductPricesCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pProductPricesCached.Reverse() 
      If vHowMany > 0 AndAlso pProductPricesCached.Count > vHowMany Then 
        Dim tmp As New clsProductPriceCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pProductPricesCached(i)) 
        Next 
        pProductPricesCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pProductPricesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  ''' <summary>
  ''' Gets a collection of all the items for a specific ProductIDAndCustomerType, or a sub-collection defined by HowMany and Direction. To append to an existing collection, set vAppend to true (default is false)
  ''' </summary>
  ''' <param name="vHowMany">How Many (sorted by ID) - 0 for all</param>
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function FillByProductIDAndCustomerType(ByVal vProductID As Long, ByVal vCustomerType As clsEnums.enmCustomerType, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    Dim pFunctionParameters As String = String.Format("ProductID={0}, CustomerType={1}", vProductID, vCustomerType)
    Dim pFault As New clsFault
 
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillByProductIDAndCustomerType", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      Dim pProductPricesCached As clsProductPriceCol = MyController.DBCache.ProductPriceCol.CloneByProductIDAndCustomerType(vProductID, vCustomerType)
      pFault = LoadMeFromDBCache(pProductPricesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFillByProductID&CustomerType" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090624-1702", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByProductID(ByVal vProductID As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillByProductID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      Dim pProductPricesCached As clsProductPriceCol = MyController.DBCache.ProductPriceCol.CloneByProductID(vProductID)
      pFault = LoadMeFromDBCache(pProductPricesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFillByProductID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ProductPriceCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ProductPriceCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsProductPriceCol failed: " & pResponse) 
      Dim pProductPricesCached As clsProductPriceCol = MyController.DBCache.ProductPriceCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pProductPricesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lProductPrice As New clsProductPrice() 
      pFault = lProductPrice.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lProductPrice.IsEmpty Then Me.Add(lProductPrice) 
    Next 
 
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController 
      Dim pProductPrices As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pProductPrices, "clsProductPriceCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pProductPrices IsNot Nothing AndAlso Me.Count <> pProductPrices.Count Then FillFromListOfITargCCEntity(pProductPrices) 
    End If 
  ...
  Public Overrides Function FillOnTheFly(ByVal vParameters As Dictionary(Of System.Enum, Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    Dim pFunctionParameters As String = String.Format("IDFrom={0}, IDTo={1}, ProductID={2}, CustomerType={3}", vIDFrom, vIDTo, vProductID, vCustomerType)
    Dim pFault As New clsFault
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-ProductPrice-121122-2008", vRequester) 
      Dim pProductPricesCached As clsProductPriceCol = MyController.DBCache.ProductPriceCol.Clone() 
      Dim pProductPricesToUse As New clsProductPriceCol() 
      For Each l In pProductPricesCached 
        ...
        End If 
        pProductPricesToUse.Add(l) 
      Next 
      pProductPricesToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pProductPricesToUse.Reverse() 
      If vHowMany > 0 AndAlso pProductPricesToUse.Count > vHowMany Then 
        Dim tmp As New clsProductPriceCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pProductPricesToUse(i)) 
        Next 
        pProductPricesToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pProductPricesToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overrides Function FillSumOnTheFly(ByVal vParameters As Dictionary(Of [Enum], Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    Dim pFunctionParameters As String = String.Format("IDFrom={0}, IDTo={1}, ProductID={2}, CustomerType={3}, GroupByProductID={4}, GroupByCustomerType={5}", vIDFrom, vIDTo, vProductID, vCustomerType, vGroupByProductID, vGroupByCustomerType)
    Dim pFault As New clsFault
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-ProductPrice-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccProductPricesFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Overloads Sub FillFromArray(ByVal vProductPriceArray As clsProductPrice())
    Me.Clear()
    
    For Each pProductPrice As clsProductPrice In vProductPriceArray
      Me.Add(pProductPrice)
      _Clean.Add(pProductPrice.ID) 
    Next
  End Sub
  ...
  Public Overrides Function FillFromDataTable(ByVal vDataTable As DataTable, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      For Each pRow As DataRow In vDataTable.Rows 
        Dim pProductPrice As New clsProductPrice(pRow, vRequester, _WithParents) 
        Me.Add(pProductPrice) 
        _Clean.Add(pProductPrice.ID) 
      Next 
      pFault.SetOK() 
    Catch ex As Exception 
      Return pFault.LogException(ex, "", "TRGT-ProductPriceCol-130315-2118", vRequester) 
    End Try 
  ...
  Public Overrides Function CreateXML(ByRef rXML As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      rXML = MyStringBuilder.ToString() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-130515-1300", vRequester) 
    End Try 
  ...
  Public Overrides Function FillFromXML(ByVal vXML As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      Dim pXmlSerializer As New System.Xml.Serialization.XmlSerializer(pType) 
      Dim pStreamReader As New IO.StringReader(vXML) 
      Dim pProductPrices As clsProductPriceCol = CType(pXmlSerializer.Deserialize(pStreamReader), clsProductPriceCol) 
      For Each pProductPrice As clsProductPrice In pProductPrices 
        Me.Add(pProductPrice) 
        _Clean.Add(pProductPrice.ID) 
      Next 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, vXML, "TRGT-ProductPrice-130515-1329", vRequester) 
    End Try 
  ...
  Public Overrides Function LoadJSON(ByVal vJSON As String, ByVal vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      Dim pProductPrices As List(Of clsProductPrice) = Newtonsoft.Json.JsonConvert.DeserializeObject(Of List(Of clsProductPrice))(vJSON, pSettings) 
      For Each pProductPrice As clsProductPrice In pProductPrices 
        Me.Add(pProductPrice) 
        _Clean.Add(pProductPrice.ID) 
      Next 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-190720-2059", vRequester) 
    End Try 
    ...
    Try 
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream) 
          ...
          For Each lProductPrice As clsProductPrice In Me 
            Dim pByte As Byte() = lProductPrice.CreateByteArray(rFault, vRequester) : If Not rFault.isOK Then Return Nothing 
      ...
      rFault.SetOK() 
    Catch ex As Exception 
      rFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-150307-2340", vRequester) 
    End Try 
  ...
  Public Overrides Sub LoadByteArray(ByVal vBytes As Byte(), ByRef rFault As clsFault, ByVal vRequester As clsRequester) 
    ...
    Try 
      Using pMemoryStream As New System.IO.MemoryStream(vBytes) 
        Using pReader As New System.IO.BinaryReader(pMemoryStream) 
          ...
          For iCntr As Integer = 0 To pCount - 1 
            ...
            Dim pProductPrice As clsProductPrice = New clsProductPrice(pReader.ReadBytes(pLength), rFault, vRequester) 
            Me.Add(pProductPrice) : If Not rFault.isOK Then Exit Sub 
            _Clean.Add(pProductPrice.ID) 
          Next 
          pReader.Close() 
        End Using 
      ...
      rFault.SetOK() 
    Catch ex As Exception 
      rFault.LogException(ex, "", "TRGT-ProductPrice-150307-2341", vRequester) 
    End Try 
  ...
  Public Overrides Function LoadLookupAndEnumText(ByVal vRequester As clsRequester) As clsFault 
    ...
    For Each pProductPrice As clsProductPrice In Me 
      With pProductPrice 
        pFault = pProductPrice.LoadLookupAndEnumText(vRequester) 
  ...
  Public Overrides Function isEqual(vEntitiesToTest As ITargCCCollection) As Boolean 
    If Not (TypeOf (vEntitiesToTest) Is clsProductPriceCol) Then Return False 
    Dim pProductPriceColToTest As clsProductPriceCol = CType(vEntitiesToTest, clsProductPriceCol) 
    Return isEqual(pProductPriceColToTest) 
  End Function 
  ...
  ''' This checks if the objects are the same (not if the share the same address), IGNORING the dependants 
  ''' </summary> 
  ''' <param name="vProductPricesToTest"></param> 
  ''' <returns></returns> 
  Public Overloads Function isEqual(ByVal vProductPricesToTest As clsProductPriceCol) As Boolean
    If Me.Count <> vProductPricesToTest.Count Then Return False
    For i As Integer = 0 To Me.Count - 1 
      If Me(i).isEqual(vProductPricesToTest(i)) = False Then Return False
    Next
    Return True
  End Function
  ...
  Public Overrides Function CloneTargCCCollection() As ITargCCCollection 
    ...
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    If pFilledFromSumOnTheFly Then pProductPrices._FilledFromSumOnTheFly = True
    
    For Each pProductPrice As clsProductPrice In Me 
      Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone() 
      pProductPrices.Add(pProductPriceClone) 
      If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
    Next 
    Return pProductPrices 
  End Function 
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function Clone() As clsProductPriceCol
    Dim pFilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    If pFilledFromSumOnTheFly Then pProductPrices._FilledFromSumOnTheFly = True
  ...
  ''' <returns></returns> 
  ''' <remarks></remarks> 
  Public Function CloneByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long) As clsProductPriceCol 
    Dim pProductPrices As New clsProductPriceCol(_WithParents)  
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
 
    If _RecreateDictionaryForFindByID = True Then LoadIDs() 
 
    For Each pProductPrice As clsProductPrice In _SortedDictionaryForFindByID.Values.ToList() 
      If (pProductPrice.ID > vIDFrom AndAlso pProductPrice.ID <= vIDTo) Then 
        Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone() 
        pProductPrices.Add(pProductPriceClone) 
        If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
      End If 
  ...
  Public Function LoadParents(ByVal vRequester As clsRequester) As clsFault
    Dim pFault As New clsFault
    For Each pProductPrice As clsProductPrice In Me
      pFault = pProductPrice.LoadParents(vRequester)
      If pFault.isOK = False Then Return pFault
    Next
    _WithParents = clsEnums.enmLoadParent.EntireObject 
    pFault.SetOK()
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function FindByID(ByVal vID As Long) As clsProductPrice
    If Me.Count = 0 Then Return New clsProductPrice 
    ...
    Dim pLocalDict As Dictionary(Of Long, clsProductPrice) = _SortedDictionaryForFindByID 
    ...
    If pLocalDict IsNot Nothing Then pLocalDict.TryGetValue(vID, pProductPrice) 
    If pProductPrice IsNot Nothing Then Return pProductPrice Else Return New clsProductPrice() 
  End Function
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByProductID(ByVal vProductID As Long) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    Dim pTempDist As Dictionary(Of Long, clsProductPrice) = _SortedDictionaryForFindByID 
 
    ' 3. READ FROM THE SNAPSHOT (Lock-Free Speed) 
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.ProductID = vProductID Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
    ...
    Else 
      ...
      Dim pList As clsProductPriceCol = Me.Clone() 
      For Each pProductPrice As clsProductPrice In pList 
        If pProductPrice.ProductID = vProductID Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByCustomerType(ByVal vCustomerType As clsEnums.enmCustomerType) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.CustomerType = vCustomerType Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneBySellingPrice(ByVal vSellingPrice As Decimal) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.SellingPrice = vSellingPrice Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByMinQuantity(ByVal vMinQuantity As Integer) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.MinQuantity = vMinQuantity Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByDiscountPercent(ByVal vDiscountPercent As Decimal) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.DiscountPercent = vDiscountPercent Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByNotes(ByVal vNotes As String) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.Notes.ToLowerInvariant() = vNotes Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByTag(ByVal vTag As String) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If pTempDist IsNot Nothing AndAlso pTempDist.Count > 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In pTempDist.Values
        If pProductPrice.Tag.ToLowerInvariant() = vTag Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          If Not _FilledFromSumOnTheFly Then pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
  ...
  ''' <summary>
  ''' This returns a clone of the collection, including only those object with the defined ProductIDAndCustomerType
  ''' </summary>
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function CloneByProductIDAndCustomerType(ByVal vProductID As Long, ByVal vCustomerType As clsEnums.enmCustomerType) As clsProductPriceCol
    Dim pProductPrices As New clsProductPriceCol(_WithParents) 
    pProductPrices._FilledFromSumOnTheFly = _FilledFromSumOnTheFly 
    ...
    If Not _SortedDictionaryForFindByID.Count = 0 Then 
      'This is faster 
      For Each pProductPrice As clsProductPrice In _SortedDictionaryForFindByID.Values.ToList()
        If pProductPrice.ProductID = vProductID AndAlso pProductPrice.CustomerType = vCustomerType Then
          Dim pProductPriceClone As clsProductPrice = pProductPrice.Clone()
          pProductPrices.Add(pProductPriceClone)
          pProductPrices._Clean.Add(pProductPrice.ID) 
        End If
      Next
    Else 
      'We have no ID, so we can't use it 
  ...
  Public Overrides Function LoadMeIntoDataTable(ByVal vDataTable As DataTable, ByVal vRequester As clsRequester) As clsFault 
    ...
    For Each pProductPrice As clsProductPrice In Me 
      ...
      pFault = pProductPrice.LoadDataRow(pRow, vRequester) 
      If pFault.isOK = False Then Return pFault 
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.Update
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceView, "clsProductPriceCol_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    'Check for new rows 
    For Each p As clsProductPrice In Me 
    ...
    If _Clean IsNot Nothing Then 'Since it's private, it will always be nothing when coming from a web service. (The same process was already done in WSController) 
      For Each pCleanID As Long In _Clean 
        ...
        Dim pFound As clsProductPrice = Me.FindByID(pCleanID) 
        If pFound.ID = 0 Then 
          Dim pProductPriceToKill As New clsProductPrice 
          pProductPriceToKill.ID = pCleanID 
          pProductPriceToKill.ccStatus = clsEnums.enmObjectStatus.Deleted 
          Me.Add(pProductPriceToKill) 
        End If 
      Next 
    End If 
    pFault.SetOK() 
    ...
    For Each pExists As clsProductPrice In Me 
      ...
      ElseIf pExists.ccStatus = clsEnums.enmObjectStatus.UD Then 
        ...
        Return pFault.LogFreeTextFault("Status should not be UD ", pFunctionParameters, "TRGT-ProductPrice-130415-0942", vRequester) 
      End If 
    Next 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceUpdate, "clsProductPriceCol_UpdateFromCollection", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPriceCol_Delete", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccProductPricesDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      pFault = MyController.DBCache.SaveData(New clsProductPriceCol(), vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  ''' <summary>
  ''' Deletes a collection of all the items for a specific ProductIDAndCustomerType 
  ''' </summary>
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Shared Function DeleteByProductIDAndCustomerType(ByVal vProductID As Long, ByVal vCustomerType As clsEnums.enmCustomerType, ByVal vRequester As clsRequester) As clsFault
    Dim pFunctionParameters As String = String.Format("ProductID={0}, CustomerType={1}", vProductID, vCustomerType)
    Dim pFault As New clsFault
 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPriceCol_DeleteByProductIDAndCustomerType", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccProductPricesDeleteByProductID&CustomerType"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllProductPrices As New clsProductPriceCol() : pAllProductPrices.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredProductPrices As clsProductPriceCol = pAllProductPrices.CloneByProductIDAndCustomerType(vProductID, vCustomerType) 
      For Each l In pFilteredProductPrices 
        pAllProductPrices.Remove(pAllProductPrices.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllProductPrices, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByProductID(ByVal vProductID As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPriceCol_DeleteByProductID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccProductPricesDeleteByProductID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllProductPrices As New clsProductPriceCol() : pAllProductPrices.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredProductPrices As clsProductPriceCol = pAllProductPrices.CloneByProductID(vProductID) 
      For Each l In pFilteredProductPrices 
        pAllProductPrices.Remove(pAllProductPrices.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllProductPrices, vRequester) 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_ProductPriceDelete, "clsProductPriceCol_DeleteByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccProductPricesDeleteByBoundedID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-ProductPrice-150216-2148", vRequester) 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-090210-1341", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Sub SortByID()
    Me.Sort(New clsProductPriceCol.CompareByID)
  End Sub
  Private Class CompareByID
    Implements IComparer(Of clsProductPrice)
    Private Function Compare(ByVal x As clsProductPrice, ByVal y As clsProductPrice) As Integer Implements System.Collections.Generic.IComparer(Of clsProductPrice).Compare
  ...
  Public Sub SortByProductID()
    Me.Sort(New clsProductPriceCol.CompareByProductID)
  End Sub
  ...
  Public Sub SortByProductText()
    Me.Sort(New clsProductPriceCol.CompareByProductText)
  End Sub
  ...
  Public Sub SortByCustomerType()
    Me.Sort(New clsProductPriceCol.CompareByCustomerType)
  End Sub
  ...
  Public Sub SortByCustomerTypeText()
    Me.Sort(New clsProductPriceCol.CompareByCustomerTypeText)
  End Sub
  ...
  Public Sub SortBySellingPrice()
    Me.Sort(New clsProductPriceCol.CompareBySellingPrice)
  End Sub
  ...
  Public Sub SortByMinQuantity()
    Me.Sort(New clsProductPriceCol.CompareByMinQuantity)
  End Sub
  ...
  Public Sub SortByDiscountPercent()
    Me.Sort(New clsProductPriceCol.CompareByDiscountPercent)
  End Sub
  ...
  Public Sub SortByNotes()
    Me.Sort(New clsProductPriceCol.CompareByNotes)
  End Sub
  ...
  Public Sub SortByTag()
    Me.Sort(New clsProductPriceCol.CompareByTag)
  End Sub
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    While vReader.Read()
      pProductPrice = New clsProductPrice(_WithParents) 
      pFault = pProductPrice.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pProductPrice)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pProductPrice.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedProductPriceCol As clsProductPriceCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedProductPrice As clsProductPrice In vCachedProductPriceCol 
        pCachedProductPrice.SetWithParents(_WithParents) 
        pProductPrice = New clsProductPrice(pCachedProductPrice) 
        If _WithParents = clsEnums.enmLoadParent.DoNotLoad Then 
          pProductPrice.ProductText = "." 
        End If 
        pProductPrice.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pProductPrice) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pProductPrice.ID) 
      Next 
      If _WithParents = clsEnums.enmLoadParent.EntireObject Then 
      ...
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-ProductPrice-121122-1513", vRequester) 
      End Try 
...
#End Region 
  ...
  Private Overloads Sub Clear() 
    ...
    _SortedDictionaryForFindByID = New Dictionary(Of Long, clsProductPrice) 
    _RecreateDictionaryForFindByID = False 
  End Sub 
