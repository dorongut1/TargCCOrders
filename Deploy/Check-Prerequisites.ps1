...
if (Test-Path $ancm) { Ok 'ASP.NET Core Module V2 present' }
else { Bad 'aspnetcorev2.dll not found — the Hosting Bundle did not register with IIS.' 'Install the Hosting Bundle AFTER IIS, then run: iisreset' }
...
if (-not (Test-Path $PublishPath)) {
    ...
    if (Test-Path $cfg) {
        ...
        if ($txt -match 'value="Localhost~TargCCOrdersNew~500"\s*/>') {
            Bad '  connection string is still the development default.' 'Set TargCCOrders.Controller to <SERVER>~TargCCOrdersNew~500~TargCCApp~<password>.'
        } else { Ok '  connection string has been customised' }
