Public Class csAlertMessage
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "csAlertMessageGetByID" 
      ...
      'Use the response to build the AlertMessage 
      LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-150308-1015", vRequester) 
    End Try 
  ...
  Public Function GetByNumber(ByVal vNumber As Integer, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "csAlertMessageGetByNumber" 
      Dim pParametersToLog = $"Number: {vNumber};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault  
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    Dim pAlertMessage As New csAlertMessage 
    If Me.isEqual(pAlertMessage) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", "", "TRGT-AlertMessage-100113-1813", vRequester) 
    End If 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "csAlertMessageUpdate" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-150314-1803", vRequester) 
    End Try 
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "csAlertMessageDelete" 
      Dim pParametersToLog = $"ID: {ID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      'Run the request  
      Dim pFunction As String = "csAlertMessageDeleteByID" 
      Dim pParametersToLog = $"ID: {vID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Assign the value  
    Catch ex As Exception 
      Return New clsFault().LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-231207-1707", vRequester) 
    End Try 
...
Public Class csAlertMessageCol
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColFill" 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vDescription 
          If vDescription Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vDescription) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
      ...
      Dim pFunction As String = "csAlertMessageColFillByDescription" 
      Dim pParametersToLog = $"Description: {vDescription};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Use the response to build the AlertMessage  
      If vAppend = True Then 
        Dim pAlertMessages As New csAlertMessageCol 
        pAlertMessages.LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
        Me.AddRange(pAlertMessages) 
      Else 
        LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-150407-2142", vRequester) 
    End Try 
  ...
  ''' <summary>
  ''' Gets a collection of all the items for a specific Type and Severity, or a sub-collection defined by HowMany and Direction. To append to an existing collection, set vAppend to true (default is false)
  ''' </summary>
  ''' <param name="vHowMany">How Many (sorted by ID) - 0 for all</param>
  ''' <param name="vDir">ASCendined or DESCending</param>
  ''' <param name="vAppend">True of False</param>
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          ' 
          'vSeverity 
          pBinaryWriter.Write(vSeverity.ToString()) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
      ...
      Dim pFunction As String = "csAlertMessageColFillByTypeAndSeverity" 
      Dim pParametersToLog = $"TypeAndSeverity: {vType};{vSeverity};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColFillByBoundedID" 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vDescriptionFrom 
          If vDescriptionFrom Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vDescriptionFrom) 
          ' 
          'vDescriptionTo 
          If vDescriptionTo Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vDescriptionTo) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
      ...
      Dim pFunction As String = "csAlertMessageColFillByBoundedDescription" 
      Dim pParametersToLog = $"Description: {vDescriptionFrom};{vDescriptionTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vNumberFrom 
          pBinaryWriter.Write(vNumberFrom) 
          ' 
          'vNumberTo 
          pBinaryWriter.Write(vNumberTo) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
      ...
      Dim pFunction As String = "csAlertMessageColFillByBoundedNumber" 
      Dim pParametersToLog = $"Number: {vNumberFrom};{vNumberTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If pHasValue = True Then pBinaryWriter.Write(vDescription) 
          ' 
          pBinaryWriter.Write(vDescriptionWildcardType.FastToString())
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
          pBinaryWriter.Write(_IsLocalized) 
      ...
      Dim pFunction As String = "csAlertMessageColFillByWildCardDescription" 
      Dim pParametersToLog = $"Description: {vDescription};{vDescriptionWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColFillByListOfID" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-231207-1750", vRequester) 
    End Try 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          'Number 
          pBinaryWriter.Write(vNumberFrom.HasValue) 
          If vNumberFrom.HasValue Then pBinaryWriter.Write(vNumberFrom.Value) : pParametersToLog &= $"NumberFrom={vNumberFrom};"  
          pBinaryWriter.Write(vNumberTo.HasValue) 
          If vNumberTo.HasValue Then pBinaryWriter.Write(vNumberTo.Value) : pParametersToLog &= $"NumberTo={vNumberTo};"  
          'Description 
          If vDescription Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vDescription) : pBinaryWriter.Write(vDescriptionWildcardType.FastToString()) : pParametersToLog &= $"Description={vDescription};" : pParametersToLog &= $"DescriptionWildcardType={vDescriptionWildcardType};"  
          'Type 
          pBinaryWriter.Write(vType.ToString()) : pParametersToLog &= $"Type={vType};"  
          'Severity 
          pBinaryWriter.Write(vSeverity.ToString()) : pParametersToLog &= $"Severity={vSeverity};"  
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(_IsLocalized) 
          pBinaryWriter.Write(_LocalizedLanguage.FastToString()) 
      ...
      Dim pFunction As String = "csAlertMessageColFillOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If vDescription Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vDescription) : pBinaryWriter.Write(vDescriptionWildcardType.FastToString()) 
          'Type 
          pBinaryWriter.Write(vType.ToString()) : pParametersToLog &= $"Type={vType};"  
          'Severity 
          pBinaryWriter.Write(vSeverity.ToString()) : pParametersToLog &= $"Severity={vSeverity};"  
          pBinaryWriter.Write(vGroupByDescription) : pParametersToLog &= $"GroupByDescription={vGroupByDescription};"  
          pBinaryWriter.Write(vGroupByType) : pParametersToLog &= $"GroupByType={vGroupByType};"  
          pBinaryWriter.Write(vGroupBySeverity) : pParametersToLog &= $"GroupBySeverity={vGroupBySeverity};"  
          pBinaryWriter.Write(_IsLocalized) 
          pBinaryWriter.Write(_LocalizedLanguage.FastToString()) 
          pBinaryWriter.Close() 
        End Using 
      ...
      Dim pFunction As String = "csAlertMessageColFillSumOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "csAlertMessageColUpdateFromCollection" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDelete" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-AlertMessage-200709-0852-", vRequester) 
    End Try 
  ...
  Public Shared Function DeleteByDescription(ByVal vDescription As String, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByDescription" 
      Dim pParametersToLog = $"Description: {vDescription};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByTypeAndSeverity(ByVal vType As clsEnums.enmFaultType, ByVal vSeverity As clsEnums.enmFaultSeverity, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByTypeAndSeverity" 
      Dim pParametersToLog = $"TypeAndSeverity: {vType};{vSeverity};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByBoundedID" 
      Dim pParametersToLog = $"ID: {vIDFrom};{vIDTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedDescription(ByVal vDescriptionFrom As String, ByVal vDescriptionTo As String, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByBoundedDescription" 
      Dim pParametersToLog = $"Description: {vDescriptionFrom};{vDescriptionTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedNumber(ByVal vNumberFrom As Integer, ByVal vNumberTo As Integer, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByBoundedNumber" 
      Dim pParametersToLog = $"Number: {vNumberFrom};{vNumberTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByWildCardDescription(ByVal vDescription As String, ByVal vDescriptionWildcardType As clsEnums.enmWildCardType, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "csAlertMessageColDeleteByWildCardDescription" 
      Dim pParametersToLog = $"Description: {vDescription};{vDescriptionWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
