Public Class ctlc_LookupCol
  ...
  Public Event evtUpdated(ByVal vLookup As csLookup) 
  Private Event evtBeforeDelete(ByVal vLookup As csLookup, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vLookup As csLookup) 
  Public Event evtRowDoubleClicked(ByVal vLookup As csLookup, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As csLookup.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property CbosDoNotLoad As List(Of csLookup.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of csLookup.enmProperty) 
    Public Property ColumnsHide As List(Of csLookup.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csLookup.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csLookup.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csLookup.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csLookup.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of csLookup.enmParentProperty) 
      _ColumnsReadOnly = New List(Of csLookup.enmProperty) 
      _ColumnsHide = New List(Of csLookup.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csLookup.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csLookup.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csLookup.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csLookup.enmProperty, String) 
  ...
  Private WithEvents _LookupCol As csLookupCol
  Private WithEvents _LookupColFullLength As csLookupCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedLookup]() As csLookup 
    Get 
      If dgvLookup.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvLookup.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvLookup.Rows.Count - 1 Then dgvLookup.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _LookupCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [LookupCol]() As csLookupCol 
    Get 
      Return _LookupCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pLookupCol As New csLookupCol(vIsLocalized:=True) 
    If LocalizedTextLanguage <> clsEnums.enmLanguage.UD Then pLookupCol.OverrideDefaultLanguage(LocalizedTextLanguage) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pLookupCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pLookupCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vLookupCol As csLookupCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vLookupCol) 
  End Function
  
  Private Function LoadControl(vLookupCol As csLookupCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csLookup.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csLookup.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csLookup.enmProperty.ID Then colID.ReadOnly = True 
      If l = csLookup.enmProperty.ParentLookupType Then colParentLookupType.ReadOnly = True 
      If l = csLookup.enmProperty.ParentCode Then colParentCode.ReadOnly = True 
      If l = csLookup.enmProperty.LookupType Then colLookupType.ReadOnly = True 
      If l = csLookup.enmProperty.Code Then colCode.ReadOnly = True 
      If l = csLookup.enmProperty.Text Then colText.ReadOnly = True 
      If l = csLookup.enmProperty.Description Then colDescription.ReadOnly = True 
    Next 
    ...
    dgvLookup.DoubleBuffered(True) 
 
    pFault = vLookupCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _LookupColFullLength = vLookupCol.Clone() 
 
    'Truncate the strings 
    _LookupCol = vLookupCol 
    If _LoadParameters.TruncateStrings Then 
      _LookupCol.TruncateStrings() 
    Else 
      dgvLookup.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvLookup.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      ...
      Else 
        If (_Requester.UILang = clsEnums.enmLanguage.en AndAlso LocalizedTextLanguage = clsEnums.enmLanguage.UD) OrElse LocalizedTextLanguage = clsEnums.enmLanguage.en Then 
          ...
          colDescriptionLocalized.ReadOnly = True 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvLookup.Columns 
    ...
    'Hide columns  
    For Each p As csLookup.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvLookup.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvLookup.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvLookup.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvLookup.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    bsCtlLookup.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Lookup-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvLookup.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvLookup_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvLookup.SelectedRows.Count > 0 Then 
      pRowIndex = dgvLookup.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvLookup.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvLookup.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlLookup.DataSource = Nothing 
    bsCtlLookup.DataSource = _LookupCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvLookup.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _LookupCol.Count - 2 Then 
          dgvLookup.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _LookupCol.Count - 1 Then 
          dgvLookup.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'EnumParentLookupType
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(csLookup.enmParentProperty.ParentLookupType, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.Lookup, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmLookup.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmLookup.UD, pPrompt) 
    bsParentLookupType.DataSource = pEnumCol 
    colParentLookupType.Tag = pPrompt 

    'EnumLookupType
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(csLookup.enmParentProperty.LookupType, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.Lookup, _Requester) 
      If pFault.isOK = False Then Return pFault 
    ...
    End If 
    pEnumCol.AddToTop(clsEnums.enmLookup.UD, pPrompt) 
    bsLookupType.DataSource = pEnumCol 
    colLookupType.Tag = pPrompt 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_LookupUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_LookupUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_LookupDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      dgvLookup.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvLookup.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvLookup.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvLookup.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvLookup.AllowUserToDeleteRows = False 
      dgvLookup.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvLookup.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvLookup.CurrentRow.Index 
      '  dgvLookup.CurrentCell = dgvLookup.Rows(pCurrentRow).Cells(0) 
      '  dgvLookup.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("ParentLookupType (DB Code), ") 
    pFieldList.Append("ParentCode, ") 
    pFieldList.Append("LookupType (DB Code), ") 
    pFieldList.Append("Code, ") 
    pFieldList.Append("Text, ") 
    pFieldList.Append("TextLocalized (" & _LookupCol.LocalizedLanguage.FastToString() & "), ") 
    pFieldList.Append("Description, ") 
    pFieldList.Append("DescriptionLocalized (" & _LookupCol.LocalizedLanguage.FastToString() & "), ") 
    ...
    pMessage &= Environment.NewLine 
    pMessage &= Environment.NewLine 
    pMessage &= "If there is no ID field (the 1st field is ParentLookupType), then I will delete the table and recreate it with the data in this spreadsheet" 
    pMessage &= Environment.NewLine 
    pMessage &= Environment.NewLine 
    ...
    'Now Load the data 
    Dim pIncomingLookups As New csLookupCol(vIsLocalized:=True) 
    If LocalizedTextLanguage <> clsEnums.enmLanguage.UD Then pIncomingLookups.OverrideDefaultLanguage(LocalizedTextLanguage) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingLookup As New csLookup(vIsLocalized:=True) 
          If LocalizedTextLanguage <> clsEnums.enmLanguage.UD Then pIncomingLookup.OverrideDefaultLanguage(LocalizedTextLanguage) 
          pIncomingLookup.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = csLookupCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingLookup.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingLookups.Add(pIncomingLookup) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingLookup.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
 
          Try 
            pFieldNo += 1 
            pFieldName = "ParentLookupType" 
            pIncomingLookup.ParentLookupType = clsEnums.TranslateEnmLookup(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "ParentCode" 
            pIncomingLookup.ParentCode = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LookupType" 
            pIncomingLookup.LookupType = clsEnums.TranslateEnmLookup(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Code" 
            pIncomingLookup.Code = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Text" 
            pIncomingLookup.Text = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingLookup.TextLocalized = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingLookup.Description = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "DescriptionLocalized" 
            pIncomingLookup.DescriptionLocalized = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingLookup.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingLookup.Tag = "Row " & pRow.ToString Then 
            pIncomingLookup.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingLookups.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingLookups.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingLookups 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
  ...
  Private Sub DoEdit() 
    ...
    If dgvLookup.Focused = True AndAlso dgvLookup.SelectedRows.Count > 0 Then 
      pCellRow = dgvLookup.CurrentCell.RowIndex 
      pCellCol = dgvLookup.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvLookup.CurrentCell = dgvLookup.Rows(0).Cells(0) 
      dgvLookup.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _LookupCol.Count > 0 AndAlso _LookupCol(_LookupCol.Count - 1).ID = 0 Then 
      _LookupCol.RemoveAt(_LookupCol.Count - 1) 
      bsCtlLookup.DataSource = Nothing 
      bsCtlLookup.DataSource = _LookupCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvLookup.Focus() 
      dgvLookup.CurrentCell = dgvLookup.Rows(pCellRow).Cells(pCellCol) 
      dgvLookup.CurrentCell.Selected = True 
    ElseIf _LookupCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    Dim pEntity As csLookup 
    pEntity = CType(bsCtlLookup.Current, csLookup) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvLookup.CurrentCell Is Nothing Then Return pFault 
    
    If dgvLookup.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pLookup As csLookup 
      pLookup = CType(bsCtlLookup.Current, csLookup) 
      If pLookup Is Nothing Then 
        pFault.LogFreeTextFault("There is no Lookup to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvLookup.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvLookup.CurrentCell.RowIndex 
      'show row as selected  
      dgvLookup.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvLookup.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvLookup.CurrentRow.Selected = True 
      If pLookup.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pLookup.LookupType.FastToString() & " --> " & pLookup.Code & " (" & pLookup.Text & ")" & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pLookup, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvLookup.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvLookup.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvLookup.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pLookup.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlLookup.Remove(bsCtlLookup.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pLookup As csLookup = CType(bsCtlLookup.Current, csLookup) 
    If pLookup IsNot Nothing Then 
      If pLookup.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _LookupCol.Count > 0 AndAlso dgvLookup.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvLookup.Columns.Count - 1 
        If dgvLookup.Columns(i).Visible Then 
          dgvLookup.CurrentCell = dgvLookup.Rows(dgvLookup.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvLookup.Refresh() 
      dgvLookup.Rows(dgvLookup.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvLookup.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvLookup_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvLookup.RowValidating 
    If _Loading = True OrElse dgvLookup.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvLookup.CurrentCell = dgvLookup.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvLookup_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvLookup.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvLookup.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvLookup.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvLookup.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pLookup Is Nothing Then pLookup = CType(dgvLookup.Rows(e.RowIndex).DataBoundItem, csLookup) ' Only assign it if needed 
    '  If pLookup.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pLookup.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvLookup.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pLookup Is Nothing Then pLookup = CType(dgvLookup.Rows(e.RowIndex).DataBoundItem, csLookup) ' Only assign it if needed
    '  If pLookup.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pLookup.RAV - pLookup.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvLookup.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvLookup.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvLookup.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvLookup.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvLookup.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvLookup.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvLookup.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvLookup_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvLookup.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvLookup_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvLookup.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvLookup.Columns(e.ColumnIndex)
    If bsCtlLookup.Current Is Nothing Then Exit Sub
    ...
    Dim pLookupCol As csLookupCol
    pLookupCol = CType(bsCtlLookup.DataSource, csLookupCol)

    Dim pSummaryRow As csLookup = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pLookupCol(pLookupCol.Count - 1) 
      pLookupCol.RemoveAt(pLookupCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pLookupCol.Count - 1 
          pLookupCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pLookupCol.SortByID()
      ElseIf pNewColumn Is colParentLookupType Then
        pLookupCol.SortByParentLookupType()
      ElseIf pNewColumn Is colParentCode Then
        pLookupCol.SortByParentCode()
      ElseIf pNewColumn Is colLookupType Then
        pLookupCol.SortByLookupType()
      ElseIf pNewColumn Is colCode Then
        pLookupCol.SortByCode()
      ElseIf pNewColumn Is colText Then
        pLookupCol.SortByText()
      ElseIf pNewColumn Is colTextLocalized Then
        pLookupCol.SortByTextLocalized()
      ElseIf pNewColumn Is colDescription Then
        pLookupCol.SortByDescription()
      ElseIf pNewColumn Is colDescriptionLocalized Then
        pLookupCol.SortByDescriptionLocalized()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csLookup In pLookupCol 
            If p.ID <> pTest Then iCntr += 1 : pTest = p.ID 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colParentLookupType Then
          Dim pTest As clsEnums.enmLookup = clsEnums.enmLookup.UD 
          For Each p As csLookup In pLookupCol 
            If p.ParentLookupType <> pTest Then iCntr += 1 : pTest = p.ParentLookupType 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colParentCode Then
          Dim pTest As String = "" 
          For Each p As csLookup In pLookupCol 
            If p.ParentCode <> pTest Then iCntr += 1 : pTest = p.ParentCode 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLookupType Then
          Dim pTest As clsEnums.enmLookup = clsEnums.enmLookup.UD 
          For Each p As csLookup In pLookupCol 
            If p.LookupType <> pTest Then iCntr += 1 : pTest = p.LookupType 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colCode Then
          Dim pTest As String = "" 
          For Each p As csLookup In pLookupCol 
            If p.Code <> pTest Then iCntr += 1 : pTest = p.Code 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colText Then
          Dim pTest As String = "" 
        ...
        ElseIf pNewColumn Is colDescriptionLocalized Then
          Dim pTest As String = "" 
          For Each p As csLookup In pLookupCol 
            If p.DescriptionLocalized <> pTest Then iCntr += 1 : pTest = p.DescriptionLocalized 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pLookupCol.SortByTag() 
      End If 
    ...
    If _Summarized = True Then 
      pLookupCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlLookup.Position = bsCtlLookup.IndexOf(pLookupCol.FindByID(pID))
    End If

    'dgvLookup.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvLookup.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pLookupCol As csLookupCol 
      pLookupCol = CType(bsCtlLookup.DataSource, csLookupCol) 
      Dim pLookup As csLookup = pLookupCol.FindByID(pID) 
      If Not pLookup.IsEmpty Then 
        bsCtlLookup.Position = bsCtlLookup.IndexOf(pLookupCol.FindByID(pID)) 
        dgvLookup.Rows(bsCtlLookup.Position).Selected = True 
      Else 
        dgvLookup.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvLookup_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvLookup.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvLookup.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvLookup_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvLookup.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvLookup_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvLookup.Scroll
    dgvLookup.Invalidate() 
  End Sub
 
  Private Sub dgvLookup_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvLookup.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvLookup.Rows.Count - 1 Then Exit Sub
 
    'If dgvLookup.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Lookup', the row with an ID of " & dgvLookup.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Lookup', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvLookup.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvLookup.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Lookup-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvLookup(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvLookup(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    ...
    'End If 
  End Sub
  Private Sub dgvLookup_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvLookup.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvLookup.CurrentCell = dgvLookup.Rows(dgvLookup.CurrentCell.RowIndex).Cells(0) 
      dgvLookup.Refresh() 
    End If 
  End Sub 
  Private Sub dgvLookup_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvLookup.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvLookup_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvLookup.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvLookup_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvLookup.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvLookup.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvLookup(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pLookup As csLookup = _LookupCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pLookup, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Lookup Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_Lookup", pLookup, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvLookup_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvLookup.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvLookup.Rows.Count - 1 Then dgvLookup.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pLookup As csLookup = _LookupCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pLookup) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvLookup_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvLookup.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvLookup.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvLookup.Columns(pNewCol).Visible = False Then 
      dgvLookup.Columns(pNewCol).Visible = True 
      dgvLookup.CurrentCell = dgvLookup.CurrentRow.Cells(pNewCol) 
      dgvLookup.CurrentCell = dgvLookup.CurrentRow.Cells(pOriginalCol) 
      dgvLookup.Columns(pNewCol).Visible = False 
    Else 
      dgvLookup.CurrentCell = dgvLookup.CurrentRow.Cells(pNewCol) 
      dgvLookup.CurrentCell = dgvLookup.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pLookup, csLookup), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pLookup.Update(_Requester) 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvLookup.Columns 
  ...
  Private Sub Summarize() 
    If _LookupCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As csLookup In _LookupCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\LookupCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\LookupCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\LookupCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\LookupCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\LookupCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvLookup.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvLookup.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _LookupColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _LookupColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _LookupColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _LookupColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _LookupColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Lookup-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Lookups" 
        .SubTitleRight = "Rows: " & _LookupCol.Count.ToString 
        ...
        End If 
        .DataSource = _LookupCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Lookup-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Lookup-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvLookup.Rows 
    ...
    For Each row As DataGridViewRow In dgvLookup.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlLookup), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvLookup.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Lookup-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvLookup.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvLookup.Width - 30) / dgvLookup.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvLookup.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Lookup-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colParentLookupType.HeaderText = pStrg : mnuColVisibleParentLookupType.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Lookup", "ParentCode", _Requester) 
    If pStrg <> "" Then colParentCode.HeaderText = pStrg : mnuColVisibleParentCode.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Lookup", "LookupType", _Requester) 
    If pStrg <> "" Then colLookupType.HeaderText = pStrg : mnuColVisibleLookupType.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Lookup", "Code", _Requester) 
    If pStrg <> "" Then colCode.HeaderText = pStrg : mnuColVisibleCode.Text = pStrg
    ...
    If pStrg <> "" Then colDescriptionLocalized.HeaderText = pStrg : mnuColVisibleDescriptionLocalized.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleParentLookupType.Click, mnuColVisibleParentCode.Click, mnuColVisibleLookupType.Click, mnuColVisibleCode.Click, mnuColVisibleText.Click, mnuColVisibleTextLocalized.Click, mnuColVisibleDescription.Click, mnuColVisibleDescriptionLocalized.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvLookup.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvLookup.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleParentLookupType.Checked = True Then mnuColVisibleParentLookupType.PerformClick() 
    If mnuColVisibleParentCode.Checked = True Then mnuColVisibleParentCode.PerformClick() 
    If mnuColVisibleLookupType.Checked = True Then mnuColVisibleLookupType.PerformClick() 
    If mnuColVisibleCode.Checked = True Then mnuColVisibleCode.PerformClick() 
    ...
    If mnuColVisibleDescriptionLocalized.Checked = True Then mnuColVisibleDescriptionLocalized.PerformClick() 
    'Show Defaults 
    If mnuColVisibleLookupType.Checked = False Then mnuColVisibleLookupType.PerformClick() 
    If mnuColVisibleCode.Checked = False Then mnuColVisibleCode.PerformClick() 
    If mnuColVisibleText.Checked = False Then mnuColVisibleText.PerformClick() 
    If mnuColVisibleTextLocalized.Checked = True Then mnuColVisibleTextLocalized.PerformClick() 
  ...
  Private Sub dgvLookup_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvLookup.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Lookup to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvLookup_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvLookup.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvLookup.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvLookup.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvLookup.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvLookup.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvLookup(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvLookup.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvLookup.BeginEdit(True) 
        DirectCast(dgvLookup.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvLookup_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvLookup.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvLookup.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvLookup.MultiSelect = True 
        dgvLookup.ClearSelection() 
  ...
  Private Sub ctlc_LookupCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvLookup.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_LookupCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvLookup.Width 
  ...
  Private Sub ctlc_LookupCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlc_LookupCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvLookup_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvLookup.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvLookup.Rows(e.RowIndex).Selected Then 
        dgvLookup.ClearSelection() 
        dgvLookup.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvLookup.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvLookup.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvLookup.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _LookupCol.Count Then Exit Sub 
    Dim pLookup As csLookup = _LookupCol(pRowIndex) 
    Dim pTitle As String = "Lookup #" & pLookup.ID.ToString() 
    Dim pKey As String = "Lookup_" & pLookup.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_Lookup" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pLookup, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pLookup 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Lookup_" & pLookup.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Lookup #" & pLookup.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_Lookup", pLookup, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvLookup.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvLookup.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _LookupCol.Count Then 
        Dim pLookup As csLookup = _LookupCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pLookup.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvLookup.SelectedRows(0) 
