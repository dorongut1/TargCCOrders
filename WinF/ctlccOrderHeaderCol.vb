Public Class ctlccOrderHeaderCol
  ...
  Public Event evtUpdated(ByVal vOrderHeader As clsOrderHeader) 
  Private Event evtBeforeDelete(ByVal vOrderHeader As clsOrderHeader, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vOrderHeader As clsOrderHeader) 
  Public Event evtRowDoubleClicked(ByVal vOrderHeader As clsOrderHeader, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As clsOrderHeader.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of clsOrderHeader.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property CbosDoNotLoad As List(Of clsOrderHeader.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of clsOrderHeader.enmProperty) 
    Public Property ColumnsHide As List(Of clsOrderHeader.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of clsOrderHeader.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of clsOrderHeader.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of clsOrderHeader.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of clsOrderHeader.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of clsOrderHeader.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of clsOrderHeader.enmParentProperty) 
      _ColumnsReadOnly = New List(Of clsOrderHeader.enmProperty) 
      _ColumnsHide = New List(Of clsOrderHeader.enmProperty) 
      _ColumnsFormat = New Dictionary(Of clsOrderHeader.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of clsOrderHeader.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of clsOrderHeader.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of clsOrderHeader.enmProperty, String) 
  ...
  Private WithEvents _OrderHeaderCol As clsOrderHeaderCol
  Private WithEvents _OrderHeaderColFullLength As clsOrderHeaderCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedOrderHeader]() As clsOrderHeader 
    Get 
      If dgvOrderHeader.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvOrderHeader.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvOrderHeader.Rows.Count - 1 Then dgvOrderHeader.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _OrderHeaderCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [OrderHeaderCol]() As clsOrderHeaderCol 
    Get 
      Return _OrderHeaderCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pOrderHeaderCol As New clsOrderHeaderCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pOrderHeaderCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pOrderHeaderCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pOrderHeaderCol As New clsOrderHeaderCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "Customer" 
          pFault = pOrderHeaderCol.FillByCustomerID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vOrderHeaderCol As clsOrderHeaderCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vOrderHeaderCol) 
  End Function
  
  Private Function LoadControl(vOrderHeaderCol As clsOrderHeaderCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of clsOrderHeader.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of clsOrderHeader.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = clsOrderHeader.enmProperty.ID Then colID.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.OrderNumber Then colOrderNumber.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.Customer Then colCustomer.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.OrderDate Then colOrderDate.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.TotalAmount Then colTotalAmount.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.VATAmount Then colVATAmount.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.TotalWithVAT Then colTotalWithVAT.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.PaymentMethod Then colPaymentMethod.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.PaymentStatus Then colPaymentStatus.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.PaymentDate Then colPaymentDate.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.InvoiceNumber Then colInvoiceNumber.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.DeliveryMethod Then colDeliveryMethod.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.DeliveryDate Then colDeliveryDate.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.DeliveryDay Then colDeliveryDay.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.OrderStatus Then colOrderStatus.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.Notes Then colNotes.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.Notes2 Then colNotes2.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.OrderMonth Then colOrderMonth.ReadOnly = True 
      If l = clsOrderHeader.enmProperty.Quarter Then colQuarter.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As clsOrderHeader.enmParentProperty = clsOrderHeader.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of clsOrderHeader.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvOrderHeader.DoubleBuffered(True) 
 
    pFault = vOrderHeaderCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _OrderHeaderColFullLength = vOrderHeaderCol.Clone() 
 
    'Truncate the strings 
    _OrderHeaderCol = vOrderHeaderCol 
    If _LoadParameters.TruncateStrings Then 
      _OrderHeaderCol.TruncateStrings() 
    Else 
      dgvOrderHeader.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvOrderHeader.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        ...
        If colCustomer.DataGridView IsNot Nothing Then dgvOrderHeader.Columns.Remove(colCustomer) 
      Else 
        If colCustomer.ReadOnly = False Then 
          If colCustomerText.DataGridView IsNot Nothing Then dgvOrderHeader.Columns.Remove(colCustomerText) 
        Else 
          colCustomer.Name = colCustomer.Name & "zzzz" 
          colCustomerText.Name = colCustomer.Name.Replace("zzzz", "") 
          If colCustomer.DataGridView IsNot Nothing Then dgvOrderHeader.Columns.Remove(colCustomer) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(clsOrderHeader.enmParentProperty.Customer) Then 
            _LoadParameters.CbosDoNotLoad.Add(clsOrderHeader.enmParentProperty.Customer) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvOrderHeader.Columns 
    ...
    'Hide columns  
    For Each p As clsOrderHeader.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvOrderHeader.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvOrderHeader.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvOrderHeader.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvOrderHeader.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvOrderHeader.ClearSelection()
    bsCtlOrderHeader.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-OrderHeader-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvOrderHeader.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvOrderHeader_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvOrderHeader.SelectedRows.Count > 0 Then 
      pRowIndex = dgvOrderHeader.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvOrderHeader.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvOrderHeader.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlOrderHeader.DataSource = Nothing 
    bsCtlOrderHeader.DataSource = _OrderHeaderCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvOrderHeader.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _OrderHeaderCol.Count - 2 Then 
          dgvOrderHeader.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _OrderHeaderCol.Count - 1 Then 
          dgvOrderHeader.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'Customer
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = clsOrderHeader.enmParentProperty.Customer) = clsOrderHeader.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.ccCustomerDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.Customer, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-OrderHeaderCol-200806-1015", _Requester) 
        End If 
      End If 
    ...
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.PaymentMethod, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.PaymentMethod, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmPaymentMethod.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmPaymentMethod.UD, pPrompt) 
    bsPaymentMethod.DataSource = pEnumCol 
    colPaymentMethod.Tag = pPrompt 

    'EnumPaymentStatus
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.PaymentStatus, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.PaymentStatus, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmPaymentStatus.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmPaymentStatus.UD, pPrompt) 
    bsPaymentStatus.DataSource = pEnumCol 
    colPaymentStatus.Tag = pPrompt 

    'EnumDeliveryMethod
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.DeliveryMethod, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
    ...
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.DeliveryDay, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.DeliveryDay, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmDeliveryDay.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmDeliveryDay.UD, pPrompt) 
    bsDeliveryDay.DataSource = pEnumCol 
    colDeliveryDay.Tag = pPrompt 

    'EnumOrderStatus
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsOrderHeader.enmParentProperty.OrderStatus, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.OrderStatus, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmOrderStatus.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmOrderStatus.UD, pPrompt) 
    bsOrderStatus.DataSource = pEnumCol 
    colOrderStatus.Tag = pPrompt 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderHeaderUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderHeaderUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderHeaderDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      ...
      colTotalWithVAT.ReadOnly = True 
      colOrderMonth.ReadOnly = True 
      colQuarter.ReadOnly = True 
      dgvOrderHeader.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvOrderHeader.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvOrderHeader.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvOrderHeader.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvOrderHeader.AllowUserToDeleteRows = False 
      dgvOrderHeader.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvOrderHeader.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvOrderHeader.CurrentRow.Index 
      '  dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(pCurrentRow).Cells(0) 
      '  dgvOrderHeader.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
    ...
    Else 
      If _OrderHeaderCol.Count = 0 Then 
        btnSpreadsheet.Enabled = False 
        btnReport.Enabled = False 
      Else 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("OrderNumber, ") 
    pFieldList.Append("CustomerID (DB Code), ") 
    pFieldList.Append("OrderDate, ") 
    pFieldList.Append("PaymentMethod (DB Code), ") 
    pFieldList.Append("PaymentStatus (DB Code), ") 
    pFieldList.Append("PaymentDate, ") 
    pFieldList.Append("InvoiceNumber, ") 
    pFieldList.Append("DeliveryMethod (DB Code), ") 
    pFieldList.Append("DeliveryDate, ") 
    pFieldList.Append("DeliveryDay (DB Code), ") 
    pFieldList.Append("OrderStatus (DB Code), ") 
    pFieldList.Append("Notes, ") 
    pFieldList.Append("Notes2, ") 
    ...
    'Now Load the data 
    Dim pIncomingOrderHeaders As New clsOrderHeaderCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingOrderHeader As New clsOrderHeader(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingOrderHeader.Tag = "Row " & pRow.ToString 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingOrderHeaders.Add(pIncomingOrderHeader) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingOrderHeader.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
 
          Try 
            pFieldNo += 1 
            pFieldName = "OrderNumber" 
            pIncomingOrderHeader.OrderNumber = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingOrderHeader.CustomerID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "OrderDate" 
            pIncomingOrderHeader.OrderDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "PaymentMethod" 
            pIncomingOrderHeader.PaymentMethod = clsEnums.TranslateEnmPaymentMethod(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "PaymentStatus" 
            pIncomingOrderHeader.PaymentStatus = clsEnums.TranslateEnmPaymentStatus(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "PaymentDate" 
            pIncomingOrderHeader.PaymentDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "InvoiceNumber" 
            pIncomingOrderHeader.InvoiceNumber = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingOrderHeader.DeliveryMethod = clsEnums.TranslateEnmDeliveryMethod(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingOrderHeader.DeliveryDate = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "DeliveryDay" 
            pIncomingOrderHeader.DeliveryDay = clsEnums.TranslateEnmDeliveryDay(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "OrderStatus" 
            pIncomingOrderHeader.OrderStatus = clsEnums.TranslateEnmOrderStatus(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Notes" 
            pIncomingOrderHeader.Notes = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Notes2" 
            pIncomingOrderHeader.Notes2 = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderHeader.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingOrderHeader.Tag = "Row " & pRow.ToString Then 
            pIncomingOrderHeader.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingOrderHeaders.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingOrderHeaders.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingOrderHeaders 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _OrderHeaderCol = pIncomingOrderHeaders 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvOrderHeader.Focused = True AndAlso dgvOrderHeader.SelectedRows.Count > 0 Then 
      pCellRow = dgvOrderHeader.CurrentCell.RowIndex 
      pCellCol = dgvOrderHeader.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(0).Cells(0) 
      dgvOrderHeader.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _OrderHeaderCol.Count > 0 AndAlso _OrderHeaderCol(_OrderHeaderCol.Count - 1).ID = 0 Then 
      _OrderHeaderCol.RemoveAt(_OrderHeaderCol.Count - 1) 
      bsCtlOrderHeader.DataSource = Nothing 
      bsCtlOrderHeader.DataSource = _OrderHeaderCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvOrderHeader.Focus() 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(pCellRow).Cells(pCellCol) 
      dgvOrderHeader.CurrentCell.Selected = True 
    ElseIf _OrderHeaderCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlOrderHeader.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As clsOrderHeader 
    pEntity = CType(bsCtlOrderHeader.Current, clsOrderHeader) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvOrderHeader.CurrentCell Is Nothing Then Return pFault 
    
    If dgvOrderHeader.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pOrderHeader As clsOrderHeader 
      pOrderHeader = CType(bsCtlOrderHeader.Current, clsOrderHeader) 
      If pOrderHeader Is Nothing Then 
        pFault.LogFreeTextFault("There is no OrderHeader to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvOrderHeader.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvOrderHeader.CurrentCell.RowIndex 
      'show row as selected  
      dgvOrderHeader.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvOrderHeader.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvOrderHeader.CurrentRow.Selected = True 
      If pOrderHeader.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pOrderHeader.OrderNumber.ToString() & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pOrderHeader, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvOrderHeader.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvOrderHeader.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvOrderHeader.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pOrderHeader.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlOrderHeader.Remove(bsCtlOrderHeader.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pOrderHeader As clsOrderHeader = CType(bsCtlOrderHeader.Current, clsOrderHeader) 
    If pOrderHeader IsNot Nothing Then 
      If pOrderHeader.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _OrderHeaderCol.Count > 0 AndAlso dgvOrderHeader.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvOrderHeader.Columns.Count - 1 
        If dgvOrderHeader.Columns(i).Visible Then 
          dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(dgvOrderHeader.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvOrderHeader.Refresh() 
      dgvOrderHeader.Rows(dgvOrderHeader.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvOrderHeader.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvOrderHeader_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvOrderHeader.RowValidating 
    If _Loading = True OrElse dgvOrderHeader.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvOrderHeader_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvOrderHeader.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvOrderHeader.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvOrderHeader.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvOrderHeader.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pOrderHeader Is Nothing Then pOrderHeader = CType(dgvOrderHeader.Rows(e.RowIndex).DataBoundItem, clsOrderHeader) ' Only assign it if needed 
    '  If pOrderHeader.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pOrderHeader.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvOrderHeader.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pOrderHeader Is Nothing Then pOrderHeader = CType(dgvOrderHeader.Rows(e.RowIndex).DataBoundItem, clsOrderHeader) ' Only assign it if needed
    '  If pOrderHeader.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pOrderHeader.RAV - pOrderHeader.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvOrderHeader.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvOrderHeader.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvOrderHeader.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvOrderHeader.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If e.ColumnIndex = colTotalAmount.Index Then 
      If CType(e.Value, Decimal) = 0D Then e.Value = "" 
    End If 
    
    If e.ColumnIndex = colVATAmount.Index Then 
      If CType(e.Value, Decimal) = 0D Then e.Value = "" 
    End If 
    
    If e.ColumnIndex = colTotalWithVAT.Index Then 
      If CType(e.Value, Decimal) = 0D Then e.Value = "" 
    End If 
    ...
    If dgvOrderHeader.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvOrderHeader.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvOrderHeader.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvOrderHeader_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvOrderHeader.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvOrderHeader_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvOrderHeader.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvOrderHeader.Columns(e.ColumnIndex)
    If bsCtlOrderHeader.Current Is Nothing Then Exit Sub
    ...
    Dim pOrderHeaderCol As clsOrderHeaderCol
    pOrderHeaderCol = CType(bsCtlOrderHeader.DataSource, clsOrderHeaderCol)

    Dim pSummaryRow As clsOrderHeader = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pOrderHeaderCol(pOrderHeaderCol.Count - 1) 
      pOrderHeaderCol.RemoveAt(pOrderHeaderCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pOrderHeaderCol.Count - 1 
          pOrderHeaderCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pOrderHeaderCol.SortByID()
      ElseIf pNewColumn Is colOrderNumber Then
        pOrderHeaderCol.SortByOrderNumber()
      ElseIf pNewColumn Is colCustomer OrElse pNewColumn Is colCustomerText Then
        pOrderHeaderCol.SortByCustomerText()
      ElseIf pNewColumn Is colOrderDate Then
        pOrderHeaderCol.SortByOrderDate()
      ElseIf pNewColumn Is colTotalAmount Then
        pOrderHeaderCol.SortByTotalAmount()
      ElseIf pNewColumn Is colVATAmount Then
        pOrderHeaderCol.SortByVATAmount()
      ElseIf pNewColumn Is colTotalWithVAT Then
        pOrderHeaderCol.SortByTotalWithVAT()
      ElseIf pNewColumn Is colPaymentMethod Then
        pOrderHeaderCol.SortByPaymentMethod()
      ElseIf pNewColumn Is colPaymentStatus Then
        pOrderHeaderCol.SortByPaymentStatus()
      ElseIf pNewColumn Is colPaymentDate Then
        pOrderHeaderCol.SortByPaymentDate()
      ElseIf pNewColumn Is colInvoiceNumber Then
        pOrderHeaderCol.SortByInvoiceNumber()
      ElseIf pNewColumn Is colDeliveryMethod Then
        pOrderHeaderCol.SortByDeliveryMethod()
      ElseIf pNewColumn Is colDeliveryDate Then
        pOrderHeaderCol.SortByDeliveryDate()
      ElseIf pNewColumn Is colDeliveryDay Then
        pOrderHeaderCol.SortByDeliveryDay()
      ElseIf pNewColumn Is colOrderStatus Then
        pOrderHeaderCol.SortByOrderStatus()
      ElseIf pNewColumn Is colNotes Then
        pOrderHeaderCol.SortByNotes()
      ElseIf pNewColumn Is colNotes2 Then
        pOrderHeaderCol.SortByNotes2()
      ElseIf pNewColumn Is colOrderMonth Then
        pOrderHeaderCol.SortByOrderMonth()
      ElseIf pNewColumn Is colQuarter Then
        pOrderHeaderCol.SortByQuarter()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As clsOrderHeader In pOrderHeaderCol 
        ...
        ElseIf pNewColumn Is colOrderNumber Then
          ...
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.OrderNumber <> pTest Then iCntr += 1 : pTest = p.OrderNumber 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colCustomer OrElse pNewColumn Is colCustomerText Then
          Dim pTest As String = "" 
        ...
        ElseIf pNewColumn Is colOrderDate Then
          ...
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.OrderDate <> pTest Then iCntr += 1 : pTest = p.OrderDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colTotalAmount Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.TotalAmount <> pTest Then iCntr += 1 : pTest = p.TotalAmount 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colVATAmount Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.VATAmount <> pTest Then iCntr += 1 : pTest = p.VATAmount 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colTotalWithVAT Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.TotalWithVAT <> pTest Then iCntr += 1 : pTest = p.TotalWithVAT 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colPaymentMethod Then
          Dim pTest As clsEnums.enmPaymentMethod = clsEnums.enmPaymentMethod.UD 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.PaymentMethod <> pTest Then iCntr += 1 : pTest = p.PaymentMethod 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colPaymentStatus Then
          Dim pTest As clsEnums.enmPaymentStatus = clsEnums.enmPaymentStatus.UD 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.PaymentStatus <> pTest Then iCntr += 1 : pTest = p.PaymentStatus 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colPaymentDate Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.PaymentDate <> pTest Then iCntr += 1 : pTest = p.PaymentDate 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colInvoiceNumber Then
          Dim pTest As String = "" 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.InvoiceNumber <> pTest Then iCntr += 1 : pTest = p.InvoiceNumber 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDeliveryMethod Then
        ...
        ElseIf pNewColumn Is colDeliveryDate Then
          ...
          Next 
        ElseIf pNewColumn Is colDeliveryDay Then
          Dim pTest As clsEnums.enmDeliveryDay = clsEnums.enmDeliveryDay.UD 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.DeliveryDay <> pTest Then iCntr += 1 : pTest = p.DeliveryDay 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colOrderStatus Then
          Dim pTest As clsEnums.enmOrderStatus = clsEnums.enmOrderStatus.UD 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.OrderStatus <> pTest Then iCntr += 1 : pTest = p.OrderStatus 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colNotes Then
          Dim pTest As String = "" 
        ...
        ElseIf pNewColumn Is colNotes2 Then
          Dim pTest As String = "" 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.Notes2 <> pTest Then iCntr += 1 : pTest = p.Notes2 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colOrderMonth Then
          Dim pTest As String = "" 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.OrderMonth <> pTest Then iCntr += 1 : pTest = p.OrderMonth 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colQuarter Then
          Dim pTest As String = "" 
          For Each p As clsOrderHeader In pOrderHeaderCol 
            If p.Quarter <> pTest Then iCntr += 1 : pTest = p.Quarter 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pOrderHeaderCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pOrderHeaderCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pOrderHeaderCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlOrderHeader.Position = bsCtlOrderHeader.IndexOf(pOrderHeaderCol.FindByID(pID))
    End If

    'dgvOrderHeader.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvOrderHeader.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pOrderHeaderCol As clsOrderHeaderCol 
      pOrderHeaderCol = CType(bsCtlOrderHeader.DataSource, clsOrderHeaderCol) 
      Dim pOrderHeader As clsOrderHeader = pOrderHeaderCol.FindByID(pID) 
      If Not pOrderHeader.IsEmpty Then 
        bsCtlOrderHeader.Position = bsCtlOrderHeader.IndexOf(pOrderHeaderCol.FindByID(pID)) 
        dgvOrderHeader.Rows(bsCtlOrderHeader.Position).Selected = True 
      Else 
        dgvOrderHeader.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvOrderHeader_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvOrderHeader.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvOrderHeader.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvOrderHeader_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvOrderHeader.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvOrderHeader_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvOrderHeader.Scroll
    dgvOrderHeader.Invalidate() 
  End Sub
 
  Private Sub dgvOrderHeader_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvOrderHeader.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvOrderHeader.Rows.Count - 1 Then Exit Sub
 
    'If dgvOrderHeader.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'OrderHeader', the row with an ID of " & dgvOrderHeader.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'OrderHeader', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvOrderHeader.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvOrderHeader.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-OrderHeader-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvOrderHeader(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvOrderHeader(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvOrderHeader_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvOrderHeader.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvOrderHeader.CurrentCell = dgvOrderHeader.Rows(dgvOrderHeader.CurrentCell.RowIndex).Cells(0) 
      dgvOrderHeader.Refresh() 
    End If 
  End Sub 
  Private Sub dgvOrderHeader_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvOrderHeader.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvOrderHeader_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvOrderHeader.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvOrderHeader_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvOrderHeader.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvOrderHeader.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvOrderHeader(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pOrderHeader As clsOrderHeader = _OrderHeaderCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pOrderHeader, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "OrderHeader Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlccOrderHeader", pOrderHeader, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvOrderHeader_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvOrderHeader.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvOrderHeader.Rows.Count - 1 Then dgvOrderHeader.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pOrderHeader As clsOrderHeader = _OrderHeaderCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pOrderHeader) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvOrderHeader_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvOrderHeader.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvOrderHeader.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvOrderHeader.Columns(pNewCol).Visible = False Then 
      dgvOrderHeader.Columns(pNewCol).Visible = True 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.CurrentRow.Cells(pNewCol) 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.CurrentRow.Cells(pOriginalCol) 
      dgvOrderHeader.Columns(pNewCol).Visible = False 
    Else 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.CurrentRow.Cells(pNewCol) 
      dgvOrderHeader.CurrentCell = dgvOrderHeader.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pOrderHeader, clsOrderHeader), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pOrderHeader.Update(_Requester) 
    ...
    Else 
      If pFault.isOK = False Then 'AndAlso pFault.Severity = clsEnums.enmFaultSeverity.LogOnly  
        ShowFault(pFault, _Requester) 
      End If 
      dgvOrderHeader.EndEdit() 
      _DVGDirty = False 
      'Reset the OrderHeader collection 
      MyCache.ClearComboList(clsEnums.enmComboListType.ccOrderHeaderDefaultByID) 
      RaiseEvent evtUpdated(pOrderHeader) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvOrderHeader.Columns 
  ...
  Private Sub Summarize() 
    If _OrderHeaderCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pOrderNumber As Integer 
    Dim pTotalAmount As Decimal 
    Dim pVATAmount As Decimal 
    Dim pTotalWithVAT As Decimal 
    For Each pExistingRow As clsOrderHeader In _OrderHeaderCol 
      If _SummaryOverFlow.IndexOf("#OrderNumber#") < 0 Then 
        Try 
          pOrderNumber += pExistingRow.OrderNumber 
        Catch ex As System.OverflowException 
          pOrderNumber = -99999999 
          _SummaryOverFlow &= "OrderNumber#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#TotalAmount#") < 0 Then 
        Try 
          pTotalAmount += pExistingRow.TotalAmount 
        Catch ex As System.OverflowException 
          pTotalAmount = -99999999 
          _SummaryOverFlow &= "TotalAmount#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#VATAmount#") < 0 Then 
        Try 
          pVATAmount += pExistingRow.VATAmount 
        Catch ex As System.OverflowException 
          pVATAmount = -99999999 
          _SummaryOverFlow &= "VATAmount#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#TotalWithVAT#") < 0 Then 
        Try 
          pTotalWithVAT += pExistingRow.TotalWithVAT 
        Catch ex As System.OverflowException 
          pTotalWithVAT = -99999999 
          _SummaryOverFlow &= "TotalWithVAT#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderHeader.enmSummarizeableProperty.OrderNumber) = clsOrderHeader.enmSummarizeableProperty.OrderNumber Then pOrderNumber = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderHeader.enmSummarizeableProperty.TotalAmount) = clsOrderHeader.enmSummarizeableProperty.TotalAmount Then pTotalAmount = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderHeader.enmSummarizeableProperty.VATAmount) = clsOrderHeader.enmSummarizeableProperty.VATAmount Then pVATAmount = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderHeader.enmSummarizeableProperty.TotalWithVAT) = clsOrderHeader.enmSummarizeableProperty.TotalWithVAT Then pTotalWithVAT = 0
    Dim pSummaryRow As New clsOrderHeader( _ 
        vID:=0 _ 
      , vOrderNumber:=pOrderNumber _ 
      , vCustomerID:=0 _ 
      , vCustomerText:="" _ 
      , vOrderDate:=Nothing _ 
      , vTotalAmount:=pTotalAmount _ 
      , vVATAmount:=pVATAmount _ 
      , vTotalWithVAT:=pTotalWithVAT _ 
      , vPaymentMethod:=clsEnums.enmPaymentMethod.UD _ 
      , vPaymentMethodText:="" _ 
      , vPaymentStatus:=clsEnums.enmPaymentStatus.UD _ 
      , vPaymentStatusText:="" _ 
      , vPaymentDate:=Nothing _ 
      , vInvoiceNumber:="" _ 
      , vDeliveryMethod:=clsEnums.enmDeliveryMethod.UD _ 
      , vDeliveryMethodText:="" _ 
      , vDeliveryDate:=Nothing _ 
      , vDeliveryDay:=clsEnums.enmDeliveryDay.UD _ 
      , vDeliveryDayText:="" _ 
      , vOrderStatus:=clsEnums.enmOrderStatus.UD _ 
      , vOrderStatusText:="" _ 
      , vNotes:="" _ 
      , vNotes2:="" _ 
      , vOrderMonth:="" _ 
    ...
    _OrderHeaderCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\OrderHeaderCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\OrderHeaderCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\OrderHeaderCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\OrderHeaderCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\OrderHeaderCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvOrderHeader.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvOrderHeader.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _OrderHeaderColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _OrderHeaderColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _OrderHeaderColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _OrderHeaderColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _OrderHeaderColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-OrderHeader-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "OrderHeaders" 
        .SubTitleRight = "Rows: " & _OrderHeaderCol.Count.ToString 
        ...
        End If 
        .DataSource = _OrderHeaderCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-OrderHeader-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-OrderHeader-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvOrderHeader.Rows 
    ...
    For Each row As DataGridViewRow In dgvOrderHeader.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlOrderHeader), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvOrderHeader.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-OrderHeader-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvOrderHeader.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvOrderHeader.Width - 30) / dgvOrderHeader.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'colNotes2.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvOrderHeader.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-OrderHeader-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colOrderNumber.HeaderText = pStrg : mnuColVisibleOrderNumber.Text = pStrg
    ...
    If pStrg <> "" Then colOrderDate.HeaderText = pStrg : mnuColVisibleOrderDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "TotalAmount", _Requester) 
    If pStrg <> "" Then colTotalAmount.HeaderText = pStrg : mnuColVisibleTotalAmount.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "VATAmount", _Requester) 
    If pStrg <> "" Then colVATAmount.HeaderText = pStrg : mnuColVisibleVATAmount.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "TotalWithVAT", _Requester) 
    If pStrg <> "" Then colTotalWithVAT.HeaderText = pStrg : mnuColVisibleTotalWithVAT.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "PaymentMethod", _Requester) 
    If pStrg <> "" Then colPaymentMethod.HeaderText = pStrg : mnuColVisiblePaymentMethod.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "PaymentStatus", _Requester) 
    If pStrg <> "" Then colPaymentStatus.HeaderText = pStrg : mnuColVisiblePaymentStatus.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "PaymentDate", _Requester) 
    If pStrg <> "" Then colPaymentDate.HeaderText = pStrg : mnuColVisiblePaymentDate.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "InvoiceNumber", _Requester) 
    If pStrg <> "" Then colInvoiceNumber.HeaderText = pStrg : mnuColVisibleInvoiceNumber.Text = pStrg
    ...
    If pStrg <> "" Then colDeliveryDay.HeaderText = pStrg : mnuColVisibleDeliveryDay.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "OrderStatus", _Requester) 
    If pStrg <> "" Then colOrderStatus.HeaderText = pStrg : mnuColVisibleOrderStatus.Text = pStrg
    ...
    If pStrg <> "" Then colNotes2.HeaderText = pStrg : mnuColVisibleNotes2.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "OrderMonth", _Requester) 
    If pStrg <> "" Then colOrderMonth.HeaderText = pStrg : mnuColVisibleOrderMonth.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderHeader", "Quarter", _Requester) 
    If pStrg <> "" Then colQuarter.HeaderText = pStrg : mnuColVisibleQuarter.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleOrderNumber.Click, mnuColVisibleCustomer.Click, mnuColVisibleOrderDate.Click, mnuColVisibleTotalAmount.Click, mnuColVisibleVATAmount.Click, mnuColVisibleTotalWithVAT.Click, mnuColVisiblePaymentMethod.Click, mnuColVisiblePaymentStatus.Click, mnuColVisiblePaymentDate.Click, mnuColVisibleInvoiceNumber.Click, mnuColVisibleDeliveryMethod.Click, mnuColVisibleDeliveryDate.Click, mnuColVisibleDeliveryDay.Click, mnuColVisibleOrderStatus.Click, mnuColVisibleNotes.Click, mnuColVisibleNotes2.Click, mnuColVisibleOrderMonth.Click, mnuColVisibleQuarter.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvOrderHeader.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvOrderHeader.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleOrderNumber.Checked = True Then mnuColVisibleOrderNumber.PerformClick() 
    If mnuColVisibleCustomer.Checked = True Then mnuColVisibleCustomer.PerformClick() 
    If mnuColVisibleOrderDate.Checked = True Then mnuColVisibleOrderDate.PerformClick() 
    If mnuColVisibleTotalAmount.Checked = True Then mnuColVisibleTotalAmount.PerformClick() 
    If mnuColVisibleVATAmount.Checked = True Then mnuColVisibleVATAmount.PerformClick() 
    If mnuColVisibleTotalWithVAT.Checked = True Then mnuColVisibleTotalWithVAT.PerformClick() 
    If mnuColVisiblePaymentMethod.Checked = True Then mnuColVisiblePaymentMethod.PerformClick() 
    If mnuColVisiblePaymentStatus.Checked = True Then mnuColVisiblePaymentStatus.PerformClick() 
    If mnuColVisiblePaymentDate.Checked = True Then mnuColVisiblePaymentDate.PerformClick() 
    If mnuColVisibleInvoiceNumber.Checked = True Then mnuColVisibleInvoiceNumber.PerformClick() 
    ...
    If mnuColVisibleDeliveryDay.Checked = True Then mnuColVisibleDeliveryDay.PerformClick() 
    If mnuColVisibleOrderStatus.Checked = True Then mnuColVisibleOrderStatus.PerformClick() 
    If mnuColVisibleNotes.Checked = True Then mnuColVisibleNotes.PerformClick() 
    If mnuColVisibleNotes2.Checked = True Then mnuColVisibleNotes2.PerformClick() 
    If mnuColVisibleOrderMonth.Checked = True Then mnuColVisibleOrderMonth.PerformClick() 
    If mnuColVisibleQuarter.Checked = True Then mnuColVisibleQuarter.PerformClick() 
    'Show Defaults 
    If mnuColVisibleOrderNumber.Checked = False Then mnuColVisibleOrderNumber.PerformClick() 
    
    _Loading = False 
    'dgvOrderHeader.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvOrderHeader_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvOrderHeader.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the OrderHeader to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvOrderHeader_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvOrderHeader.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvOrderHeader.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvOrderHeader.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvOrderHeader.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvOrderHeader.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvOrderHeader(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvOrderHeader.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvOrderHeader.BeginEdit(True) 
        DirectCast(dgvOrderHeader.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvOrderHeader_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvOrderHeader.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvOrderHeader.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvOrderHeader.MultiSelect = True 
        dgvOrderHeader.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvOrderHeader.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvOrderHeader.MultiSelect = True 
    Else 
      dgvOrderHeader.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlccOrderHeaderCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvOrderHeader.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlccOrderHeaderCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvOrderHeader.Width 
  ...
  Private Sub ctlccOrderHeaderCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlccOrderHeaderCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvOrderHeader_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvOrderHeader.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvOrderHeader.Rows(e.RowIndex).Selected Then 
        dgvOrderHeader.ClearSelection() 
        dgvOrderHeader.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvOrderHeader.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvOrderHeader.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvOrderHeader.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _OrderHeaderCol.Count Then Exit Sub 
    Dim pOrderHeader As clsOrderHeader = _OrderHeaderCol(pRowIndex) 
    Dim pTitle As String = "OrderHeader #" & pOrderHeader.ID.ToString() 
    Dim pKey As String = "OrderHeader_" & pOrderHeader.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlccOrderHeader" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pOrderHeader, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pOrderHeader 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "OrderHeader_" & pOrderHeader.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "OrderHeader #" & pOrderHeader.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlccOrderHeader", pOrderHeader, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvOrderHeader.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvOrderHeader.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _OrderHeaderCol.Count Then 
        Dim pOrderHeader As clsOrderHeader = _OrderHeaderCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pOrderHeader.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvOrderHeader.SelectedRows(0) 
