Public Class ctlc_JobCol
  ...
  Public Event evtUpdated(ByVal vJob As csJob) 
  Private Event evtBeforeDelete(ByVal vJob As csJob, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vJob As csJob) 
  Public Event evtRowDoubleClicked(ByVal vJob As csJob, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As csJob.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of csJob.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property CbosDoNotLoad As List(Of csJob.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of csJob.enmProperty) 
    Public Property ColumnsHide As List(Of csJob.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csJob.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csJob.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csJob.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csJob.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of csJob.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of csJob.enmParentProperty) 
      _ColumnsReadOnly = New List(Of csJob.enmProperty) 
      _ColumnsHide = New List(Of csJob.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csJob.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csJob.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csJob.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csJob.enmProperty, String) 
  ...
  Private WithEvents _JobCol As csJobCol
  Private WithEvents _JobColFullLength As csJobCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedJob]() As csJob 
    Get 
      If dgvJob.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvJob.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvJob.Rows.Count - 1 Then dgvJob.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _JobCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [JobCol]() As csJobCol 
    Get 
      Return _JobCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = pJobCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pJobCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vJobCol As csJobCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vJobCol) 
  End Function
  
  Private Function LoadControl(vJobCol As csJobCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csJob.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csJob.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csJob.enmProperty.ID Then colID.ReadOnly = True 
      If l = csJob.enmProperty.Job Then colJob.ReadOnly = True 
      If l = csJob.enmProperty.JobRunner Then colJobRunner.ReadOnly = True 
      If l = csJob.enmProperty.Description Then colDescription.ReadOnly = True 
      If l = csJob.enmProperty.Instructions Then colInstructions.ReadOnly = True 
      If l = csJob.enmProperty.JobType Then colJobType.ReadOnly = True 
      If l = csJob.enmProperty.WhenToRun Then colWhenToRun.ReadOnly = True 
      If l = csJob.enmProperty.CyclicCount Then colCyclicCount.ReadOnly = True 
      If l = csJob.enmProperty.SendNotificationOnSuccess Then colSendNotificationOnSuccess.ReadOnly = True 
      If l = csJob.enmProperty.SendAlarmOnMissed Then colSendAlarmOnMissed.ReadOnly = True 
      If l = csJob.enmProperty.TimeOutSec Then colTimeOutSec.ReadOnly = True 
      If l = csJob.enmProperty.Active Then colActive.ReadOnly = True 
      If l = csJob.enmProperty.ActivatingUser Then colActivatingUser.ReadOnly = True 
      If l = csJob.enmProperty.NextRunTime Then colNextRunTime.ReadOnly = True 
      If l = csJob.enmProperty.LastRunTime Then colLastRunTime.ReadOnly = True 
      If l = csJob.enmProperty.JobStatus Then colJobStatus.ReadOnly = True 
      If l = csJob.enmProperty.WarningMailSent Then colWarningMailSent.ReadOnly = True 
      If l = csJob.enmProperty.IsManaged Then colIsManaged.ReadOnly = True 
      If l = csJob.enmProperty.LastRunBy Then colLastRunBy.ReadOnly = True 
    Next 
    ...
    dgvJob.DoubleBuffered(True) 
 
    pFault = vJobCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _JobColFullLength = vJobCol.Clone() 
 
    'Truncate the strings 
    _JobCol = vJobCol 
    If _LoadParameters.TruncateStrings Then 
      _JobCol.TruncateStrings() 
    Else 
      dgvJob.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvJob.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvJob.Columns 
    ...
    'Hide columns  
    For Each p As csJob.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvJob.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvJob.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvJob.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvJob.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvJob.ClearSelection()
    bsCtlJob.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Job-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvJob.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvJob_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvJob.SelectedRows.Count > 0 Then 
      pRowIndex = dgvJob.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvJob.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvJob.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvJob.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _JobCol.Count - 2 Then 
          dgvJob.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _JobCol.Count - 1 Then 
          dgvJob.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    pPrompt = pChoose 
    RaiseEvent evtOverrideLoadCbo(csJob.enmParentProperty.Job, Nothing, Nothing, pTestLookupCol, pPrompt) 
    If pTestLookupCol Is Nothing Then 
      pComboList = New clsComboList() 
      pFault = pComboList.FillLookup(clsEnums.enmLookup.Job, _Requester) : If pFault.isOK = False Then Return pFault 
      pPrompt = pChoose 
    ...
    pTestLookupCol = Nothing 
    pPrompt = pChoose 
    RaiseEvent evtOverrideLoadCbo(csJob.enmParentProperty.JobRunner, Nothing, Nothing, pTestLookupCol, pPrompt) 
    If pTestLookupCol Is Nothing Then 
      pComboList = New clsComboList() 
      pFault = pComboList.FillLookup(clsEnums.enmLookup.JobRunner, _Requester) : If pFault.isOK = False Then Return pFault 
      pPrompt = pChoose 
      pComboList.AddToTop("", pPrompt) 
    Else 
      pComboList = pTestLookupCol 
    End If 
    bsJobRunner.DataSource = pComboList 
    colJobRunner.Tag = pPrompt 

    'EnumJobType
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(csJob.enmParentProperty.JobType, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
      pFault = pEnumCol.FillEnums(clsEnums.enmEnum.JobType, _Requester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      pFault.SetOK() 
    End If 
    pEnumCol.Remove(pEnumCol.FindByKey(clsEnums.enmJobType.UD)) 
    pEnumCol.SortByText() 
    If pPrompt = "" Then 
      pPrompt = pChoose 
    End If 
    pEnumCol.AddToTop(clsEnums.enmJobType.UD, pPrompt) 
    bsJobType.DataSource = pEnumCol 
    colJobType.Tag = pPrompt 

    'EnumJobStatus
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(csJob.enmParentProperty.JobStatus, Nothing, Nothing, pEnumCol, pPrompt) 
    If pEnumCol Is Nothing Then 
      pEnumCol = New clsComboList 
    ...
    bsJobStatus.DataSource = pEnumCol 
    colJobStatus.Tag = pPrompt 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_JobUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_JobUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_c_JobDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      colActivatingUser.ReadOnly = True 
      colNextRunTime.ReadOnly = True 
      colLastRunTime.ReadOnly = True 
      colJobStatus.ReadOnly = True 
      colWarningMailSent.ReadOnly = True 
      dgvJob.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
    ...
    Else 
      ...
      'Don't automatically set the 1st one If dgvJob.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvJob.CurrentRow.Index 
      '  dgvJob.CurrentCell = dgvJob.Rows(pCurrentRow).Cells(0) 
      '  dgvJob.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("JobCode, ") 
    pFieldList.Append("JobRunnerCode, ") 
    pFieldList.Append("Description, ") 
    pFieldList.Append("Instructions, ") 
    pFieldList.Append("JobType (DB Code), ") 
    pFieldList.Append("WhenToRun, ") 
    pFieldList.Append("CyclicCount, ") 
    pFieldList.Append("SendNotificationOnSuccess, ") 
    pFieldList.Append("SendAlarmOnMissed, ") 
    pFieldList.Append("TimeOutSec, ") 
    pFieldList.Append("Active, ") 
    pFieldList.Append("IsManaged, ") 
    pFieldList.Append("LastRunBy, ") 
    ...
    'Now Load the data 
    Dim pIncomingJobs As New csJobCol() 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingJob As New csJob() 
          pIncomingJob.Tag = "Row " & pRow.ToString 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingJob.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingJobs.Add(pIncomingJob) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingJob.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "JobCode" 
            pIncomingJob.JobCode = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "JobRunnerCode" 
            pIncomingJob.JobRunnerCode = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Description" 
            pIncomingJob.Description = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Instructions" 
            pIncomingJob.Instructions = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "JobType" 
            pIncomingJob.JobType = clsEnums.TranslateEnmJobType(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "WhenToRun" 
            pIncomingJob.WhenToRun = CType(pCurrentRow(pFieldNo), Date) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "CyclicCount" 
            pIncomingJob.CyclicCount = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "SendNotificationOnSuccess" 
            pIncomingJob.SendNotificationOnSuccess = CType(pCurrentRow(pFieldNo), Boolean) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "SendAlarmOnMissed" 
            pIncomingJob.SendAlarmOnMissed = CType(pCurrentRow(pFieldNo), Boolean) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "TimeOutSec" 
            pIncomingJob.TimeOutSec = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Active" 
            pIncomingJob.Active = CType(pCurrentRow(pFieldNo), Boolean) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "IsManaged" 
            pIncomingJob.IsManaged = CType(pCurrentRow(pFieldNo), Boolean) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LastRunBy" 
            pIncomingJob.LastRunBy = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingJob.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingJob.Tag = "Row " & pRow.ToString Then 
            pIncomingJob.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingJobs.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingJobs.UpdateFromCollection(_Requester) 
  ...
  Private Sub DoEdit() 
    ...
    If dgvJob.Focused = True AndAlso dgvJob.SelectedRows.Count > 0 Then 
      pCellRow = dgvJob.CurrentCell.RowIndex 
      pCellCol = dgvJob.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvJob.CurrentCell = dgvJob.Rows(0).Cells(0) 
      dgvJob.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _JobCol.Count > 0 AndAlso _JobCol(_JobCol.Count - 1).ID = 0 Then 
      _JobCol.RemoveAt(_JobCol.Count - 1) 
      bsCtlJob.DataSource = Nothing 
      bsCtlJob.DataSource = _JobCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvJob.Focus() 
      dgvJob.CurrentCell = dgvJob.Rows(pCellRow).Cells(pCellCol) 
      dgvJob.CurrentCell.Selected = True 
    ElseIf _JobCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    Dim pEntity As csJob 
    pEntity = CType(bsCtlJob.Current, csJob) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvJob.CurrentCell Is Nothing Then Return pFault 
    
    If dgvJob.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pJob As csJob 
      pJob = CType(bsCtlJob.Current, csJob) 
      If pJob Is Nothing Then 
        pFault.LogFreeTextFault("There is no Job to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvJob.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvJob.CurrentCell.RowIndex 
      'show row as selected  
      dgvJob.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvJob.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvJob.CurrentRow.Selected = True 
      If pJob.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pJob.JobCode & " on " & pJob.JobRunnerCode & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pJob, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvJob.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvJob.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvJob.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pJob.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlJob.Remove(bsCtlJob.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pJob As csJob = CType(bsCtlJob.Current, csJob) 
    If pJob IsNot Nothing Then 
      If pJob.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _JobCol.Count > 0 AndAlso dgvJob.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvJob.Columns.Count - 1 
        If dgvJob.Columns(i).Visible Then 
          dgvJob.CurrentCell = dgvJob.Rows(dgvJob.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvJob.Refresh() 
      dgvJob.Rows(dgvJob.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvJob.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvJob_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvJob.RowValidating 
    If _Loading = True OrElse dgvJob.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvJob.CurrentCell = dgvJob.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvJob_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvJob.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvJob.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvJob.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvJob.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pJob Is Nothing Then pJob = CType(dgvJob.Rows(e.RowIndex).DataBoundItem, csJob) ' Only assign it if needed 
    '  If pJob.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pJob.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvJob.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pJob Is Nothing Then pJob = CType(dgvJob.Rows(e.RowIndex).DataBoundItem, csJob) ' Only assign it if needed
    '  If pJob.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pJob.RAV - pJob.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvJob.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvJob.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvJob.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvJob.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvJob.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvJob.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvJob.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvJob_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvJob.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvJob_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvJob.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvJob.Columns(e.ColumnIndex)
    If bsCtlJob.Current Is Nothing Then Exit Sub
    ...
    Dim pJobCol As csJobCol
    pJobCol = CType(bsCtlJob.DataSource, csJobCol)

    Dim pSummaryRow As csJob = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pJobCol(pJobCol.Count - 1) 
      pJobCol.RemoveAt(pJobCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pJobCol.Count - 1 
          pJobCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pJobCol.SortByID()
      ElseIf pNewColumn Is colJob Then
        pJobCol.SortByJobText()
      ElseIf pNewColumn Is colJobRunner Then
        pJobCol.SortByJobRunnerText()
      ElseIf pNewColumn Is colDescription Then
        pJobCol.SortByDescription()
      ElseIf pNewColumn Is colInstructions Then
        pJobCol.SortByInstructions()
      ElseIf pNewColumn Is colJobType Then
        pJobCol.SortByJobType()
      ElseIf pNewColumn Is colWhenToRun Then
        pJobCol.SortByWhenToRun()
      ElseIf pNewColumn Is colCyclicCount Then
        pJobCol.SortByCyclicCount()
      ElseIf pNewColumn Is colSendNotificationOnSuccess Then
        pJobCol.SortBySendNotificationOnSuccess()
      ElseIf pNewColumn Is colSendAlarmOnMissed Then
        pJobCol.SortBySendAlarmOnMissed()
      ElseIf pNewColumn Is colTimeOutSec Then
        pJobCol.SortByTimeOutSec()
      ElseIf pNewColumn Is colActive Then
        pJobCol.SortByActive()
      ElseIf pNewColumn Is colActivatingUser Then
        pJobCol.SortByActivatingUser()
      ElseIf pNewColumn Is colNextRunTime Then
        pJobCol.SortByNextRunTime()
      ElseIf pNewColumn Is colLastRunTime Then
        pJobCol.SortByLastRunTime()
      ElseIf pNewColumn Is colJobStatus Then
        pJobCol.SortByJobStatus()
      ElseIf pNewColumn Is colWarningMailSent Then
        pJobCol.SortByWarningMailSent()
      ElseIf pNewColumn Is colIsManaged Then
        pJobCol.SortByIsManaged()
      ElseIf pNewColumn Is colLastRunBy Then
        pJobCol.SortByLastRunBy()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csJob In pJobCol 
        ...
        ElseIf pNewColumn Is colJobRunner Then
          Dim pTest As String = "" 
          For Each p As csJob In pJobCol 
            If p.JobRunnerText <> pTest Then iCntr += 1 : pTest = p.JobRunnerText 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDescription Then
          Dim pTest As String = "" 
          For Each p As csJob In pJobCol 
            If p.Description <> pTest Then iCntr += 1 : pTest = p.Description 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colInstructions Then
          Dim pTest As String = "" 
          For Each p As csJob In pJobCol 
            If p.Instructions <> pTest Then iCntr += 1 : pTest = p.Instructions 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colJobType Then
          Dim pTest As clsEnums.enmJobType = clsEnums.enmJobType.UD 
          For Each p As csJob In pJobCol 
            If p.JobType <> pTest Then iCntr += 1 : pTest = p.JobType 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colWhenToRun Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As csJob In pJobCol 
            If p.WhenToRun <> pTest Then iCntr += 1 : pTest = p.WhenToRun 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colCyclicCount Then
          Dim pTest As Integer = 0 
          For Each p As csJob In pJobCol 
            If p.CyclicCount <> pTest Then iCntr += 1 : pTest = p.CyclicCount 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colSendNotificationOnSuccess Then
          Dim pTest As Boolean = False 
          For Each p As csJob In pJobCol 
            If p.SendNotificationOnSuccess <> pTest Then iCntr += 1 : pTest = p.SendNotificationOnSuccess 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colSendAlarmOnMissed Then
          Dim pTest As Boolean = False 
          For Each p As csJob In pJobCol 
            If p.SendAlarmOnMissed <> pTest Then iCntr += 1 : pTest = p.SendAlarmOnMissed 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colTimeOutSec Then
          Dim pTest As Integer = 0 
          For Each p As csJob In pJobCol 
            If p.TimeOutSec <> pTest Then iCntr += 1 : pTest = p.TimeOutSec 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colActive Then
          Dim pTest As Boolean = False 
          For Each p As csJob In pJobCol 
            If p.Active <> pTest Then iCntr += 1 : pTest = p.Active 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colActivatingUser Then
          Dim pTest As String = "" 
        ...
        ElseIf pNewColumn Is colNextRunTime Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As csJob In pJobCol 
            If p.NextRunTime <> pTest Then iCntr += 1 : pTest = p.NextRunTime 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLastRunTime Then
          Dim pTest As Date = #12:00:00 AM# 
          For Each p As csJob In pJobCol 
            If p.LastRunTime <> pTest Then iCntr += 1 : pTest = p.LastRunTime 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colJobStatus Then
          Dim pTest As clsEnums.enmJobStatus = clsEnums.enmJobStatus.UD 
          For Each p As csJob In pJobCol 
            If p.JobStatus <> pTest Then iCntr += 1 : pTest = p.JobStatus 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colWarningMailSent Then
          Dim pTest As Boolean = False 
          For Each p As csJob In pJobCol 
            If p.WarningMailSent <> pTest Then iCntr += 1 : pTest = p.WarningMailSent 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colIsManaged Then
          Dim pTest As Boolean = False 
          For Each p As csJob In pJobCol 
            If p.IsManaged <> pTest Then iCntr += 1 : pTest = p.IsManaged 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLastRunBy Then
          Dim pTest As String = "" 
    ...
    If _Summarized = True Then 
      pJobCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlJob.Position = bsCtlJob.IndexOf(pJobCol.FindByID(pID))
    End If

    'dgvJob.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvJob.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pJobCol As csJobCol 
      pJobCol = CType(bsCtlJob.DataSource, csJobCol) 
      Dim pJob As csJob = pJobCol.FindByID(pID) 
      If Not pJob.IsEmpty Then 
        bsCtlJob.Position = bsCtlJob.IndexOf(pJobCol.FindByID(pID)) 
        dgvJob.Rows(bsCtlJob.Position).Selected = True 
      Else 
        dgvJob.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvJob_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvJob.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvJob.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvJob_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvJob.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvJob_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvJob.Scroll
    dgvJob.Invalidate() 
  End Sub
 
  Private Sub dgvJob_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvJob.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvJob.Rows.Count - 1 Then Exit Sub
 
    'If dgvJob.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Job', the row with an ID of " & dgvJob.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Job', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvJob.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvJob.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Job-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvJob(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvJob(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    ...
    'End If 
  End Sub
  Private Sub dgvJob_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvJob.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvJob.CurrentCell = dgvJob.Rows(dgvJob.CurrentCell.RowIndex).Cells(0) 
      dgvJob.Refresh() 
    End If 
  End Sub 
  Private Sub dgvJob_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvJob.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvJob_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvJob.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvJob_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvJob.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvJob.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvJob(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pJob As csJob = _JobCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pJob, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Job Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_Job", pJob, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvJob_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvJob.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvJob.Rows.Count - 1 Then dgvJob.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pJob As csJob = _JobCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pJob) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvJob_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvJob.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvJob.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvJob.Columns(pNewCol).Visible = False Then 
      dgvJob.Columns(pNewCol).Visible = True 
      dgvJob.CurrentCell = dgvJob.CurrentRow.Cells(pNewCol) 
      dgvJob.CurrentCell = dgvJob.CurrentRow.Cells(pOriginalCol) 
      dgvJob.Columns(pNewCol).Visible = False 
    Else 
      dgvJob.CurrentCell = dgvJob.CurrentRow.Cells(pNewCol) 
      dgvJob.CurrentCell = dgvJob.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pJob, csJob), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pJob.Update(_Requester) 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvJob.Columns 
  ...
  Private Sub Summarize() 
    If _JobCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pCyclicCount As Integer 
    Dim pTimeOutSec As Integer 
    For Each pExistingRow As csJob In _JobCol 
      If _SummaryOverFlow.IndexOf("#CyclicCount#") < 0 Then 
        Try 
          pCyclicCount += pExistingRow.CyclicCount 
        Catch ex As System.OverflowException 
          pCyclicCount = -99999999 
          _SummaryOverFlow &= "CyclicCount#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#TimeOutSec#") < 0 Then 
        Try 
          pTimeOutSec += pExistingRow.TimeOutSec 
        Catch ex As System.OverflowException 
          pTimeOutSec = -99999999 
          _SummaryOverFlow &= "TimeOutSec#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csJob.enmSummarizeableProperty.CyclicCount) = csJob.enmSummarizeableProperty.CyclicCount Then pCyclicCount = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csJob.enmSummarizeableProperty.TimeOutSec) = csJob.enmSummarizeableProperty.TimeOutSec Then pTimeOutSec = 0
    Dim pSummaryRow As New csJob( _ 
        vID:=0 _ 
      , vJobCode:="" _ 
      , vJobText:="" _ 
      , vJobRunnerCode:="" _ 
      ...
      , vDescription:="" _ 
      , vInstructions:="" _ 
      , vJobType:=clsEnums.enmJobType.UD _ 
      , vJobTypeText:="" _ 
      , vWhenToRun:=Nothing _ 
      , vCyclicCount:=pCyclicCount _ 
      , vSendNotificationOnSuccess:=False _ 
      , vSendAlarmOnMissed:=False _ 
      , vTimeOutSec:=pTimeOutSec _ 
      , vActive:=False _ 
      , vActivatingUser:="" _ 
      , vNextRunTime:=Nothing _ 
      , vLastRunTime:=Nothing _ 
      , vJobStatus:=clsEnums.enmJobStatus.UD _ 
      , vJobStatusText:="" _ 
      , vWarningMailSent:=False _ 
      , vIsManaged:=False _ 
      , vLastRunBy:="" _ 
      , vTag:="" _ 
      , vDateAdded:=Nothing _ 
      )
    _JobCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\JobCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\JobCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\JobCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\JobCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\JobCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvJob.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvJob.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _JobColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _JobColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _JobColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _JobColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _JobColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Job-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Jobs" 
        .SubTitleRight = "Rows: " & _JobCol.Count.ToString 
    ...
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Job-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Job-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvJob.Rows 
    ...
    For Each row As DataGridViewRow In dgvJob.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlJob), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvJob.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Job-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvJob.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvJob.Width - 30) / dgvJob.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'Set Wrap 
    'colDescription.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    'colInstructions.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvJob.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Job-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colJobRunner.HeaderText = pStrg : mnuColVisibleJobRunner.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "Description", _Requester) 
    If pStrg <> "" Then colDescription.HeaderText = pStrg : mnuColVisibleDescription.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "Instructions", _Requester) 
    If pStrg <> "" Then colInstructions.HeaderText = pStrg : mnuColVisibleInstructions.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "JobType", _Requester) 
    If pStrg <> "" Then colJobType.HeaderText = pStrg : mnuColVisibleJobType.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "WhenToRun", _Requester) 
    If pStrg <> "" Then colWhenToRun.HeaderText = pStrg : mnuColVisibleWhenToRun.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "CyclicCount", _Requester) 
    If pStrg <> "" Then colCyclicCount.HeaderText = pStrg : mnuColVisibleCyclicCount.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "SendNotificationOnSuccess", _Requester) 
    If pStrg <> "" Then colSendNotificationOnSuccess.HeaderText = pStrg : mnuColVisibleSendNotificationOnSuccess.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "SendAlarmOnMissed", _Requester) 
    If pStrg <> "" Then colSendAlarmOnMissed.HeaderText = pStrg : mnuColVisibleSendAlarmOnMissed.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "TimeOutSec", _Requester) 
    If pStrg <> "" Then colTimeOutSec.HeaderText = pStrg : mnuColVisibleTimeOutSec.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "Active", _Requester) 
    If pStrg <> "" Then colActive.HeaderText = pStrg : mnuColVisibleActive.Text = pStrg
    ...
    If pStrg <> "" Then colNextRunTime.HeaderText = pStrg : mnuColVisibleNextRunTime.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "LastRunTime", _Requester) 
    If pStrg <> "" Then colLastRunTime.HeaderText = pStrg : mnuColVisibleLastRunTime.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "JobStatus", _Requester) 
    If pStrg <> "" Then colJobStatus.HeaderText = pStrg : mnuColVisibleJobStatus.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "WarningMailSent", _Requester) 
    If pStrg <> "" Then colWarningMailSent.HeaderText = pStrg : mnuColVisibleWarningMailSent.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Job", "IsManaged", _Requester) 
    If pStrg <> "" Then colIsManaged.HeaderText = pStrg : mnuColVisibleIsManaged.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleJob.Click, mnuColVisibleJobRunner.Click, mnuColVisibleDescription.Click, mnuColVisibleInstructions.Click, mnuColVisibleJobType.Click, mnuColVisibleWhenToRun.Click, mnuColVisibleCyclicCount.Click, mnuColVisibleSendNotificationOnSuccess.Click, mnuColVisibleSendAlarmOnMissed.Click, mnuColVisibleTimeOutSec.Click, mnuColVisibleActive.Click, mnuColVisibleActivatingUser.Click, mnuColVisibleNextRunTime.Click, mnuColVisibleLastRunTime.Click, mnuColVisibleJobStatus.Click, mnuColVisibleWarningMailSent.Click, mnuColVisibleIsManaged.Click, mnuColVisibleLastRunBy.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvJob.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvJob.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleJobRunner.Checked = True Then mnuColVisibleJobRunner.PerformClick() 
    If mnuColVisibleDescription.Checked = True Then mnuColVisibleDescription.PerformClick() 
    If mnuColVisibleInstructions.Checked = True Then mnuColVisibleInstructions.PerformClick() 
    If mnuColVisibleJobType.Checked = True Then mnuColVisibleJobType.PerformClick() 
    If mnuColVisibleWhenToRun.Checked = True Then mnuColVisibleWhenToRun.PerformClick() 
    If mnuColVisibleCyclicCount.Checked = True Then mnuColVisibleCyclicCount.PerformClick() 
    If mnuColVisibleSendNotificationOnSuccess.Checked = True Then mnuColVisibleSendNotificationOnSuccess.PerformClick() 
    If mnuColVisibleSendAlarmOnMissed.Checked = True Then mnuColVisibleSendAlarmOnMissed.PerformClick() 
    If mnuColVisibleTimeOutSec.Checked = True Then mnuColVisibleTimeOutSec.PerformClick() 
    If mnuColVisibleActive.Checked = True Then mnuColVisibleActive.PerformClick() 
    If mnuColVisibleActivatingUser.Checked = True Then mnuColVisibleActivatingUser.PerformClick() 
    If mnuColVisibleNextRunTime.Checked = True Then mnuColVisibleNextRunTime.PerformClick() 
    If mnuColVisibleLastRunTime.Checked = True Then mnuColVisibleLastRunTime.PerformClick() 
    If mnuColVisibleJobStatus.Checked = True Then mnuColVisibleJobStatus.PerformClick() 
    If mnuColVisibleWarningMailSent.Checked = True Then mnuColVisibleWarningMailSent.PerformClick() 
    If mnuColVisibleIsManaged.Checked = True Then mnuColVisibleIsManaged.PerformClick() 
    If mnuColVisibleLastRunBy.Checked = True Then mnuColVisibleLastRunBy.PerformClick() 
    'Show Defaults 
    If mnuColVisibleJob.Checked = False Then mnuColVisibleJob.PerformClick() 
    If mnuColVisibleJobRunner.Checked = False Then mnuColVisibleJobRunner.PerformClick() 
    
    _Loading = False 
    'dgvJob.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvJob_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvJob.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Job to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvJob_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvJob.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvJob.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvJob.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvJob.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvJob.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvJob(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvJob.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvJob.BeginEdit(True) 
        DirectCast(dgvJob.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvJob_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvJob.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvJob.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvJob.MultiSelect = True 
        dgvJob.ClearSelection() 
  ...
  Private Sub ctlc_JobCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvJob.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_JobCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvJob.Width 
  ...
  Private Sub ctlc_JobCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlc_JobCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub cc_evtBeforeLoad() Handles Me.evtBeforeLoad 
    ...
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.ID) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.Description) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.Instructions) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.WhenToRun) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.CyclicCount) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.WarningMailSent) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.TimeOutSec) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.ActivatingUser) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.LastRunBy) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.SendNotificationOnSuccess) 
    _LoadParameters.ColumnsHide.Add(csJob.enmProperty.SendAlarmOnMissed) 
  End Sub 
 
  Private _isFirstLoad As Boolean = True 
  Private Sub cc_ctlc_JobCol_evtLoaded() Handles Me.evtLoaded 
    If _isFirstLoad Then 
      Dim pE As New System.Windows.Forms.DataGridViewCellMouseEventArgs(colJob.Index, 0, 0, 0, New System.Windows.Forms.MouseEventArgs(System.Windows.Forms.MouseButtons.Left, 0, 0, 0, 0)) 
      dgvJob_ColumnHeaderMouseClick(Me, pE) 
      _isFirstLoad = False 
    End If 
  End Sub 
  ...
  Private Sub dgvJob_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvJob.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvJob.Rows(e.RowIndex).Selected Then 
        dgvJob.ClearSelection() 
        dgvJob.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvJob.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvJob.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvJob.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _JobCol.Count Then Exit Sub 
    Dim pJob As csJob = _JobCol(pRowIndex) 
    Dim pTitle As String = "Job #" & pJob.ID.ToString() 
    Dim pKey As String = "Job_" & pJob.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_Job" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pJob, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pJob 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Job_" & pJob.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Job #" & pJob.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_Job", pJob, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvJob.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvJob.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _JobCol.Count Then 
        Dim pJob As csJob = _JobCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pJob.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvJob.SelectedRows(0) 
