Public Class ctlccOrderLineCol
  ...
  Public Event evtUpdated(ByVal vOrderLine As clsOrderLine) 
  Private Event evtBeforeDelete(ByVal vOrderLine As clsOrderLine, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vOrderLine As clsOrderLine) 
  Public Event evtRowDoubleClicked(ByVal vOrderLine As clsOrderLine, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As clsOrderLine.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of clsOrderLine.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property CbosDoNotLoad As List(Of clsOrderLine.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of clsOrderLine.enmProperty) 
    Public Property ColumnsHide As List(Of clsOrderLine.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of clsOrderLine.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of clsOrderLine.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of clsOrderLine.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of clsOrderLine.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of clsOrderLine.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of clsOrderLine.enmParentProperty) 
      _ColumnsReadOnly = New List(Of clsOrderLine.enmProperty) 
      _ColumnsHide = New List(Of clsOrderLine.enmProperty) 
      _ColumnsFormat = New Dictionary(Of clsOrderLine.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of clsOrderLine.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of clsOrderLine.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of clsOrderLine.enmProperty, String) 
  ...
  Private WithEvents _OrderLineCol As clsOrderLineCol
  Private WithEvents _OrderLineColFullLength As clsOrderLineCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedOrderLine]() As clsOrderLine 
    Get 
      If dgvOrderLine.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvOrderLine.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvOrderLine.Rows.Count - 1 Then dgvOrderLine.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _OrderLineCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [OrderLineCol]() As clsOrderLineCol 
    Get 
      Return _OrderLineCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pOrderLineCol As New clsOrderLineCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pOrderLineCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pOrderLineCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pOrderLineCol As New clsOrderLineCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "OrderHeader" 
          pFault = pOrderLineCol.FillByOrderHeaderID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case "Product" 
          pFault = pOrderLineCol.FillByProductID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vOrderLineCol As clsOrderLineCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vOrderLineCol) 
  End Function
  
  Private Function LoadControl(vOrderLineCol As clsOrderLineCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of clsOrderLine.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of clsOrderLine.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = clsOrderLine.enmProperty.ID Then colID.ReadOnly = True 
      If l = clsOrderLine.enmProperty.OrderHeader Then colOrderHeader.ReadOnly = True 
      If l = clsOrderLine.enmProperty.Product Then colProduct.ReadOnly = True 
      If l = clsOrderLine.enmProperty.Quantity Then colQuantity.ReadOnly = True 
      If l = clsOrderLine.enmProperty.UnitPrice Then colUnitPrice.ReadOnly = True 
      If l = clsOrderLine.enmProperty.DiscountPercent Then colDiscountPercent.ReadOnly = True 
      If l = clsOrderLine.enmProperty.UnitCost Then colUnitCost.ReadOnly = True 
      If l = clsOrderLine.enmProperty.LineNumber Then colLineNumber.ReadOnly = True 
      If l = clsOrderLine.enmProperty.LineTotal Then colLineTotal.ReadOnly = True 
      If l = clsOrderLine.enmProperty.TotalCost Then colTotalCost.ReadOnly = True 
      If l = clsOrderLine.enmProperty.Profit Then colProfit.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As clsOrderLine.enmParentProperty = clsOrderLine.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of clsOrderLine.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvOrderLine.DoubleBuffered(True) 
 
    pFault = vOrderLineCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _OrderLineColFullLength = vOrderLineCol.Clone() 
 
    'Truncate the strings 
    _OrderLineCol = vOrderLineCol 
    If _LoadParameters.TruncateStrings Then 
      _OrderLineCol.TruncateStrings() 
    Else 
      dgvOrderLine.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvOrderLine.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        ...
        If colOrderHeader.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colOrderHeader) 
        colProduct.Name = colProduct.Name & "zzzz" 
        colProductText.Name = colProduct.Name.Replace("zzzz", "") 
        If colProduct.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colProduct) 
      Else 
        If colOrderHeader.ReadOnly = False Then 
          If colOrderHeaderText.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colOrderHeaderText) 
        Else 
          colOrderHeader.Name = colOrderHeader.Name & "zzzz" 
          colOrderHeaderText.Name = colOrderHeader.Name.Replace("zzzz", "") 
          If colOrderHeader.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colOrderHeader) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(clsOrderLine.enmParentProperty.OrderHeader) Then 
            _LoadParameters.CbosDoNotLoad.Add(clsOrderLine.enmParentProperty.OrderHeader) 
          End If 
        End If 
        If colProduct.ReadOnly = False Then 
          If colProductText.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colProductText) 
        Else 
          colProduct.Name = colProduct.Name & "zzzz" 
          colProductText.Name = colProduct.Name.Replace("zzzz", "") 
          If colProduct.DataGridView IsNot Nothing Then dgvOrderLine.Columns.Remove(colProduct) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(clsOrderLine.enmParentProperty.Product) Then 
            _LoadParameters.CbosDoNotLoad.Add(clsOrderLine.enmParentProperty.Product) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvOrderLine.Columns 
    ...
    'Hide columns  
    For Each p As clsOrderLine.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvOrderLine.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvOrderLine.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvOrderLine.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvOrderLine.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvOrderLine.ClearSelection()
    bsCtlOrderLine.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-OrderLine-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvOrderLine.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvOrderLine_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvOrderLine.SelectedRows.Count > 0 Then 
      pRowIndex = dgvOrderLine.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvOrderLine.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvOrderLine.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlOrderLine.DataSource = Nothing 
    bsCtlOrderLine.DataSource = _OrderLineCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvOrderLine.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _OrderLineCol.Count - 2 Then 
          dgvOrderLine.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _OrderLineCol.Count - 1 Then 
          dgvOrderLine.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'OrderHeader
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = clsOrderLine.enmParentProperty.OrderHeader) = clsOrderLine.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.ccOrderHeaderDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(clsOrderLine.enmParentProperty.OrderHeader, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-OrderLineCol-200806-1015", _Requester) 
        End If 
      End If 
    ...
    'Product
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = clsOrderLine.enmParentProperty.Product) = clsOrderLine.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.ccProductDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(clsOrderLine.enmParentProperty.Product, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        pFault = MyCache.GetComboList(pComboListTypeToLoad, pComboList) : If Not pFault.isOK() Then Return pFault 
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-OrderLineCol-200806-1015", _Requester) 
      ...
      If pPrompt = "" Then pPrompt = pChoose 
        pComboList.AddToTop(ccHelper.ToLong(0), pPrompt) 
      bsProduct.DataSource = pComboList 
      colProduct.Tag = pPrompt 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderLineUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderLineUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_OrderLineDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      ...
      dgvOrderLine.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvOrderLine.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvOrderLine.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvOrderLine.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvOrderLine.AllowUserToDeleteRows = False 
      dgvOrderLine.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvOrderLine.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvOrderLine.CurrentRow.Index 
      '  dgvOrderLine.CurrentCell = dgvOrderLine.Rows(pCurrentRow).Cells(0) 
      '  dgvOrderLine.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    pFieldList.Append("ProductID (DB Code), ") 
    pFieldList.Append("Quantity, ") 
    pFieldList.Append("UnitPrice, ") 
    pFieldList.Append("DiscountPercent, ") 
    pFieldList.Append("LineNumber, ") 
    
    Dim pNumberOfFields As Integer = 7 
    ...
    'Now Load the data 
    Dim pIncomingOrderLines As New clsOrderLineCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingOrderLine As New clsOrderLine(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingOrderLine.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = clsOrderLineCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingOrderLines.Add(pIncomingOrderLine) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingOrderLine.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            ...
            pIncomingOrderLine.OrderHeaderID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingOrderLine.ProductID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Quantity" 
            pIncomingOrderLine.Quantity = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "UnitPrice" 
            pIncomingOrderLine.UnitPrice = CType(pCurrentRow(pFieldNo), Decimal) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingOrderLine.DiscountPercent = CType(pCurrentRow(pFieldNo), Decimal) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "LineNumber" 
            pIncomingOrderLine.LineNumber = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingOrderLine.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingOrderLine.Tag = "Row " & pRow.ToString Then 
            pIncomingOrderLine.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingOrderLines.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingOrderLines.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingOrderLines 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _OrderLineCol = pIncomingOrderLines 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvOrderLine.Focused = True AndAlso dgvOrderLine.SelectedRows.Count > 0 Then 
      pCellRow = dgvOrderLine.CurrentCell.RowIndex 
      pCellCol = dgvOrderLine.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvOrderLine.CurrentCell = dgvOrderLine.Rows(0).Cells(0) 
      dgvOrderLine.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _OrderLineCol.Count > 0 AndAlso _OrderLineCol(_OrderLineCol.Count - 1).ID = 0 Then 
      _OrderLineCol.RemoveAt(_OrderLineCol.Count - 1) 
      bsCtlOrderLine.DataSource = Nothing 
      bsCtlOrderLine.DataSource = _OrderLineCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvOrderLine.Focus() 
      dgvOrderLine.CurrentCell = dgvOrderLine.Rows(pCellRow).Cells(pCellCol) 
      dgvOrderLine.CurrentCell.Selected = True 
    ElseIf _OrderLineCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlOrderLine.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As clsOrderLine 
    pEntity = CType(bsCtlOrderLine.Current, clsOrderLine) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvOrderLine.CurrentCell Is Nothing Then Return pFault 
    
    If dgvOrderLine.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pOrderLine As clsOrderLine 
      pOrderLine = CType(bsCtlOrderLine.Current, clsOrderLine) 
      If pOrderLine Is Nothing Then 
        pFault.LogFreeTextFault("There is no OrderLine to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvOrderLine.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvOrderLine.CurrentCell.RowIndex 
      'show row as selected  
      dgvOrderLine.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvOrderLine.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvOrderLine.CurrentRow.Selected = True 
      If pOrderLine.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pOrderLine.ProductID.ToString() & " - Qty: " & pOrderLine.Quantity.ToString() & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pOrderLine, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvOrderLine.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvOrderLine.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvOrderLine.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pOrderLine.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlOrderLine.Remove(bsCtlOrderLine.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pOrderLine As clsOrderLine = CType(bsCtlOrderLine.Current, clsOrderLine) 
    If pOrderLine IsNot Nothing Then 
      If pOrderLine.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _OrderLineCol.Count > 0 AndAlso dgvOrderLine.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvOrderLine.Columns.Count - 1 
        If dgvOrderLine.Columns(i).Visible Then 
          dgvOrderLine.CurrentCell = dgvOrderLine.Rows(dgvOrderLine.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvOrderLine.Refresh() 
      dgvOrderLine.Rows(dgvOrderLine.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvOrderLine.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvOrderLine_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvOrderLine.RowValidating 
    If _Loading = True OrElse dgvOrderLine.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvOrderLine.CurrentCell = dgvOrderLine.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvOrderLine_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvOrderLine.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvOrderLine.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvOrderLine.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvOrderLine.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pOrderLine Is Nothing Then pOrderLine = CType(dgvOrderLine.Rows(e.RowIndex).DataBoundItem, clsOrderLine) ' Only assign it if needed 
    '  If pOrderLine.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pOrderLine.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvOrderLine.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pOrderLine Is Nothing Then pOrderLine = CType(dgvOrderLine.Rows(e.RowIndex).DataBoundItem, clsOrderLine) ' Only assign it if needed
    '  If pOrderLine.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pOrderLine.RAV - pOrderLine.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvOrderLine.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvOrderLine.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvOrderLine.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvOrderLine.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If e.ColumnIndex = colUnitCost.Index Then 
      If CType(e.Value, Decimal) = 0D Then e.Value = "" 
    End If 
    ...
    If dgvOrderLine.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvOrderLine.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvOrderLine.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvOrderLine_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvOrderLine.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvOrderLine_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvOrderLine.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvOrderLine.Columns(e.ColumnIndex)
    If bsCtlOrderLine.Current Is Nothing Then Exit Sub
    ...
    Dim pOrderLineCol As clsOrderLineCol
    pOrderLineCol = CType(bsCtlOrderLine.DataSource, clsOrderLineCol)

    Dim pSummaryRow As clsOrderLine = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pOrderLineCol(pOrderLineCol.Count - 1) 
      pOrderLineCol.RemoveAt(pOrderLineCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pOrderLineCol.Count - 1 
          pOrderLineCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      ...
      ElseIf pNewColumn Is colOrderHeader OrElse pNewColumn Is colOrderHeaderText Then
        pOrderLineCol.SortByOrderHeaderText()
      ElseIf pNewColumn Is colProduct OrElse pNewColumn Is colProductText Then
        pOrderLineCol.SortByProductText()
      ElseIf pNewColumn Is colQuantity Then
        pOrderLineCol.SortByQuantity()
      ElseIf pNewColumn Is colUnitPrice Then
        pOrderLineCol.SortByUnitPrice()
      ElseIf pNewColumn Is colDiscountPercent Then
        pOrderLineCol.SortByDiscountPercent()
      ElseIf pNewColumn Is colUnitCost Then
        pOrderLineCol.SortByUnitCost()
      ElseIf pNewColumn Is colLineNumber Then
        pOrderLineCol.SortByLineNumber()
      ElseIf pNewColumn Is colLineTotal Then
        pOrderLineCol.SortByLineTotal()
      ElseIf pNewColumn Is colTotalCost Then
        pOrderLineCol.SortByTotalCost()
      ElseIf pNewColumn Is colProfit Then
        pOrderLineCol.SortByProfit()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As clsOrderLine In pOrderLineCol 
        ...
        ElseIf pNewColumn Is colProduct OrElse pNewColumn Is colProductText Then
          Dim pTest As String = "" 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.ProductText <> pTest Then iCntr += 1 : pTest = p.ProductText 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colQuantity Then
          Dim pTest As Integer = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.Quantity <> pTest Then iCntr += 1 : pTest = p.Quantity 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colUnitPrice Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.UnitPrice <> pTest Then iCntr += 1 : pTest = p.UnitPrice 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colDiscountPercent Then
          Dim pTest As Decimal = 0 
        ...
        ElseIf pNewColumn Is colUnitCost Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.UnitCost <> pTest Then iCntr += 1 : pTest = p.UnitCost 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLineNumber Then
          Dim pTest As Integer = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.LineNumber <> pTest Then iCntr += 1 : pTest = p.LineNumber 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colLineTotal Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.LineTotal <> pTest Then iCntr += 1 : pTest = p.LineTotal 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colTotalCost Then
          Dim pTest As Decimal = 0 
        ...
        ElseIf pNewColumn Is colProfit Then
          Dim pTest As Decimal = 0 
          For Each p As clsOrderLine In pOrderLineCol 
            If p.Profit <> pTest Then iCntr += 1 : pTest = p.Profit 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pOrderLineCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pOrderLineCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pOrderLineCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlOrderLine.Position = bsCtlOrderLine.IndexOf(pOrderLineCol.FindByID(pID))
    End If

    'dgvOrderLine.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvOrderLine.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pOrderLineCol As clsOrderLineCol 
      pOrderLineCol = CType(bsCtlOrderLine.DataSource, clsOrderLineCol) 
      Dim pOrderLine As clsOrderLine = pOrderLineCol.FindByID(pID) 
      If Not pOrderLine.IsEmpty Then 
        bsCtlOrderLine.Position = bsCtlOrderLine.IndexOf(pOrderLineCol.FindByID(pID)) 
        dgvOrderLine.Rows(bsCtlOrderLine.Position).Selected = True 
      Else 
        dgvOrderLine.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvOrderLine_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvOrderLine.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvOrderLine.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvOrderLine_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvOrderLine.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvOrderLine_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvOrderLine.Scroll
    dgvOrderLine.Invalidate() 
  End Sub
 
  Private Sub dgvOrderLine_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvOrderLine.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvOrderLine.Rows.Count - 1 Then Exit Sub
 
    'If dgvOrderLine.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'OrderLine', the row with an ID of " & dgvOrderLine.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'OrderLine', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvOrderLine.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvOrderLine.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-OrderLine-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvOrderLine(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvOrderLine(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvOrderLine_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvOrderLine.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvOrderLine.CurrentCell = dgvOrderLine.Rows(dgvOrderLine.CurrentCell.RowIndex).Cells(0) 
      dgvOrderLine.Refresh() 
    End If 
  End Sub 
  Private Sub dgvOrderLine_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvOrderLine.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvOrderLine_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvOrderLine.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvOrderLine_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvOrderLine.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvOrderLine.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvOrderLine(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pOrderLine As clsOrderLine = _OrderLineCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pOrderLine, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "OrderLine Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlccOrderLine", pOrderLine, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvOrderLine_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvOrderLine.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvOrderLine.Rows.Count - 1 Then dgvOrderLine.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pOrderLine As clsOrderLine = _OrderLineCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pOrderLine) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvOrderLine_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvOrderLine.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvOrderLine.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvOrderLine.Columns(pNewCol).Visible = False Then 
      dgvOrderLine.Columns(pNewCol).Visible = True 
      dgvOrderLine.CurrentCell = dgvOrderLine.CurrentRow.Cells(pNewCol) 
      dgvOrderLine.CurrentCell = dgvOrderLine.CurrentRow.Cells(pOriginalCol) 
      dgvOrderLine.Columns(pNewCol).Visible = False 
    Else 
      dgvOrderLine.CurrentCell = dgvOrderLine.CurrentRow.Cells(pNewCol) 
      dgvOrderLine.CurrentCell = dgvOrderLine.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pOrderLine, clsOrderLine), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pOrderLine.Update(_Requester) 
    ...
    Else 
      If pFault.isOK = False Then 'AndAlso pFault.Severity = clsEnums.enmFaultSeverity.LogOnly  
        ShowFault(pFault, _Requester) 
      End If 
      dgvOrderLine.EndEdit() 
      _DVGDirty = False 
      'Reset the OrderLine collection 
      MyCache.ClearComboList(clsEnums.enmComboListType.ccOrderLineDefaultByID) 
      RaiseEvent evtUpdated(pOrderLine) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvOrderLine.Columns 
  ...
  Private Sub Summarize() 
    If _OrderLineCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pQuantity As Integer 
    Dim pUnitPrice As Decimal 
    Dim pDiscountPercent As Decimal 
    Dim pUnitCost As Decimal 
    Dim pLineNumber As Integer 
    Dim pLineTotal As Decimal 
    Dim pTotalCost As Decimal 
    Dim pProfit As Decimal 
    For Each pExistingRow As clsOrderLine In _OrderLineCol 
      If _SummaryOverFlow.IndexOf("#Quantity#") < 0 Then 
        Try 
          pQuantity += pExistingRow.Quantity 
        Catch ex As System.OverflowException 
          pQuantity = -99999999 
          _SummaryOverFlow &= "Quantity#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#UnitPrice#") < 0 Then 
        Try 
          pUnitPrice += pExistingRow.UnitPrice 
        Catch ex As System.OverflowException 
          pUnitPrice = -99999999 
          _SummaryOverFlow &= "UnitPrice#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#DiscountPercent#") < 0 Then 
        ...
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#UnitCost#") < 0 Then 
        Try 
          pUnitCost += pExistingRow.UnitCost 
        Catch ex As System.OverflowException 
          pUnitCost = -99999999 
          _SummaryOverFlow &= "UnitCost#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#LineNumber#") < 0 Then 
        Try 
          pLineNumber += pExistingRow.LineNumber 
        Catch ex As System.OverflowException 
          pLineNumber = -99999999 
          _SummaryOverFlow &= "LineNumber#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#LineTotal#") < 0 Then 
        Try 
          pLineTotal += pExistingRow.LineTotal 
        Catch ex As System.OverflowException 
          pLineTotal = -99999999 
          _SummaryOverFlow &= "LineTotal#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#TotalCost#") < 0 Then 
        ...
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#Profit#") < 0 Then 
        Try 
          pProfit += pExistingRow.Profit 
        Catch ex As System.OverflowException 
          pProfit = -99999999 
          _SummaryOverFlow &= "Profit#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.Quantity) = clsOrderLine.enmSummarizeableProperty.Quantity Then pQuantity = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.UnitPrice) = clsOrderLine.enmSummarizeableProperty.UnitPrice Then pUnitPrice = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.DiscountPercent) = clsOrderLine.enmSummarizeableProperty.DiscountPercent Then pDiscountPercent = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.UnitCost) = clsOrderLine.enmSummarizeableProperty.UnitCost Then pUnitCost = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.LineNumber) = clsOrderLine.enmSummarizeableProperty.LineNumber Then pLineNumber = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.LineTotal) = clsOrderLine.enmSummarizeableProperty.LineTotal Then pLineTotal = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.TotalCost) = clsOrderLine.enmSummarizeableProperty.TotalCost Then pTotalCost = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsOrderLine.enmSummarizeableProperty.Profit) = clsOrderLine.enmSummarizeableProperty.Profit Then pProfit = 0
    Dim pSummaryRow As New clsOrderLine( _ 
        vID:=0 _ 
      , vOrderHeaderID:=0 _ 
      , vOrderHeaderText:="" _ 
      , vProductID:=0 _ 
      , vProductText:="" _ 
      , vQuantity:=pQuantity _ 
      , vUnitPrice:=pUnitPrice _ 
      , vDiscountPercent:=pDiscountPercent _ 
      , vUnitCost:=pUnitCost _ 
      , vLineNumber:=pLineNumber _ 
      , vLineTotal:=pLineTotal _ 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\OrderLineCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\OrderLineCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\OrderLineCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\OrderLineCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\OrderLineCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvOrderLine.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvOrderLine.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _OrderLineColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _OrderLineColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _OrderLineColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _OrderLineColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _OrderLineColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-OrderLine-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "OrderLines" 
        .SubTitleRight = "Rows: " & _OrderLineCol.Count.ToString 
        ...
        End If 
        .DataSource = _OrderLineCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-OrderLine-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-OrderLine-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvOrderLine.Rows 
    ...
    For Each row As DataGridViewRow In dgvOrderLine.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlOrderLine), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvOrderLine.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-OrderLine-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvOrderLine.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvOrderLine.Width - 30) / dgvOrderLine.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvOrderLine.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-OrderLine-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colProduct.HeaderText = pStrg : mnuColVisibleProduct.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderLine", "Quantity", _Requester) 
    If pStrg <> "" Then colQuantity.HeaderText = pStrg : mnuColVisibleQuantity.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderLine", "UnitPrice", _Requester) 
    If pStrg <> "" Then colUnitPrice.HeaderText = pStrg : mnuColVisibleUnitPrice.Text = pStrg
    ...
    If pStrg <> "" Then colUnitCost.HeaderText = pStrg : mnuColVisibleUnitCost.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderLine", "LineNumber", _Requester) 
    If pStrg <> "" Then colLineNumber.HeaderText = pStrg : mnuColVisibleLineNumber.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("OrderLine", "LineTotal", _Requester) 
    If pStrg <> "" Then colLineTotal.HeaderText = pStrg : mnuColVisibleLineTotal.Text = pStrg
    ...
    If pStrg <> "" Then colProfit.HeaderText = pStrg : mnuColVisibleProfit.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleOrderHeader.Click, mnuColVisibleProduct.Click, mnuColVisibleQuantity.Click, mnuColVisibleUnitPrice.Click, mnuColVisibleDiscountPercent.Click, mnuColVisibleUnitCost.Click, mnuColVisibleLineNumber.Click, mnuColVisibleLineTotal.Click, mnuColVisibleTotalCost.Click, mnuColVisibleProfit.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvOrderLine.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvOrderLine.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleProduct.Checked = True Then mnuColVisibleProduct.PerformClick() 
    If mnuColVisibleQuantity.Checked = True Then mnuColVisibleQuantity.PerformClick() 
    If mnuColVisibleUnitPrice.Checked = True Then mnuColVisibleUnitPrice.PerformClick() 
    If mnuColVisibleDiscountPercent.Checked = True Then mnuColVisibleDiscountPercent.PerformClick() 
    If mnuColVisibleUnitCost.Checked = True Then mnuColVisibleUnitCost.PerformClick() 
    If mnuColVisibleLineNumber.Checked = True Then mnuColVisibleLineNumber.PerformClick() 
    If mnuColVisibleLineTotal.Checked = True Then mnuColVisibleLineTotal.PerformClick() 
    If mnuColVisibleTotalCost.Checked = True Then mnuColVisibleTotalCost.PerformClick() 
    If mnuColVisibleProfit.Checked = True Then mnuColVisibleProfit.PerformClick() 
    'Show Defaults 
    If mnuColVisibleProduct.Checked = False Then mnuColVisibleProduct.PerformClick() 
    If mnuColVisibleQuantity.Checked = False Then mnuColVisibleQuantity.PerformClick() 
    
    _Loading = False 
    'dgvOrderLine.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvOrderLine_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvOrderLine.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the OrderLine to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvOrderLine_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvOrderLine.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvOrderLine.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvOrderLine.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvOrderLine.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvOrderLine.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvOrderLine(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvOrderLine.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvOrderLine.BeginEdit(True) 
        DirectCast(dgvOrderLine.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvOrderLine_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvOrderLine.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvOrderLine.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvOrderLine.MultiSelect = True 
        dgvOrderLine.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvOrderLine.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvOrderLine.MultiSelect = True 
    Else 
      dgvOrderLine.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlccOrderLineCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvOrderLine.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlccOrderLineCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvOrderLine.Width 
  ...
  Private Sub ctlccOrderLineCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlccOrderLineCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvOrderLine_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvOrderLine.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvOrderLine.Rows(e.RowIndex).Selected Then 
        dgvOrderLine.ClearSelection() 
        dgvOrderLine.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvOrderLine.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvOrderLine.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvOrderLine.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _OrderLineCol.Count Then Exit Sub 
    Dim pOrderLine As clsOrderLine = _OrderLineCol(pRowIndex) 
    Dim pTitle As String = "OrderLine #" & pOrderLine.ID.ToString() 
    Dim pKey As String = "OrderLine_" & pOrderLine.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlccOrderLine" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pOrderLine, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pOrderLine 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "OrderLine_" & pOrderLine.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "OrderLine #" & pOrderLine.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlccOrderLine", pOrderLine, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvOrderLine.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvOrderLine.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _OrderLineCol.Count Then 
        Dim pOrderLine As clsOrderLine = _OrderLineCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pOrderLine.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvOrderLine.SelectedRows(0) 
