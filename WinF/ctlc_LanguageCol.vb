Public Class ctlc_LanguageCol
  ...
  Public Event evtRowClicked(ByVal vLanguage As csLanguage) 
  Public Event evtRowDoubleClicked(ByVal vLanguage As csLanguage, ByRef rHandled As Boolean) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property ColumnsReadOnly As List(Of csLanguage.enmProperty) 
    Public Property ColumnsHide As List(Of csLanguage.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csLanguage.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csLanguage.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csLanguage.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csLanguage.enmProperty, String) 
    ...
    Public Sub New() 
      ...
      _ReportTitle = "" 
      _ReadOnly = False 
      _ColumnsReadOnly = New List(Of csLanguage.enmProperty) 
      _ColumnsHide = New List(Of csLanguage.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csLanguage.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csLanguage.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csLanguage.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csLanguage.enmProperty, String) 
  ...
  Private WithEvents _LanguageCol As csLanguageCol
  Private WithEvents _LanguageColFullLength As csLanguageCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedLanguage]() As csLanguage 
    Get 
      If dgvLanguage.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvLanguage.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvLanguage.Rows.Count - 1 Then dgvLanguage.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _LanguageCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [LanguageCol]() As csLanguageCol 
    Get 
      Return _LanguageCol 
    End Get 
  End Property 
  ...
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pLanguageCol As New csLanguageCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pLanguageCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pLanguageCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vLanguageCol As csLanguageCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vLanguageCol) 
  End Function
  
  Private Function LoadControl(vLanguageCol As csLanguageCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csLanguage.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csLanguage.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csLanguage.enmProperty.ID Then colID.ReadOnly = True 
      If l = csLanguage.enmProperty.Code Then colCode.ReadOnly = True 
      If l = csLanguage.enmProperty.Name Then colName.ReadOnly = True 
      If l = csLanguage.enmProperty.NameLoc Then colNameLoc.ReadOnly = True 
      If l = csLanguage.enmProperty.Culture Then colCulture.ReadOnly = True 
    Next 
    ...
    dgvLanguage.DoubleBuffered(True) 
 
    pFault = vLanguageCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _LanguageColFullLength = vLanguageCol.Clone() 
 
    'Truncate the strings 
    _LanguageCol = vLanguageCol 
    If _LoadParameters.TruncateStrings Then 
      _LanguageCol.TruncateStrings() 
    Else 
      dgvLanguage.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvLanguage.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvLanguage.Columns 
    ...
    'Hide columns  
    For Each p As csLanguage.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvLanguage.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvLanguage.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvLanguage.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvLanguage.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    bsCtlLanguage.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-Language-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvLanguage.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvLanguage_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvLanguage.SelectedRows.Count > 0 Then 
      pRowIndex = dgvLanguage.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvLanguage.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvLanguage.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlLanguage.DataSource = Nothing 
    bsCtlLanguage.DataSource = _LanguageCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvLanguage.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _LanguageCol.Count - 2 Then 
          dgvLanguage.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _LanguageCol.Count - 1 Then 
          dgvLanguage.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    dgvLanguage.EditMode = DataGridViewEditMode.EditProgrammatically 
    dgvLanguage.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
    dgvLanguage.AllowUserToDeleteRows = False 
    dgvLanguage.AllowUserToAddRows = False 
    If _LanguageCol.Count = 0 Then 
  ...
  'ExternalButtons 
  'CellFormatting  
  Private Sub dgvLanguage_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvLanguage.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvLanguage.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvLanguage.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvLanguage.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pLanguage Is Nothing Then pLanguage = CType(dgvLanguage.Rows(e.RowIndex).DataBoundItem, csLanguage) ' Only assign it if needed 
    '  If pLanguage.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pLanguage.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvLanguage.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pLanguage Is Nothing Then pLanguage = CType(dgvLanguage.Rows(e.RowIndex).DataBoundItem, csLanguage) ' Only assign it if needed
    '  If pLanguage.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pLanguage.RAV - pLanguage.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvLanguage.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvLanguage.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvLanguage.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvLanguage.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvLanguage.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvLanguage.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvLanguage.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvLanguage_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvLanguage.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvLanguage_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvLanguage.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvLanguage.Columns(e.ColumnIndex)
    If bsCtlLanguage.Current Is Nothing Then Exit Sub
    ...
    dgvLanguage.SuspendLayout()

    Dim pLanguage As csLanguage
    Dim pID As Long = 0 
    If dgvLanguage.SelectedRows.Count > 0 Then 
    pLanguage = CType(bsCtlLanguage.Current, csLanguage)
      pID = pLanguage.ID 
    End If 
    ...
    Dim pLanguageCol As csLanguageCol
    pLanguageCol = CType(bsCtlLanguage.DataSource, csLanguageCol)

    Dim pSummaryRow As csLanguage = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pLanguageCol(pLanguageCol.Count - 1) 
      pLanguageCol.RemoveAt(pLanguageCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pLanguageCol.Count - 1 
          pLanguageCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pLanguageCol.SortByID()
      ElseIf pNewColumn Is colCode Then
        pLanguageCol.SortByCode()
      ElseIf pNewColumn Is colName Then
        pLanguageCol.SortByName()
      ElseIf pNewColumn Is colNameLoc Then
        pLanguageCol.SortByNameLoc()
      ElseIf pNewColumn Is colCulture Then
        pLanguageCol.SortByCulture()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csLanguage In pLanguageCol 
        ...
        ElseIf pNewColumn Is colNameLoc Then
          Dim pTest As String = "" 
          For Each p As csLanguage In pLanguageCol 
            If p.NameLoc <> pTest Then iCntr += 1 : pTest = p.NameLoc 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colCulture Then
          Dim pTest As String = "" 
          For Each p As csLanguage In pLanguageCol 
            If p.Culture <> pTest Then iCntr += 1 : pTest = p.Culture 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pLanguageCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pLanguageCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pLanguageCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlLanguage.Position = bsCtlLanguage.IndexOf(pLanguageCol.FindByID(pID))
    End If

    'dgvLanguage.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvLanguage.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pLanguageCol As csLanguageCol 
      pLanguageCol = CType(bsCtlLanguage.DataSource, csLanguageCol) 
      Dim pLanguage As csLanguage = pLanguageCol.FindByID(pID) 
      If Not pLanguage.IsEmpty Then 
        bsCtlLanguage.Position = bsCtlLanguage.IndexOf(pLanguageCol.FindByID(pID)) 
        dgvLanguage.Rows(bsCtlLanguage.Position).Selected = True 
      Else 
        dgvLanguage.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvLanguage_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvLanguage.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvLanguage.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvLanguage_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvLanguage.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvLanguage_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvLanguage.Scroll
    dgvLanguage.Invalidate() 
  End Sub
 
  Private Sub dgvLanguage_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvLanguage.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvLanguage.Rows.Count - 1 Then Exit Sub
 
    'If dgvLanguage.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'Language', the row with an ID of " & dgvLanguage.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'Language', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvLanguage.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvLanguage.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-Language-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvLanguage(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvLanguage(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvLanguage_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvLanguage.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvLanguage_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvLanguage.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvLanguage_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvLanguage.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvLanguage.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvLanguage(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pLanguage As csLanguage = _LanguageCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pLanguage, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "Language Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_Language", pLanguage, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvLanguage_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvLanguage.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvLanguage.Rows.Count - 1 Then dgvLanguage.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pLanguage As csLanguage = _LanguageCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pLanguage) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvLanguage_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvLanguage.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvLanguage.Columns 
  ...
  Private Sub Summarize() 
    If _LanguageCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    For Each pExistingRow As csLanguage In _LanguageCol 
    Next 
    _Summarized = False 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\LanguageCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\LanguageCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\LanguageCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\LanguageCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\LanguageCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvLanguage.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvLanguage.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _LanguageColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _LanguageColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _LanguageColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _LanguageColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _LanguageColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-Language-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "Languages" 
        .SubTitleRight = "Rows: " & _LanguageCol.Count.ToString 
        ...
        End If 
        .DataSource = _LanguageCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-Language-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-Language-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvLanguage.Rows 
    ...
    For Each row As DataGridViewRow In dgvLanguage.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlLanguage), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvLanguage.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-Language-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvLanguage.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvLanguage.Width - 30) / dgvLanguage.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvLanguage.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-Language-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colNameLoc.HeaderText = pStrg : mnuColVisibleNameLoc.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_Language", "Culture", _Requester) 
    If pStrg <> "" Then colCulture.HeaderText = pStrg : mnuColVisibleCulture.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleCode.Click, mnuColVisibleName.Click, mnuColVisibleNameLoc.Click, mnuColVisibleCulture.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvLanguage.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvLanguage.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleNameLoc.Checked = True Then mnuColVisibleNameLoc.PerformClick() 
    If mnuColVisibleCulture.Checked = True Then mnuColVisibleCulture.PerformClick() 
    'Show Defaults 
  ...
  Private Sub dgvLanguage_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvLanguage.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the Language to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvLanguage_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvLanguage.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvLanguage.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvLanguage.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvLanguage.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvLanguage.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvLanguage(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvLanguage.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvLanguage.BeginEdit(True) 
        DirectCast(dgvLanguage.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvLanguage_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvLanguage.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvLanguage.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvLanguage.MultiSelect = True 
        dgvLanguage.ClearSelection() 
  ...
  Private Sub ctlc_LanguageCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvLanguage.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_LanguageCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvLanguage.Width 
  ...
  Private Sub ctlc_LanguageCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub dgvLanguage_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvLanguage.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvLanguage.Rows(e.RowIndex).Selected Then 
        dgvLanguage.ClearSelection() 
        dgvLanguage.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvLanguage.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvLanguage.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvLanguage.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _LanguageCol.Count Then Exit Sub 
    Dim pLanguage As csLanguage = _LanguageCol(pRowIndex) 
    Dim pTitle As String = "Language #" & pLanguage.ID.ToString() 
    Dim pKey As String = "Language_" & pLanguage.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_Language" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pLanguage, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pLanguage 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "Language_" & pLanguage.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "Language #" & pLanguage.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_Language", pLanguage, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvLanguage.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvLanguage.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _LanguageCol.Count Then 
        Dim pLanguage As csLanguage = _LanguageCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pLanguage.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvLanguage.SelectedRows(0) 
