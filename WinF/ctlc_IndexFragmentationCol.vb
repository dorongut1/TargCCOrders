Public Class ctlc_IndexFragmentationCol
  ...
  Public Event evtRowClicked(ByVal vIndexFragmentation As csIndexFragmentation) 
  Public Event evtRowDoubleClicked(ByVal vIndexFragmentation As csIndexFragmentation, ByRef rHandled As Boolean) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of csIndexFragmentation.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property ColumnsReadOnly As List(Of csIndexFragmentation.enmProperty) 
    Public Property ColumnsHide As List(Of csIndexFragmentation.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of csIndexFragmentation.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of csIndexFragmentation.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of csIndexFragmentation.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of csIndexFragmentation.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of csIndexFragmentation.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _ColumnsReadOnly = New List(Of csIndexFragmentation.enmProperty) 
      _ColumnsHide = New List(Of csIndexFragmentation.enmProperty) 
      _ColumnsFormat = New Dictionary(Of csIndexFragmentation.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of csIndexFragmentation.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of csIndexFragmentation.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of csIndexFragmentation.enmProperty, String) 
  ...
  Private WithEvents _IndexFragmentationCol As csIndexFragmentationCol
  Private WithEvents _IndexFragmentationColFullLength As csIndexFragmentationCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedIndexFragmentation]() As csIndexFragmentation 
    Get 
      If dgvIndexFragmentation.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvIndexFragmentation.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvIndexFragmentation.Rows.Count - 1 Then dgvIndexFragmentation.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _IndexFragmentationCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [IndexFragmentationCol]() As csIndexFragmentationCol 
    Get 
      Return _IndexFragmentationCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pIndexFragmentationCol As New csIndexFragmentationCol() 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pIndexFragmentationCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pIndexFragmentationCol)
    Return pFault 
  End Function 
 
  Public Function LoadControl(vIndexFragmentationCol As csIndexFragmentationCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vIndexFragmentationCol) 
  End Function
  
  Private Function LoadControl(vIndexFragmentationCol As csIndexFragmentationCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of csIndexFragmentation.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of csIndexFragmentation.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = csIndexFragmentation.enmProperty.ID Then colID.ReadOnly = True 
      If l = csIndexFragmentation.enmProperty.TableName Then colTableName.ReadOnly = True 
      If l = csIndexFragmentation.enmProperty.IndexName Then colIndexName.ReadOnly = True 
      If l = csIndexFragmentation.enmProperty.IndexType Then colIndexType.ReadOnly = True 
      If l = csIndexFragmentation.enmProperty.FragmentationPct Then colFragmentationPct.ReadOnly = True 
      If l = csIndexFragmentation.enmProperty.PageCount Then colPageCount.ReadOnly = True 
    Next 
    ...
    dgvIndexFragmentation.DoubleBuffered(True) 
 
    pFault = vIndexFragmentationCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _IndexFragmentationColFullLength = vIndexFragmentationCol.Clone() 
 
    'Truncate the strings 
    _IndexFragmentationCol = vIndexFragmentationCol 
    If _LoadParameters.TruncateStrings Then 
      _IndexFragmentationCol.TruncateStrings() 
    Else 
      dgvIndexFragmentation.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvIndexFragmentation.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
 
    _LoadParameters.ReadOnly = True 'Since it's a view
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvIndexFragmentation.Columns 
    ...
    'Hide columns  
    For Each p As csIndexFragmentation.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvIndexFragmentation.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvIndexFragmentation.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvIndexFragmentation.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvIndexFragmentation.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvIndexFragmentation.ClearSelection()
    bsCtlIndexFragmentation.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-IndexFragmentation-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvIndexFragmentation.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvIndexFragmentation_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvIndexFragmentation.SelectedRows.Count > 0 Then 
      pRowIndex = dgvIndexFragmentation.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvIndexFragmentation.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvIndexFragmentation.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlIndexFragmentation.DataSource = Nothing 
    bsCtlIndexFragmentation.DataSource = _IndexFragmentationCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvIndexFragmentation.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _IndexFragmentationCol.Count - 2 Then 
          dgvIndexFragmentation.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _IndexFragmentationCol.Count - 1 Then 
          dgvIndexFragmentation.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    dgvIndexFragmentation.EditMode = DataGridViewEditMode.EditProgrammatically 
    dgvIndexFragmentation.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
    dgvIndexFragmentation.AllowUserToDeleteRows = False 
    dgvIndexFragmentation.AllowUserToAddRows = False 
    If _IndexFragmentationCol.Count = 0 Then 
    ...
    lblStatus.Text = "" 
    dgvIndexFragmentation.Refresh() 
  End Sub
  'ExternalButtons 
  'CellFormatting  
  Private Sub dgvIndexFragmentation_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvIndexFragmentation.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvIndexFragmentation.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvIndexFragmentation.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'If dgvIndexFragmentation.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pIndexFragmentation Is Nothing Then pIndexFragmentation = CType(dgvIndexFragmentation.Rows(e.RowIndex).DataBoundItem, csIndexFragmentation) ' Only assign it if needed 
    '  If pIndexFragmentation.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pIndexFragmentation.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvIndexFragmentation.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pIndexFragmentation Is Nothing Then pIndexFragmentation = CType(dgvIndexFragmentation.Rows(e.RowIndex).DataBoundItem, csIndexFragmentation) ' Only assign it if needed
    '  If pIndexFragmentation.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pIndexFragmentation.RAV - pIndexFragmentation.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvIndexFragmentation.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvIndexFragmentation.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvIndexFragmentation.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvIndexFragmentation.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvIndexFragmentation.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvIndexFragmentation.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvIndexFragmentation.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvIndexFragmentation_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvIndexFragmentation.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvIndexFragmentation_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvIndexFragmentation.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvIndexFragmentation.Columns(e.ColumnIndex)
    If bsCtlIndexFragmentation.Current Is Nothing Then Exit Sub
    ...
    dgvIndexFragmentation.SuspendLayout()

    Dim pIndexFragmentation As csIndexFragmentation
    Dim pID As Long = -1 
    If dgvIndexFragmentation.SelectedRows.Count > 0 Then 
    pIndexFragmentation = CType(bsCtlIndexFragmentation.Current, csIndexFragmentation)
      pID = pIndexFragmentation.ID 
    End If 
    ...
    Dim pIndexFragmentationCol As csIndexFragmentationCol
    pIndexFragmentationCol = CType(bsCtlIndexFragmentation.DataSource, csIndexFragmentationCol)

    Dim pSummaryRow As csIndexFragmentation = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pIndexFragmentationCol(pIndexFragmentationCol.Count - 1) 
      pIndexFragmentationCol.RemoveAt(pIndexFragmentationCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pIndexFragmentationCol.Count - 1 
          pIndexFragmentationCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pIndexFragmentationCol.SortByID()
      ElseIf pNewColumn Is colTableName Then
        pIndexFragmentationCol.SortByTableName()
      ElseIf pNewColumn Is colIndexName Then
        pIndexFragmentationCol.SortByIndexName()
      ElseIf pNewColumn Is colIndexType Then
        pIndexFragmentationCol.SortByIndexType()
      ElseIf pNewColumn Is colFragmentationPct Then
        pIndexFragmentationCol.SortByFragmentationPct()
      ElseIf pNewColumn Is colPageCount Then
        pIndexFragmentationCol.SortByPageCount()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As csIndexFragmentation In pIndexFragmentationCol 
        ...
        ElseIf pNewColumn Is colIndexName Then
          Dim pTest As String = "" 
          For Each p As csIndexFragmentation In pIndexFragmentationCol 
            If p.IndexName <> pTest Then iCntr += 1 : pTest = p.IndexName 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colIndexType Then
          Dim pTest As String = "" 
          For Each p As csIndexFragmentation In pIndexFragmentationCol 
            If p.IndexType <> pTest Then iCntr += 1 : pTest = p.IndexType 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colFragmentationPct Then
          Dim pTest As Decimal = 0 
          For Each p As csIndexFragmentation In pIndexFragmentationCol 
            If p.FragmentationPct <> pTest Then iCntr += 1 : pTest = p.FragmentationPct 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        ElseIf pNewColumn Is colPageCount Then
          Dim pTest As Integer = 0 
          For Each p As csIndexFragmentation In pIndexFragmentationCol 
            If p.PageCount <> pTest Then iCntr += 1 : pTest = p.PageCount 
            p.Tag = iCntr.ToString("0000000000") & p.Tag 
          Next 
        End If 
        pIndexFragmentationCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pIndexFragmentationCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pIndexFragmentationCol.Add(pSummaryRow) 
    End If 
 
    If pID > -1 Then
      bsCtlIndexFragmentation.Position = bsCtlIndexFragmentation.IndexOf(pIndexFragmentationCol.FindByID(pID))
    End If

    'dgvIndexFragmentation.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvIndexFragmentation.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > -1 Then 
      Dim pIndexFragmentationCol As csIndexFragmentationCol 
      pIndexFragmentationCol = CType(bsCtlIndexFragmentation.DataSource, csIndexFragmentationCol) 
      Dim pIndexFragmentation As csIndexFragmentation = pIndexFragmentationCol.FindByID(pID) 
      If Not pIndexFragmentation.IsEmpty Then 
        bsCtlIndexFragmentation.Position = bsCtlIndexFragmentation.IndexOf(pIndexFragmentationCol.FindByID(pID)) 
        dgvIndexFragmentation.Rows(bsCtlIndexFragmentation.Position).Selected = True 
      Else 
        dgvIndexFragmentation.ClearSelection() 
      End If 
    ElseIf pID = -1 Then 
  ...
  'Grid Resize
  Private Sub dgvIndexFragmentation_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvIndexFragmentation.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvIndexFragmentation.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvIndexFragmentation_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvIndexFragmentation.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvIndexFragmentation_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvIndexFragmentation.Scroll
    dgvIndexFragmentation.Invalidate() 
  End Sub
 
  Private Sub dgvIndexFragmentation_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvIndexFragmentation.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvIndexFragmentation.Rows.Count - 1 Then Exit Sub
 
    'If dgvIndexFragmentation.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'IndexFragmentation', the row with an ID of " & dgvIndexFragmentation.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'IndexFragmentation', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvIndexFragmentation.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvIndexFragmentation.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-IndexFragmentation-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvIndexFragmentation(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvIndexFragmentation(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvIndexFragmentation_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvIndexFragmentation.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvIndexFragmentation_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvIndexFragmentation.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvIndexFragmentation_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvIndexFragmentation.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvIndexFragmentation.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvIndexFragmentation(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pIndexFragmentation As csIndexFragmentation = _IndexFragmentationCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pIndexFragmentation, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "IndexFragmentation Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlc_IndexFragmentation", pIndexFragmentation, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvIndexFragmentation_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvIndexFragmentation.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvIndexFragmentation.Rows.Count - 1 Then dgvIndexFragmentation.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pIndexFragmentation As csIndexFragmentation = _IndexFragmentationCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pIndexFragmentation) 
    Finally
  ...
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvIndexFragmentation.Columns 
  ...
  Private Sub Summarize() 
    If _IndexFragmentationCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
 
    Dim pFragmentationPct As Decimal 
    Dim pPageCount As Integer 
    For Each pExistingRow As csIndexFragmentation In _IndexFragmentationCol 
      If _SummaryOverFlow.IndexOf("#FragmentationPct#") < 0 Then 
        Try 
          pFragmentationPct += pExistingRow.FragmentationPct 
        Catch ex As System.OverflowException 
          pFragmentationPct = -99999999 
          _SummaryOverFlow &= "FragmentationPct#" 
        End Try 
      End If 
      If _SummaryOverFlow.IndexOf("#PageCount#") < 0 Then 
        Try 
          pPageCount += pExistingRow.PageCount 
        Catch ex As System.OverflowException 
          pPageCount = -99999999 
          _SummaryOverFlow &= "PageCount#" 
        End Try 
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csIndexFragmentation.enmSummarizeableProperty.FragmentationPct) = csIndexFragmentation.enmSummarizeableProperty.FragmentationPct Then pFragmentationPct = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = csIndexFragmentation.enmSummarizeableProperty.PageCount) = csIndexFragmentation.enmSummarizeableProperty.PageCount Then pPageCount = 0
    Dim pSummaryRow As New csIndexFragmentation( _ 
        vID:=0 _ 
      , vTableName:="" _ 
      , vIndexName:="" _ 
      , vIndexType:="" _ 
      , vFragmentationPct:=pFragmentationPct _ 
      , vPageCount:=pPageCount _ 
      , vTag:="" _ 
      , vDateAdded:=Nothing _ 
      )
    _IndexFragmentationCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\IndexFragmentationCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\IndexFragmentationCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\IndexFragmentationCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\IndexFragmentationCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\IndexFragmentationCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvIndexFragmentation.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvIndexFragmentation.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _IndexFragmentationColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _IndexFragmentationColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _IndexFragmentationColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _IndexFragmentationColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _IndexFragmentationColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-IndexFragmentation-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "IndexFragmentations" 
        .SubTitleRight = "Rows: " & _IndexFragmentationCol.Count.ToString 
        ...
        End If 
        .DataSource = _IndexFragmentationCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-IndexFragmentation-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-IndexFragmentation-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvIndexFragmentation.Rows 
    ...
    For Each row As DataGridViewRow In dgvIndexFragmentation.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlIndexFragmentation), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvIndexFragmentation.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-IndexFragmentation-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvIndexFragmentation.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvIndexFragmentation.Width - 30) / dgvIndexFragmentation.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    'Set Wrap 
    'colTableName.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    'colIndexName.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvIndexFragmentation.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-IndexFragmentation-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub LoadLocalizedText() 
    ...
    If pStrg <> "" Then colIndexName.HeaderText = pStrg : mnuColVisibleIndexName.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_IndexFragmentation", "IndexType", _Requester) 
    If pStrg <> "" Then colIndexType.HeaderText = pStrg : mnuColVisibleIndexType.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_IndexFragmentation", "FragmentationPct", _Requester) 
    If pStrg <> "" Then colFragmentationPct.HeaderText = pStrg : mnuColVisibleFragmentationPct.Text = pStrg
 
    pStrg = ccHelper.GetLocalizedFieldName("c_IndexFragmentation", "PageCount", _Requester) 
    If pStrg <> "" Then colPageCount.HeaderText = pStrg : mnuColVisiblePageCount.Text = pStrg
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleTableName.Click, mnuColVisibleIndexName.Click, mnuColVisibleIndexType.Click, mnuColVisibleFragmentationPct.Click, mnuColVisiblePageCount.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvIndexFragmentation.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvIndexFragmentation.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleIndexName.Checked = True Then mnuColVisibleIndexName.PerformClick() 
    If mnuColVisibleIndexType.Checked = True Then mnuColVisibleIndexType.PerformClick() 
    If mnuColVisibleFragmentationPct.Checked = True Then mnuColVisibleFragmentationPct.PerformClick() 
    If mnuColVisiblePageCount.Checked = True Then mnuColVisiblePageCount.PerformClick() 
    'Show Defaults 
    If mnuColVisibleTableName.Checked = False Then mnuColVisibleTableName.PerformClick() 
    
    _Loading = False 
    'dgvIndexFragmentation.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvIndexFragmentation_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvIndexFragmentation.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the IndexFragmentation to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvIndexFragmentation_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvIndexFragmentation.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvIndexFragmentation.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvIndexFragmentation.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvIndexFragmentation.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvIndexFragmentation.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvIndexFragmentation(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvIndexFragmentation.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvIndexFragmentation.BeginEdit(True) 
        DirectCast(dgvIndexFragmentation.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvIndexFragmentation_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvIndexFragmentation.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvIndexFragmentation.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvIndexFragmentation.MultiSelect = True 
        dgvIndexFragmentation.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvIndexFragmentation.Rows(i).Selected = True 
        Next 
      End If 
    ElseIf (pModifiers And Keys.Control) = Keys.Control Then 
      dgvIndexFragmentation.MultiSelect = True 
    Else 
      dgvIndexFragmentation.MultiSelect = False 
    End If 
  End Sub 
  ...
  Private Sub ctlc_IndexFragmentationCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvIndexFragmentation.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlc_IndexFragmentationCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvIndexFragmentation.Width 
  ...
  Private Sub ctlc_IndexFragmentationCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub dgvIndexFragmentation_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvIndexFragmentation.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvIndexFragmentation.Rows(e.RowIndex).Selected Then 
        dgvIndexFragmentation.ClearSelection() 
        dgvIndexFragmentation.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvIndexFragmentation.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvIndexFragmentation.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvIndexFragmentation.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _IndexFragmentationCol.Count Then Exit Sub 
    Dim pIndexFragmentation As csIndexFragmentation = _IndexFragmentationCol(pRowIndex) 
    Dim pTitle As String = "IndexFragmentation #" & pIndexFragmentation.ID.ToString() 
    Dim pKey As String = "IndexFragmentation_" & pIndexFragmentation.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlc_IndexFragmentation" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pIndexFragmentation, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pIndexFragmentation 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "IndexFragmentation_" & pIndexFragmentation.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "IndexFragmentation #" & pIndexFragmentation.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlc_IndexFragmentation", pIndexFragmentation, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvIndexFragmentation.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvIndexFragmentation.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _IndexFragmentationCol.Count Then 
        Dim pIndexFragmentation As csIndexFragmentation = _IndexFragmentationCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pIndexFragmentation.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvIndexFragmentation.SelectedRows(0) 
