Public Class ctlccProductPriceCol
  ...
  Public Event evtUpdated(ByVal vProductPrice As clsProductPrice) 
  Private Event evtBeforeDelete(ByVal vProductPrice As clsProductPrice, ByRef rCancel As Nullable(Of Boolean)) 
  
  Public Event evtRowClicked(ByVal vProductPrice As clsProductPrice) 
  Public Event evtRowDoubleClicked(ByVal vProductPrice As clsProductPrice, ByRef rHandled As Boolean) 
  Public Event evtUnChosen() 
 
  Public Event evtOverrideLoadCbo(ByVal vParentName As clsProductPrice.enmParentProperty, ByRef rComboListTypeToLoad As clsEnums.enmComboListType, ByRef rParentID As Long, ByRef rComboList As clsComboList, ByRef rPrompt As String) 
  ...
  Public Class clsLoadParameters 
    ...
    Public Property DoNotSummarizeProperties As List(Of clsProductPrice.enmSummarizeableProperty) 
    Public Property SpreadsheetShowAllFields As Nullable(Of Boolean) 
    Public Property GridTitle As String 
    Public Property ReportTitle As String 
    Public Property [ReadOnly] As Boolean 
    Public Property CbosDoNotLoad As List(Of clsProductPrice.enmParentProperty) 
    Public Property ColumnsReadOnly As List(Of clsProductPrice.enmProperty) 
    Public Property ColumnsHide As List(Of clsProductPrice.enmProperty) 
    Public Property ColumnsFormat As Dictionary(Of clsProductPrice.enmProperty, String) 
    Public Property ColumnsOrdinalPosition As Dictionary(Of clsProductPrice.enmProperty, Integer) 
    Public Property ColumnsAlignment As Dictionary(Of clsProductPrice.enmProperty, DataGridViewContentAlignment) 
    Public Property ColumnsHeaderText As Dictionary(Of clsProductPrice.enmProperty, String) 
    ...
    Public Sub New() 
      _SummarizeGrid = True 
      _DoNotSummarizeProperties = New List(Of clsProductPrice.enmSummarizeableProperty) 
      _SpreadsheetShowAllFields = Nothing 
      _GridTitle = "" 
      _ReportTitle = "" 
      _ReadOnly = False 
      _CbosDoNotLoad = New List(Of clsProductPrice.enmParentProperty) 
      _ColumnsReadOnly = New List(Of clsProductPrice.enmProperty) 
      _ColumnsHide = New List(Of clsProductPrice.enmProperty) 
      _ColumnsFormat = New Dictionary(Of clsProductPrice.enmProperty, String) 
      _ColumnsOrdinalPosition = New Dictionary(Of clsProductPrice.enmProperty, Integer) 
      _ColumnsAlignment = New Dictionary(Of clsProductPrice.enmProperty, DataGridViewContentAlignment) 
      _ColumnsHeaderText = New Dictionary(Of clsProductPrice.enmProperty, String) 
  ...
  Private WithEvents _ProductPriceCol As clsProductPriceCol
  Private WithEvents _ProductPriceColFullLength As clsProductPriceCol
  ...
  'Properties 
  Public ReadOnly Property [SelectedProductPrice]() As clsProductPrice 
    Get 
      If dgvProductPrice.SelectedRows.Count = 0 OrElse _Loading = True Then Return Nothing 
      Dim RowIndex As Integer = dgvProductPrice.SelectedRows(0).Cells(0).RowIndex 
      If RowIndex < 0 Then Return Nothing 
      If _Summarized = True AndAlso RowIndex = dgvProductPrice.Rows.Count - 1 Then dgvProductPrice.ClearSelection() : RaiseEvent evtUnChosen() : Return Nothing 
      Return _ProductPriceCol(RowIndex) 
    End Get 
  End Property 
  
  Public ReadOnly Property [ProductPriceCol]() As clsProductPriceCol 
    Get 
      Return _ProductPriceCol 
    End Get 
  End Property 
 
  Public Function LoadControl(ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    Dim pProductPriceCol As New clsProductPriceCol(clsEnums.enmLoadParent.TextOnly) 
    Dim pFault As clsFault 
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    pFault = pProductPriceCol.Fill(_Requester) 
    If pFault.isOK = False Then Return pFault 
 
    pFault = LoadControl(pProductPriceCol)
    Return pFault 
  End Function 
  ...
  Public Function LoadControl(ByVal vUniqueCode As Object, ByVal vParentObjectType As String, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
    ...
    Dim pProductPriceCol As New clsProductPriceCol(clsEnums.enmLoadParent.EntireObject) 
    Dim pFault As clsFault 
    ...
    If pDoIt Then 
      Select Case vParentObjectType 
        Case "Product" 
          pFault = pProductPriceCol.FillByProductID(ccHelper.ToLong(vUniqueCode), _Requester) 
        Case Else 
  ...
  Public Function LoadControl(vProductPriceCol As clsProductPriceCol, ByVal vLoadParameters As clsLoadParameters, ByVal vRequester As clsRequester) As clsFault
 
    _Requester = vRequester 
    _LoadParameters = vLoadParameters 
 
    Return LoadControl(vProductPriceCol) 
  End Function
  
  Private Function LoadControl(vProductPriceCol As clsProductPriceCol) As clsFault
    ...
    'keep safe in case 
    Dim pColumnsHides As List(Of clsProductPrice.enmProperty) = Nothing 
    If _LoadParameters.IsSumFillOnTheFly Then 
      pColumnsHides = New List(Of clsProductPrice.enmProperty) 
    ...
    For Each l In _LoadParameters.ColumnsReadOnly 
      If l = clsProductPrice.enmProperty.ID Then colID.ReadOnly = True 
      If l = clsProductPrice.enmProperty.Product Then colProduct.ReadOnly = True 
      If l = clsProductPrice.enmProperty.CustomerType Then colCustomerType.ReadOnly = True 
      If l = clsProductPrice.enmProperty.SellingPrice Then colSellingPrice.ReadOnly = True 
      If l = clsProductPrice.enmProperty.MinQuantity Then colMinQuantity.ReadOnly = True 
      If l = clsProductPrice.enmProperty.DiscountPercent Then colDiscountPercent.ReadOnly = True 
      If l = clsProductPrice.enmProperty.Notes Then colNotes.ReadOnly = True 
    Next 
 
    For Each l In _LoadParameters.ColumnsHide 
      'Parents only 
      Dim pParentProperty As clsProductPrice.enmParentProperty = clsProductPrice.enmParentProperty.UD 
      Dim pSuccess As Boolean = [Enum].TryParse(Of clsProductPrice.enmParentProperty)(l.ToString(), ignoreCase:=False, pParentProperty) 
    ...
    dgvProductPrice.DoubleBuffered(True) 
 
    pFault = vProductPriceCol.LoadLookupAndEnumText(_Requester) : If Not pFault.isOK Then Return pFault 
    
    'Now transfer to local collection 
    _ProductPriceColFullLength = vProductPriceCol.Clone() 
 
    'Truncate the strings 
    _ProductPriceCol = vProductPriceCol 
    If _LoadParameters.TruncateStrings Then 
      _ProductPriceCol.TruncateStrings() 
    Else 
      dgvProductPrice.DefaultCellStyle.WrapMode = DataGridViewTriState.True 
      dgvProductPrice.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCellsExceptHeaders 
    End If 
    ...
    If sReadOnlyHandled = False Then 
      If _LoadParameters.ReadOnly = True Then 
        ...
        If colProduct.DataGridView IsNot Nothing Then dgvProductPrice.Columns.Remove(colProduct) 
      Else 
        If colProduct.ReadOnly = False Then 
          If colProductText.DataGridView IsNot Nothing Then dgvProductPrice.Columns.Remove(colProductText) 
        Else 
          colProduct.Name = colProduct.Name & "zzzz" 
          colProductText.Name = colProduct.Name.Replace("zzzz", "") 
          If colProduct.DataGridView IsNot Nothing Then dgvProductPrice.Columns.Remove(colProduct) 
          If Not _LoadParameters.CbosDoNotLoad.Contains(clsProductPrice.enmParentProperty.Product) Then 
            _LoadParameters.CbosDoNotLoad.Add(clsProductPrice.enmParentProperty.Product) 
          End If 
        End If 
      End If 
    ...
    Next 
    For Each lCol As DataGridViewColumn In dgvProductPrice.Columns 
    ...
    'Hide columns  
    For Each p As clsProductPrice.enmProperty In _LoadParameters.ColumnsHide 
    ...
    For Each pD In _LoadParameters.ColumnsHeaderText 
      dgvProductPrice.Columns("col" & pD.Key.ToString).HeaderText = pD.Value 
    Next 
 
    'Format Columns 
    For Each pD In _LoadParameters.ColumnsFormat 
      dgvProductPrice.Columns("col" & pD.Key.ToString).DefaultCellStyle.Format = pD.Value 
    Next 
 
    'ordinal position 
    For Each pD In _LoadParameters.ColumnsOrdinalPosition 
      dgvProductPrice.Columns("col" & pD.Key.ToString).DisplayIndex = pD.Value 
    Next 
    _GridSettings.Update(Me, _Requester)
    
    'Align Columns 
    For Each pD In _LoadParameters.ColumnsAlignment 
      dgvProductPrice.Columns("col" & pD.Key.ToString).DefaultCellStyle.Alignment = pD.Value 
    Next 
    ...
    dgvProductPrice.ClearSelection()
    bsCtlProductPrice.DataSource = Nothing 
    ...
    Catch ex As Exception
      Return pFault.LogException(ex, "LoadGrid", "TRGT-ProductPrice-090124-2345", _Requester) 
    End Try
    ...
    lblStatus.Text = dgvProductPrice.RowCount & " rows" 
    ...
    If _SortList IsNot Nothing Then 
      ...
      For Each i In _SortList 
        ...
        dgvProductPrice_ColumnHeaderMouseClick(Me, pE) 
      Next 
      _AutoSorting = False 
    End If 
  ...
  Private Sub LoadGrid()
    ...
    If dgvProductPrice.SelectedRows.Count > 0 Then 
      pRowIndex = dgvProductPrice.SelectedRows(0).Cells(0).RowIndex 
    Else 
      If dgvProductPrice.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
        pRowIndex = dgvProductPrice.CurrentCellAddress.Y 
      End If 
    End If 
    ...
    bsCtlProductPrice.DataSource = Nothing 
    bsCtlProductPrice.DataSource = _ProductPriceCol
    ...
    For Each p As ToolStripMenuItem In btnColumns.DropDownItems 
      ...
      p.Checked = dgvProductPrice.Columns("col" & pMenuItemProprty).Visible 
    Next 
 
    If pRowIndex >= 0 Then 
      If _Summarized = True Then 
        If pRowIndex <= _ProductPriceCol.Count - 2 Then 
          dgvProductPrice.Rows(pRowIndex).Selected = True 
        End If 
      Else 
        If pRowIndex <= _ProductPriceCol.Count - 1 Then 
          dgvProductPrice.Rows(pRowIndex).Selected = True 
        End If 
      End If 
    End If 
  ...
  Private Function LoadSupportingCombos() As clsFault
    ...
    'Load comboLists 
    'Product
    If _LoadParameters.ReadOnly = False AndAlso _LoadParameters.CbosDoNotLoad.Find(Function(p) p = clsProductPrice.enmParentProperty.Product) = clsProductPrice.enmParentProperty.UD Then 
      'enable using an external list if needed 
      pComboList = Nothing 
      pPrompt = "" 
      Dim pComboListTypeToLoad As clsEnums.enmComboListType = clsEnums.enmComboListType.ccProductDefaultByID 
      Dim pParentID As Long = 0 
      RaiseEvent evtOverrideLoadCbo(clsProductPrice.enmParentProperty.Product, pComboListTypeToLoad, pParentID, pComboList, pPrompt) 
      If pComboList Is Nothing Then 
        ...
        If MyCache.GetLevel(pComboListTypeToLoad) = Cache.enmLevel.AlwaysPageFromServer Then 
          Return pFault.LogFreeTextFault($"In {Me.Name}, {pComboListTypeToLoad.FastToString()} is defined as AlwaysPageFromServer. Either change it to AlwaysCache in evtBeforeLoad, make the column read-only, or make this grid read-only", "", "TRGT-ProductPriceCol-200806-1015", _Requester) 
        End If 
      End If 
    ...
    pPrompt = "" 
    pEnumCol = Nothing 
    RaiseEvent evtOverrideLoadCbo(clsProductPrice.enmParentProperty.CustomerType, Nothing, Nothing, pEnumCol, pPrompt) 
  ...
  Private Sub SetUpBNButtons(ByVal vInEdit As Boolean)
    ...
    Else 
      ...
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_ProductPriceUpdate, _Requester) = True AndAlso _LoadParameters.ImportButtonHide = False Then btnImport.Visible = vInEdit Else btnImport.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_ProductPriceUpdate, _Requester) = True Then btnAdd.Visible = vInEdit Else btnAdd.Visible = False 
      If ccSecurity.GetPermissionForUI(clsEnums.enmProcess.tbl_ProductPriceDelete, _Requester) = True Then btnDelete.Visible = vInEdit Else btnDelete.Visible = False 
    ...
    If vInEdit = True AndAlso _LoadParameters.AddEditDeleteButtonsHide = False Then 
      colID.ReadOnly = True 
      dgvProductPrice.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
      dgvProductPrice.SelectionMode = DataGridViewSelectionMode.CellSelect 
      _DVGDirty = False 
    Else 
      dgvProductPrice.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvProductPrice.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvProductPrice.AllowUserToDeleteRows = False 
      dgvProductPrice.AllowUserToAddRows = False 
      'Don't automatically set the 1st one If dgvProductPrice.Rows.Count > 0 Then 
      '  Dim pCurrentRow As Integer 
      '  pCurrentRow = dgvProductPrice.CurrentRow.Index 
      '  dgvProductPrice.CurrentCell = dgvProductPrice.Rows(pCurrentRow).Cells(0) 
      '  dgvProductPrice.Rows(pCurrentRow).Selected = True 
      'End If 
    End If 
    ...
    Else 
      If _ProductPriceCol.Count = 0 Then 
        btnSpreadsheet.Enabled = False 
        btnReport.Enabled = False 
      Else 
  ...
  Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click 
    ...
    'Now Load the data 
    Dim pIncomingProductPrices As New clsProductPriceCol(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
    ...
    Using pReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(ofd.FileName, System.Text.Encoding.Unicode) 
      ...
      While Not pReader.EndOfData 
        ...
        Try 
          Dim pIncomingProductPrice As New clsProductPrice(vWithParents:=clsEnums.enmLoadParent.DoNotLoad) 
          pIncomingProductPrice.Tag = "Row " & pRow.ToString 
          pCurrentRow = pReader.ReadFields() 
          If pRow = 0 Then 
            If pCurrentRow.Length = pNumberOfFields - 1 Then 
              pNoPrimaryKey = True 
              pFault = clsProductPriceCol.Delete(_Requester) : If Not pFault.isOK Then ShowFault(pFault, _Requester) : Return 
          ...
          If pCurrentRow.Length <> pNumberOfFields Then 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ": There should be " & pNumberOfFields & " fields, but there are actually " & pCurrentRow.Length & " fields." 
            pIncomingProductPrices.Add(pIncomingProductPrice) 
            Continue While 
          End If 
          ...
          If pNoPrimaryKey = False Then 
            Try 
              pFieldNo += 1 
              pFieldName = "ID" 
              pIncomingProductPrice.ID = CType(pCurrentRow(pFieldNo), Long) 
            Catch ex As Exception 
              pErrorFound = True 
              pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
            End Try 
          End If 
          ...
          Try 
            ...
            pIncomingProductPrice.ProductID = CType(pCurrentRow(pFieldNo), Long) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingProductPrice.CustomerType = clsEnums.TranslateEnmCustomerType(pCurrentRow(pFieldNo)) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingProductPrice.SellingPrice = CType(pCurrentRow(pFieldNo), Decimal) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingProductPrice.MinQuantity = CType(pCurrentRow(pFieldNo), Integer) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            ...
            pIncomingProductPrice.DiscountPercent = CType(pCurrentRow(pFieldNo), Decimal) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
          ...
          Try 
            pFieldNo += 1 
            pFieldName = "Notes" 
            pIncomingProductPrice.Notes = CType(pCurrentRow(pFieldNo), String) 
          Catch ex As Exception 
            pErrorFound = True 
            pIncomingProductPrice.Tag &= ccHelper.NewLine & "Field: " & pFieldName & "; Value: " & pCurrentRow(pFieldNo) & "; Problem: " & ex.Message 
          End Try 
 
          If pIncomingProductPrice.Tag = "Row " & pRow.ToString Then 
            pIncomingProductPrice.Tag &= ": OK" 
          End If 
      ...
      If pErrorFound = True Then 
        Cursor = Cursors.Default 
        Try 
          My.Computer.FileSystem.WriteAllText(pFilenameOut, pIncomingProductPrices.ToCSV, False) 
        Catch ex As Exception 
    ...
    pFault = pIncomingProductPrices.UpdateFromCollection(_Requester) 
    ...
    pErrorFound = False 
    For Each p In pIncomingProductPrices 
      If p.Tag <> "OK" Then 
        pErrorFound = True 
      End If 
    Next 
    If pErrorFound = True Then 
      ...
      Exit Sub 
    Else 
      _ProductPriceCol = pIncomingProductPrices 
      LoadGrid() 
  ...
  Private Sub DoEdit() 
    ...
    If dgvProductPrice.Focused = True AndAlso dgvProductPrice.SelectedRows.Count > 0 Then 
      pCellRow = dgvProductPrice.CurrentCell.RowIndex 
      pCellCol = dgvProductPrice.CurrentCell.ColumnIndex 
    End If 
 
    Try 'in case it's empty 
      dgvProductPrice.CurrentCell = dgvProductPrice.Rows(0).Cells(0) 
      dgvProductPrice.CurrentCell.Selected = True 
    Catch ex As Exception 
    End Try 
 
 
    'remove summary row 
    If _LoadParameters.SummarizeGrid = True AndAlso _ProductPriceCol.Count > 0 AndAlso _ProductPriceCol(_ProductPriceCol.Count - 1).ID = 0 Then 
      _ProductPriceCol.RemoveAt(_ProductPriceCol.Count - 1) 
      bsCtlProductPrice.DataSource = Nothing 
      bsCtlProductPrice.DataSource = _ProductPriceCol 
      _Summarized = False 
    End If 
    ...
    If pCellRow >= 0 AndAlso pCellCol >= 0 Then 
      dgvProductPrice.Focus() 
      dgvProductPrice.CurrentCell = dgvProductPrice.Rows(pCellRow).Cells(pCellCol) 
      dgvProductPrice.CurrentCell.Selected = True 
    ElseIf _ProductPriceCol.Count = 0 Then 
    Else 
  ...
  Private Sub DoAdd() 
    ...
    bsCtlProductPrice.AddNew() 
 
    'Now choose any needed fields 
    Dim pEntity As clsProductPrice 
    pEntity = CType(bsCtlProductPrice.Current, clsProductPrice) 
  ...
  Private Function DoDelete() As clsFault 
    ...
    If dgvProductPrice.CurrentCell Is Nothing Then Return pFault 
    
    If dgvProductPrice.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      Dim pProductPrice As clsProductPrice 
      pProductPrice = CType(bsCtlProductPrice.Current, clsProductPrice) 
      If pProductPrice Is Nothing Then 
        pFault.LogFreeTextFault("There is no ProductPrice to delete", "", "TRGT-110303-165408", _Requester) 
        Return pFault 
      End If 
      Dim pOriginalCol As Integer = dgvProductPrice.CurrentCell.ColumnIndex 
      Dim pOriginalRow As Integer = dgvProductPrice.CurrentCell.RowIndex 
      'show row as selected  
      dgvProductPrice.SelectionMode = DataGridViewSelectionMode.FullRowSelect 
      dgvProductPrice.EditMode = DataGridViewEditMode.EditProgrammatically 
      dgvProductPrice.CurrentRow.Selected = True 
      If pProductPrice.ID > 0 Then 
        Dim pRequest As String = "Are you sure you want to delete '" & pProductPrice.ProductID.ToString() & " - " & pProductPrice.CustomerType.FastToString() & ": ?" & pProductPrice.SellingPrice.ToString() & "'?" 
        Dim pCancel As Nullable(Of Boolean) = Nothing 
        RaiseEvent evtBeforeDelete(pProductPrice, pCancel) 
        ...
        ElseIf pCancel Is Nothing Then 
          ...
          If pResponse = frmMessageOrInputBox.enmButtonReturned.No Then 
            dgvProductPrice.SelectionMode = DataGridViewSelectionMode.CellSelect 
            dgvProductPrice.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 
            dgvProductPrice.Rows(pOriginalRow).Cells(pOriginalCol).Selected = True 
            Return pFault 
          End If 
        End If 
        pFault = pProductPrice.Delete(_Requester) : If pFault.isOK = False Then Return pFault 
      End If 
      bsCtlProductPrice.Remove(bsCtlProductPrice.Current) 
      LoadGrid() 
    End If 
    Return pFault 
  End Function 
  Private Sub DoCeaseEdit() 
    ...
    If _DVGDirty = True Then 
      ...
      Exit Sub 
    End If 
    Dim pProductPrice As clsProductPrice = CType(bsCtlProductPrice.Current, clsProductPrice) 
    If pProductPrice IsNot Nothing Then 
      If pProductPrice.ID = 0 Then 
    ...
    End If 
    SetUpBNButtons(False) 
    If _ProductPriceCol.Count > 0 AndAlso dgvProductPrice.CurrentCell IsNot Nothing Then 
      For i As Integer = 0 To dgvProductPrice.Columns.Count - 1 
        If dgvProductPrice.Columns(i).Visible Then 
          dgvProductPrice.CurrentCell = dgvProductPrice.Rows(dgvProductPrice.CurrentCell.RowIndex).Cells(i) 
          Exit For 
        End If 
      Next 
      dgvProductPrice.Refresh() 
      dgvProductPrice.Rows(dgvProductPrice.CurrentCell.RowIndex).Selected = True 
    Else 
      dgvProductPrice.Refresh() 
    End If 
  End Sub 
  'Grid RowValidating 
  Private Sub dgvProductPrice_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles dgvProductPrice.RowValidating 
    If _Loading = True OrElse dgvProductPrice.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
    Dim pCancel As Boolean 
    pCancel = UpdateRow() 
    If pCancel = True Then 
      e.Cancel = True 
      dgvProductPrice.CurrentCell = dgvProductPrice.Rows(e.RowIndex).Cells(e.ColumnIndex) 
    End If 
  End Sub 
  'CellFormatting  
  Private Sub dgvProductPrice_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvProductPrice.CellFormatting 
    '_Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
 
    If dgvProductPrice.EditMode = DataGridViewEditMode.EditOnKeystrokeOrF2 Then 
      If dgvProductPrice.Columns(e.ColumnIndex).ReadOnly = False Then 
        Exit Sub 
      End If 
    End If 
    ...
    'Dim pProductPrice As clsProductPrice = Nothing 
    'If dgvProductPrice.Columns(e.ColumnIndex).Name = colRecommendedQuantityToOrder.Name Then 
    '  If pProductPrice Is Nothing Then pProductPrice = CType(dgvProductPrice.Rows(e.RowIndex).DataBoundItem, clsProductPrice) ' Only assign it if needed 
    '  If pProductPrice.CustomerOrders > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pProductPrice.CustomerOrders > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
    'If dgvProductPrice.Columns(e.ColumnIndex).Name = colRAV.Name Then 
    '  If pProductPrice Is Nothing Then pProductPrice = CType(dgvProductPrice.Rows(e.RowIndex).DataBoundItem, clsProductPrice) ' Only assign it if needed
    '  If pProductPrice.RAV > 10 Then 
    '    e.CellStyle.ForeColor = Color.Red 
    '    If pProductPrice.RAV - pProductPrice.MaximumStock > 100 Then 
    '      e.CellStyle.BackColor = Color.LightYellow 
    '    End If 
    '  End If 
    'End If 
 
    'Debug.Print("loc x,y:" & e.RowIndex & ", " & e.ColumnIndex & ": GetType" & dgvProductPrice.Columns(e.ColumnIndex).GetType.ToString & ": zValue" & e.Value.ToString) 
    If dgvProductPrice.Columns(e.ColumnIndex).GetType.ToString = "System.Windows.Forms.DataGridViewComboBoxColumn" Then 
      Dim pCol As System.Windows.Forms.DataGridViewComboBoxColumn = CType(dgvProductPrice.Columns(e.ColumnIndex), System.Windows.Forms.DataGridViewComboBoxColumn) 
      Dim pTag As String = "" 
      ...
      If pTag <> "" Then 
        If e.Value Is Nothing Then 
          'e.Value = "## INVALID ##" 
          e.Value = "* BadCode '" & dgvProductPrice.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString() & "' *" 
    ...
    If dgvProductPrice.Columns(e.ColumnIndex).GetType.Name.Equals("DataGridViewImageColumn", StringComparison.OrdinalIgnoreCase) Then 
      If e.Value Is Nothing Then 
    ...
    If _Summarized = True Then 
      If e.RowIndex = dgvProductPrice.Rows.Count - 1 Then 
        If e.Value IsNot Nothing Then 
          ...
          ElseIf e.Value.ToString = "0" Then 
            e.Value = "" 
          Else 
            If _SummaryOverFlow.IndexOf(dgvProductPrice.Columns(e.ColumnIndex).Name.Substring(3)) >= 0 Then 
              e.Value = "* OVERFLOW *" 
            End If 
          End If 
        Else 
    ...
    End If 
  End Sub 
  Private Sub dgvProductPrice_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvProductPrice.CellValueChanged 
    If e.RowIndex < 0 Then Exit Sub 
 
  End Sub 
 
  'Grid Sort
  Private Sub dgvProductPrice_ColumnHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvProductPrice.ColumnHeaderMouseClick
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Dim pNewColumn As DataGridViewColumn = dgvProductPrice.Columns(e.ColumnIndex)
    If bsCtlProductPrice.Current Is Nothing Then Exit Sub
    ...
    Dim pProductPriceCol As clsProductPriceCol
    pProductPriceCol = CType(bsCtlProductPrice.DataSource, clsProductPriceCol)

    Dim pSummaryRow As clsProductPrice = Nothing 
    If _Summarized = True Then 
      pSummaryRow = pProductPriceCol(pProductPriceCol.Count - 1) 
      pProductPriceCol.RemoveAt(pProductPriceCol.Count - 1) 
    End If 
    ...
    If pNewSortOrder = SortOrder.Ascending Then
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
        ...
        For iCntr As Integer = 0 To pProductPriceCol.Count - 1 
          pProductPriceCol(iCntr).Tag = iCntr.ToString("0000000000") 
        Next 
      End If 
      If pNewColumn Is colID Then
        pProductPriceCol.SortByID()
      ElseIf pNewColumn Is colProduct OrElse pNewColumn Is colProductText Then
        pProductPriceCol.SortByProductText()
      ElseIf pNewColumn Is colCustomerType Then
        pProductPriceCol.SortByCustomerType()
      ElseIf pNewColumn Is colSellingPrice Then
        pProductPriceCol.SortBySellingPrice()
      ElseIf pNewColumn Is colMinQuantity Then
        pProductPriceCol.SortByMinQuantity()
      ElseIf pNewColumn Is colDiscountPercent Then
        pProductPriceCol.SortByDiscountPercent()
      ElseIf pNewColumn Is colNotes Then
        pProductPriceCol.SortByNotes()
      End If
      If _PrevSortColumn IsNot Nothing AndAlso _PrevSortColumn IsNot pNewColumn Then 
      Dim iCntr As Integer = 0 
        If pNewColumn Is colID Then
          Dim pTest As Long = 0 
          For Each p As clsProductPrice In pProductPriceCol 
        ...
        ElseIf pNewColumn Is colNotes Then
          ...
          Next 
        End If 
        pProductPriceCol.SortByTag() 
      End If 
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Ascending
      _PrevSortColumn = pNewColumn
      pPrevSortOrder = SortOrder.Ascending
    Else
      pProductPriceCol.Reverse()
      pNewColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending
      pPrevSortOrder = SortOrder.Descending
    End If

    If _Summarized = True Then 
      pProductPriceCol.Add(pSummaryRow) 
    End If 
 
    If pID > 0 Then
      bsCtlProductPrice.Position = bsCtlProductPrice.IndexOf(pProductPriceCol.FindByID(pID))
    End If

    'dgvProductPrice.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
    dgvProductPrice.ResumeLayout()
  ...
  Public Sub SelectRowByObjectID(ByVal pID As Long) 
    If pID > 0 Then 
      Dim pProductPriceCol As clsProductPriceCol 
      pProductPriceCol = CType(bsCtlProductPrice.DataSource, clsProductPriceCol) 
      Dim pProductPrice As clsProductPrice = pProductPriceCol.FindByID(pID) 
      If Not pProductPrice.IsEmpty Then 
        bsCtlProductPrice.Position = bsCtlProductPrice.IndexOf(pProductPriceCol.FindByID(pID)) 
        dgvProductPrice.Rows(bsCtlProductPrice.Position).Selected = True 
      Else 
        dgvProductPrice.ClearSelection() 
      End If 
    ElseIf pID = 0 Then 
  ...
  'Grid Resize
  Private Sub dgvProductPrice_ColumnHeaderMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvProductPrice.ColumnHeaderMouseDoubleClick
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    Cursor = Cursors.WaitCursor
    dgvProductPrice.AutoResizeColumn(e.ColumnIndex)
    Cursor = Cursors.Default
  End Sub
  'Other Grid Events
  Private Sub dgvProductPrice_CurrentCellDirtyStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvProductPrice.CurrentCellDirtyStateChanged
   _DVGDirty = True 
  End Sub
  Private Sub dgvProductPrice_Scroll(sender As Object, e As ScrollEventArgs) Handles dgvProductPrice.Scroll
    dgvProductPrice.Invalidate() 
  End Sub
 
  Private Sub dgvProductPrice_DataFault(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles dgvProductPrice.DataError
    'Dim pFault As New clsFault
    '
    'If e.RowIndex = dgvProductPrice.Rows.Count - 1 Then Exit Sub
 
    'If dgvProductPrice.Columns(e.ColumnIndex).Name.StartsWith("colIDin", StringComparison.OrdinalIgnoreCase) Then Exit Sub 
    ...
    'Try 
    '  Try 
    '    pSubStrg.AppendLine("In table 'ProductPrice', the row with an ID of " & dgvProductPrice.Rows(e.RowIndex).Cells(0).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine("In grid 'ProductPrice', row index " & e.RowIndex) 
    '  End Try 
    '  Try 
    '    pSubStrg.AppendLine(" has an invalid value of " & dgvProductPrice.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString)
    '  Catch ex As Exception 
    '    pSubStrg.AppendLine(" has an invalid value of Nothing.") 
    '  End Try 
    '  pSubStrg.AppendLine(" in column " & dgvProductPrice.Columns(e.ColumnIndex).DataPropertyName) 
    'Catch ex As Exception 
    '  pSubStrg.AppendLine("; Failed trying to fill DataFault as well!") 
    'End Try 
    'pFault.LogException(209, e.Exception, pSubStrg.ToString, "TRGT-ProductPrice-100409-2248", _Requester) 
    'If pShown = False Then 
    '  Dim pCell As DataGridViewCell 
    '  Try 
    '    pCell = dgvProductPrice(e.ColumnIndex, e.RowIndex)
    '  Catch ex As Exception 
    '    pCell = dgvProductPrice(0, 0)
    '  End Try 
    '  ShowFault(pFault, _Requester) 
    '  pShown = True 
    'End If 
  End Sub
  Private Sub dgvProductPrice_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvProductPrice.KeyDown 
    If e.KeyCode = Keys.Escape Then 
      ...
      End If 
      SetUpBNButtons(False) 
      'dgvProductPrice.CurrentCell = dgvProductPrice.Rows(dgvProductPrice.CurrentCell.RowIndex).Cells(0) 
      dgvProductPrice.Refresh() 
    End If 
  End Sub 
  Private Sub dgvProductPrice_ColumnWidthChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvProductPrice.ColumnWidthChanged
    If Me.DesignMode = True Then Exit Sub 
    If _Loading = False Then SaveSizes()
  End Sub
  Private Sub dgvProductPrice_ColumnDisplayIndexChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewColumnEventArgs) Handles dgvProductPrice.ColumnDisplayIndexChanged
    Cursor = Cursors.WaitCursor
    If _Loading = False Then SaveSizes()
    Cursor = Cursors.Default
  End Sub
  Private Sub dgvProductPrice_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvProductPrice.CellDoubleClick 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    If e.RowIndex < 0 Then Exit Sub 
    If _Summarized = True AndAlso e.RowIndex = dgvProductPrice.Rows.Count - 1 Then Exit Sub 
    Cursor = Cursors.WaitCursor 
    Dim pCell As DataGridViewCell = dgvProductPrice(e.ColumnIndex, e.RowIndex) 
 
    Dim pHandled As Boolean = False 
    Dim pProductPrice As clsProductPrice = _ProductPriceCol(e.RowIndex)
    RaiseEvent evtRowDoubleClicked(pProductPrice, pHandled) 
    Cursor = Cursors.Default 
 
    If pHandled = False Then 
      If Me.ParentForm.Name.Equals("frmPopup", StringComparison.OrdinalIgnoreCase) Then Return 
      frmPopup.Text = "ProductPrice Detail" 
      Dim pFault As clsFault = frmPopup.LoadControl("ctlccProductPrice", pProductPrice, _Requester) 
      If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
      frmPopup.ShowDialog() 
    End If 
 
  End Sub 
  Private Sub dgvProductPrice_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvProductPrice.SelectionChanged 
    If btnCeaseEdit.Visible = True Then Exit Sub 
    _Requester.CallingFunctionWithinApplication = System.Reflection.MethodInfo.GetCurrentMethod().DeclaringType.Name & ":" & System.Reflection.MethodInfo.GetCurrentMethod().Name 
    ...
    If _Summarized = True AndAlso RowIndex = dgvProductPrice.Rows.Count - 1 Then dgvProductPrice.ClearSelection() : RaiseEvent evtUnChosen() : Exit Sub 
    Dim pProductPrice As clsProductPrice = _ProductPriceCol(RowIndex)
    _ProcessingSelection = True
    Try
      RaiseEvent evtRowClicked(pProductPrice) 
    Finally
      _IgnoreSelectionUntil = DateTime.Now.AddMilliseconds(500)
      _ProcessingSelection = False
    End Try
  End Sub 
  Private Sub dgvProductPrice_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvProductPrice.RowLeave 
    colID.ReadOnly = True 
  End Sub 
  ...
  Private Function UpdateRow() As Boolean 
    ...
    If pOriginalCol > 0 Then 
      ...
      Do Until dgvProductPrice.Columns(pNewCol).Visible = True OrElse pNewCol = 0 
        pNewCol = pNewCol - 1 
      Loop 
    Else 
      pNewCol = 1 
    End If 
    If dgvProductPrice.Columns(pNewCol).Visible = False Then 
      dgvProductPrice.Columns(pNewCol).Visible = True 
      dgvProductPrice.CurrentCell = dgvProductPrice.CurrentRow.Cells(pNewCol) 
      dgvProductPrice.CurrentCell = dgvProductPrice.CurrentRow.Cells(pOriginalCol) 
      dgvProductPrice.Columns(pNewCol).Visible = False 
    Else 
      dgvProductPrice.CurrentCell = dgvProductPrice.CurrentRow.Cells(pNewCol) 
      dgvProductPrice.CurrentCell = dgvProductPrice.CurrentRow.Cells(pOriginalCol) 
    End If 
    ...
    RaiseEvent evtBeforeUpdate(CType(pProductPrice, clsProductPrice), pCancel) 
    If pCancel = True Then 
      _DVGDirty = False 
      RaiseEvent evtTimerTripped() 
      Return True 
    End If 
    pFault = pProductPrice.Update(_Requester) 
    ...
    Else 
      If pFault.isOK = False Then 'AndAlso pFault.Severity = clsEnums.enmFaultSeverity.LogOnly  
        ShowFault(pFault, _Requester) 
      End If 
      dgvProductPrice.EndEdit() 
      _DVGDirty = False 
      'Reset the ProductPrice collection 
      MyCache.ClearComboList(clsEnums.enmComboListType.ccProductPriceDefaultByID) 
      RaiseEvent evtUpdated(pProductPrice) 
      Return False 
    End If 
  End Function 
  Private Sub SaveSizes() 
    ...
    For Each pCol As DataGridViewColumn In dgvProductPrice.Columns 
  ...
  Private Sub Summarize() 
    If _ProductPriceCol.Count <= 1 Then 
      _Summarized = False 
      Exit Sub 
    End If 
    ...
    For Each pExistingRow As clsProductPrice In _ProductPriceCol 
      ...
      End If 
    Next 
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsProductPrice.enmSummarizeableProperty.SellingPrice) = clsProductPrice.enmSummarizeableProperty.SellingPrice Then pSellingPrice = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsProductPrice.enmSummarizeableProperty.MinQuantity) = clsProductPrice.enmSummarizeableProperty.MinQuantity Then pMinQuantity = 0
    If _LoadParameters.DoNotSummarizeProperties.Find(Function(p) p = clsProductPrice.enmSummarizeableProperty.DiscountPercent) = clsProductPrice.enmSummarizeableProperty.DiscountPercent Then pDiscountPercent = 0
    Dim pSummaryRow As New clsProductPrice( _ 
        vID:=0 _ 
      , vProductID:=0 _ 
      , vProductText:="" _ 
    ...
    _ProductPriceCol.Add(pSummaryRow) 
    _Summarized = True 
  End Sub 
  ...
  Friend Function CreateSpreadSheet() As clsFault  
    ...
    Dim pFileNameAllFields As String = $"{pRoot}\ProductPriceCol_{pDateToShow}AllFields.csv" 
    Dim pFileNameFieldsOnGrid As String = $"{pRoot}\ProductPriceCol_{pDateToShow}FieldsOnGrid.csv" 
    Dim pFileNameAllFieldsWithIDs As String = $"{pRoot}\ProductPriceCol_{pDateToShow}AllFieldsWithIDs.csv" 
    Dim pFileNameAllFieldsXML As String = $"{pRoot}\ProductPriceCol_{pDateToShow}AllFields.xml" 
    Dim pFileNameAllFieldsJson As String = $"{pRoot}\ProductPriceCol_{pDateToShow}AllFields.json" 
    ...
    For Each Row As DataGridViewRow In dgvProductPrice.Rows 
      i += 1 
      If _LoadParameters.SummarizeGrid = True Then 
        If Row.Index = dgvProductPrice.Rows.Count - 1 Then Exit For 
      End If 
      If i Mod 500 = 0 Then 
    ...
    Try  
      If pShowAllFields = True Then  
        Dim pStrg As String = "" 
        'xml 
        pFault = _ProductPriceColFullLength.CreateXML(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsXML, pStrg, False) 
        'json 
        pFault = _ProductPriceColFullLength.CreateJSON(pStrg, _Requester) : If pFault.isOK = False Then Return pFault 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsJson, pStrg, False) 
        'default  
        My.Computer.FileSystem.WriteAllText(pFileNameAllFields, _ProductPriceColFullLength.ToCSV, False)  
        'WithIDs  
        'pFault = _ProductPriceColFullLength.LoadLookupAndEnumText(_Requester) : If pFault.isOK = False Then Return pFault (already done) 
        My.Computer.FileSystem.WriteAllText(pFileNameAllFieldsWithIDs, _ProductPriceColFullLength.ToCSV(True), False) 
      End If  
      'default  
      My.Computer.FileSystem.WriteAllText(pFileNameFieldsOnGrid, pCSV.ToString, False)  
      pFault.SetOK()  
    Catch ex As Exception  
      pFault.LogException(ex, "", "TRGT-ProductPrice-090210-1618", _Requester)  
    End Try  
    If pFault.isOK = False Then Return pFault  
  ...
  Private Sub ReportDesign() 
    ...
    Try 
      ...
      With _Report 
        ...
        .SubTitleLeft = "ProductPrices" 
        .SubTitleRight = "Rows: " & _ProductPriceCol.Count.ToString 
        ...
        End If 
        .DataSource = _ProductPriceCol 
        .HasSummaryLine = _Summarized 
      End With 
    Catch ex As Exception 
      Dim pFault As New clsFault 
      pFault.LogException(ex, "", "TRGT-ProductPrice-090210-2119", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  Friend Function CreateReport() As clsFault 
    ...
    Catch ex As Exception 
      pFault = New clsFault 
      pFault.LogException(ex, "", "TRGT-ProductPrice-090211-0746", _Requester) 
    End Try 
    Return pFault 
  End Function 
  ...
  Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged 
    ...
    If String.IsNullOrEmpty(pSearchText) Then 
      For Each pRow As DataGridViewRow In dgvProductPrice.Rows 
    ...
    For Each row As DataGridViewRow In dgvProductPrice.Rows 
      ...
      Next 
      Try 
        Dim pBS As CurrencyManager = CType(Me.BindingContext(bsCtlProductPrice), CurrencyManager) 
    ...
    lblStatus.Text = pVisibleCount & " of " & dgvProductPrice.RowCount & " rows" 
  End Sub 
  ...
  Private Function GetOrInitializeGridSettings() As clsFault 
    ...
    Catch ex As Exception 
      pFault.LogException(ex, "GetOrInitializeGridSettings", "TRGT-ProductPrice-120225-1310", _Requester) 
    End Try 
    If pFault.isOK = False Then Return pFault 
    ...
    Else 
      ...
      For Each pCol As DataGridViewColumn In dgvProductPrice.Columns 
        ...
        If pG.ColumnName = "" Then 
          pG.ColumnDisplayIndex = pCol.DisplayIndex 
          pG.ColumnWidth = ccHelper.ToInteger((dgvProductPrice.Width - 30) / dgvProductPrice.Columns.Count) 
  ...
  Private Sub LoadColumns() 
    ...
    Try 
      For Each lGridSetting In _GridSettings 
        Try 
          ...
          With dgvProductPrice.Columns(lGridSetting.ColumnName) 
    ...
    Catch ex As Exception 'Fault in getting GridSettings  
      pFault.LogException(204, ex, "", "TRGT-ProductPrice-090120-1502", _Requester) : ShowFault(pFault, _Requester) : Exit Sub 
    End Try 
  End Sub 
  ...
  Private Sub mnuColVisible_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColVisibleID.Click, mnuColVisibleProduct.Click, mnuColVisibleCustomerType.Click, mnuColVisibleSellingPrice.Click, mnuColVisibleMinQuantity.Click, mnuColVisibleDiscountPercent.Click, mnuColVisibleNotes.Click
    Cursor = Cursors.WaitCursor 
    Dim pToolStripItem As System.Windows.Forms.ToolStripMenuItem = CType(sender, System.Windows.Forms.ToolStripMenuItem) 
    dgvProductPrice.Columns("col" & pToolStripItem.Name.Substring(13)).Visible = pToolStripItem.Checked 
    If _Loading = False Then SaveSizes() 
    Cursor = Cursors.Default 
  End Sub 
  Private Sub mnuColsReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsReset.Click 
    ...
    pNewWidth = ccHelper.ToInteger((dgvProductPrice.Width - 30) / pVisibleColumns) 
  ...
  Private Sub mnuColsHideMost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuColsHideMost.Click 
    ...
    If mnuColVisibleSellingPrice.Checked = False Then mnuColVisibleSellingPrice.PerformClick() 
    
    _Loading = False 
    'dgvProductPrice.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells) 
  End Sub 
  
  Private Sub dgvProductPrice_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvProductPrice.CellMouseClick 
    If e.Button = MouseButtons.Right Then 
      Dim pMessageBox As frmMessageOrInputBox.enmButtonReturned = frmMessageOrInputBox.ShowMsg("How do you want to copy the ProductPrice to the clipboard?", frmMessageOrInputBox.enmIconType.QuestionMark, frmMessageOrInputBox.enmButtons.YesNoCancel, vYesText:="Text", vNoText:="CSV") 
  ...
  Private Sub dgvProductPrice_MouseDown(sender As Object, e As MouseEventArgs) Handles dgvProductPrice.MouseDown 
    '--- Save anchor on normal click (no modifiers) ---
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And (Keys.Shift Or Keys.Control)) = 0 AndAlso dgvProductPrice.CurrentRow IsNot Nothing Then 
      _SelectionAnchor = dgvProductPrice.CurrentRow.Index 
    End If 
    'This removes on click from the update 
    If dgvProductPrice.EditMode = DataGridViewEditMode.EditProgrammatically Then Exit Sub 
 
    Dim pCell As DataGridView.HitTestInfo = dgvProductPrice.HitTest(e.X, e.Y) 
    ...
    If pCell.Type = DataGridViewHitTestType.Cell Then 
      ...
      Try 
        pCurrentCell = dgvProductPrice(pCell.ColumnIndex, pCell.RowIndex) 
        If pCurrentCell.ReadOnly Then Exit Sub 
        dgvProductPrice.CurrentCell = pCurrentCell 
      Catch ex As Exception 
        Exit Sub  
      End Try 
      ...
      If pCurrentCell.GetType().Name.Equals("DataGridViewComboBoxCell", StringComparison.OrdinalIgnoreCase) Then 
        dgvProductPrice.BeginEdit(True) 
        DirectCast(dgvProductPrice.EditingControl, DataGridViewComboBoxEditingControl).DroppedDown = True 
      End If 
    End If 
  End Sub 
 
  Private Sub dgvProductPrice_MouseUp(sender As Object, e As MouseEventArgs) Handles dgvProductPrice.MouseUp 
    Dim pModifiers = Control.ModifierKeys 
    If (pModifiers And Keys.Shift) = Keys.Shift AndAlso _SelectionAnchor >= 0 Then 
      Dim hit = dgvProductPrice.HitTest(e.X, e.Y) 
      If hit.RowIndex >= 0 Then 
        dgvProductPrice.MultiSelect = True 
        dgvProductPrice.ClearSelection() 
        ...
        For i As Integer = pFrom To pTo 
          dgvProductPrice.Rows(i).Selected = True 
        Next 
      End If 
  ...
  Private Sub ctlccProductPriceCol_FontChanged(sender As Object, e As EventArgs) Handles Me.FontChanged 
    'Set the font for the BN 
        If MyFont Is Nothing Then Return 
    BN.Font = New Font(MyFont.Name, MyFont.Size) 
    dgvProductPrice.RowTemplate.Height = ccHelper.ToInteger(23 * MyFont.Size / 9) 
  End Sub 
 
  Private Sub ctlccProductPriceCol_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged 
    If Me.ParentForm Is Nothing Then Exit Sub 
    Dim pParent As String = Me.ParentForm.Name 
    Dim pResponse As Boolean = Me.Visible 
    Dim pSize As Integer = dgvProductPrice.Width 
  ...
  Private Sub ctlccProductPriceCol_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged 
  ...
  Private Sub ctlccProductPriceCol_Leave(sender As Object, e As EventArgs) Handles Me.Leave 
  ...
  Private Sub dgvProductPrice_CellMouseDown(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvProductPrice.CellMouseDown 
    ReleaseStuckModifierKeys() 'Fix sticky SHIFT/CTRL before selection changes 
    If e.Button = MouseButtons.Right AndAlso e.RowIndex >= 0 Then 
      If Not dgvProductPrice.Rows(e.RowIndex).Selected Then 
        dgvProductPrice.ClearSelection() 
        dgvProductPrice.Rows(e.RowIndex).Selected = True 
      End If 
    End If 
  End Sub 
  ...
  Private Sub cmsGrid_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmsGrid.Opening 
    Dim pCount As Integer = dgvProductPrice.SelectedRows.Count 
  ...
  Private Sub tsmiOpenDetail_Click(sender As Object, e As EventArgs) Handles tsmiOpenDetail.Click 
    If dgvProductPrice.SelectedRows.Count <> 1 Then Exit Sub 
    Dim pRowIndex As Integer = dgvProductPrice.SelectedRows(0).Index 
    If pRowIndex < 0 OrElse pRowIndex >= _ProductPriceCol.Count Then Exit Sub 
    Dim pProductPrice As clsProductPrice = _ProductPriceCol(pRowIndex) 
    Dim pTitle As String = "ProductPrice #" & pProductPrice.ID.ToString() 
    Dim pKey As String = "ProductPrice_" & pProductPrice.ID.ToString() 
    ...
    Dim pCtlName As String = "ctlccProductPrice" 
    ...
    Dim pFault As clsFault = CType(pLoad.Invoke(pControl, New Object() {pProductPrice, _Requester}), clsFault) 
    If Not pFault.isOK Then ShowFault(pFault, _Requester) : Exit Sub 
    'Return to Tab click handler - sends entity to a new tab in frmMain 
    Dim pEntityRef As Object = pProductPrice 
  ...
  Private Sub tsmiOpenInTab_Click(sender As Object, e As EventArgs) Handles tsmiOpenInTab.Click 
    ...
    Dim pWinKey As String = "ProductPrice_" & pProductPrice.ID.ToString() 
    If _openDetailWindows.ContainsKey(pWinKey) Then 
      ...
      End If 
    End If 
    Dim pTabTitle As String = "ProductPrice #" & pProductPrice.ID.ToString() 
    Dim pFault As clsFault = pFrmMain.OpenEntityInNewTab("ctlccProductPrice", pProductPrice, _Requester, pTabTitle) 
  ...
  Private Sub tsmiCopyID_Click(sender As Object, e As EventArgs) Handles tsmiCopyID.Click 
    If dgvProductPrice.SelectedRows.Count = 0 Then Exit Sub 
    Dim pIDs As New System.Text.StringBuilder() 
    For Each pSelectedRow As DataGridViewRow In dgvProductPrice.SelectedRows 
      If pSelectedRow.Index >= 0 AndAlso pSelectedRow.Index < _ProductPriceCol.Count Then 
        Dim pProductPrice As clsProductPrice = _ProductPriceCol(pSelectedRow.Index) 
        If pIDs.Length > 0 Then pIDs.Append(", ") 
        pIDs.Append(pProductPrice.ID.ToString()) 
      End If 
    Next 
  ...
  Private Sub tsmiCopyRowHeaders_Click(sender As Object, e As EventArgs) Handles tsmiCopyRowHeaders.Click 
    ...
    Dim pFirstRow As DataGridViewRow = dgvProductPrice.SelectedRows(0) 
