Public Class clsSupplierOrder
  ...
  Friend Sub CleanEntityForXML() 
    ...
    _SupplierEmail = ccHelper.RemoveChrW0(_SupplierEmail) 
    _EmailSubject = ccHelper.RemoveChrW0(_EmailSubject) 
    _EmailBody = ccHelper.RemoveChrW0(_EmailBody) 
    _RequestedDeliveryDay = ccHelper.RemoveChrW0(_RequestedDeliveryDay) 
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrder_GetByPrimaryKey", vRequester) 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrder_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrder_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = 0 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"SupplierOrder not found for GetByID, since its value is 0", pFunctionParameters, "TRGT-SupplierOrder-210927-1527", vRequester, vAdditionalMessageToUser:=$"SupplierOrder not found") 
      pFault.SetOK() 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      pFault = LoadMeFromDBCache(MyController.DBCache.SupplierOrderCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrderGetByID" 
      ...
      Try 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"SupplierOrder not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-SupplierOrder-210625-0950", vRequester, vAdditionalMessageToUser:=$"SupplierOrder not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function AddUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityAddable.AddUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderUpdate, "clsSupplierOrder_AddUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function EditUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityEditable.EditUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderUpdate, "clsSupplierOrder_EditUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Friend Function UpdateFriend(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderUpdate, "clsSupplierOrder_UpdateFriend", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Dim pSupplierOrder As New clsSupplierOrder(_WithParents) 
    If Me.isEqual(pSupplierOrder) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", pFunctionParameters, "TRGT-SupplierOrder-100113-1638", vRequester) 
    End If 
    ...
    Dim pCommandText As String = "ccSupplierOrderUpdateFriend"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pCachedSupplierOrder As clsSupplierOrder 
      If _ID = 0 Then 
        pCachedSupplierOrder = New clsSupplierOrder(_WithParents) 
        'get last ID 
        Dim pSupplierOrderCol As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.Clone() 
        If pSupplierOrderCol.Count = 0 Then 
          _ID = 1 
        Else 
          pSupplierOrderCol.SortByID() 
          Dim pLastID As Long = pSupplierOrderCol(pSupplierOrderCol.Count - 1).ID
          _ID = pLastID + 1 
        End If 
        bPrimaryKey = _ID 
        MyController.DBCache.SupplierOrderCol.Add(pCachedSupplierOrder) 
      Else  
        pCachedSupplierOrder = MyController.DBCache.SupplierOrderCol.FindByID(_ID) 
      End If 
      pCachedSupplierOrder.AssignValues(Me) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.SupplierOrderCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
        ...
        pLastReadVariableName = "SupplierEmail" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 255).Value = ccHelper.ObjectNullable(_SupplierEmail) 
        pLastReadVariableName = "EmailSubject" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 500).Value = ccHelper.ObjectNullable(_EmailSubject) 
        pLastReadVariableName = "EmailBody" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar).Value = ccHelper.ObjectNullable(_EmailBody) 
        pLastReadVariableName = "enmEmailStatus" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 50).Value = (_EmailStatus.FastToString()) 
        pLastReadVariableName = "SentDate" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(_SentDate) 
        pLastReadVariableName = "blg_TotalCost" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Decimal).Value = (_TotalCost) 
        ...
        pLastReadVariableName = "RequestedDeliveryDate" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Date).Value = ccHelper.DateNullable(_RequestedDeliveryDate) 
        pLastReadVariableName = "RequestedDeliveryDay" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 10).Value = ccHelper.ObjectNullable(_RequestedDeliveryDay) 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            bPrimaryKey = pID 
            If pID = 0 Then 
              pFault.LogFreeTextFault(51, "ID returned is 0!", pFunctionParameters, "TRGT-SupplierOrder-151114-1147", vRequester) 
            End If 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-SupplierOrder-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090623-1809", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderUpdate, "clsSupplierOrder_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Dim pCommandText As String = "ccSupplierOrderUpdate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrder_Delete", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    Dim pCommandText As String = "ccSupplierOrderDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      MyController.DBCache.SupplierOrderCol.Remove(MyController.DBCache.SupplierOrderCol.FindByID(_ID)) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.SupplierOrderCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-SupplierOrder-151114-1147", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-SupplierOrder-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090623-1813", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrder_DeleteByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      MyController.DBCache.SupplierOrderCol.Remove(MyController.DBCache.SupplierOrderCol.FindByID(vID)) 
      'Save File  
      pFault = MyController.DBCache.SaveData(MyController.DBCache.SupplierOrderCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pID As Integer = ccHelper.ToInteger(pTargCCReader(0)) 
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-SupplierOrder-231207-0845", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-SupplierOrder-231207-0844", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-231207-0843", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If 
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Try
      ...
      pLastReadVariableName = "SupplierEmail" 
      If Not vReader.IsDBNull(2) Then _SupplierEmail = vReader.GetString(2) 
      pLastReadVariableName = "EmailSubject" 
      If Not vReader.IsDBNull(3) Then _EmailSubject = vReader.GetString(3) 
      pLastReadVariableName = "EmailBody" 
      If Not vReader.IsDBNull(4) Then _EmailBody = vReader.GetString(4) 
      pLastReadVariableName = "enmEmailStatus" 
      If Not vReader.IsDBNull(5) Then _EmailStatus = clsEnums.TranslateEnmEmailStatus(vReader.GetString(5))
      pLastReadVariableName = "SentDate" 
      If Not vReader.IsDBNull(6) Then _SentDate = vReader.GetDateTime(6)
      pLastReadVariableName = "blg_TotalCost" 
      If Not vReader.IsDBNull(7) Then _TotalCost = vReader.GetDecimal(7)
      pLastReadVariableName = "enmDeliveryMethod" 
      If Not vReader.IsDBNull(8) Then _DeliveryMethod = clsEnums.TranslateEnmDeliveryMethod(vReader.GetString(8))
      pLastReadVariableName = "RequestedDeliveryDate" 
      If Not vReader.IsDBNull(9) Then _RequestedDeliveryDate = vReader.GetDateTime(9)
      pLastReadVariableName = "RequestedDeliveryDay" 
      If Not vReader.IsDBNull(10) Then _RequestedDeliveryDay = vReader.GetString(10) 
      pLastReadVariableName = "Notes" 
      If Not vReader.IsDBNull(11) Then _Notes = vReader.GetString(11) 
      pLastReadVariableName = "bDateAdded" 
      If Not vReader.IsDBNull(12) Then bDateAdded = vReader.GetDateTime(12)   
      If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
        pLastReadVariableName = "OrderHeaderText" 
        If Not vReader.IsDBNull(13) Then _OrderHeaderText = vReader.GetString(13) 
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedSupplierOrder As clsSupplierOrder, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedSupplierOrder) 
      ...
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-121122-1607", vRequester) 
    End Try 
...
Public Class clsSupplierOrderCol
  ...
  Public Sub CleanCollectionForXML() 
    ...
    For Each lSupplierOrder As clsSupplierOrder In Me 
      lSupplierOrder.CleanEntityForXML() 
    Next 
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.Clone() 
      pSupplierOrdersCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pSupplierOrdersCached.Reverse() 
      If vHowMany > 0 AndAlso pSupplierOrdersCached.Count > vHowMany Then 
        Dim tmp As New clsSupplierOrderCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pSupplierOrdersCached(i)) 
        Next 
        pSupplierOrdersCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pSupplierOrdersCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByOrderHeaderID(ByVal vOrderHeaderID As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillByOrderHeaderID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.CloneByOrderHeaderID(vOrderHeaderID)
      pFault = LoadMeFromDBCache(pSupplierOrdersCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillByOrderHeaderID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090624-1702", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillBySentDate(ByVal vSentDate As Date, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillBySentDate", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.CloneBySentDate(vSentDate)
      pFault = LoadMeFromDBCache(pSupplierOrdersCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillBySentDate" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        'set parameters 
        pLastReadVariableName = "SentDate" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = ccHelper.DateNullable(vSentDate) 
        pLastReadVariableName = "WithParentText" 
        If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
          pDALParameters.Add("WithParentText", ccDAL.enmSQLDataType.Bit).Value = True 
        Else 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pSupplierOrdersCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByBoundedSentDate(ByVal vSentDateStart As Date, ByVal vSentDateEnd As Date, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillByBoundedSentDate", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.SupplierOrderCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.SupplierOrderCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsSupplierOrderCol failed: " & pResponse) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.CloneByBoundedSentDate(vSentDateStart, vSentDateEnd)
      pFault = LoadMeFromDBCache(pSupplierOrdersCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillByBoundedSentDate" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
   
      Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters 
        pLastReadVariableName = "SentDateFrom" 
        pDALParameters.Add("bndSentDateFrom", ccDAL.enmSQLDataType.DateTime).Value = (vSentDateStart) 
        pLastReadVariableName = "SentDateTo" 
        pDALParameters.Add("bndSentDateTo", ccDAL.enmSQLDataType.DateTime).Value = (vSentDateEnd) 
        pLastReadVariableName = "WithParentText" 
        If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
          pDALParameters.Add("WithParentText", ccDAL.enmSQLDataType.Bit).Value = True 
        Else 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lSupplierOrder As New clsSupplierOrder() 
      pFault = lSupplierOrder.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lSupplierOrder.IsEmpty Then Me.Add(lSupplierOrder) 
    Next 
 
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController 
      Dim pSupplierOrders As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pSupplierOrders, "clsSupplierOrderCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pSupplierOrders IsNot Nothing AndAlso Me.Count <> pSupplierOrders.Count Then FillFromListOfITargCCEntity(pSupplierOrders) 
    End If 
  ...
  Public Overrides Function FillOnTheFly(ByVal vParameters As Dictionary(Of System.Enum, Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-SupplierOrder-121122-2008", vRequester) 
      Dim pSupplierOrdersCached As clsSupplierOrderCol = MyController.DBCache.SupplierOrderCol.Clone() 
      Dim pSupplierOrdersToUse As New clsSupplierOrderCol() 
      For Each l In pSupplierOrdersCached 
        ...
        End If 
        If vSentDateStart.HasValue Then 
          If vSentDateEnd.HasValue Then 
            If l.SentDate < vSentDateStart OrElse l.SentDate > vSentDateEnd.Value Then Continue For 
          Else 
            If l.SentDate <> vSentDateStart.Value Then Continue For 
          End If 
        End If 
        pSupplierOrdersToUse.Add(l) 
      Next 
      pSupplierOrdersToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pSupplierOrdersToUse.Reverse() 
      If vHowMany > 0 AndAlso pSupplierOrdersToUse.Count > vHowMany Then 
        Dim tmp As New clsSupplierOrderCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pSupplierOrdersToUse(i)) 
        Next 
        pSupplierOrdersToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pSupplierOrdersToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pLastReadVariableName = "SentDateFrom" 
        pDALParameters.Add("bndSentDateFrom", ccDAL.enmSQLDataType.DateTime).Value = ccHelper.ObjectNullable(vSentDateStart) 
        pLastReadVariableName = "SentDateTo" 
        pDALParameters.Add("bndSentDateTo", ccDAL.enmSQLDataType.DateTime).Value = ccHelper.ObjectNullable(vSentDateEnd) 
        pLastReadVariableName = "WithParentText" 
        If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
          pDALParameters.Add("WithParentText", ccDAL.enmSQLDataType.Bit).Value = True 
        Else 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overrides Function FillSumOnTheFly(ByVal vParameters As Dictionary(Of [Enum], Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-SupplierOrder-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccSupplierOrdersFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pLastReadVariableName = "SentDate" 
        pDALParameters.Add("GroupBySentDate", ccDAL.enmSQLDataType.Bit).Value = vGroupBySentDate
        pLastReadVariableName = "WithParentText" 
        If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
          pDALParameters.Add("WithParentText", ccDAL.enmSQLDataType.Bit).Value = True 
        Else 
  ...
  Public Function LoadParents(ByVal vRequester As clsRequester) As clsFault
    Dim pFault As New clsFault
    For Each pSupplierOrder As clsSupplierOrder In Me
      pFault = pSupplierOrder.LoadParents(vRequester)
      If pFault.isOK = False Then Return pFault
    Next
    _WithParents = clsEnums.enmLoadParent.EntireObject 
    pFault.SetOK()
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.Update
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderView, "clsSupplierOrderCol_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each pExists As clsSupplierOrder In Me 
      ...
      ElseIf pExists.ccStatus = clsEnums.enmObjectStatus.UD Then 
        ...
        Return pFault.LogFreeTextFault("Status should not be UD ", pFunctionParameters, "TRGT-SupplierOrder-130415-0942", vRequester) 
      End If 
    Next 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderUpdate, "clsSupplierOrderCol_UpdateFromCollection", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrderCol_Delete", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccSupplierOrdersDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      pFault = MyController.DBCache.SaveData(New clsSupplierOrderCol(), vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByOrderHeaderID(ByVal vOrderHeaderID As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrderCol_DeleteByOrderHeaderID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccSupplierOrdersDeleteByOrderHeaderID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllSupplierOrders As New clsSupplierOrderCol() : pAllSupplierOrders.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredSupplierOrders As clsSupplierOrderCol = pAllSupplierOrders.CloneByOrderHeaderID(vOrderHeaderID) 
      For Each l In pFilteredSupplierOrders 
        pAllSupplierOrders.Remove(pAllSupplierOrders.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllSupplierOrders, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteBySentDate(ByVal vSentDate As Date, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrderCol_DeleteBySentDate", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccSupplierOrdersDeleteBySentDate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllSupplierOrders As New clsSupplierOrderCol() : pAllSupplierOrders.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredSupplierOrders As clsSupplierOrderCol = pAllSupplierOrders.CloneBySentDate(vSentDate) 
      For Each l In pFilteredSupplierOrders 
        pAllSupplierOrders.Remove(pAllSupplierOrders.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllSupplierOrders, vRequester) 
    ...
    Else 
      ...
      Try 
        'set parameters  
        pLastReadVariableName = "SentDate" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.DateTime).Value = (vSentDate) 
        pLastReadVariableName = "ChangedBy" 
        pDALParameters.Add("ChangedBy", ccDAL.enmSQLDataType.NVarChar, 50).Value = vRequester.UserName
        pLastReadVariableName = "UpdatingLoginID" 
        pDALParameters.Add("UpdatingLoginID", ccDAL.enmSQLDataType.BigInt).Value = vRequester.LoggedLoginID
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrderCol_DeleteByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccSupplierOrdersDeleteByBoundedID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-SupplierOrder-150216-2148", vRequester) 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-090210-1341", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByBoundedSentDate(ByVal vSentDateStart As Date, ByVal vSentDateEnd As Date, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_SupplierOrderDelete, "clsSupplierOrderCol_DeleteByBoundedSentDate", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "ccSupplierOrdersDeleteByBoundedSentDate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    While vReader.Read()
      pSupplierOrder = New clsSupplierOrder(_WithParents) 
      pFault = pSupplierOrder.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pSupplierOrder)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pSupplierOrder.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedSupplierOrderCol As clsSupplierOrderCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedSupplierOrder As clsSupplierOrder In vCachedSupplierOrderCol 
        pCachedSupplierOrder.SetWithParents(_WithParents) 
        pSupplierOrder = New clsSupplierOrder(pCachedSupplierOrder) 
        If _WithParents = clsEnums.enmLoadParent.DoNotLoad Then 
          pSupplierOrder.OrderHeaderText = "." 
        End If 
        pSupplierOrder.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pSupplierOrder) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pSupplierOrder.ID) 
      Next 
      If _WithParents = clsEnums.enmLoadParent.EntireObject Then 
      ...
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-SupplierOrder-121122-1513", vRequester) 
      End Try 
