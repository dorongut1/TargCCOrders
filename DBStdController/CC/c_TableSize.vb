Public Class csTableSize
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSize_GetByPrimaryKey", vRequester) 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSize_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function GetByTableName(ByVal vTableName As String, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSize_GetByTableName", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccTableSizeCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccTableSizeCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csTableSizeCol failed: " & pResponse) 
      ' Not Implemented Yet!!  pFault = LoadMeFromDBCache(MyController.DBCache.ccTableSizeCol.FindByTableName(vTableName), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      'Create the Stored procedure manually, based on other objects 'GetByTableName' 
      Dim pCommandText As String = "mnViewTableSizeGetByTableName" 
      ...
      Try 
        ...
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 200).Value = (vTableName) 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"TableSize not found for GetByTableName. See FunctionParameters for values", pFunctionParameters, "TRGT-TableSize-210625-0950", vRequester, vAdditionalMessageToUser:=$"TableSize not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSize_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = -1 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"TableSize not found for GetByID, since its value is -1", pFunctionParameters, "TRGT-TableSize-210927-1527", vRequester, vAdditionalMessageToUser:=$"TableSize not found") 
      pFault.SetOK() 
      RaiseEvent evtAfterGet() 
      RaiseEvent evtAfterGetWithRequester(vRequester, pFault) 
      Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccTableSizeCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccTableSizeCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csTableSizeCol failed: " & pResponse) 
      ' Not Implemented Yet!!  pFault = LoadMeFromDBCache(MyController.DBCache.ccTableSizeCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      'Create the Stored procedure manually, based on other objects 'GetByID' 
      Dim pCommandText As String = "mnViewTableSizeGetByID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        'Execute query 
        pFault = ccDAL.ExecuteQuery(pCommandText, pDALParameters, vRequester, Me) : If Not pFault.isOK Then Return pFault 
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"TableSize not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-TableSize-210625-0950", vRequester, vAdditionalMessageToUser:=$"TableSize not found") 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-090623-1648", vRequester) 
      End Try 
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Try
      ...
      pLastReadVariableName = "NumberOfRows" 
      If Not vReader.IsDBNull(2) Then _NumberOfRows = vReader.GetInt32(2)
      pLastReadVariableName = "ReservedSizeKb" 
      If Not vReader.IsDBNull(3) Then _ReservedSizeKb = vReader.GetInt32(3)
      pLastReadVariableName = "DataSizeKb" 
      If Not vReader.IsDBNull(4) Then _DataSizeKb = vReader.GetInt32(4)
      pLastReadVariableName = "IndexSizeKb" 
      If Not vReader.IsDBNull(5) Then _IndexSizeKb = vReader.GetInt32(5)
      pLastReadVariableName = "UnusedSizeKb" 
      If Not vReader.IsDBNull(6) Then _UnusedSizeKb = vReader.GetInt32(6)
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedTableSize As csTableSize, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedTableSize) 
      bccStatus = clsEnums.enmObjectStatus.Clean 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-121122-1607", vRequester) 
    End Try 
...
Public Class csTableSizeCol
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccTableSizeCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccTableSizeCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csTableSizeCol failed: " & pResponse) 
      Dim pTableSizesCached As csTableSizeCol = MyController.DBCache.ccTableSizeCol.Clone() 
      pTableSizesCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pTableSizesCached.Reverse() 
      If vHowMany > 0 AndAlso pTableSizesCached.Count > vHowMany Then 
        Dim tmp As New csTableSizeCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pTableSizesCached(i)) 
        Next 
        pTableSizesCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pTableSizesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      'Create the Stored procedure manually, based on other objects 'Fill' 
      Dim pCommandText As String = "c__TableSizesFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByBoundedTableName(ByVal vTableNameFrom As String, ByVal vTableNameTo As String, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillByBoundedTableName", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccTableSizeCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccTableSizeCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csTableSizeCol failed: " & pResponse) 
      Dim pTableSizesCached As csTableSizeCol = MyController.DBCache.ccTableSizeCol.CloneByBoundedTableName(vTableNameFrom, vTableNameTo)
      pFault = LoadMeFromDBCache(pTableSizesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_TableSizesFillByBoundedTableName" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pDALParameters.Add("bndTableNameFrom", ccDAL.enmSQLDataType.NVarChar, 200).Value = (vTableNameFrom) 
        pLastReadVariableName = "TableNameTo" 
        pDALParameters.Add("bndTableNameTo", ccDAL.enmSQLDataType.NVarChar, 200).Value = (vTableNameTo) 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccTableSizeCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccTableSizeCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csTableSizeCol failed: " & pResponse) 
      Dim pTableSizesCached As csTableSizeCol = MyController.DBCache.ccTableSizeCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pTableSizesCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_TableSizesFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByWildCardTableName(ByVal vTableName As String, ByVal vTableNameWildcardType As clsEnums.enmWildCardType, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillByWildCardTableName", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillByWildCardOrderingCode has not been handled yet locally", pFunctionParameters, "TRGT-TableSize-121122-2008", vRequester) 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_TableSizesFillByWildCardTableName" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pDALParameters.Add("wldTableName", ccDAL.enmSQLDataType.NVarChar, 200).Value = (pWCTableName) 
        pLastReadVariableName = "HowMany" 
        pDALParameters.Add("Top", ccDAL.enmSQLDataType.Int).Value = vHowMany 
        pLastReadVariableName = "Dir" 
        pDALParameters.Add("Dir", ccDAL.enmSQLDataType.VarChar, 4).Value = vDir.FastToString() 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lTableSize As New csTableSize() 
      pFault = lTableSize.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lTableSize.IsEmpty Then Me.Add(lTableSize) 
    Next 
 
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController 
      Dim pTableSizes As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pTableSizes, "csTableSizeCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pTableSizes IsNot Nothing AndAlso Me.Count <> pTableSizes.Count Then FillFromListOfITargCCEntity(pTableSizes) 
    End If 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-TableSize-121122-2008", vRequester) 
      Dim pTableSizesCached As csTableSizeCol = MyController.DBCache.ccTableSizeCol.Clone() 
      Dim pTableSizesToUse As New csTableSizeCol() 
      For Each l In pTableSizesCached 
        ...
        If Not String.IsNullOrEmpty(vTableName) Then 
          ...
          End If 
        End If 
        pTableSizesToUse.Add(l) 
      Next 
      pTableSizesToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pTableSizesToUse.Reverse() 
      If vHowMany > 0 AndAlso pTableSizesToUse.Count > vHowMany Then 
        Dim tmp As New csTableSizeCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pTableSizesToUse(i)) 
        Next 
        pTableSizesToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pTableSizesToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_TableSizesFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Try 
        ...
        pLastReadVariableName = "TableName" 
        pDALParameters.Add("wldTableName", ccDAL.enmSQLDataType.NVarChar, 200).Value = ccHelper.ObjectNullable(pWCTableName) 
        pLastReadVariableName = "HowMany" 
        pDALParameters.Add("Top", ccDAL.enmSQLDataType.Int).Value = vHowMany 
        pLastReadVariableName = "Dir" 
        pDALParameters.Add("Dir", ccDAL.enmSQLDataType.VarChar, 4).Value = vDir.FastToString() 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_c_TableSizeView, "csTableSizeCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-TableSize-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_TableSizesFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    While vReader.Read()
      pTableSize = New csTableSize() 
      pFault = pTableSize.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pTableSize)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pTableSize.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedTableSizeCol As csTableSizeCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedTableSize As csTableSize In vCachedTableSizeCol 
        pTableSize = New csTableSize(pCachedTableSize) 
        pTableSize.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pTableSize) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pTableSize.ID) 
      Next 
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-TableSize-121122-1513", vRequester) 
      End Try 
