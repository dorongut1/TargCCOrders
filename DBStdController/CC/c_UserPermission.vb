Public Class csUserPermission
  ...
  Friend Sub CleanEntityForXML() 
    ...
    _ComputerIdentifier = ccHelper.RemoveChrW0(_ComputerIdentifier) 
    _ComputerName = ccHelper.RemoveChrW0(_ComputerName) 
    _ExternalIP = ccHelper.RemoveChrW0(_ExternalIP) 
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermission_GetByPrimaryKey", vRequester) 
    ...
    If pEnteredHere = True Then pFault = ccSecurity.GetPermissionForExternalIndentityTypeForEntity(Me, "csUserPermission_GetByPrimaryKey", vRequester) : If Not pFault.isOK Then CreateEmpty() : Return pFault 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermission_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    'only check if this function was accessed directly from outside DBController 
    If pEnteredHere = True Then pFault = ccSecurity.GetPermissionForExternalIndentityTypeForEntity(Me, "csUserPermission_GetByParameters", vRequester) : If Not pFault.isOK Then CreateEmpty() : Return pFault 
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermission_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = 0 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"UserPermission not found for GetByID, since its value is 0", pFunctionParameters, "TRGT-UserPermission-210927-1527", vRequester, vAdditionalMessageToUser:=$"UserPermission not found") 
      pFault.SetOK() 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      pFault = LoadMeFromDBCache(MyController.DBCache.ccUserPermissionCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionGetByID" 
      ...
      Try 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"UserPermission not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-UserPermission-210625-0950", vRequester, vAdditionalMessageToUser:=$"UserPermission not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
       
    'only check if this function was accessed directly from outside DBController 
    If pEnteredHere = True Then pFault = ccSecurity.GetPermissionForExternalIndentityTypeForEntity(Me, "csUserPermission_GetByID", vRequester) : If Not pFault.isOK Then CreateEmpty() : Return pFault 
  ...
  Public Function AddUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityAddable.AddUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionUpdate, "csUserPermission_AddUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function EditUpdate(vRequester As clsRequester, Optional vReload As Boolean = True) As clsFault Implements ITargCCEntityEditable.EditUpdate 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionUpdate, "csUserPermission_EditUpdate", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionUpdate, "csUserPermission_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    Dim pUserPermission As New csUserPermission(_WithParents) 
    If Me.isEqual(pUserPermission) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", pFunctionParameters, "TRGT-UserPermission-100113-1638", vRequester) 
    End If 
    ...
    Dim pCommandText As String = "c_UserPermissionUpdate"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pCachedUserPermission As csUserPermission 
      If _ID = 0 Then 
        pCachedUserPermission = New csUserPermission(_WithParents) 
        'get last ID 
        Dim pUserPermissionCol As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.Clone() 
        If pUserPermissionCol.Count = 0 Then 
          _ID = 1 
        Else 
          pUserPermissionCol.SortByID() 
          Dim pLastID As Long = pUserPermissionCol(pUserPermissionCol.Count - 1).ID
          _ID = pLastID + 1 
        End If 
        bPrimaryKey = _ID 
        MyController.DBCache.ccUserPermissionCol.Add(pCachedUserPermission) 
      Else  
        pCachedUserPermission = MyController.DBCache.ccUserPermissionCol.FindByID(_ID) 
      End If 
      pCachedUserPermission.AssignValues(Me) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccUserPermissionCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
        ...
        pLastReadVariableName = "ComputerIdentifier" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 100).Value = ccHelper.ObjectNullable(_ComputerIdentifier) 
        pLastReadVariableName = "ComputerName" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 50).Value = ccHelper.ObjectNullable(_ComputerName) 
        pLastReadVariableName = "ExternalIP" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.VarChar, 100).Value = ccHelper.ObjectNullable(_ExternalIP) 
        pLastReadVariableName = "HasPermission" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.Bit).Value = (_HasPermission) 
        pLastReadVariableName = "Comments" 
        pDALParameters.Add(pLastReadVariableName, ccDAL.enmSQLDataType.NVarChar, 200).Value = ccHelper.ObjectNullable(_Comments) 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            bPrimaryKey = pID 
            If pID = 0 Then 
              pFault.LogFreeTextFault(51, "ID returned is 0!", pFunctionParameters, "TRGT-UserPermission-151114-1147", vRequester) 
            End If 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-UserPermission-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090623-1809", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermission_Delete", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    Dim pCommandText As String = "c_UserPermissionDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      MyController.DBCache.ccUserPermissionCol.Remove(MyController.DBCache.ccUserPermissionCol.FindByID(_ID)) 
      'Save File 
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccUserPermissionCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            ...
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-UserPermission-151114-1147", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-UserPermission-160324-1700", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090623-1813", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermission_DeleteByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      MyController.DBCache.ccUserPermissionCol.Remove(MyController.DBCache.ccUserPermissionCol.FindByID(vID)) 
      'Save File  
      pFault = MyController.DBCache.SaveData(MyController.DBCache.ccUserPermissionCol, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      ...
      Try 
        ...
        If pFault.isOK = True Then 
          If pTargCCReader.HasRows = True Then 
            pTargCCReader.Read() 
            Dim pID As Integer = ccHelper.ToInteger(pTargCCReader(0)) 
            If pID <> -1 Then pFault.LogFreeTextFault(51, "I expected to get -1 back!", pFunctionParameters, "TRGT-UserPermission-231207-0845", vRequester) 
          Else 
            pFault.LogFreeTextFault(51, "No response returned from SQL query!", pFunctionParameters, "TRGT-UserPermission-231207-0844", vRequester) 
          End If 
        End If 
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-231207-0843", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If 
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Try
      ...
      pLastReadVariableName = "ComputerIdentifier" 
      If Not vReader.IsDBNull(2) Then _ComputerIdentifier = vReader.GetString(2) 
      pLastReadVariableName = "ApplicationName" 
      If Not vReader.IsDBNull(3) Then _ApplicationName = vReader.GetString(3) 
      pLastReadVariableName = "ComputerName" 
      If Not vReader.IsDBNull(4) Then _ComputerName = vReader.GetString(4) 
      pLastReadVariableName = "ExternalIP" 
      If Not vReader.IsDBNull(5) Then _ExternalIP = vReader.GetString(5) 
      pLastReadVariableName = "HasPermission" 
      If Not vReader.IsDBNull(6) Then _HasPermission = vReader.GetBoolean(6)
      pLastReadVariableName = "Comments" 
      If Not vReader.IsDBNull(7) Then _Comments = vReader.GetString(7) 
      pLastReadVariableName = "LastAccessTime" 
      If Not vReader.IsDBNull(8) Then _LastAccessTime = vReader.GetDateTime(8)
      pLastReadVariableName = "LoggedLoginID" 
      If Not vReader.IsDBNull(9) Then _LoggedLoginID = vReader.GetInt64(9)
      pLastReadVariableName = "bDateAdded" 
      If Not vReader.IsDBNull(10) Then bDateAdded = vReader.GetDateTime(10)   
      If _WithParents = clsEnums.enmLoadParent.TextOnly Then 
        pLastReadVariableName = "UserText" 
        If Not vReader.IsDBNull(11) Then _UserText = vReader.GetString(11) 
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedUserPermission As csUserPermission, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedUserPermission) 
      ...
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-121122-1607", vRequester) 
    End Try 
...
#End Region
  ...
  Private Sub csUserPermission_evtBeforeUpdate(vWhichColumn As enmUpdateType, ByRef rCancel As Boolean) Handles Me.evtBeforeUpdate 
    If _ExternalIP.IndexOf(" or *") > 0 Then 
      _HasPermission = False 
    End If 
  End Sub 
...
Public Class csUserPermissionCol
  ...
  Public Sub CleanCollectionForXML() 
    ...
    For Each lUserPermission As csUserPermission In Me 
      lUserPermission.CleanEntityForXML() 
    Next 
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillByParameters", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.Clone() 
      pUserPermissionsCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pUserPermissionsCached.Reverse() 
      If vHowMany > 0 AndAlso pUserPermissionsCached.Count > vHowMany Then 
        Dim tmp As New csUserPermissionCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pUserPermissionsCached(i)) 
        Next 
        pUserPermissionsCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pUserPermissionsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Function FillByUserID(ByVal vUserID As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByUserID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.CloneByUserID(vUserID)
      pFault = LoadMeFromDBCache(pUserPermissionsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillByUserID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090624-1702", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillByUserID", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Function FillByUserIDAndApplicationName(ByVal vUserID As Long, ByVal vApplicationName As String, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByUserIDAndApplicationName", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.CloneByUserIDAndApplicationName(vUserID, vApplicationName)
      pFault = LoadMeFromDBCache(pUserPermissionsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillByUserID&ApplicationName" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillByUserIDAndApplicationName", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pUserPermissionsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillByBoundedID", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Function FillByBoundedUserIDAndApplicationName(ByVal vUserID As Long, ByVal vApplicationNameFrom As String, ByVal vApplicationNameTo As String, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByBoundedUserIDAndApplicationName", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.ccUserPermissionCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.ccUserPermissionCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load csUserPermissionCol failed: " & pResponse) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.CloneByBoundedUserIDAndApplicationName(vUserID, vApplicationNameFrom, vApplicationNameTo)
      pFault = LoadMeFromDBCache(pUserPermissionsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillByBoundedUserID&ApplicationName" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillByBoundedUserIDAndApplicationName", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lUserPermission As New csUserPermission() 
      pFault = lUserPermission.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lUserPermission.IsEmpty Then Me.Add(lUserPermission) 
    Next 
  ...
  Public Overrides Function FillOnTheFly(ByVal vParameters As Dictionary(Of System.Enum, Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillOnTheFly", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-UserPermission-121122-2008", vRequester) 
      Dim pUserPermissionsCached As csUserPermissionCol = MyController.DBCache.ccUserPermissionCol.Clone() 
      Dim pUserPermissionsToUse As New csUserPermissionCol() 
      For Each l In pUserPermissionsCached 
        ...
        If Not String.IsNullOrEmpty(vApplicationName) Then 
          ...
          End If 
        End If 
        pUserPermissionsToUse.Add(l) 
      Next 
      pUserPermissionsToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pUserPermissionsToUse.Reverse() 
      If vHowMany > 0 AndAlso pUserPermissionsToUse.Count > vHowMany Then 
        Dim tmp As New csUserPermissionCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pUserPermissionsToUse(i)) 
        Next 
        pUserPermissionsToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pUserPermissionsToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overrides Function FillSumOnTheFly(ByVal vParameters As Dictionary(Of [Enum], Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController
      Dim pUserPermissions As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pUserPermissions, "csUserPermissionCol_FillSumOnTheFly", vRequester) : If Not pFault.isOK Then Return pFault 
      If pUserPermissions IsNot Nothing AndAlso Me.Count <> pUserPermissions.Count Then FillFromListOfITargCCEntity(pUserPermissions) 
    End If 
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-UserPermission-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "c_UserPermissionsFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function LoadParents(ByVal vRequester As clsRequester) As clsFault
    Dim pFault As New clsFault
    For Each pUserPermission As csUserPermission In Me
      pFault = pUserPermission.LoadParents(vRequester)
      If pFault.isOK = False Then Return pFault
    Next
    _WithParents = clsEnums.enmLoadParent.EntireObject 
    pFault.SetOK()
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.Update
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionView, "csUserPermissionCol_Update", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each pExists As csUserPermission In Me 
      ...
      ElseIf pExists.ccStatus = clsEnums.enmObjectStatus.UD Then 
        ...
        Return pFault.LogFreeTextFault("Status should not be UD ", pFunctionParameters, "TRGT-UserPermission-130415-0942", vRequester) 
      End If 
    Next 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionUpdate, "csUserPermissionCol_UpdateFromCollection", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermissionCol_Delete", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_UserPermissionsDelete" 
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      pFault = MyController.DBCache.SaveData(New csUserPermissionCol(), vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByUserID(ByVal vUserID As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermissionCol_DeleteByUserID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_UserPermissionsDeleteByUserID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllUserPermissions As New csUserPermissionCol() : pAllUserPermissions.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredUserPermissions As csUserPermissionCol = pAllUserPermissions.CloneByUserID(vUserID) 
      For Each l In pFilteredUserPermissions 
        pAllUserPermissions.Remove(pAllUserPermissions.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllUserPermissions, vRequester) 
      If pFault.isOK = False Then Return pFault 
    Else 
      Dim pLastReadVariableName As String = "" 
      Try 
  ...
  Public Shared Function DeleteByUserIDAndApplicationName(ByVal vUserID As Long, ByVal vApplicationName As String, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermissionCol_DeleteByUserIDAndApplicationName", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_UserPermissionsDeleteByUserID&ApplicationName"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Dim pAllUserPermissions As New csUserPermissionCol() : pAllUserPermissions.Fill(vRequester) : If Not pFault.isOK Then Return pFault 
      Dim pFilteredUserPermissions As csUserPermissionCol = pAllUserPermissions.CloneByUserIDAndApplicationName(vUserID, vApplicationName) 
      For Each l In pFilteredUserPermissions 
        pAllUserPermissions.Remove(pAllUserPermissions.FindByID(l.ID)) 
      Next 
      pFault = MyController.DBCache.SaveData(pAllUserPermissions, vRequester) 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermissionCol_DeleteByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_UserPermissionsDeleteByBoundedID"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
 
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("Function not implemented", pFunctionParameters, "TRGT-UserPermission-150216-2148", vRequester) 
    Else 
        Dim pLastReadVariableName As String = "" 
      Try 
        'set parameters  
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-090210-1341", vRequester) 
      End Try 
      If pFault.isOK = False Then Return pFault 
    End If
  ...
  Public Shared Function DeleteByBoundedUserIDAndApplicationName(ByVal vUserID As Long, ByVal vApplicationNameFrom As String, ByVal vApplicationNameTo As String, ByVal vRequester As clsRequester) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.tbl_c_UserPermissionDelete, "csUserPermissionCol_DeleteByBoundedUserIDAndApplicationName", vRequester) 
    If pFault.isOK = False Then Return pFault 
 
    Dim pCommandText As String = "c_UserPermissionsDeleteByBoundedUserID&ApplicationName"
    Dim pDALParameters As New ccDAL.csTargCCParameterCol 
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    While vReader.Read()
      pUserPermission = New csUserPermission(_WithParents) 
      pFault = pUserPermission.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pUserPermission)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pUserPermission.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedUserPermissionCol As csUserPermissionCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedUserPermission As csUserPermission In vCachedUserPermissionCol 
        pCachedUserPermission.SetWithParents(_WithParents) 
        pUserPermission = New csUserPermission(pCachedUserPermission) 
        If _WithParents = clsEnums.enmLoadParent.DoNotLoad Then 
          pUserPermission.UserText = "." 
        End If 
        pUserPermission.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pUserPermission) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pUserPermission.ID) 
      Next 
      If _WithParents = clsEnums.enmLoadParent.EntireObject Then 
      ...
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-UserPermission-121122-1513", vRequester) 
      End Try 
