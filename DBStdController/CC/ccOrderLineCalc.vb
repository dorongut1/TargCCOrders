Public Class clsOrderLineCalc
  ...
  Public Overrides Function GetByPrimaryKey(ByVal vPrimaryKeyValue As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalc_GetByPrimaryKey", vRequester) 
  ...
  Public Overrides Function GetByParameters(ByVal vParametersType As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalc_GetByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalc_GetByID", vRequester) 
    If Not pFault.isOK Then Return pFault 
    
    CreateEmpty() 
    
    If vID = -1 Then 
      If vMustExist Then Return pFault.LogFreeTextFault(70, $"OrderLineCalc not found for GetByID, since its value is -1", pFunctionParameters, "TRGT-OrderLineCalc-210927-1527", vRequester, vAdditionalMessageToUser:=$"OrderLineCalc not found") 
      pFault.SetOK() 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      ' Not Implemented Yet!!  pFault = LoadMeFromDBCache(MyController.DBCache.OrderLineCalcCol.FindByID(vID), vRequester) : If pFault.isOK = False Then Return pFault
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcGetByID" 
      ...
      Try 
        ...
        If (vMustExist AndAlso Me.IsEmpty) Then pFault.LogFreeTextFault(70, $"OrderLineCalc not found for GetByID. See FunctionParameters for values", pFunctionParameters, "TRGT-OrderLineCalc-210625-0950", vRequester, vAdditionalMessageToUser:=$"OrderLineCalc not found") 
      Catch ex As Exception 
        ...
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-090623-1648", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
...
#Region "Load Entity" 
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Catch ex As Exception
      If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
      pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-090624-1143", vRequester)
    End Try
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedOrderLineCalc As clsOrderLineCalc, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      AssignValues(vCachedOrderLineCalc) 
      bccStatus = clsEnums.enmObjectStatus.Clean 
      pFault.SetOK() 
    Catch ex As Exception 
      pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-121122-1607", vRequester) 
    End Try 
...
Public Class clsOrderLineCalcCol
  ...
  Public Sub CleanCollectionForXML() 
    ...
    For Each lOrderLineCalc As clsOrderLineCalc In Me 
      lOrderLineCalc.CleanEntityForXML() 
    Next 
  ...
  Public Overrides Function FillByParameters(ByVal vWhichParameterCombination As [Enum], ByVal vParameters As List(Of Object), ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault 
    ...
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByParameters", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_Fill", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.Clone() 
      pOrderLineCalcsCached.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pOrderLineCalcsCached.Reverse() 
      If vHowMany > 0 AndAlso pOrderLineCalcsCached.Count > vHowMany Then 
        Dim tmp As New clsOrderLineCalcCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pOrderLineCalcsCached(i)) 
        Next 
        pOrderLineCalcsCached = tmp 
      End If 
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFill"
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-090624-1625", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByOrderHeaderID(ByVal vOrderHeaderID As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByOrderHeaderID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.CloneByOrderHeaderID(vOrderHeaderID)
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillByOrderHeaderID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-090624-1702", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByProductID(ByVal vProductID As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByProductID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.CloneByProductID(vProductID)
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillByProductID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByBoundedOrderHeaderID(ByVal vOrderHeaderIDFrom As Long, ByVal vOrderHeaderIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByBoundedOrderHeaderID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.CloneByBoundedOrderHeaderID(vOrderHeaderIDFrom, vOrderHeaderIDTo)
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillByBoundedOrderHeaderID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-151113-1405", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Function FillByBoundedProductID(ByVal vProductIDFrom As Long, ByVal vProductIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByBoundedProductID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.CloneByBoundedProductID(vProductIDFrom, vProductIDTo)
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillByBoundedProductID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByBoundedID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      If MyController.DBCache.OrderLineCalcCol.Count = 0 Then Dim pResponse As String = MyController.DBCache.LoadTableFromFileSystem(MyController.DBCache.OrderLineCalcCol) : If Not pResponse.Equals("OK", StringComparison.OrdinalIgnoreCase) Then Throw New Exception("Load clsOrderLineCalcCol failed: " & pResponse) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.CloneByBoundedID(vIDFrom, vIDTo)
      pFault = LoadMeFromDBCache(pOrderLineCalcsCached, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
        Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillByBoundedID" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillByListOfID", vRequester) 
    If pFault.isOK = False Then Return pFault 
    ...
    For Each l In vIDs 
      ...
      Dim lOrderLineCalc As New clsOrderLineCalc() 
      pFault = lOrderLineCalc.GetByID(l, vRequester) : If Not pFault.isOK Then Return pFault 
      If Not lOrderLineCalc.IsEmpty Then Me.Add(lOrderLineCalc) 
    Next 
 
    If pEnteredHere = True Then 'only check if this function was accessed directly from outside DBController 
      Dim pOrderLineCalcs As Generic.List(Of ITargCCEntity) = Nothing 
      pFault = ccSecurity.GetPermissionForExternalIndentityTypeForCollection(Me, pOrderLineCalcs, "clsOrderLineCalcCol_Fill", vRequester) : If Not pFault.isOK Then Return pFault 
      If pOrderLineCalcs IsNot Nothing AndAlso Me.Count <> pOrderLineCalcs.Count Then FillFromListOfITargCCEntity(pOrderLineCalcs) 
    End If 
  ...
  Public Overrides Function FillOnTheFly(ByVal vParameters As Dictionary(Of System.Enum, Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      'Return pFault.LogFreeTextFault("FillOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-OrderLineCalc-121122-2008", vRequester) 
      Dim pOrderLineCalcsCached As clsOrderLineCalcCol = MyController.DBCache.OrderLineCalcCol.Clone() 
      Dim pOrderLineCalcsToUse As New clsOrderLineCalcCol() 
      For Each l In pOrderLineCalcsCached 
        ...
        If vProductIDFrom.HasValue Then 
          ...
          End If 
        End If 
        pOrderLineCalcsToUse.Add(l) 
      Next 
      pOrderLineCalcsToUse.SortByID() : If vDir = clsEnums.enmFillDirection.DESC Then pOrderLineCalcsToUse.Reverse() 
      If vHowMany > 0 AndAlso pOrderLineCalcsToUse.Count > vHowMany Then 
        Dim tmp As New clsOrderLineCalcCol 
        For i = 0 To vHowMany - 1 
          tmp.Add(pOrderLineCalcsToUse(i)) 
        Next 
        pOrderLineCalcsToUse = tmp 
      End If 
      pFault = LoadMeFromDBCache(pOrderLineCalcsToUse, vRequester) : If pFault.isOK = False Then Return pFault 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
      ...
      Catch ex As Exception 
        If pLastReadVariableName <> "" Then pFunctionParameters = "Failed while reading variable: " & pLastReadVariableName & "; " & pFunctionParameters 
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-090303-1658", vRequester) 
      End Try 
      If Not pFault.isOK Then Return pFault 
    End If
  ...
  Public Overrides Function FillSumOnTheFly(ByVal vParameters As Dictionary(Of [Enum], Object), ByVal vRequester As clsRequester) As clsFault 
    ...
    Dim pEnteredHere As Boolean = String.IsNullOrEmpty(vRequester.EntryFunction) 
    pFault = ccSecurity.GetPermissionForDBControllerFunction(clsEnums.enmProcess.viw_vwOrderLineCalcView, "clsOrderLineCalcCol_FillSumOnTheFly", vRequester) 
    If pFault.isOK = False Then Return pFault 
  ...
  Public Overloads Function FillSumOnTheFly( _
    ...
    If MyController.DBType = MyController.enmDBType.FileSystem Then 
      Return pFault.LogFreeTextFault("FillSumOnTheFly has not been handled yet locally", pFunctionParameters, "TRGT-OrderLineCalc-121122-2008", vRequester) 
    Else 
      Dim pCancel As Boolean = False 
      Dim pCommandText As String = "ccOrderLineCalcsFillSumOnTheFly" 
      Dim pDALParameters As New ccDAL.csTargCCParameterCol 
...
#Region "Load Collection"  
  Friend Function LoadMeFromIDataReader(vReader As IDataReader, vRequester As clsRequester) As clsFault Implements ITargCCDataReaderUser.LoadMeFromIDataReader 
    ...
    Dim pOrderLineCalc As clsOrderLineCalc
  
    While vReader.Read()
      pOrderLineCalc = New clsOrderLineCalc() 
      pFault = pOrderLineCalc.LoadMeFromIDataReader(vReader, vRequester) : If Not pFault.isOK Then Return pFault 
       
      Me.Add(pOrderLineCalc)
      If Not _FilledFromSumOnTheFly Then _Clean.Add(pOrderLineCalc.ID) 
    End While
    vRequester.Tag = "" 
  ...
  Private Function LoadMeFromDBCache(ByVal vCachedOrderLineCalcCol As clsOrderLineCalcCol, ByVal vRequester As clsRequester) As clsFault 
    Dim pFunctionParameters As String = "" 
    Dim pFault As New clsFault 
    ...
    Try 
      ...
      For Each pCachedOrderLineCalc As clsOrderLineCalc In vCachedOrderLineCalcCol 
        pOrderLineCalc = New clsOrderLineCalc(pCachedOrderLineCalc) 
        pOrderLineCalc.ccStatus = clsEnums.enmObjectStatus.Clean 
        Me.Add(pOrderLineCalc) 
        If Not _FilledFromSumOnTheFly Then _Clean.Add(pOrderLineCalc.ID) 
      Next 
      pFault.SetOK() 
      Catch ex As Exception 
        pFault.LogException(ex, pFunctionParameters, "TRGT-OrderLineCalc-121122-1513", vRequester) 
      End Try 
