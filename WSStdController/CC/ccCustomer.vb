Public Class clsCustomer
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsCustomerGetByID" 
      ...
      'Use the response to build the Customer 
      LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Customer-150308-1015", vRequester) 
    End Try 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream) 
          Dim pHasValue As Boolean = False 
          'vCustomerCode 
          If vCustomerCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCode) 
          ' 
          pBinaryWriter.Write(vMustExist) 
          pBinaryWriter.Close() 
        End Using 
      ...
      'Run the request 
      Dim pFunction As String = "clsCustomerGetByCustomerCode" 
      Dim pParametersToLog = $"CustomerCode: {vCustomerCode};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault  
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    If Me.isEqual(pCustomer) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", "", "TRGT-Customer-100113-1813", vRequester) 
    End If 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsCustomerUpdate" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Customer-150314-1803", vRequester) 
    End Try 
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsCustomerDelete" 
      Dim pParametersToLog = $"ID: {ID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      'Run the request  
      Dim pFunction As String = "clsCustomerDeleteByID" 
      Dim pParametersToLog = $"ID: {vID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Assign the value  
    Catch ex As Exception 
      Return New clsFault().LogException(73, ex, pFunctionParameters, "TRGT-Customer-231207-1707", vRequester) 
    End Try 
...
Public Class clsCustomerCol
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColFill" 
  ...
  Public Function FillByCustomerType(ByVal vCustomerType As clsEnums.enmCustomerType, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColFillByCustomerType" 
      Dim pParametersToLog = $"CustomerType: {vCustomerType};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Use the response to build the Customer  
      If vAppend = True Then 
        Dim pCustomers As New clsCustomerCol 
        pCustomers.LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
        Me.AddRange(pCustomers) 
      Else 
        LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Customer-150407-2142", vRequester) 
    End Try 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColFillByBoundedID" 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vCustomerCodeFrom 
          If vCustomerCodeFrom Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCodeFrom) 
          ' 
          'vCustomerCodeTo 
          If vCustomerCodeTo Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCodeTo) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
      ...
      Dim pFunction As String = "clsCustomerColFillByBoundedCustomerCode" 
      Dim pParametersToLog = $"CustomerCode: {vCustomerCodeFrom};{vCustomerCodeTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCode) 
          ' 
          pBinaryWriter.Write(vCustomerCodeWildcardType.FastToString())
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
          pBinaryWriter.Close() 
      ...
      Dim pFunction As String = "clsCustomerColFillByWildCardCustomerCode" 
      Dim pParametersToLog = $"CustomerCode: {vCustomerCode};{vCustomerCodeWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColFillByListOfID" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Customer-231207-1750", vRequester) 
    End Try 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If vCustomerCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCode) : pBinaryWriter.Write(vCustomerCodeWildcardType.FastToString()) : pParametersToLog &= $"CustomerCode={vCustomerCode};" : pParametersToLog &= $"CustomerCodeWildcardType={vCustomerCodeWildcardType};"  
          'CustomerType 
      ...
      Dim pFunction As String = "clsCustomerColFillOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If vCustomerCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vCustomerCode) : pBinaryWriter.Write(vCustomerCodeWildcardType.FastToString()) 
          'CustomerType 
          pBinaryWriter.Write(vCustomerType.ToString()) : pParametersToLog &= $"CustomerType={vCustomerType};"  
      ...
      Dim pFunction As String = "clsCustomerColFillSumOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsCustomerColUpdateFromCollection" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColDelete" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Customer-200709-0852-", vRequester) 
    End Try 
  ...
  Public Shared Function DeleteByCustomerType(ByVal vCustomerType As clsEnums.enmCustomerType, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColDeleteByCustomerType" 
      Dim pParametersToLog = $"CustomerType: {vCustomerType};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColDeleteByBoundedID" 
      Dim pParametersToLog = $"ID: {vIDFrom};{vIDTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedCustomerCode(ByVal vCustomerCodeFrom As String, ByVal vCustomerCodeTo As String, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColDeleteByBoundedCustomerCode" 
      Dim pParametersToLog = $"CustomerCode: {vCustomerCodeFrom};{vCustomerCodeTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByWildCardCustomerCode(ByVal vCustomerCode As String, ByVal vCustomerCodeWildcardType As clsEnums.enmWildCardType, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsCustomerColDeleteByWildCardCustomerCode" 
      Dim pParametersToLog = $"CustomerCode: {vCustomerCode};{vCustomerCodeWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
