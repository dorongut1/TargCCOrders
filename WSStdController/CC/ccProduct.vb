Public Class clsProduct
  ...
  Public Function GetByID(ByVal vID As Long, ByVal vRequester As clsRequester, Optional vMustExist As Boolean = False) As clsFault
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsProductGetByID" 
      ...
      'Use the response to build the Product 
      LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Product-150308-1015", vRequester) 
    End Try 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream) 
          Dim pHasValue As Boolean = False 
          'vProductCode 
          If vProductCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vProductCode) 
          ' 
          pBinaryWriter.Write(vMustExist) 
          pBinaryWriter.Close() 
        End Using 
      ...
      'Run the request 
      Dim pFunction As String = "clsProductGetByProductCode" 
      Dim pParametersToLog = $"ProductCode: {vProductCode};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault  
  ...
  Public Function Update(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault
    ...
    If Me.isEqual(pProduct) Then 
      Return pFault.LogFreeTextFault(53, "Object is 'New' - contains no data ", "", "TRGT-Product-100113-1813", vRequester) 
    End If 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsProductUpdate" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Product-150314-1803", vRequester) 
    End Try 
  ...
  Public Function Delete(ByVal vRequester As clsRequester) As clsFault Implements ITargCCEntityDeletable.Delete 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsProductDelete" 
      Dim pParametersToLog = $"ID: {ID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByID(vID As Long, vRequester As clsRequester) As clsFault 
    ...
    Try 
      ...
      'Run the request  
      Dim pFunction As String = "clsProductDeleteByID" 
      Dim pParametersToLog = $"ID: {vID}" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Assign the value  
    Catch ex As Exception 
      Return New clsFault().LogException(73, ex, pFunctionParameters, "TRGT-Product-231207-1707", vRequester) 
    End Try 
...
Public Class clsProductCol
  ...
  Public Overrides Function Fill(ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColFill" 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vCategory 
          pBinaryWriter.Write(vCategory.ToString()) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
      ...
      Dim pFunction As String = "clsProductColFillByCategory" 
      Dim pParametersToLog = $"Category: {vCategory};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
      'Use the response to build the Product  
      If vAppend = True Then 
        Dim pProducts As New clsProductCol 
        pProducts.LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
        Me.AddRange(pProducts) 
      Else 
        LoadByteArray(pResponse, pFault, vRequester) : If Not pFault.isOK Then Return pFault 
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Product-150407-2142", vRequester) 
    End Try 
  ...
  Public Function FillByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester, Optional ByVal vHowMany As Integer = 0, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColFillByBoundedID" 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          Dim pHasValue As Boolean = False 
          'vProductCodeFrom 
          If vProductCodeFrom Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vProductCodeFrom) 
          ' 
          'vProductCodeTo 
          If vProductCodeTo Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vProductCodeTo) 
          ' 
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
      ...
      Dim pFunction As String = "clsProductColFillByBoundedProductCode" 
      Dim pParametersToLog = $"ProductCode: {vProductCodeFrom};{vProductCodeTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If pHasValue = True Then pBinaryWriter.Write(vProductCode) 
          ' 
          pBinaryWriter.Write(vProductCodeWildcardType.FastToString())
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Write(False) 
          pBinaryWriter.Close() 
      ...
      Dim pFunction As String = "clsProductColFillByWildCardProductCode" 
      Dim pParametersToLog = $"ProductCode: {vProductCode};{vProductCodeWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function FillByListOfID(vIDs As List(Of Long), vRequester As clsRequester, Optional ByVal vDir As clsEnums.enmFillDirection = clsEnums.enmFillDirection.ASC, Optional vAppend As Boolean = False) As clsFault 
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColFillByListOfID" 
      ...
      End If 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Product-231207-1750", vRequester) 
    End Try 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If vProductCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vProductCode) : pBinaryWriter.Write(vProductCodeWildcardType.FastToString()) : pParametersToLog &= $"ProductCode={vProductCode};" : pParametersToLog &= $"ProductCodeWildcardType={vProductCodeWildcardType};"  
          'Category 
          pBinaryWriter.Write(vCategory.ToString()) : pParametersToLog &= $"Category={vCategory};"  
          pBinaryWriter.Write(vHowMany) 
          pBinaryWriter.Write(vDir.FastToString()) 
          pBinaryWriter.Close() 
        End Using 
      ...
      Dim pFunction As String = "clsProductColFillOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
    ...
    Try 
      ...
      Using pMemoryStream As New System.IO.MemoryStream() 
        Using pBinaryWriter As New System.IO.BinaryWriter(pMemoryStream, Text.Encoding.UTF8) 
          ...
          If vProductCode Is Nothing Then pHasValue = False Else pHasValue = True 
          pBinaryWriter.Write(pHasValue) 
          If pHasValue = True Then pBinaryWriter.Write(vProductCode) : pBinaryWriter.Write(vProductCodeWildcardType.FastToString()) 
          'Category 
          pBinaryWriter.Write(vCategory.ToString()) : pParametersToLog &= $"Category={vCategory};"  
          pBinaryWriter.Write(vGroupByCategory) : pParametersToLog &= $"GroupByCategory={vGroupByCategory};"  
          pBinaryWriter.Close() 
        End Using 
        pRequest = pMemoryStream.ToArray() 
        pMemoryStream.Close() 
      ...
      Dim pFunction As String = "clsProductColFillSumOnTheFly" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Function UpdateFromCollection(ByVal vRequester As clsRequester, Optional ByVal vReload As Boolean = True) As clsFault Implements ITargCCCollectionUpdateable.UpdateFromCollection 
    ...
    Try 
      ...
      'Run the request 
      Dim pFunction As String = "clsProductColUpdateFromCollection" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function Delete(ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColDelete" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, "Parameters: None", pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
 
    Catch ex As Exception 
      Return pFault.LogException(73, ex, pFunctionParameters, "TRGT-Product-200709-0852-", vRequester) 
    End Try 
  ...
  Public Shared Function DeleteByCategory(ByVal vCategory As clsEnums.enmCategory, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColDeleteByCategory" 
      Dim pParametersToLog = $"Category: {vCategory};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedID(ByVal vIDFrom As Long, ByVal vIDTo As Long, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColDeleteByBoundedID" 
      Dim pParametersToLog = $"ID: {vIDFrom};{vIDTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByBoundedProductCode(ByVal vProductCodeFrom As String, ByVal vProductCodeTo As String, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColDeleteByBoundedProductCode" 
      Dim pParametersToLog = $"ProductCode: {vProductCodeFrom};{vProductCodeTo};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
  ...
  Public Shared Function DeleteByWildCardProductCode(ByVal vProductCode As String, ByVal vProductCodeWildcardType As clsEnums.enmWildCardType, ByVal vRequester As clsRequester) As clsFault
    ...
    Try 
      ...
      Dim pFunction As String = "clsProductColDeleteByWildCardProductCode" 
      Dim pParametersToLog = $"ProductCode: {vProductCode};{vProductCodeWildcardType.FastToString()};" 
      pFault = WebAPI.RunAPI(pFunction, pRequest, pParametersToLog, pResponse, vRequester) : If Not pFault.isOK Then Return pFault 
